...
# - Self-critique pipeline (draft -> critique -> refined)
...
#     - Controlled by GUI switch-style ON/OFF toggle
#     - Polls clipboard every 1 second
#     - Triggers only on NEW clipboard text
# - GUI visuals for health, safe mode, net status, intent, anomalies,
#   context memory, critique cycles, mind stream, evolution log
...
# ========= GLOBAL STATE (SAFE MODE, HEALTH, CLIPBOARD, QUEUE) =========
...
# Unified input queue: console input + clipboard input
...
class IntentRouterOrgan:
    """
    Classifies each input into a coarse intent:
    - CODE, DOCS, CHAT, DEBUG, RESEARCH, OTHER
    Uses the local LLM for classification.
    """
    def __init__(self):
...
""".strip()
    ...
    def choose_system_prompt(self, base_prompt: str, intent: str) -> str:
        """
        Augment the system prompt based on detected intent.
        """
        intent = intent.upper()
        if intent == "CODE":
...
class AnomalyDetectorOrgan:
    """
    Watches inputs and outputs to flag anomalies:
    - Highly repetitive refined answers
    - Very short or empty refined answers
    ...
    def _similarity(self, a: str, b: str) -> float:
        # Very crude similarity: Jaccard on word sets
        a_set = set(a.split())
        b_set = set(b.split())
    ...
    def analyze(self, user: str, draft: str, refined: str) -> str | None:
        ...
        # Check repetition with last refined
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _init_ui(self):
        ...
        # Left: organs + vitals + prediction + web probe + health/safe/net + clipboard toggle
        ...
        self.organs_list.setFixedHeight(320)
        ...
        # Switch-style clipboard listener toggle
        ...
        self.btn_clip_off.setChecked(True)  # default OFF
        ...
        # Center: mind stream + context + intent/anomaly
...
# ========= BRAIN LOOP (QUEUE-DRIVEN, INTENT, ANOMALY, CONTEXT) =========
...
def brain_loop(thought_model: ThoughtStreamModel, cfg: dict):
    thought_model.append("[SYSTEM] Brain online. Type into this console OR copy text while clipboard listener is ON.")

    # Initialize panels at startup
    thought_model.update_context(CONTEXT_MEMORY.render())
    thought_model.update_intent("Intent: UNKNOWN\nExplanation: none yet.")
    thought_model.update_anomaly(ANOMALY_DETECTOR.render())

    # Console input reader (push to queue)
    def console_reader():
        while True:
            try:
...
# ========= CLIPBOARD LISTENER (POLL EVERY 1s, ONLY NEW TEXT) =========
