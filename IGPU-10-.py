# golden_star_event_horizon_swarm_civilization_raft.py
...
def ensure_directories():
    ...
    if not os.path.exists(POLICY_FILE):
        default_policy = {
            ...
            "last_tune": None,
            "epochs": [],
            "rl_q_table_global": {},   # global RL
            "rl_q_table_local": {},    # local RL per node_id
            "heuristic_version": 0,
        }
...
class RaftNode(QThread):
    ...
    log_committed = pyqtSignal(dict)  # committed entry
...
# -------------------------------
# Task Scheduler (multi-agent RL + self-modifying heuristics)
# -------------------------------
...
class BorgControlTower(QWidget):
    def __init__(self, telemetry_engine, scheduler, encoder_worker, compute_worker,
        ...
        self.setWindowTitle("Borg Collective Control Tower – Golden Star Event Horizon (Raft)")
