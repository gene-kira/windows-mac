#!/usr/bin/env python3
# guardian_hybrid_full_platform.py
...
def load_config() -> Dict[str, Any]:
    ...
    if not os.path.exists(path):
        return {"state_path": None, "swarm_endpoint": None, "update_url": None, "llm_active": None}
    try:
...
class SwarmSyncOrgan:
    ...
    def is_enabled(self) -> bool:
        return bool(self.swarm_endpoint)
    ...
    def publish_event(self, event_type: str, payload: Dict[str, Any]):
        ...
        # TODO: real HTTP POST
        self.log(f"[SWARM] Would publish: {data}")

    def fetch_directives(self) -> List[Dict[str, Any]]:
        if not self.is_enabled():
            return []
        # TODO: real HTTP GET
        self.log("[SWARM] Would fetch directives from swarm.")
        # fake directive
        directives = []
        with self.lock:
            directives.extend(self.incoming_directives)
            self.incoming_directives.clear()
        return directives
...
class UpdateOrgan:
    ...
    def check_for_updates(self) -> Dict[str, Any]:
        if not self.update_url:
            self.log("[UPDATE] No update URL configured.")
            return {"status": "disabled"}
        self.log("[UPDATE] Would check remote version.")
        return {
            "status": "ok",
...
class GuardianApp(QMainWindow):
    ...
    def init_settings_tab(self):
        ...
        self.llm_selector.addItem("dummy")
    ...
    def apply_persistent_states_to_ui(self):
        ...
        active_llm = self.config.get("llm_active") or "dummy"
