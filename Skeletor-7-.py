...
# =========================
# Change-point detection (CUSUM) + regime tagging
# =========================
@dataclass
class ChangePointState:
    ...
    drift: float = 0.0
    regime: str = "stable"  # "stable", "spike", "drift"
...
# =========================
# Kalman filter for level + slope (simple 1D state-space)
# =========================
@dataclass
class KalmanState:
    level: float = 0.5
...
# =========================
# Hazard-based ETA (survival-style score)
# =========================
def hazard_eta(series: List[float], target: float, bands: Tuple[List[float], List[float]], hz: float = 10.0) -> Optional[Tuple[float, float]]:
    lo, hi = bands
    if not lo or not hi or len(lo) != len(hi): return None
    # Find earliest index where band overlaps target
    idx = None
    for i, (l, h) in enumerate(zip(lo, hi)):
        if (l <= target <= h) or h >= target and l >= target*0.9:
            idx = i; break
    if idx is None: return None
    # Confidence from band tightness near crossing
...
# =========================
# Backtesting calibrator for bands & risk meters
# =========================
@dataclass
class BacktestCal:
    ...
    def update_alert_quality(self, predicted_events: List[Tuple[str, float]], realized_events: List[str]):
        # predicted_events: (label, confidence), realized_events: labels actually occurred
        realized_set = set(realized_events)
        for label, conf in predicted_events:
            if label in realized_set:
                self.tp += 1
            else:
                self.fp += 1
        for label in realized_set:
...
# =========================
# Causality detection (lagged correlations) + Granger-lite check
# =========================
...
def granger_lite(a: List[float], b: List[float], lag: int) -> float:
    # Simple improvement metric: variance reduction of b after regressing on lagged a
    if len(a) < lag+10 or len(b) < lag+10: return 0.0
    aL = a[:-lag]; bL = b[lag:]
    mb = sum(bL)/len(bL)
    var_b = sum((x-mb)**2 for x in bL) / max(1, len(bL)-1)
    # naive linear regression of bL on aL
    ma = sum(aL)/len(aL)
...
class ScenarioWeights:
    ...
    def adjust(self, outcomes: Dict[str, bool]):
        # Increase weight if scenario predicted an issue that realized; decrease otherwise
        for name, hit in outcomes.items():
            if name == "cpu_burst_30":
                self.cpu_burst = min(2.0, self.cpu_burst * (1.08 if hit else 0.95))
            elif name == "net_double":
                self.net_double = min(2.0, self.net_double * (1.08 if hit else 0.95))
            elif name == "mem_leak":
                self.mem_leak = min(2.0, self.mem_leak * (1.08 if hit else 0.95))
...
# =========================
# Preemptive strategy stack with hysteresis/dwell
# =========================
@dataclass
class StrategyPlan:
...
# =========================
# Autonomous engine (threaded, calibrated, change-point, hazard ETAs, causality)
# =========================
class AutonomousLightDark:
    def __init__(self, channels: List[str], state_path: str = "light_dark_state.json", log_path: str = "light_dark_telemetry.jsonl"):
        ...
        self.history: Dict[str, RingBuffer] = {ch: RingBuffer(16000) for ch in channels}
        ...
        # Tri-horizon predictions and bands
        ...
        # Causality map
        self.causal_links: List[Tuple[str, str, int, float, float]] = []  # (source, target, lag, corr, granger_improve)
    ...
    def _schedule_forecasts(self):
        for ch in self.channels:
            series = self.history[ch].tail(1800)
            # Update Kalman first with last observation for smoother short-term
            if series:
            ...
            def f_short():
                ...
                f, lo, hi = forecast_short(series, steps=self.pred_horizons["S"])
                # Blend 60/40 Kalman/short
                ff = [min(1.0, max(0.0, 0.6*a + 0.4*b)) for a,b in zip(fk, f)]
                lo2 = [min(1.0, max(0.0, 0.6*al + 0.4*bl)) for al, bl in zip(lok, lo)]
                hi2 = [min(1.0, max(0.0, 0.6*ah + 0.4*bh)) for ah, bh in zip(hik, hi)]
                return ff, lo2, hi2
            # Mid
            def f_mid():
        ...
        for ch in self.channels:
            for horiz in ("S","M","L"):
                ...
                if fut and fut.done():
                    try:
                        f, lo, hi = fut.result()
                        # Calibrate bands via backtesting scaler
    ...
    def _schedule_causality(self):
        ...
        def compute():
            ...
            for a,b in pairs:
                ...
                improve = granger_lite(la, lb, lag) if lag > 0 else 0.0
                if abs(corr) > 0.25 and improve > 0.05:
                    links.append((a, b, lag, corr, improve))
            return links
    ...
    def _run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Update signals, classify, update change-point regimes
                for ch in self.channels:
                    ...
                    # Change-point update and regime tagging
                    cp = self.cusum[ch]
                    cp = update_cusum(cp, sig.intensity, mean_ref=0.5, k=0.02)
                    self.cusum[ch] = cp
                    self.profiles[ch].regime = cp.regime
                    # Regime-aware threshold nudging
                ...
                # Risk and hazard ETA (use mid bands for robustness)
                self.pred_anomaly_risk = risk_from_jitter(self.anomaly_density.to_list(), window=300)
                bands_mid = self.bands.get("load", {}).get("M", ([], []))
                eta_pack = hazard_eta(self.history["load"].tail(240), target=self.thresholds["load"].high_on, bands=bands_mid, hz=self.rate_hz)
                if eta_pack:
                ...
                # Backtesting calibrator: compare realized to short bands (if prior exists)
                for ch in self.channels:
                ...
                # Alert quality metrics (basic: predicted future labels vs. labels that occurred recently)
                realized_labels = []
                # Heuristic: high cpu or mem crossing -> label occurs
                if cpuS and max(self.history["cpu"].tail(60)) > 0.85: realized_labels.append("cpu_forecast_high")
                if memM and max(self.history["mem"].tail(120)) > 0.9: realized_labels.append("mem_forecast_pressure")
    ...
    def _refresh_plans(self, now: float):
        ...
        eta_conf = self.pred_strategy_eta_conf
        has_mutate = any(p.action == "mutate" for p in self.plan_stack)
        has_revert = any(p.action == "revert" for p in self.plan_stack)
        # Minimum dwell before scheduling opposite plan
        dwell_ok = (now - self.last_strategy_change_ts) > 10.0
        if high_risk and eta is not None and eta_conf > 0.5 and not has_mutate and dwell_ok:
    ...
    def _judge_repair_and_think(self, protected: ProtectedState) -> List[Dict]:
        ...
        for a,b,lag,corr,imp in self.causal_links[:4]:
        ...
        if len(self.thought_log) > 3000:
            self.thought_log = self.thought_log[-3000:]
...
# =========================
# Compact scientific GUI (Queue-decoupled, 480ms refresh, toggles, alert quality strip)
# =========================
...
class ScientificGUI:
    def __init__(self, engine: AutonomousLightDark):
        self.engine = engine
        self.root = tk.Tk()
        self.root.title("Predictive Skeleton Monitor (Ultra-Predictive)")
        self.root.geometry("920x680")
        self.root.configure(bg="#111111")

        # Decoupled payload queue
        ...
        for i, ch in enumerate(engine.channels):
            c_series = tk.Canvas(self.chart_grid, width=280, height=130, bg="#1b1b1b", highlightthickness=0)
            c_series.grid(row=(i // 3)*2, column=i % 3, padx=3, pady=3, sticky="nsew"); self.series_canvases[ch] = c_series
            c_conf = tk.Canvas(self.chart_grid, width=280, height=50, bg="#151515", highlightthickness=0)
        ...
        # Analytics: timelines + heatmap + anomaly bar + scenarios + future thoughts + causality + alert quality strip
        ...
        self.timeline_canvas = tk.Canvas(self.analytics, width=880, height=60, bg="#1b1b1b", highlightthickness=0)
        self.timeline_canvas.pack(fill="x", padx=4, pady=4)
        self.heat_canvas = tk.Canvas(self.analytics, width=880, height=160, bg="#1b1b1b", highlightthickness=0)
        self.heat_canvas.pack(fill="x", padx=4, pady=4)
        self.anomaly_canvas = tk.Canvas(self.analytics, width=880, height=45, bg="#151515", highlightthickness=0)
        ...
        self.quality_canvas = tk.Canvas(self.analytics, width=880, height=40, bg="#151515", highlightthickness=0)
        ...
        for col in ("label","conclusion","confidence","evidence","notes","ts"):
            ...
            self.th_tree.column(col, width=110 if col!="evidence" else 220, stretch=True)
        ...
        # Start engine + slow GUI refresh (~480ms)
    ...
    def refresh(self):
        ...
        if payload:
            ...
            # Series + tri-horizon with toggles
            ...
            self.update_causality(payload.get("causal_links", []))
    ...
    def update_causality(self, links: List[Tuple[str,str,int,float,float]]):
        self.causality_list.delete(0, "end")
        for a,b,lag,corr,gain in links[-10:]:
            self.causality_list.insert("end", f"{a} → {b} | lag={lag}s | corr≈{corr:.2f} | gain≈{gain:.2f}")
    ...
    def draw_quality(self, canvas: tk.Canvas, cal: Dict[str, float]):
        ...
        canvas.delete("all")
        w = int(canvas.winfo_width() or 880); h = int(canvas.winfo_height() or 40)
    ...
    def draw_series(self, canvas: tk.Canvas, series: List[float], th: Thresholds,
        ...
        w = int(canvas.winfo_width() or 280); h = int(canvas.winfo_height() or 130)
        canvas.delete("all")
    ...
    def draw_confidence(self, canvas: tk.Canvas, conf_series: List[float]):
        w = int(canvas.winfo_width() or 280); h = int(canvas.winfo_height() or 50)
        canvas.delete("all")
        canvas.create_rectangle(0, 0, w, h, fill="#151515", outline="")
        if conf_series:
            n = len(conf_series)
    ...
    def draw_timeline(self, canvas: tk.Canvas, events: List[Tuple[float, str, str]]):
        w = int(canvas.winfo_width() or 880); h = int(canvas.winfo_height() or 60)
        canvas.delete("all")
        canvas.create_rectangle(0, 0, w, h, fill="#1b1b1b", outline="")
    ...
    def draw_heatmap(self, canvas: tk.Canvas, series_map: Dict[str, List[float]]):
        canvas.delete("all")
        w = int(canvas.winfo_width() or 880); h = int(canvas.winfo_height() or 160)
        canvas.create_rectangle(0, 0, w, h, fill="#1b1b1b", outline="")
    ...
    def draw_anomaly_bar(self, canvas: tk.Canvas, anomaly_series: List[float]):
        w = int(canvas.winfo_width() or 880); h = int(canvas.winfo_height() or 45)
        canvas.delete("all")
        canvas.create_rectangle(0, 0, w, h, fill="#151515", outline="")
