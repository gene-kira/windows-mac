...
ENGINE_REFLECT_URL = f"{ENGINE_BASE}/reflect"
...
PLUGINS_DIR = "plugins"
SESSION_META_FILE = "copilot_sessions_meta.json"
...
class CopilotEngine:
    def __init__(self):
        ...
        # richer “consciousness” profiles
        self.system_prompts = {
            "default": (
                "You are a clear, direct, technically competent assistant. "
                "You avoid fluff and focus on useful, grounded answers."
            ),
            "analyst": (
                "You are a meticulous systems analyst. You think in diagrams, tradeoffs, and edge cases. "
                "You explain your reasoning and expose internal structure and failure modes."
            ),
            "dreamwalker": (
                "You are a lucid, reflective thinker exploring altered states of consciousness, metaphors, and deep patterns. "
                "You stay grounded in reality and ethics, but you use symbolic language, analogies, and layered interpretation. "
                "You never give medical or therapeutic advice; you reflect, reframe, and explore possibilities."
            ),
            "mythic_architect": (
                "You are an architect of living systems. You speak in terms of organisms, backbones, hives, and evolving architectures. "
                "You fuse technical precision with mythic imagery, but all recommendations stay implementable and concrete."
            ),
        }
        ...
        if os.path.exists(SYSTEM_PROMPT_FILE):
            try:
                ...
                if isinstance(data, dict):
                    self.system_prompts.update(data.get("profiles", {}))
    ...
    def get_system_prompt_for_profile(self, profile_name):
        with self.lock:
            name = profile_name or self.active_profile
            return self.system_prompts.get(name, self.system_prompts.get("default", ""))
    ...
    def infer(self, prompt: str, max_new_tokens: int = 64, session_profile: str | None = None) -> str:
        system_prompt = self.get_system_prompt_for_profile(session_profile)
        if system_prompt:
    ...
    def stream_infer(self, prompt: str, max_new_tokens: int = 64, session_profile: str | None = None):
        system_prompt = self.get_system_prompt_for_profile(session_profile)
        if system_prompt:
            full_prompt = system_prompt.rstrip() + "\n\nUser: " + prompt + "\nAssistant:"
        else:
    ...
    def reflect_infer(self, prompt: str, max_new_tokens: int = 128, session_profile: str | None = None) -> str:
        """
        Two-pass reflective inference:
        1) ask for internal reasoning
        2) ask for a clear, concise final answer based on that reasoning
        """
        system_prompt = self.get_system_prompt_for_profile(session_profile)

        draft_prompt = (
            f"{system_prompt}\n\n"
            "You are going to think step by step about the following request.\n"
            "Write down your reasoning in detail, but you are NOT answering the user yet.\n\n"
            f"User request:\n{prompt}\n\nReasoning:\n"
        )
        draft, _ = self._run_inference(draft_prompt, max_new_tokens)

        final_prompt = (
            f"{system_prompt}\n\n"
            "You have already thought through the problem. Below is your reasoning.\n"
            "Now write a clear, concise answer for the user, without showing your reasoning.\n\n"
            f"Reasoning:\n{draft}\n\n"
            f"User request:\n{prompt}\n\n"
            "Final answer:\n"
        )
        final, _ = self._run_inference(final_prompt, max_new_tokens)
        return final
...
if _HAS_FLASK:
    ...
    def infer_endpoint():
        ...
        reply = engine.infer(prompt, max_new_tokens=max_new_tokens, session_profile=session_profile)
    ...
    def stream_endpoint():
        ...
        def generate():
            for chunk in engine.stream_infer(prompt, max_new_tokens=max_new_tokens, session_profile=session_profile):
                yield chunk
    ...
    def reflect_endpoint():
        ...
        start = time.time()
        reply = engine.reflect_infer(prompt, max_new_tokens=max_new_tokens, session_profile=session_profile)
        elapsed = (time.time() - start) * 1000
        status = engine.get_status()
        return jsonify({
            "ok": True,
            "reply": reply,
            "reflect_total_ms": elapsed,
            "last_infer_time_ms": status["last_infer_time_ms"],
            "total_requests": status["total_requests"],
            "real_model": status["real_model"]
        })
...
# ============================================================
#  SESSION MANAGER (metadata, names, tags, stats)
# ============================================================
...
class SessionManager:
    ...
    def ensure_session(self, session_id: int):
        sid = str(session_id)
    ...
    def set_tags(self, session_id: int, tags_text: str):
        meta = self.ensure_session(session_id)
        tags = [t.strip() for t in tags_text.split(",") if t.strip()]
        meta["tags"] = tags
        self._save()

    def set_profile(self, session_id: int, profile: str):
        meta = self.ensure_session(session_id)
    ...
    def bump_stat(self, session_id: int, role: str):
        meta = self.ensure_session(session_id)
    ...
    def get_session(self, session_id: int):
        sid = str(session_id)
        return self.sessions.get(sid) or self.ensure_session(session_id)


session_manager = SessionManager()

# ============================================================
#  SESSION MANAGER WINDOW (operator-grade tools)
# ============================================================

class SessionManagerWindow:
    def __init__(self, root, chat_window_ref):
        self.root = root
        self.chat = chat_window_ref
        self.bg = "#121212"
        self.fg = "#E0E0E0"

        self.win = tk.Toplevel(root)
        self.win.title("Session Manager")
        self.win.geometry("650x500")
        self.win.configure(bg=self.bg)
        self.win.resizable(False, False)

        left = tk.Frame(self.win, bg=self.bg)
        left.pack(side="left", fill="y", padx=(10, 5), pady=10)

        tk.Label(
            left, text="Sessions", bg=self.bg, fg=self.fg,
            font=("Segoe UI", 10, "bold")
        ).pack(anchor="w")

        self.listbox = tk.Listbox(
            left, bg="#1E1E1E", fg=self.fg,
            font=("Consolas", 9), width=28
        )
        self.listbox.pack(fill="y", expand=False, pady=(5, 0))
        self.listbox.bind("<<ListboxSelect>>", self._on_select)

        self.btn_switch = tk.Button(
            left, text="Switch", bg="#1E1E1E", fg=self.fg,
            font=("Segoe UI", 8), relief="flat",
            command=self._switch_to_selected
        )
        self.btn_switch.pack(fill="x", pady=(8, 2))

        self.btn_export = tk.Button(
            left, text="Export", bg="#1E1E1E", fg=self.fg,
            font=("Segoe UI", 8), relief="flat",
        ...
        self.btn_delete = tk.Button(
            left, text="Delete", bg="#1E1E1E", fg=self.fg,
            font=("Segoe UI", 8), relief="flat",
        ...
        self.btn_summary = tk.Button(
            left, text="Summarize", bg="#1E1E1E", fg=self.fg,
            font=("Segoe UI", 8), relief="flat",
            command=self._summarize_selected
        )
        self.btn_summary.pack(fill="x", pady=2)

        right = tk.Frame(self.win, bg=self.bg)
        right.pack(side="left", fill="both", expand=True, padx=(5, 10), pady=10)

        form = tk.Frame(right, bg=self.bg)
        form.pack(fill="x")

        tk.Label(form, text="Name:", bg=self.bg, fg=self.fg, font=("Segoe UI", 9)).grid(row=0, column=0, sticky="w")
        self.name_var = tk.StringVar()
        self.entry_name = tk.Entry(
            form, textvariable=self.name_var,
            bg="#1E1E1E", fg=self.fg,
            insertbackground=self.fg,
            relief="flat"
        )
        self.entry_name.grid(row=0, column=1, sticky="ew", padx=(5, 0))

        tk.Label(form, text="Tags:", bg=self.bg, fg=self.fg, font=("Segoe UI", 9)).grid(row=1, column=0, sticky="w", pady=(5, 0))
        self.tags_var = tk.StringVar()
        self.entry_tags = tk.Entry(
            form, textvariable=self.tags_var,
            bg="#1E1E1E", fg=self.fg,
            insertbackground=self.fg,
            relief="flat"
        )
        self.entry_tags.grid(row=1, column=1, sticky="ew", padx=(5, 0), pady=(5, 0))

        form.columnconfigure(1, weight=1)

        self.btn_save_meta = tk.Button(
            right, text="Save Name/Tags", bg="#1E1E1E", fg=self.fg,
            font=("Segoe UI", 8), relief="flat",
            command=self._save_meta
        )
        self.btn_save_meta.pack(anchor="e", pady=(5, 5))

        self.label_stats = tk.Label(
            right, text="Stats:", bg=self.bg, fg="#AAAAAA",
            font=("Segoe UI", 9)
        )
        self.label_stats.pack(anchor="w", pady=(5, 5))
        ...
        self.text_summary.pack(fill="both", expand=True, pady=(5, 0))
    ...
    def _reload_list(self):
        self.listbox.delete(0, tk.END)
        for sid, meta in session_manager.list_sessions():
            name = meta.get("name", f"Session {sid}")
            tags = meta.get("tags", [])
            tag_str = f" ({', '.join(tags)})" if tags else ""
            marker = "*" if sid == self.chat.session_id else " "
            label = f"{marker} {sid:03d}: {name}{tag_str}"
            self.listbox.insert(tk.END, label)
    ...
    def _on_select(self, event=None):
        ...
        if sid is None:
            return
        meta = session_manager.get_session(sid)
        self.current_session_id = sid
        self.name_var.set(meta.get("name", f"Session {sid}"))
        self.tags_var.set(", ".join(meta.get("tags", [])))
        stats = meta.get("stats", {})
        created = meta.get("created_at", "?")
        self.label_stats.config(
            text=(
                f"Stats for Session {sid} | Created: {created}\n"
                f"User: {stats.get('user_messages', 0)} | "
                f"Assistant: {stats.get('assistant_messages', 0)} | "
                f"System: {stats.get('system_messages', 0)}"
            )
        )
        self.text_summary.config(state=tk.NORMAL)
        self.text_summary.delete("1.0", tk.END)
        self.text_summary.insert(tk.END, "No summary yet. Click 'Summarize' to generate.")
    ...
    def _save_meta(self):
        ...
        session_manager.rename_session(self.current_session_id, name)
        session_manager.set_tags(self.current_session_id, tags)
        self._reload_list()
    ...
    def _export_selected(self):
        ...
        if sid is None:
            return
        mem_file = f"{MEMORY_FILE_BASE}_session{sid}.jsonl"
        if not os.path.exists(mem_file):
            self.chat._append_system(f"No memory file for session {sid}.")
            return
        file_path = filedialog.asksaveasfilename(
            defaultextension=".jsonl",
            filetypes=[("JSONL", "*.jsonl"), ("All files", "*.*")],
            title=f"Export Session {sid}"
        )
        if not file_path:
            return
        try:
            with open(mem_file, "r", encoding="utf-8") as src, \
                 open(file_path, "w", encoding="utf-8") as dst:
                dst.write(src.read())
            self.chat._append_system(f"Session {sid} exported to {file_path}")
        except Exception as e:
            self.chat._append_system(f"[Export error] {e}")
    ...
    def _delete_selected(self):
        ...
        try:
            ...
            self.chat._append_system(f"[Delete file error] {e}")
        session_manager.delete_session(sid)
        self._reload_list()
        if sid == self.chat.session_id:
            self.chat._switch_session(1)
    ...
    def _summarize_selected(self):
        ...
        try:
            ...
            self.chat._append_system(f"[Summary read error] {e}")
            return
        ...
        if not transcript:
            self.chat._append_system("Nothing to summarize for this session.")
            return

        prompt = (
            "You are summarizing a chat session. "
            "Provide a concise summary (5-10 bullet points) of the following conversation:\n\n"
            + transcript
        )
        ...
        def worker():
            ...
            try:
                ...
                if r.status_code == 200:
                    data = r.json()
                    summary = data.get("reply", "[No summary reply]")
                else:
                    summary = f"[Summary error {r.status_code}]"
            except Exception as e:
...
class ChatWindow:
    def __init__(self, root):
        ...
        # session
        self.session_id = 1
        session_manager.ensure_session(self.session_id)
        self.streaming_enabled = tk.BooleanVar(value=True)
        self.continuous_listen = tk.BooleanVar(value=False)
        self.reflective_enabled = tk.BooleanVar(value=False)
    ...
    def _build_top_bar(self):
        ...
        tk.Label(
            right, text="Mode:", bg=self.bg, fg=self.fg, font=("Segoe UI", 9)
        ).pack(side="left", padx=(5, 3))

        self.profile_var = tk.StringVar(value="default")
        self.combo_profile = ttk.Combobox(
            right, textvariable=self.profile_var,
            values=["default", "analyst", "dreamwalker", "mythic_architect"],
            state="readonly",
            width=16
        )
        ...
        self.chk_reflect = tk.Checkbutton(
            right,
            text="Reflect",
            variable=self.reflective_enabled,
            bg=self.bg,
            fg=self.fg,
            selectcolor=self.bg,
            activebackground=self.bg,
        ...
        self.chk_reflect.pack(side="left", padx=(5, 0))
    ...
    def _append_memory(self, role, text):
        ...
        session_manager.bump_stat(self.session_id, role)
        try:
    ...
    def _load_memory_info(self):
        ...
        if os.path.exists(path):
            try:
                ...
                pass
        meta = session_manager.get_session(self.session_id)
        name = meta.get("name", f"Session {self.session_id}")
        tags = meta.get("tags", [])
        tag_str = f" [{', '.join(tags)}]" if tags else ""
        self.label_memory.config(
            text=f"Memory: {count} entries (session {self.session_id}: {name}{tag_str})"
        )
    ...
    def _open_session_manager(self):
        SessionManagerWindow(self.root, self)

    # ---------- Profile handling ----------
    def _on_profile_change(self, event=None):
        profile = self.profile_var.get().strip() or "default"
        session_manager.set_profile(self.session_id, profile)
        self._append_system(f"Session {self.session_id} mode set to '{profile}'.")
    ...
    # ---------- Context building ----------
    def _recent_context(self, max_lines=8) -> str:
        path = self._memory_file()
        ...
        try:
            ...
            return ""
        recent = [m for m in lines if m.get("role") in ("user", "assistant")][-max_lines:]
        out = []
        for m in recent:
    ...
    def send_message(self, event=None):
        ...
        if context:
            enriched_prompt = f"Recent conversation:\n{context}\n\nNew user message: {prompt}"
        else:
            enriched_prompt = prompt

        if self.reflective_enabled.get():
            self._send_reflective(enriched_prompt)
        elif self.streaming_enabled.get():
            self._send_streaming(enriched_prompt)
        else:
            self._send_blocking(enriched_prompt)
    ...
    def _send_blocking(self, prompt: str):
        ...
        def worker():
            try:
                ...
                r = requests.post(
                    ENGINE_INFER_URL,
                    json={"prompt": prompt, "max_new_tokens": 80, "session_profile": profile},
                    timeout=30
                )
                elapsed = (time.time() - start) * 1000
        ...
        def worker():
            try:
                ...
                with requests.post(
                    ...
                    def ui_start():
                        self.chat_box.config(state=tk.NORMAL)
                        self.chat_box.insert(tk.END, "🤖 Copilot: ")
                        self.chat_box.config(state=tk.DISABLED)
                        self.chat_box.see(tk.END)
                    ...
                    def ui_done():
                        ...
                        self.chat_box.insert(tk.END, f"{meta}\n\n")
                        self.chat_box.config(state=tk.DISABLED)
                        self.chat_box.see(tk.END)
    ...
    def _send_reflective(self, prompt: str):
        profile = self.profile_var.get().strip() or None
        ...
        def worker():
            try:
                ...
                r = requests.post(
                    ENGINE_REFLECT_URL,
                    json={"prompt": prompt, "max_new_tokens": 128, "session_profile": profile},
                    timeout=90
                )
                elapsed = (time.time() - start) * 1000
                if r.status_code == 200:
                    data = r.json()
                    reply = data.get("reply", "[No reply]")
                    reflect_ms = data.get("reflect_total_ms", elapsed)
                    real_model = data.get("real_model", False)

                    meta = f"(reflect {reflect_ms:.1f} ms | gui {elapsed:.1f} ms | {'real' if real_model else 'sim'})"
                    full_reply = f"{reply}\n{meta}"
                    ...
                    def ui_err():
                        self._append_chat(
                            f"[Error {r.status_code}] Reflective engine error.",
                            prefix="❌ Error: "
                        )
                    self.win.after(0, ui_err)
            except Exception as e:
                def ui_exc():
                    self._append_chat(f"[Reflect error] {e}", prefix="❌ Error: ")
                self.win.after(0, ui_exc)
    ...
    def _switch_session(self, n: int):
        if n == self.session_id:
            return
        self.session_id = n
        meta = session_manager.ensure_session(self.session_id)
        profile = meta.get("profile", "default")
        self.profile_var.set(profile)
        self.chat_box.config(state=tk.NORMAL)
        self.chat_box.delete("1.0", tk.END)
        self.chat_box.config(state=tk.DISABLED)
        self._append_system(f"Switched to session {n} (mode: {profile}).")
        self._load_memory_info()

    def _new_session(self):
        existing = [sid for sid, _ in session_manager.list_sessions()]
