...
class RouteHistory:
    ...
    def add_sample(self, status: RouteStatus):
        ...
        # compute anomaly and record fingerprint if high
    ...
    def volatility(self, series: List[float]) -> float:
        ...
        var = sum((x - mean) ** 2 for x in series) / len(series)
        std = var ** 0.5
        # normalize volatility into [0,1] using a soft cap
...
class ShortHorizonForecaster:
    ...
    def forecast_routes(self, routes: Dict[str, RouteStatus]) -> Dict[str, RouteForecast]:
        ...
        for name, status in routes.items():
            ...
            # volatility-aware forecast: dampen trend if volatility is high
            ...
            # health penalized by volatility
...
# ============================================================
# THREAT CONE 2.0 (USES TRENDS, ANOMALY, VOLATILITY)
# ============================================================
class ThreatCone:
...
# ============================================================
# PREEMPTIVE REROUTE BRAIN (COOLDOWN + STANCE THRESHOLDS)
# ============================================================
    ...
    def maybe_preemptive_reroute(
        ...
        for name, status in snapshot.items():
            ...
            if p.risk_score < cur_proj.risk_score and p.projected_color_30s in (
                ...
                safer_candidates.append((p.risk_score, status, p))
...
class TelemetrySignals(QObject):
    event_received = pyqtSignal(str, str)  # category, text
...
class MainWindow(QMainWindow):
    ...
    # TELEMETRY HANDLER (STANCE AUTO-SWITCHING + FEEDBACK)
