...
# ---------------- PowerShell helpers (UWP, Provisioned, Features, Updates, Tasks, Drivers, Firewall, Restore, Events) ----------------
...
def _run_powershell_text(ps_command: str):
    cmd = [
        "powershell",
        "-NoProfile",
        "-Command",
...
def get_windows_updates_text():
    return _run_powershell_text(
        "Get-WindowsUpdateLog -LogPath $env:TEMP\\WULOG.txt; "
        "Get-Content $env:TEMP\\WULOG.txt -Tail 200"
    )


def list_scheduled_tasks_text():
    return _run_powershell_text("schtasks /query /v /fo LIST")


def run_scheduled_task(task_name: str):
    return _run_powershell_text(f'schtasks /run /tn "{task_name}"')


def delete_scheduled_task(task_name: str):
    return _run_powershell_text(f'schtasks /delete /tn "{task_name}" /f')


def list_drivers_text():
    return _run_powershell_text("Get-WmiObject Win32_SystemDriver | Select Name,DisplayName,State,StartMode,PathName | Format-Table -AutoSize")


def list_firewall_rules_text():
    return _run_powershell_text("Get-NetFirewallRule | Select DisplayName,Direction,Action,Enabled,Profile | Format-Table -AutoSize")


def create_firewall_rule_text(name, direction, action, program=None, port=None):
    base = f"New-NetFirewallRule -DisplayName '{name}' -Direction {direction} -Action {action}"
    if program:
        base += f" -Program '{program}'"
    if port:
        base += f" -LocalPort {port} -Protocol TCP"
    return _run_powershell_text(base)


def list_restore_points_text():
    return _run_powershell_text("vssadmin list shadows")


def create_restore_point_text(desc: str):
    return _run_powershell_text(
        f"Checkpoint-Computer -Description '{desc}' -RestorePointType 'MODIFY_SETTINGS'"
    )


def list_event_log_text(log_name="System", max_entries=200):
    return _run_powershell_text(
        f"Get-EventLog -LogName {log_name} -Newest {max_entries} | "
        "Select TimeGenerated,EntryType,Source,EventID,Message | Format-Table -AutoSize"
    )
...
class SystemConsole(QMainWindow):
    def __init__(self):
        ...
        self.process_tree_tab = self._build_process_tree_tab()
        ...
        self.tabs.addTab(self.gpu_tab, "GPU Monitor")
        ...
        self.tabs.addTab(self.updates_tab, "Windows Update")
        ...
        self.tabs.addTab(self.restore_tab, "System Restore")
        self.tabs.addTab(self.events_tab, "Event Viewer")
    ...
    def _build_dashboard_tab(self) -> QWidget:
        ...
        self.db_cpu_label = QLabel("CPU Usage:")
        self.db_cpu_bar = QProgressBar()
        self.db_cpu_bar.setRange(0, 100)

        self.db_ram_label = QLabel("RAM Usage:")
        self.db_ram_bar = QProgressBar()
        self.db_ram_bar.setRange(0, 100)

        self.db_gpu_label = QLabel("GPU Usage:")
        self.db_gpu_bar = QProgressBar()
        self.db_gpu_bar.setRange(0, 100)

        self.db_net_label = QLabel("Network: 0 KB/s up, 0 KB/s down")

        self.db_disk_label = QLabel("Disk Activity:")
        self.db_disk_bar = QProgressBar()
        self.db_disk_bar.setRange(0, 100)

        self.db_health_label = QLabel("System Health: OK")
        self.db_health_label.setStyleSheet("QLabel { font-weight: bold; }")

        layout.addWidget(self.db_cpu_label)
        layout.addWidget(self.db_cpu_bar)
        layout.addWidget(self.db_ram_label)
        layout.addWidget(self.db_ram_bar)
        layout.addWidget(self.db_gpu_label)
        layout.addWidget(self.db_gpu_bar)
        layout.addWidget(self.db_disk_label)
        layout.addWidget(self.db_disk_bar)
        layout.addWidget(self.db_net_label)
        layout.addWidget(self.db_health_label)
        layout.addStretch()
        return w
    ...
    def update_dashboard(self):
        ...
        self.db_cpu_bar.setValue(int(cpu))
        self.db_cpu_label.setText(f"CPU Usage: {cpu:.1f}%")

        self.db_ram_bar.setValue(int(ram))
        self.db_ram_label.setText(f"RAM Usage: {ram:.1f}%")
        ...
        if GPU_AVAILABLE:
            try:
                ...
                gpu_usage = util.gpu
                self.db_gpu_bar.setValue(int(gpu_usage))
                self.db_gpu_label.setText(f"GPU Usage: {gpu_usage:.1f}%")
            except Exception:
                self.db_gpu_label.setText("GPU Usage: (error)")
        else:
            self.db_gpu_label.setText("GPU Usage: (no GPU API)")

        # Network
        now_net = psutil.net_io_counters()
        sent = now_net.bytes_sent - self.last_net.bytes_sent
        recv = now_net.bytes_recv - self.last_net.bytes_recv
        self.last_net = now_net
        up_kb = sent / 1024
        down_kb = recv / 1024
        self.db_net_label.setText(f"Network: {up_kb:.1f} KB/s up, {down_kb:.1f} KB/s down")

        # Disk
        now_disk = psutil.disk_io_counters()
        read = now_disk.read_bytes - self.last_disk.read_bytes
        write = now_disk.write_bytes - self.last_disk.write_bytes
        self.last_disk = now_disk
        total_rw = (read + write) / (1024 * 1024)
        disk_percent = min(100, total_rw * 5)
        self.db_disk_bar.setValue(int(disk_percent))
        self.db_disk_label.setText(f"Disk Activity: {total_rw:.2f} MB/s")

        # Simple health heuristic
        if cpu < 80 and ram < 80 and disk_percent < 80:
            self.db_health_label.setText("System Health: OK")
            self.db_health_label.setStyleSheet("QLabel { color: #00cc44; font-weight: bold; }")
        else:
            self.db_health_label.setText("System Health: STRESSED")
            self.db_health_label.setStyleSheet("QLabel { color: #ff4444; font-weight: bold; }")
    ...
    def _build_process_tree_tab(self) -> QWidget:
        w = QWidget()
        layout = QVBoxLayout(w)

        self.proc_tree = QTreeWidget()
        self.proc_tree.setHeaderLabels(["Process", "PID", "CPU %", "RAM MB"])
        self.proc_tree.header().setSectionResizeMode(0, QHeaderView.Stretch)
        self.proc_tree.header().setSectionResizeMode(1, QHeaderView.ResizeToContents)
        self.proc_tree.header().setSectionResizeMode(2, QHeaderView.ResizeToContents)
        self.proc_tree.header().setSectionResizeMode(3, QHeaderView.ResizeToContents)

        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("Refresh Tree")
        btn_refresh.clicked.connect(self.populate_process_tree)
        btn_kill = QPushButton("Kill Selected")
        btn_kill.clicked.connect(self.kill_selected_tree_process)
        btn_row.addWidget(btn_refresh)
    ...
    def populate_process_tree(self):
        ...
        def add_node(pid, parent_item=None):
            ...
            item = QTreeWidgetItem([name, str(pid), str(cpu), str(mem_mb)])
            if name.lower() in PROTECTED_PROCESSES:
                item.setBackground(0, QColor(80, 80, 0))
                item.setBackground(1, QColor(80, 80, 0))
                item.setBackground(2, QColor(80, 80, 0))
                item.setBackground(3, QColor(80, 80, 0))
        ...
        roots = [pid for pid in procs.keys() if procs[pid].get("ppid", 0) not in procs]
        for r in roots:
            add_node(r)

        self.proc_tree.expandToDepth(1)

    def kill_selected_tree_process(self):
    ...
    def _build_gpu_tab(self) -> QWidget:
        ...
        self.gpu_table.setHorizontalHeaderLabels(["GPU", "Usage %", "Memory Used MB", "Temperature °C"])
        self.gpu_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        layout.addWidget(self.gpu_table)

        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("Refresh GPU")
        btn_refresh.clicked.connect(self.populate_gpu)
        btn_row.addWidget(btn_refresh)
        btn_row.addStretch()
        layout.addLayout(btn_row)
    ...
    def populate_gpu(self):
        ...
        if not GPU_AVAILABLE:
            self.gpu_status_label.setText("GPU monitoring not available (pynvml not installed or no NVIDIA GPU).")
            return

        try:
            count = pynvml.nvmlDeviceGetCount()
            self.gpu_table.setRowCount(count)
            for i in range(count):
                ...
                self.gpu_table.setItem(i, 0, QTableWidgetItem(name))
                self.gpu_table.setItem(i, 1, QTableWidgetItem(str(util.gpu)))
                self.gpu_table.setItem(i, 2, QTableWidgetItem(str(mem.used // (1024 * 1024))))
                self.gpu_table.setItem(i, 3, QTableWidgetItem(str(temp)))

            self.gpu_status_label.setText("GPU data refreshed.")
        except Exception as e:
            self.gpu_status_label.setText(f"GPU error: {e}")
    ...
    def _start_network_timer(self):
        ...
        self.network_timer.timeout.connect(self.populate_network)
        ...
        for c in conns:
            ...
            if pid:
                try:
                    ...
                    name = ""
            rows.append((pid, name, laddr, raddr, status, proto))

        self.net_table.setRowCount(len(rows))
        for i, (pid, name, laddr, raddr, status, proto) in enumerate(rows):
            self.net_table.setItem(i, 0, QTableWidgetItem(str(pid)))
            self.net_table.setItem(i, 1, QTableWidgetItem(name))
            self.net_table.setItem(i, 2, QTableWidgetItem(laddr))
            self.net_table.setItem(i, 3, QTableWidgetItem(raddr))
            self.net_table.setItem(i, 4, QTableWidgetItem(status))
            self.net_table.setItem(i, 5, QTableWidgetItem(proto))
    ...
    def _build_updates_tab(self) -> QWidget:
        ...
        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("Show Recent Windows Update Log Tail")
        btn_refresh.clicked.connect(self.refresh_updates)
        btn_row.addWidget(btn_refresh)
        btn_row.addStretch()

        self.updates_view = QPlainTextEdit()
        self.updates_view.setReadOnly(True)

        layout.addLayout(btn_row)
        layout.addWidget(self.updates_view)
        return w

    def refresh_updates(self):
        text = get_windows_updates_text()
        self.updates_view.setPlainText(text)
        self.log("Refreshed Windows Update log tail")
    ...
    def _build_tasks_tab(self) -> QWidget:
        ...
        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("Refresh Scheduled Tasks")
        ...
        layout.addLayout(btn_row)
        layout.addWidget(self.tasks_view)
        return w

    def refresh_tasks(self):
        text = list_scheduled_tasks_text()
        self.tasks_view.setPlainText(text)
        self.log("Refreshed scheduled tasks")
    ...
    def _build_drivers_tab(self) -> QWidget:
        ...
        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("Refresh Drivers")
        btn_refresh.clicked.connect(self.refresh_drivers)
        btn_row.addWidget(btn_refresh)
        btn_row.addStretch()

        self.drivers_view = QPlainTextEdit()
        self.drivers_view.setReadOnly(True)
    ...
    def refresh_drivers(self):
        text = list_drivers_text()
        self.drivers_view.setPlainText(text)
        self.log("Refreshed drivers list")
    ...
    def _build_firewall_tab(self) -> QWidget:
        ...
        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("Refresh Firewall Rules")
        btn_refresh.clicked.connect(self.refresh_firewall)
        btn_row.addWidget(btn_refresh)
        btn_row.addStretch()

        self.firewall_view = QPlainTextEdit()
        self.firewall_view.setReadOnly(True)

        layout.addLayout(btn_row)
        layout.addWidget(self.firewall_view)
        return w

    def refresh_firewall(self):
        text = list_firewall_rules_text()
        self.firewall_view.setPlainText(text)
        self.log("Refreshed firewall rules")
    ...
    def _build_restore_tab(self) -> QWidget:
        ...
        btn_row = QHBoxLayout()
        btn_refresh = QPushButton("List Restore Points")
        btn_refresh.clicked.connect(self.refresh_restore)
        btn_create = QPushButton("Create Restore Point")
        btn_create.clicked.connect(self.create_restore_point_ui)
        btn_row.addWidget(btn_refresh)
        btn_row.addWidget(btn_create)
        btn_row.addStretch()

        self.restore_view = QPlainTextEdit()
        self.restore_view.setReadOnly(True)

        layout.addLayout(btn_row)
        layout.addWidget(self.restore_view)
        return w

    def refresh_restore(self):
        text = list_restore_points_text()
        self.restore_view.setPlainText(text)
        self.log("Refreshed restore points")

    def create_restore_point_ui(self):
        desc, ok = QInputDialogWithText.get_text(self, "Create Restore Point", "Description:")
        if not ok or not desc.strip():
            return
        text = create_restore_point_text(desc.strip())
        self.restore_view.setPlainText(text)
        self.log(f"Created restore point: {desc.strip()}")
    ...
    def _build_events_tab(self) -> QWidget:
        ...
        top_row = QHBoxLayout()
        self.events_log_combo = QComboBox()
        self.events_log_combo.addItems(["System", "Application", "Security"])
        btn_refresh = QPushButton("Refresh Events")
        btn_refresh.clicked.connect(self.refresh_events)
        top_row.addWidget(QLabel("Log:"))
        top_row.addWidget(self.events_log_combo)
    ...
    def refresh_events(self):
        log_name = self.events_log_combo.currentText()
        text = list_event_log_text(log_name=log_name, max_entries=200)
        self.events_view.setPlainText(text)
        self.log(f"Refreshed event log: {log_name}")
