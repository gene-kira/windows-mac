...
# =========================================================
# 1. GRID WORLD ENVIRONMENT (MULTI-AGENT + ASCII VIEW)
# =========================================================
...
class GridWorldEnv:
    ...
    def _get_obs(self):
        """
        Returns an array of shape [num_agents, 4]:
        [ax, ay, tx, ty] normalized to [0,1]
        """
        obs_list = []
        for (ax, ay), (tx, ty) in zip(self.agent_positions, self.target_positions):
            ...
            txn = tx / (self.size - 1)
            tyn = ty / (self.size - 1)
            obs_list.append([axn, ayn, txn, tyn])
    ...
    def ascii_view(self, max_agents_display=4):
        ...
        for idx, (tx, ty) in enumerate(self.target_positions[:max_agents_display]):
            if grid[ty][tx] == ".":
                ...
                # agent and target overlap (should mean success recently)
                grid[ty][tx] = "*"
...
# =========================================================
# 2. NEURAL BRAIN (APPROXIMATED Q-FUNCTION)
# =========================================================

class SimpleBrain(nn.Module):
    """
    Approximates Q(s, a) for 4 actions.
    Input: 4 floats (agent_x, agent_y, target_x, target_y) normalized.
    Output: 4 Q-values.
    """
...
class MemoryManager:
    def __init__(self, brain, config_path=None):
        ...
        if config_path is None:
            ...
            config_dir = os.path.join(home, ".grid_organism")
    ...
    def save_brain(self, extra_meta=None):
        ...
        meta = {
            "saved_at": now,
            "model_type": "SimpleBrainGridMulti",
        }
        if extra_meta:
...
# =========================================================
# 7. TRAINING LOOP (MULTI-AGENT, Q-LIKE)
# =========================================================
...
def training_loop(env: GridWorldEnv, brain: SimpleBrain,
    ...
    while not stop_event.is_set():
        ...
        while not done_env and not stop_event.is_set():
            # store ascii snapshot occasionally
            if env.steps % 10 == 0:
                stats.set_ascii(env.ascii_view())

            obs_tensor = torch.from_numpy(obs)  # [N,4]
            num_agents = obs.shape[0]

            with torch.no_grad():
                q_values = brain(obs_tensor)  # [N,4]
            ...
            next_obs, rewards, dones, done_env = env.step(actions)
            next_obs_tensor = torch.from_numpy(next_obs)

            with torch.no_grad():
                q_next = brain(next_obs_tensor)  # [N,4]
                max_q_next, _ = torch.max(q_next, dim=1)  # [N]

            q_current = brain(obs_tensor)  # [N,4]
            q_chosen = q_current[range(num_agents), actions]  # [N]
        ...
        stats.update_skill("grid_navigation_multi", perf)
...
def main():
    print("===========================================")
    print("  GRID ORGANISM - FULL PYTHON NODE START")
    print("===========================================")
