...
MEMORY_FILE = "roaming_spot_memory.json"
...
class LearningState:
    def __init__(self, memory: MemoryManager, history_size=50):
        ...
        # load template stats from memory
...
class SherlockDogEngine:
    def __init__(self, log: NarrativeLog, learning: LearningState):
        self.iteration = 1
        # seed question tries to honor focus theme if any
...
class RoamingSpotGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Sherlock & Curious Dog — Roaming Spot (Auto + Chameleon + Memory)")
        ...
        tk.Label(header, text="FOREST REACHES — The Question That Remembers",
        ...
        right = tk.Frame(top, bg="#0b0b0b", width=420)
        ...
        tk.Label(right, text="Memory (journeys & failures)", font=("Helvetica", 12, "bold"),
                 fg="#eeeeee", bg="#0b0b0b").pack(anchor="w", padx=6, pady=(6, 2))
    ...
    def _render_memory(self, limit=30):
        ...
        lessons = self.memory.data.get("lessons", [])[-limit:]
        ...
        self.memory_text.insert("end", "\nLessons:\n")
        for l in lessons:
            self.memory_text.insert("end", f"- [{l.get('time','')}] {l.get('lesson','')}\n")
    ...
    def save_memory(self):
        self.memory.save()
        self.pulse.config(text=f"Memory saved → {MEMORY_FILE}")
