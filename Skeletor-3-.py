...
def ewma_forecast(series: List[float], alpha: float = 0.35, steps: int = 60) -> Tuple[List[float], List[float], List[float]]:
    if not series:
        base = [0.5]*steps
    ...
    lower = [max(0.0, level - 0.8*std*(1 + i/steps)) for i in range(steps)]
    upper = [min(1.0, level + 0.8*std*(1 + i/steps)) for i in range(steps)]
    return forecast, lower, upper

def linear_trend_forecast(series: List[float], steps: int = 60) -> Tuple[List[float], List[float], List[float]]:
    n = len(series)
    if n < 3:
        base = [series[-1] if series else 0.5]*steps
        return base, base, base
    W = min(180, n)
    xs = list(range(W))
    ys = series[-W:]
    ...
    for k in range(1, steps+1):
        ...
        y = min(1.0, max(0.0, y))
        out.append(y)
        band = 0.9*rstd*(1 + k/steps)
        lower.append(max(0.0, y - band))
        upper.append(min(1.0, y + band))
    return out, lower, upper

def hybrid_forecast(series: List[float], steps: int = 60) -> Tuple[List[float], List[float], List[float]]:
    f1, l1, u1 = ewma_forecast(series, steps=steps)
    f2, l2, u2 = linear_trend_forecast(series, steps=steps)
    f = [min(1.0, max(0.0, 0.6*a + 0.4*b)) for a, b in zip(f1, f2)]
    lo = [min(1.0, max(0.0, 0.6*a + 0.4*b)) for a, b in zip(l1, l2)]
    hi = [min(1.0, max(0.0, 0.6*a + 0.4*b)) for a, b in zip(u1, u2)]
    return f, lo, hi

def risk_from_jitter(series: List[float], window: int = 120) -> float:
    if len(series) < 3:
        return 0.0
...
class AutonomousLightDark:
    def __init__(self, channels: List[str], state_path: str = "light_dark_state.json", log_path: str = "light_dark_telemetry.jsonl"):
        ...
        self.history: Dict[str, RingBuffer] = {ch: RingBuffer(8192) for ch in channels}
        self.last_states: Dict[str, Illumination] = {ch: Illumination.AMBIENT for ch in channels}
        self.conf_trends: Dict[str, RingBuffer] = {ch: RingBuffer(2048) for ch in channels}
        self.anomaly_density: RingBuffer = RingBuffer(2048)

        self.noise: float = 0.008
        self.rate_hz: int = 10
        ...
        # Prediction buffers (longer horizon)
        self.pred_horizon = 120  # ~12s at 10Hz
        self.pred_series: Dict[str, List[float]] = {ch: [] for ch in channels}
        self.pred_bands: Dict[str, Tuple[List[float], List[float]]] = {ch: ([], []) for ch in channels}  # (lower, upper)
        self.pred_conf: Dict[str, List[float]] = {ch: [] for ch in channels}
        self.pred_anomaly_risk: float = 0.0

        # Future thought queue
    ...
    def _run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Advanced predictions (longer horizon + confidence bands)
                self.pred_series = {}
                self.pred_bands = {}
                self.pred_conf = {}
                for ch in self.channels:
                    s = self.history[ch].tail(300)
                ...
                self.pred_anomaly_risk = risk_from_jitter(self.anomaly_density.to_list(), window=120)

                # Future thoughts (early warnings)
...
# =========================
# Compact scientific GUI with predictive overlays, confidence bands, risk gauges, future thoughts
# =========================
...
class ScientificGUI:
    def __init__(self, engine: AutonomousLightDark):
        ...
        for i, ch in enumerate(engine.channels):
            ...
            # Inline risk gauge per channel (sparkline-style)
        ...
        # Analytics: timelines + heatmap + anomaly bar (compact) + future thoughts list
        ...
        self.future_list = tk.Listbox(self.analytics, height=5)
        self.future_list.pack(fill="x", padx=4, pady=4)
    ...
    def refresh(self):
        if self.payload:
            self.status_text.set(f"Loop={self.payload['loop_ms']:.1f}ms | Bottleneck={self.payload['bottleneck_ms']:.1f}ms | Strategy={self.payload['strategy']} | Health={self.payload['health']:.2f} | Risk={self.payload.get('pred_anomaly_risk',0.0):.2f}")
            ...
            # Update risk gauges per channel using last predicted conf value
            for ch in self.engine.channels:
                g = getattr(self, f"gauge_{ch}")
                self.draw_gauge(g, self.payload["pred_conf"].get(ch, []))

            # Series + confidence + prediction overlays + bands
            for ch in self.engine.channels:
    ...
    def draw_series(self, canvas: tk.Canvas, series: List[float], th: Thresholds, pred: List[float], bands: Tuple[List[float], List[float]]):
        ...
        # Prediction band (confidence band shaded) — use solid color (no alpha)
        lo, hi = bands
    ...
    def draw_heatmap(self, canvas: tk.Canvas, series_map: Dict[str, List[float]]):
        ...
        for r, ch in enumerate(channels):
            ...
            for c in range(cols):
                v = (vals[c] - vmin) / rng
                if v < 0.35: color = f"#{int(30+v*100):02x}44cc"   # e.g., #3e44cc
                elif v > 0.65: color = f"#ff{int(120+v*100):02x}66" # e.g., #ffb666
                else: color = f"#aa66{int(120+v*100):02x}"          # e.g., #aa66b6
