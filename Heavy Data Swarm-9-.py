...
# ============================================================
# ROUTE HISTORY, ANOMALY, VOLATILITY & FINGERPRINTS
# ============================================================
class RouteHistory:
    def __init__(self):
    ...
    def add_sample(self, status: RouteStatus):
        ...
        if anomaly > 0.4:
            ...
            self._cluster_anomalies(status.name)
    ...
    def _record_anomaly_fingerprint(self, status: RouteStatus, anomaly: float):
        ...
        fp_list.append(fp)
        if len(fp_list) > 50:
            self.anomaly_fingerprints[status.name] = fp_list[-50:]

    def _cluster_anomalies(self, route_name: str):
        """
        Very simple clustering:
        - Group anomalies into 'low', 'medium', 'high' buckets based on latency+loss+congestion.
        - This is a placeholder for future ML-based clustering.
        """
        ...
        for fp in fps:
            ...
            if score < 40:
                clusters["low"].append(fp)
            elif score < 70:
                clusters["medium"].append(fp)
            else:
                clusters["high"].append(fp)
        self.anomaly_clusters[route_name] = [
            {"bucket": k, "count": len(v)} for k, v in clusters.items()
        ]
...
class QueenDecisionLog:
    ...
    def record_decision(self, chosen: RouteStatus, all_routes: Dict[str, RouteStatus]):
        reason = (
            ...
            f"and lowest comparative score among acceptable routes."
        )
        self.entries.append(
            DecisionEntry(
                timestamp=time.time(),
...
# ============================================================
# MULTI-ROUTE COMPARATIVE SCORING
# ============================================================
def comparative_route_score(
    ...
    Lower is better.
    Combines:
    - Current latency/loss/congestion
    - Threat risk (if available)
    - Bandit score (RL)
    """
    ...
    rl_bonus = bandit_score(arm) * 30.0
...
def choose_best_route_with_stance(
    ...
    scored = [(comparative_route_score(r, queen, projections), r) for r in candidates]
...
# ============================================================
# PREEMPTIVE REROUTE BRAIN (COOLDOWN + STANCE THRESHOLDS + COMPARATIVE)
# ============================================================
    ...
    def maybe_preemptive_reroute(
        ...
        for name, status in snapshot.items():
            ...
            if p.risk_score < cur_proj.risk_score and p.projected_color_30s in (
                ...
                score = comparative_route_score(status, self.queen, projections)
                safer_candidates.append((score, status, p))

        if not safer_candidates:
            return None

        safer_candidates.sort(key=lambda x: x[0])
        new_status = safer_candidates[0][1]
        new_proj = safer_candidates[0][2]

        self.last_reroute_time = now
        reason = (
            f"Preemptive reroute due to high risk on {current_best.name} "
            f"(risk={cur_proj.risk_score:.2f}, anomaly={cur_proj.anomaly:.2f}, "
            f"stance={self.queen.stance.value}) -> "
            f"{new_status.name} (risk={new_proj.risk_score:.2f}, "
            f"comparative_score={safer_candidates[0][0]:.1f})"
        )
