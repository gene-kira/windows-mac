...
# =========================
# Utilities: ring buffer, atomic write, structured logging (async)
# =========================
...
# =========================
# Autonomous engine (threaded prediction + causality + strategy stack)
# =========================
class AutonomousLightDark:
    def __init__(self, channels: List[str], state_path: str = "light_dark_state.json", log_path: str = "light_dark_telemetry.jsonl"):
        ...
        # Thread pool for predictions + scenarios + causality
        ...
        self.causal_links: List[Tuple[str, str, int, float]] = []
    ...
    def _schedule_forecasts(self):
        for ch in self.channels:
            series = self.history[ch].tail(1500)
            # Short
            self.futures_forecast[ch]["S"] = self.executor.submit(forecast_short, series, self.pred_horizons["S"])
            # Mid
            self.futures_forecast[ch]["M"] = self.executor.submit(forecast_mid, series, self.pred_horizons["M"])
            # Long
            self.futures_forecast[ch]["L"] = self.executor.submit(forecast_long, series, self.pred_horizons["L"])
    ...
    def _schedule_causality(self):
        ...
        self.future_causality = self.executor.submit(self._compute_causality, pairs, data)

    @staticmethod
    def _compute_causality(pairs: List[Tuple[str,str]], data: Dict[str, List[float]]):
        links = []
        for a,b in pairs:
            la, lb = data.get(a, []), data.get(b, [])
            lag, corr = lagged_corr(la, lb, max_lag=60)
            if abs(corr) > 0.25:
                links.append((a, b, lag, corr))
        return links
    ...
    def _schedule_scenarios(self, metrics: Dict[str, float]):
        self.future_scenarios = self.executor.submit(simulate_scenarios, {"cpu": metrics["cpu"], "mem": metrics["mem"], "net": metrics["net"], "load": metrics["load"]})
    ...
    def _run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Future thoughts (use short/mid horizon)
...
# =========================
# Compact scientific GUI (decoupled via Queue, slower refresh)
# =========================
...
class ScientificGUI:
    def __init__(self, engine: AutonomousLightDark):
        self.engine = engine
        self.root = tk.Tk()
        self.root.title("Predictive Skeleton Monitor (Threaded, Tri-horizon)")
        self.root.geometry("820x600")
        self.root.configure(bg="#111111")

        # Decoupled payload queue (engine -> GUI)
        ...
        self.root.after(480, self.refresh)  # slow by ~4
        self.engine.start()

    def _enqueue_payload(self, payload: dict):
        # Only keep most recent payload (drop older if queue full)
        try:
