...
"""
Foresight: cross-platform, autonomous, secure habit learner with multi-step forecasting,
context layering, adaptive decay, cluster generalization, explainability, encrypted persistence,
and intrusion detection — with a compact Tk GUI hardened against unpacking errors.
"""
...
class NextGenPredictor:
    ...
    def _score_snapshot(self, prev1: Optional[str], prev2: Optional[str], hour: int, dow: int, weekend: int,
                        habits: Dict[str, HabitStats]) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]]]:
    ...
    def predict(self, prev1: Optional[str], prev2: Optional[str], hour: int, dow: int, weekend: int,
        ...
        try:
            scores, components = self._score_snapshot(prev1, prev2, hour, dow, weekend, habits)
    ...
    def predict_plan(self, prev1: Optional[str], prev2: Optional[str], hour: int, dow: int, weekend: int,
                     habits: Dict[str, HabitStats], system_load_bucket: int, length: int = 3) -> List[Tuple[str, float]]:
        """
        Generate a short plan (next N steps). Iterative rollout using current transitions/context.
        """
        plan: List[Tuple[str, float]] = []
        p1, p2 = prev1, prev2
        try:
            for _ in range(max(1, length)):
                scores, _ = self._score_snapshot(p1, p2, hour, dow, weekend, habits)
                nxt, conf = self._choose(scores)
                if not nxt:
                    break
                plan.append((nxt, conf))
                p2, p1 = p1, nxt  # advance context window
            return plan
        except Exception:
            return plan
    ...
    def predict_next_plan(self, when_ts: Optional[float] = None, length: int = 3) -> List[Tuple[str, float]]:
        try:
            ...
            with self._lock:
                ...
                prev2 = prev2 if isinstance(prev2, str) and not prev2.startswith("advisory:") else None
                bucket = self._system_load_bucket()
            return self.predictor.predict_plan(prev1, prev2, hour, dow, weekend, self.habits, bucket, length=length)
        except Exception:
            return []
...
# ----------------------------
# GUI (compact) with safe predictor unpacking and plan display
# ----------------------------
class ForesightGUI:
    ...
    def _build_layout(self):
        ...
        # Right: Next steps plan
        right = ttk.LabelFrame(container, text="Next steps plan", padding=6)
        ...
        self.plan_tree.column("step", width=180, anchor="w")
    ...
    def _update_plan(self):
        try:
            ...
            # Plan errors shouldn't break UI
            pass
