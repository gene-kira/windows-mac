...
- RAM + Disk tiers
- Predictive engine (n-grams, heat, families)
...
- Policy brain:
    ...
    - heat_half_life
- Health score:
    score = f(hit_ratio, latency, ram_pressure, disk_pressure)
- Learning loop:
    - periodically perturbs policy
    - evaluates health
    - keeps or reverts
- Auto-tune (can be toggled)
- Tkinter GUI:
...
def record_access_and_predict(key: str) -> None:
    ...
    if policy.prefetch_enabled and policy.prefetch_fanout > 0:
        likely_next = predict_next_keys(history, current_key=key, limit=policy.prefetch_fanout)
        ...
        if fam:
            ...
            for fk in fam_keys[:policy.prefetch_fanout]:
                if fk != key:
                    try:
...
# -----------------------------
# Learning loop (policy experiments)
# -----------------------------
...
def perturb_policy(base: Policy) -> Policy:
    ...
    for attr, low, high, step in [
        ...
        if random.random() < 0.4:
            ...
            if attr == "max_ram_items" or attr == "base_ttl":
...
app = FastAPI(title="Autonomous Predictive Cache Server", version="5.0.0")
    ...
    def refresh_stats(self):
        ...
        for rec in data.get("experiments", []):
            ...
            self.exp_list.insert(
                tk.END,
                f"{ts} [{status}] Δhealth={dh:.3f} fanout {rec['before']['prefetch_fanout']}→{rec['after']['prefetch_fanout']} "
