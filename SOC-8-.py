#!/usr/bin/env python3
"""
Hive Guardian - ASI Queen v10 (Self-contained Online Learning + Admin Tab)
...
class QueenBrain:
    def __init__(self, brain_path: Optional[str] = None):
        ...
        self.ml_learning_rate: float = 0.01
    ...
    def _update_ml_weights(self, risk: float, features: List[float], action: str):
        ...
        # Simple supervision from actions:
        # - QUARANTINE/KILL should correspond to higher risk (>= 6)
        # - WATCH should correspond to lower risk (<= 4)
        desired = None
        ...
        error = desired - risk  # positive if risk too low, negative if too high
        # Small learning rate, scaled by magnitude of error but clipped
        lr = self.ml_learning_rate
        error = max(-5.0, min(5.0, error))

        for i in range(len(self.ml_weights)):
            self.ml_weights[i] += lr * error * features[i]
    ...
    def _compute_risk_features(self, node: NodeInfo) -> Tuple[List[float], float, float]:
        """Return (features, base_risk_without_ml, trust_score)."""
        name = node.extra.get("name", node.label)
        profile = self.process_profiles.get(name)
        cpu = node.cpu
        mem = node.memory
...
class HiveGUI:
    ...
    def _build_threats_tab(self):
        ...
        self.tree_threats = ttk.Treeview(
            self.threats_frame, columns=columns, show="headings", height=20
        )
        for col in columns:
        ...
        self.tree_threats.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)
