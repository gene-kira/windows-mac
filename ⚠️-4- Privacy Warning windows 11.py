...
def launch_gui():
    ...
    timer_interval = tk.IntVar(value=10000)  # Default: 10s
    ...
    tk.Label(root, text="Auto Ritual Interval (ms):", font=("Consolas", 10)).pack()
    tk.Entry(root, textvariable=timer_interval, font=("Consolas", 10), width=10).pack()

    tk.Label(root, text="Presets:", font=("Consolas", 10)).pack(pady=5)
    preset_frame = tk.Frame(root)
    preset_frame.pack()
    tk.Button(preset_frame, text="10s", command=lambda: set_preset(10000), width=6).pack(side=tk.LEFT, padx=2)
    tk.Button(preset_frame, text="30s", command=lambda: set_preset(30000), width=6).pack(side=tk.LEFT, padx=2)
