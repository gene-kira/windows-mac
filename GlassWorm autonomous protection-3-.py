# === 🔄 AUTOLOADER ===
try:
    import os, time, json, re, hashlib, psutil, socket
...
C2_ENDPOINTS = ["api.mainnet-beta.solana.com", "calendar.google.com"]
TRUSTED_HASHES = {
    "extensionA": "abc123...",  # Replace with real hashes
    "extensionB": "def456..."
}
...
def evaluate_threat(path, data):
    if AUTONOMOUS_MODE:
        if data["threat_level"] >= 8 or data["origin"] in blocked_countries or not data.get("trusted", True):
            purge_extension(path)
            print(f"[Autonomous] Purged {path} due to threat level, origin, or hash mismatch.")
    else:
        print(f"[Manual] Threat detected: {path} | Level: {data['threat_level']} | Origin: {data['origin']}")

# === 🌐 ORIGIN PARSER ===
def extract_origin_from_manifest(path):
    try:
        with open(path, 'r', encoding='utf-8') as f:
            ...
            if "github.com" in repo:
                ...
                if len(parts) > 2:
                    user = parts[1]
                    return "US" if user.endswith("corp") else "unknown"
    except:
        pass
    return "unknown"

# === 🧬 HASH VERIFICATION ===
def verify_extension_hash(path):
    try:
        with open(path, 'rb') as f:
            content = f.read()
            hash_val = hashlib.sha256(content).hexdigest()
            return hash_val in TRUSTED_HASHES.values()
    except:
        return False

# === 🔐 ENCRYPTED TRAFFIC DETECTION ===
def detect_encrypted_traffic():
        ...
        if conn.status == 'ESTABLISHED' and conn.raddr:
            try:
                ...
                for c2 in C2_ENDPOINTS:
                    if c2 in hostname:
                        print(f"[Firewall] Suspicious traffic to {hostname}")
                        sync_event_bus("c2_detected", {"host": hostname})
                        return hostname
            except:
                continue
    return None
            ...
            if file.endswith(('.js', '.json')):
                ...
                try:
                    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        if anomalies:
                            origin = extract_origin_from_manifest(path)
                            trusted = verify_extension_hash(path)
                            glyph_cache[path] = {
...
# === 🧩 GUI PANEL ===
class CodexSentinelOverlay(QWidget):
    def __init__(self, glyph_cache, purge_callback):
        super().__init__()
        self.setWindowTitle("🧿 CodexSentinel Overlay")
    ...
    def refresh_list(self):
        ...
        for path, data in glyph_cache.items():
            trust = "Trusted" if data.get("trusted") else "Unverified"
            item = QListWidgetItem(f"{path} | Threat: {data['threat_level']} | Origin: {data['origin']} | {trust}")
...
def main():
    ...
    print("[CodexSentinel] Scanning extensions...")
    scan_extensions(extension_dir)

    print("[CodexSentinel] Monitoring encrypted traffic...")
    detect_encrypted_traffic()

    app = QApplication([])
    panel = CodexSentinelOverlay(glyph_cache, purge_extension)
    panel.show()
    app.exec_()
