...
class LocalONNXLlm:
    ...
    def ask(self, prompt, context=None):
        ...
        # Placeholder – real implementation would tokenize and run session
...
class LLMEngineManager:
    """
    Unbounded multi-LLM manager.
    - engines: name -> engine object (must implement .ask(prompt, context=None))
    - roles: role -> [engine names]
    - tiers: tier -> [engine names] (optional, for fast/deep/strategic)
    - fusion_engine_name: engine used to fuse multi-LLM answers
    """

    def __init__(self):
        self.lock = threading.Lock()
        self.engines = {}          # name -> engine
        self.engine_meta = {}      # name -> {"roles": [...], "tier": "..."}
        self.roles = {}            # role -> [names]
        self.tiers = {}            # tier -> [names]
        self.active_engine = None  # default single engine
        self.active_engines = []   # multi-selection from GUI
...
class EDRConsole(QMainWindow):
    ...
    # --- Multi-LLM explain / compare two processes ---

    def on_proc_double_click(self, row, col):
        # keep double-click as quick local explanation popup (non-LLM)
        vals = []
        for c in range(self.proc_table.columnCount()):
            item = self.proc_table.item(row, c)
            vals.append(item.text() if item else "")
    ...
    # --- Campaign incident report (LLM role: campaign) ---
...
def main():
    ...
    # Register engines – you can add as many as you want here
    # Copilot as fusion + general-purpose
    llm_manager.register_engine(
        "copilot",
        CopilotEngine(),
        roles=["fusion", "killchain", "campaign", "anomaly", "prediction"],
        tier="strategic",
        is_fusion=True,
    )

    # Optional local ONNX model
