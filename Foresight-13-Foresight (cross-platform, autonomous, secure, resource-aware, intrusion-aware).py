...
intrusion detection, and auto-play — cross-platform with a compact Tk GUI.
...
- Auto-play: executes high-confidence steps automatically with a safety delay and cancel option.
- Persistent encrypted cache including user action map and assistant settings.
...
# ----------------------------
# Foresight engine (encrypted cache + security + alerts + persistent predictor + actions + auto-play)
# ----------------------------
class Foresight:
    def __init__(self, cache_file: str = "foresight_cache.bin"):
        ...
        # Auto-play settings persisted
        self.auto_play_enabled: bool = False
        self.auto_play_threshold: float = 0.8  # 80%
    ...
    # -------- Live assistant: actions + auto-play settings --------
...
# ----------------------------
# Background scheduler with auto-play trigger
# ----------------------------
class Scheduler(threading.Thread):
    ...
    def _auto_execute(self):
        ...
        if self.gui:
            ...
            self.gui.status_var.set(("Auto-executed and recorded." if executed else "Auto-recorded.") + " Plan refreshed.")
    ...
    def maybe_autoplay(self, suggestion: Optional[str], confidence: float):
        with self.fs._lock:
            ...
            delay = self.fs.auto_play_delay_sec
        if not enabled or not suggestion:
            return
        if confidence < threshold:
            return
        # schedule auto-execution with cancel option
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                ...
                # Preview suggestion for auto-play
...
# ----------------------------
# GUI (live assistant + forecasting + auto-play + hardened handling)
# ----------------------------
class ForesightGUI:
    ...
    def _build_layout(self):
        ...
        self.plan_tree = ttk.Treeview(midR, columns=("step", "confidence"), show="headings", height=14)
        self.plan_tree.heading("step", text="Step")
        self.plan_tree.heading("confidence", text="Confidence")
        self.plan_tree.column("step", width=240, anchor="w")
        ...
        # Right: Auto-play settings summary
