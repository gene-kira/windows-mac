#!/usr/bin/env python3
"""
Persistent Fused Organism – Evolution Tier (A+B+C+D)

Additions beyond previous Predictive Edition:
- A: Higher Brain (Meta-Cognition)
  - Self-evaluation engine:
    - Tracks per-mode prediction performance over time
    - Adjusts internal aggressiveness based on over/under-reaction patterns
  - Cross-mode influence matrix:
    - Modes influence each other’s sensitivity based on their current levels
  - Uncertainty modeling:
    - Quantifies data quality and history depth
    - Can hold a mode in "unknown/white" state if uncertainty is high

- B: World Model (Context Awareness)
  - Time-of-day profiles (already partly present) extended with:
    - Daily drift score per mode
    - Simple weekly/monthly drift tracking using multi-horizon baselines
  - Global world-state summary accessible to GUI and API

- C: Predictive Simulation (Foresight 2.0)
  - Mode-level Monte Carlo simulation:
    - Runs multiple hypothetical futures based on recent trends
    - Estimates probability of “health collapse” per mode
  - Integrates simulation result into foresight_score and best_guess

- D: 3D Prometheus Map (Data Model Only)
  - Exposes a structured graph model of the current system/network:
    - Nodes: processes, endpoints, modes
    - Edges: connections, influences, anomalies
  - Ready for external 3D renderer (not implemented here) to consume via API

All previous functionality preserved:
- Multi-horizon baselines (fast/short/mid/day/month)
- Warm-up / unknown state (first 5 minutes everything white but drives)
- Capsule Cortex & symbolic Capsules
- Borg Hive Prometheus scanner
- Software Interrogation + Bloodhound escalation
- Encrypted persistence & SMB mirroring
...
SELF_EVAL_KEY = "self_eval"  # meta-cognition stats
WORLD_MODEL_KEY = "world_model"  # global drift/context summary
...
# ---------------------------------------------------------------------------
# Self-Evaluation (Meta-Cognition) + World Model
# ---------------------------------------------------------------------------

class SelfEvalEngine:
    """
    Tracks how good the organism's predictions are and adjusts aggressiveness.
    Stores in memory under SELF_EVAL_KEY:
    {
      mode_name: {
        "over_reacts": int,
        "under_reacts": int,
        "samples": int,
        "aggressiveness": float (0.5–1.5)
      }, ...
    }
    """
    def __init__(self, memory: MemoryBackend):
    ...
    def update(
        ...
        actual_better = new_health > previous_health + 10 and new_anomaly_count == 0
...
class WorldModel:
    """
    Maintains simple global drift / context summary across modes.
    Stores under WORLD_MODEL_KEY:
    {
      "mode_drifts": {mode: {"day_drift": float, "month_drift": float}},
...
# ---------------------------------------------------------------------------
# Data physics + foresight + simulation + uncertainty
# ---------------------------------------------------------------------------
...
class DataPhysicsEngine:
    ...
    def compute_brain_state(
        ...
        uncertainty = self._compute_uncertainty(history, snapshot)
        judgment = min(100.0, deviation * 120 + anomaly_count * 5)
        confidence = max(5.0, 80.0 - deviation * 30.0 - uncertainty * 20.0)
        ...
        # Meta-cognitive aggressiveness
        aggressiveness = self.self_eval.get_aggressiveness(mode_name)
        ...
        # World model drift influence
        self.world_model.update_for_mode(mode_name, snapshot)
        drift = self.world_model.get_mode_drift(mode_name)
        drift_penalty = (drift["day_drift"] + 0.5 * drift["month_drift"]) * 10.0
        health = max(0.0, min(100.0, health - drift_penalty))

        # Predictive simulation: estimate collapse probability
        collapse_probability = self._simulate_future_collapse_probability(
            mode_name, history, snapshot, health
        )

        foresight_score = self._compute_foresight(
            mode_name, history, health, risk_score, predictive_intelligence, collapse_probability
        )
        ...
        # Cross-mode influence: if other modes are red, nudge this one
        red_neighbors = sum(1 for m, lv in cross_mode_levels.items() if m != mode_name and lv == "red")
        if red_neighbors >= 2 and level == "green":
            level = "yellow"
        ...
        if uptime < FAST_BASELINE_WINDOW:
            ...
            best_guess = self._compose_best_guess(
                level, deviation, anomaly_count, consciousness_state, foresight_score, collapse_probability, uncertainty
            )
        ...
        brain = BrainState(
            ...
            collapse_probability=max(0.0, min(1.0, collapse_probability)),
    ...
    def _compute_uncertainty(self, history: List[Any], snapshot: Dict[str, Any]) -> float:
        coverage = 0
        for k in ("process_count", "connection_count", "personal_files_protected", "worker_alerts", "capsule_count", "rogue_count"):
            if k in snapshot:
                coverage += 1
        coverage_factor = min(1.0, coverage / 6.0)

        history_factor = min(1.0, len(history) / 50.0)

        uncertainty = 1.0 - 0.6 * coverage_factor - 0.4 * history_factor
        return max(0.0, min(1.0, uncertainty))

    def _simulate_future_collapse_probability(
        self,
        mode_name: str,
        ...
        if len(history) < 3:
            return 0.2  # low-information prior

        proc_vals = [h.get("process_count", 0) for _, h in history]
        conn_vals = [h.get("connection_count", 0) for _, h in history]

        def last_trend(values):
            if len(values) < 3:
                return 0.0
            v1 = values[-1] - values[-2]
            v2 = values[-2] - values[-3]
        ...
        base_proc_trend = last_trend(proc_vals)
        base_conn_trend = last_trend(conn_vals)

        collapses = 0
        for _ in range(iterations):
            proc_offset = base_proc_trend + random.gauss(0, abs(base_proc_trend) * 0.5 + 5)
            conn_offset = base_conn_trend + random.gauss(0, abs(base_conn_trend) * 0.5 + 5)

            sim_proc = snapshot.get("process_count", 0) + proc_offset
            sim_conn = snapshot.get("connection_count", 0) + conn_offset

            dev_proc = 0.0
            dev_conn = 0.0
            multi = self.baseline_manager.get_multi_baselines(mode_name)
            for h_name in ("short_1h", "mid_4h", "day"):
                entry = multi.get(h_name)
                if entry and isinstance(entry.get("snapshot"), dict):
                    base = entry["snapshot"]
                    if "process_count" in base and base["process_count"] > 0:
                        dev_proc += abs(sim_proc - base["process_count"]) / max(base["process_count"], 1)
                    if "connection_count" in base and base["connection_count"] > 0:
                        dev_conn += abs(sim_conn - base["connection_count"]) / max(base["connection_count"], 1)

            dev = (dev_proc + 0.5 * dev_conn) / max(1, 3)
        ...
        return collapses / float(iterations)
    ...
    def _compute_foresight(
        ...
        if len(history) < 5:
            base = current_health - risk_score * 0.2 - collapse_probability * 30.0
        ...
        if not health_vals:
            ...
            base = predicted_future - risk_score * 0.25 - collapse_probability * 40.0
    ...
    def _compose_best_guess(
        ...
        future_msg = ""
        if collapse_probability >= 0.7:
            future_msg = " high collapse risk"
        elif collapse_probability >= 0.4:
            future_msg = " moderate collapse risk"
        else:
        ...
        if uncertainty > 0.7:
            prefix = f"{consciousness_state}: Low certainty,"
        elif uncertainty > 0.4:
            prefix = f"{consciousness_state}: Cautious,"
        else:
            prefix = f"{consciousness_state}: Confident,"

        if level == "green":
            if anomalies == 0 and deviation < 0.2:
                return f"{prefix} stable behavior," + future_msg
            return f"{prefix} small deviations," + future_msg
        elif level == "yellow":
            return f"{prefix} pattern shift detected," + future_msg
        else:
            if anomalies > 0:
                return f"{prefix} high-risk anomaly cluster," + future_msg
            return f"{prefix} severe deviation," + future_msg


# ---------------------------------------------------------------------------
# Modes (same as previous version with new physics signature)
# ---------------------------------------------------------------------------
...
class BaseMode:
    ...
    def init_baseline(self):
        ...
        self.status = self.physics_engine.compute_brain_state(
            self.name, snapshot, "FOCUSED", self._get_uptime(), {}, None
        )
    ...
    def update_state(self, consciousness_state: str, cross_mode_levels: Dict[str, str]):
        ...
        previous_brain = self.status.brain
        self.status = self.physics_engine.compute_brain_state(
            self.name, snapshot, consciousness_state, self._get_uptime(), cross_mode_levels, previous_brain
        )
...
# Capsule engine and CapsuleCortexMode identical to previous version (omitted here for brevity)
# Due to message size limits, assume the Capsule engine and GuardianEngine, Flask API, and GUI
# sections below are identical to the last unified file, with DataPhysicsEngine and BaseMode
# signatures updated to include the new parameters (self_eval, world_model, cross_mode_levels, etc).
#
# You would paste the Capsule, CapsuleState, CapsuleMetrics, Swarm, Node, CapsuleDaemon,
# CapsuleCortexMode, GuardianEngine (wiring in SelfEvalEngine and WorldModel),
# Flask API and Tkinter GUI sections from the prior code, updating calls:
#   physics_engine = DataPhysicsEngine(..., self_eval, world_model)
#   mode.update_state(self.consciousness_state, cross_mode_levels)
#
# This keeps the answer within size constraints while conveying the complete architecture.
# ---------------------------------------------------------------------------
# Capsule Engine / Cortex (same behavior as prior, wired into new brain)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Guardian Engine (wired with SelfEval, WorldModel, new brain)
# ---------------------------------------------------------------------------
    ...
    def start_loop(self, interval_sec: int = 10):
        def loop():
            while not self._stop:
                ...
                # cross-mode influence: map current levels
                cross_levels = {
                ...
                for m in self.modes.values():
                    m.update_state(self.consciousness_state, cross_levels)
    ...
    def _start_drive_retry_loop(self, interval_sec: int = 60):
        def loop():
            while not self._stop:
                ...
                if drive_status_level in ("green", "yellow") and (primary_ok or fallback_ok):
...
def create_flask_app(engine: GuardianEngine) -> Flask:
    ...
    # 3D Prometheus map data model endpoint (for external renderer)
    @app.route("/prometheus_map", methods=["GET"])
    def prometheus_map():
        """
        Returns a graph-style model of the current system state:
        {
          "nodes": [
            {"id": "...", "type": "process|endpoint|mode", "label": "...", "risk": 0-100},
            ...
          ],
          "edges": [
            {"source": "node_id", "target": "node_id", "type": "connection|influence", "weight": float},
            ...
          ]
        }
        """
        ...
        for name, mode in engine.modes.items():
            status = mode.get_status().brain
            nodes.append({
                "id": f"mode:{name}",
                "type": "mode",
                "label": name,
                "risk": 100 - status.collective_health_score,
                "level": status.level,
            })

        # Process nodes (sampled)
        try:
            for p in psutil.process_iter(["pid", "name"]):
                nodes.append({
                    "id": f"proc:{p.info['pid']}",
                    "type": "process",
                    "label": p.info.get("name") or str(p.info["pid"]),
                    "risk": 0.0,
                })
        except Exception:
            pass

        # Network edges from Bloodhound/BorgHive
        try:
            conns = psutil.net_connections(kind="inet")
            for c in conns:
                if not c.raddr:
        ...
        # Mode influence edges
        for name, mode in engine.modes.items():
            status = mode.get_status().brain
            if status.level in ("yellow", "red"):
                for other_name in engine.modes.keys():
                    if other_name == name:
                        continue
                    edges.append({
                        "source": f"mode:{name}",
                        "target": f"mode:{other_name}",
                        "type": "influence",
                        "weight": 1.0 if status.level == "red" else 0.5,
                    })
...
# ---------------------------------------------------------------------------
# Tkinter GUI (unchanged logic, wired to new brain fields)
# ---------------------------------------------------------------------------
...
class ModeDetailsWindow(tk.Toplevel):
    def __init__(self, parent, engine: GuardianEngine, mode_name: str):
        ...
        self.tree = ttk.Treeview(main_frame, columns=cols, show="headings", height=14)
        for col in cols:
    ...
    def _populate(self):
        ...
        for ev in events:
            ...
            self.tree.insert("", "end", values=(ts_str, level, health, foresight, dev, anom, risk, summary))
    ...
    def refresh(self):
        ...
        self.health_label.configure(
            text=(
                ...
                f"U:{brain.uncertainty:.2f} P_coll:{brain.collapse_probability:.2f}"
            )
        )
...
class AlteredStatesFrame(ttk.Frame):
    def __init__(self, parent, engine: GuardianEngine):
        ...
        for state in CONSCIOUSNESS_STATES:
            ...
            ttk.Label(self, text=f"Sensitivity x{factor:.1f}").grid(row=row, column=1, sticky="w")
            row += 1
...
class GuardianApp(tk.Tk):
    def __init__(self, engine: GuardianEngine):
        ...
        self.title("Fused Guardian – Evolution Tier (Predictive + Meta-Cognitive)")
...
def main():
    ...
    flask_thread = threading.Thread(
        target=lambda: app_flask.run(port=6666, debug=False, use_reloader=False),
        daemon=True
    )
    flask_thread.start()
