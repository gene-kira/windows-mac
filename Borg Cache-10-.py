#!/usr/bin/env python
"""
The Borg Cache - Model A, Wave 1 + Wave 2 + Wave 3 + Wave 3.1 (Aggressive, Mesh, Defensive, Minimal Logging)
...
- BorgIO block-level RAM cache with:
  ...
  - predictive prefetch 3.0 (per-file adaptive + ML n-gram)
...
- Crash replay:
  ...
  - Last 100 BorgIO ops
- Wave 3:
  - ML prefetch model (n-gram, decay, cluster-aware)
...
- Wave 3.1:
  - Defensive mesh layer:
    - shared secret
    - HMAC signatures on mesh messages
    - basic trust scoring per peer
...
class HiveIdentity:
    ...
    version: str = "1.9.0-wave3.1-aggressive-mesh-defensive"
...
class BorgIO:
    def __init__(self, queen, max_cache_bytes: int = 128 * 1024 * 1024):
        ...
        # Wave 3 ML: per-file block history for n-gram model
    ...
    def _predictive_prefetch(self, key_path: str, block_index: int, prof: FileProfile):
        ...
        # Wave 3 ML prefetch: n-gram prediction
        cluster = self.queen.cluster_engine.get_cluster()
        hist = self._get_history(key_path)
        ml_predictions = self.queen.ml_model.predict_next_blocks(
            key_path, hist, cluster=cluster, max_n=4
        )
        for idx in ml_predictions:
            if (key_path, idx) in self.cache:
                continue
            pre = self._read_block_from_disk(key_path, idx)
            if pre is not None:
                ...
                p.record_access(idx)
                self._update_history(key_path, idx)
                self._log_op("ml_prefetch", key_path, idx, len(pre))
...
# ---------------- Wave 3: ML Prefetch Model (n-gram + decay + cluster-aware) ----------------

class MLPrefetchModel:
    def __init__(self, order: int = 3):
        self.order = order
        # key: (file_key, history_tuple) -> {next_block: weight}
        self.transitions: Dict[Tuple[str, Tuple[int, ...]], Dict[int, int]] = {}
        self.lock = threading.Lock()
    ...
    def observe(self, path: str, history: List[int], next_block: int, cluster: str | None = None):
        if not history:
            return
        key = (self._file_key(path, cluster), tuple(history[-self.order:]))
        with self.lock:
    ...
    def load(self, data: Dict[str, Any]):
        with self.lock:
            self.order = int(data.get("order", 3))
...
class MeshManager:
    ...
    def __init__(self, identity: HiveIdentity, shared_secret: str = "borg_mesh_secret"):
        ...
        self.conn_semaphore = threading.BoundedSemaphore(value=16)  # max concurrent TCP handlers
        self.last_sync: Dict[str, float] = {}
        self.sync_cooldown = 2.0  # seconds
        self.trust_scores: Dict[str, float] = {}  # node_id -> trust score
...
# ---------------- Wave 3: Remote Cockpit Server (with basic rate limiting) ----------------
...
class RemoteCockpitServer:
    ...
    def __init__(self, queen_get_overview_callable):
        ...
        self.client_cooldown = 0.2  # 5 req/s per IP
...
class QueenBorg:
    def __init__(self, config_path: str = "borg_cache_config.json"):
        ...
        self.ml_model = MLPrefetchModel(order=3)
    ...
    def _update_ml_model_from_ops(self):
        with self.io.lock:
            for op in self.io.op_log:
                if op["kind"] in ("read_hit", "read_miss", "prefetch", "ml_prefetch"):
                    path = op["path"]
                    block = op["block"]
                    key_path = path
...
def run_hive(telemetry: TelemetryBus, seg_telemetry: SegmentedTelemetry,
    ...
    queen = QueenBorg()
    print("The Borg Cache hive (Wave 3.1, Mesh, Defensive) is running. Ctrl+C to stop.")
    try:
        while True:
...
def run_cockpit_telemetry(telemetry: TelemetryBus, seg_telemetry: SegmentedTelemetry,
    ...
    print("The Borg Cache cockpit (telemetry mode, Wave 3.1). Ctrl+C to exit.")
    try:
        while True:
            ...
            if ov is None:
                os.system("cls" if os.name == "nt" else "clear")
                print("The Borg Cache cockpit (telemetry mode, Wave 3.1).")
...
def run_cockpit_hybrid(interval: float = 0.5):
    queen = QueenBorg()
    print("The Borg Cache cockpit (Hybrid mode, Wave 3.1). Ctrl+C to exit.")
    if HAS_MSVCRT:
...
def main():
    parser = argparse.ArgumentParser(description="The Borg Cache - Model A, Wave 3.1 (Aggressive, Mesh, Defensive)")
    parser.add_argument(
        "--mode",
