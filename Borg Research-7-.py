#!/usr/bin/env python3
# imagination_colony_tk_live_ultra_storage.py
...
# - GUI indicator of current storage path
# - Ultra project generator (streaming): 50 up to 4,000,000+ lines of Python per project

import os, sys, time, json, random, zlib, math, hashlib, signal, threading, logging, socket, subprocess
...
def choose_storage_dir(cfg: Dict) -> str:
    ...
    try:
        if sys.platform.startswith("win"):
            ...
            if psutil:
                ...
                for p in parts:
                    ...
                    if p.device.upper().startswith("C:"):
                        continue
                    # Skip removable/CD-ROM without mount or inaccessible
                    try:
            ...
            # Fallback to network drive if provided
    ...
    while not _shutdown_event.is_set():
        try:
            for step in range(cfg["iterations_per_tick"]):
                ...
                # Update storage dir frequently in case drive usage changes
                chosen_dir = choose_storage_dir(cfg)
                _set_storage_dir(chosen_dir)
                with _archive_lock:
                    archive["stats"]["storage_dir"] = chosen_dir
...
# ---------------------------
# Tkinter GUI with mute button, storage indicator, and code preview
# ---------------------------
