#!/usr/bin/env python3
# ============================================================
# SYSTEM NERVE CENTER – HybridBrain + Tri-Stance + MultiGame v3.2
...
# - Tkinter GUI: Brain Cortex + Reboot Memory + Reasoning Heatmap + B4B/Games + ONNX + Conversation
...
class GameAIConversationBridge:
    """
    Simulated AI-to-AI protocol between:
      - Virtual Game AI (speaker 1)
      - HybridBrain System AI (speaker 2)
    """
...
class GameDetector:
    ...
    GAME_MAP = {
        ...
        # World War Z (Epic) – adjust as needed
    ...
    def tick(self) -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        # 2) Fallback: unknown heavy process as "other_game"
        if not found:
            for p in psutil.process_iter(["name", "pid", "cpu_percent", "memory_info"]):
                try:
                    name = p.info["name"].lower()
...
class NerveCenterApp(tk.Tk):
    ...
    def _build_cortex_tab(self, parent):
        ...
        self.lbl_conv.grid(row=2, column=0, columnspan=2, sticky="w", padx=5, pady=2)
    ...
    def cmd_save_reboot_memory(self):
        ...
        meta = {"ts": time.time(), "version": "3.2-multigame-world-live-conv"}
    ...
    def _bg_loop(self):
        while True:
            try:
                ...
                # 3) World + game + live pattern ingestion (uses last risk)
                hour, weekday = local_time_components()
                ...
                # 7.5) Game AI ↔ System AI conversation
