#!/usr/bin/env python3
# ============================================================
# SYSTEM NERVE CENTER – HybridBrain + Tri-Stance + MultiGame v3
# - Multi-engine Best-Guess with regime-aware fusion
# - B4B + multi-game + world pattern + live events
# - Pattern memory (system + game + world + live), reinforcement, meta-states, integrity
...
# - GameDetector: multi-game (Steam/Epic) detection + profiles
# - Full B4B memory persistence (metrics, physics, episodes, game state)
# - World foresight via pattern memory + live events
...
def local_time_components() -> Tuple[int, int]:
    """Return (hour, weekday) in local time."""
    t = time.localtime()
...
class InteractionCoachOrgan:
    """
    Uses games as a training ground for human ↔ AI interaction.
    Records episodes (with game_id) and updates reinforcement.
    """
...
class PredictionEngine:
    ...
    - Multi-engine voting with:
        * EWMA/Trend/Variance/Turbulence
        * Reinforcement + Integrity + Meta-state
        * B4B physics + predicted outcome
        * World pattern + live events
        * Multi-game profile/load/pattern
    """
    ...
    def _engine_votes(self) -> Tuple[float, Dict[str, float], str, Dict[str, float]]:
        ...
        # B4B / game-specific contributions
        ...
        if game_id:
            ...
            if entry:
                raw = (entry.overload_count - entry.stable_count * 0.5 - entry.beast_win_count * 0.8) / 10.0
                game_pattern_score = clamp01(0.5 + raw)
            else:
        ...
        if entry:
            raw = (entry.overload_count - entry.stable_count * 0.5 - entry.beast_win_count * 0.8) / 10.0
            live_pattern_score = clamp01(0.5 + raw)
        else:
...
class NerveCenterApp(tk.Tk):
    ...
    def cmd_save_reboot_memory(self):
        ...
        meta = {"ts": time.time(), "version": "3.0-multigame-world-live"}
    ...
    def _bg_loop(self):
        while True:
            try:
                ...
                # World + game + live pattern ingestion
                hour, weekday = local_time_components()
                ...
                if running and not self.game_detector.last_state:
                    ...
                    print(f"[GameDetector] Game {gid} detected — profile loaded, auto-training engaged")
                    self.lbl_top_status.config(
                        text=f"Game {gid} detected — Training Active",
                        fg="#55ff55",
                    )
    ...
    def _update_b4b_panel(self):
        ...
        if self.game_detector.game_running:
            ...
            self.lbl_b4b_game.config(
                text=f"Game: {gid} RUNNING (pid={info.get('pid','?')}, cpu={info.get('cpu','?')}%)",
                foreground="#00cc66",
            )
        else:
