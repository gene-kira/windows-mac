...
class Asset:
    def __init__(self, asset_id, size_mb, kind="generic", meta=None):
        ...
        self.kind = kind                      # "texture", "map", "tab", "video", "file", ...
...
class SystemHealth:
    ...
    def record_access(self, asset, tier_name, t):
        ...
        if asset.id in self.last_evicted:
            ...
            if age < 5.0:  # within 5 seconds after eviction = regret
...
class AIGovernor:
    ...
    Hybrid brain:
      ...
      - Health feedback for resilience
    """

    def __init__(self, assets, tier_disk, tier_ram, tier_vram, mode="gaming", health=None, log_fn=None):
        self.assets = assets
        ...
        self.base_hot_threshold = 4.0 if mode == "gaming" else 3.0
        self.base_warm_threshold = 1.5 if mode == "gaming" else 1.2
        self.decay = 0.98 if mode == "gaming" else 0.995

        self.hot_threshold = self.base_hot_threshold
        self.warm_threshold = self.base_warm_threshold

        self.transition_counts = defaultdict(lambda: defaultdict(int))
        self.recent_accesses = deque(maxlen=10)
    ...
    def record_access_for_prediction(self, asset):
        if self.recent_accesses:
            prev = self.recent_accesses[-1]
            self.transition_counts[prev.id][asset.id] += 1
        self.recent_accesses.append(asset)

    def predicted_followers(self, asset_id, top_k=3):
        counts = self.transition_counts.get(asset_id, {})
        if not counts:
            return []
    ...
    def promotion_confidence(self, asset):
        base = self._base_asset_score(asset)
        followers = self.predicted_followers(asset.id, top_k=3)
        bonus = 0.0
    ...
    def execute_moves(self, moves):
        for asset, desired in moves:
            ...
            conf = self.promotion_confidence(asset)

            vram_util = self.tier_vram.used_mb / max(1.0, self.tier_vram.capacity_mb)
            if desired == "vram" and vram_util > 0.9 and conf < 0.6:
...
# ============================================================
#  REAL GAME ACCESS: PREFETCHER + SIMULATOR
# ============================================================

class GameSimulator:
    """
    For now still simulates accesses in gaming mode, but:
      - Assets are real game files if the profile has them.
      - Accesses drive the predictive brain.
    """
...
# ============================================================
#  REAL GAMING MODE: FILE SCANNER + PREFETCH
# ============================================================

def scan_game_folder(root_path, log_fn):
    """
    Scan a real game folder and create Assets from real files.
    Kind is inferred from extension and path.
    """
    assets = []
...
def prewarm_os_cache_for_assets(assets, count, log_fn):
    """
    Read the top-N assets from disk into memory to bias the OS page cache.
    """
    top = assets[:count]
    ...
    for a in top:
        ...
        try:
            with open(path, "rb") as f:
                _ = f.read(1024 * 1024)  # sample first MB to avoid excessive I/O
...
class MemoryGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Hybrid Memory Backbone - Gaming / General - Predictive VRAM Cache")
    ...
    def _select_profile_and_load(self):
        ...
        self._log(f"Mode: {mode} | Profile: {self.profile_name}")
    ...
    def _switch_mode(self, *_):
        if self.simulator:
            self.simulator.stop()
        self.save_profile()
        self._log("Switching mode; reloading profile with new brain.")
        self._reset_tiers()
        self._select_profile_and_load()
    ...
    def _init_ui(self):
        ...
        ttk.Radiobutton(mode_frame, text="Gaming Core", value="gaming",
                        variable=self.mode_var, command=self._switch_mode).pack(anchor="w")
        ...
        # Gaming controls (folder, exe, prewarm, launch)
