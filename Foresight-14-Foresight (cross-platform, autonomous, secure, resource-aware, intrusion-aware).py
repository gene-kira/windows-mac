...
"""
Foresight: live assistant + forecaster with advanced prediction, seasonality, confidence calibration,
encrypted persistence, intrusion detection, and safe auto-play — cross-platform with a compact Tk GUI.

Key capabilities:
- 7-step forecasting with iterative rollout.
- 3-step transition modeling (prev3→prev2→prev1→next) for deeper sequence learning.
- Seasonality detection: day-of-week, week-of-month, month-of-year biases.
- Confidence calibration: per-bin reliability adjustment from rolling accuracy history.
- Context layering: hour, day-of-week, weekend flag, system load.
- Hour-of-day smoothing and recency boosts.
- Cluster generalization: browser/IDE/office/terminal boosts.
- Live assistant controls: Accept, Skip, Up/Down reorder, Edit actions, Execute selected.
- Safe auto-play: executes only approved clusters or whitelisted habits with adaptive delay.
- Persistent encrypted cache including user action map, calibration, and assistant settings.
- Hardened GUI: safe handling of predictor outputs and user actions.
"""
...
# ----------------------------
# Confidence calibration (per-bin reliability)
# ----------------------------
class ConfidenceCalibrator:
    def __init__(self, bins: int = 10, history_max: int = 1000):
        self.bins = bins
        self.bin_correct = [0] * bins
        self.bin_total = [0] * bins
        self.history = deque(maxlen=history_max)  # (raw_conf, correct)
        self.alpha = 0.5  # blend raw and reliability
    ...
    def reliability(self, raw_conf: float) -> float:
        ...
        if total == 0:
            g_total = sum(self.bin_total)
            g_correct = sum(self.bin_correct)
            return (g_correct / g_total) if g_total > 0 else 0.5
        return self.bin_correct[idx] / total
...
# ----------------------------
# Next-gen predictor with seasonality, multi-step, context, recency, clustering
# ----------------------------
class NextGenPredictor:
    ...
    def _tod_smoothing(self, hour: int, habits: Dict[str, HabitStats]) -> Dict[str, float]:
        kernel = {0: 0.25, 1: 0.5, 2: 0.25}  # h-1, h, h+1
        scores = {}
    ...
    def predict_plan(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                     dt: datetime.datetime, habits: Dict[str, HabitStats], now_ts: float, length: int = 7) -> List[Tuple[str, float]]:
        ...
        try:
            for _ in range(max(1, length)):
                ...
                p3, p2, p1 = p2, p1, nxt  # advance window
            return plan
        except Exception:
            return plan
...
# ----------------------------
# Foresight engine: encrypted cache, security, calibration, actions, safe auto-play
# ----------------------------
class Foresight:
    def __init__(self, cache_file: str = "foresight_cache.bin"):
        ...
        self.auto_play_threshold: float = 0.8
        self.auto_play_delay_sec: float = 4.0
        self.auto_safe_clusters: Set[str] = {"browser", "office"}  # only these clusters auto-execute
        self.auto_whitelist_habits: Set[str] = set()  # specific habits allowed to auto-execute
        self._load_cache()
    ...
    def predict_next_plan(self, when_ts: Optional[float] = None, length: int = 7) -> List[Tuple[str, float]]:
        try:
            now_ts = when_ts if when_ts is not None else time.time()
            dt = datetime.datetime.fromtimestamp(now_ts)
            with self._lock:
...
class Scheduler(threading.Thread):
    def __init__(self, foresight: Foresight, gui_ref, stop_event: threading.Event):
        ...
        self.fs = foresight
        self.gui = gui_ref  # for cancel prompts
...
# ----------------------------
# GUI (live assistant + forecasting + auto-play + safe lists + hardened handling)
# ----------------------------
class ForesightGUI:
    ...
    def _build_layout(self):
        ...
        ttk.Label(top, textvariable=self.advisory_var, foreground="#b22222").grid(row=1, column=0, columnspan=4, sticky="w")
        ...
        self.plan_tree = ttk.Treeview(midR, columns=("step", "confidence"), show="headings", height=16)
        self.plan_tree.heading("step", text="Step")
        self.plan_tree.heading("confidence", text="Confidence")
        self.plan_tree.column("step", width=260, anchor="w")
    ...
    def _update_plan(self):
        try:
            plan = self.fs.predict_next_plan(length=7)
