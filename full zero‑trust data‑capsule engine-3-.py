...
class SwarmSyncOrgan:
    ...
    def register_peer(self, info: Dict[str, Any]):
        ...
        try:
            r = requests.post(f"{self.coord_url}/register", json=payload, timeout=3)
            r.raise_for_status()
            data = r.json()
    ...
    def push_policy(self, policy_blob: Dict[str, Any]):
        try:
            ...
            r = requests.post(f"{self.coord_url}/policy", json=payload, timeout=3)
            r.raise_for_status()
            data = r.json()
            version = data.get("version", 0)
            blob = data.get("policy", {})
...
class CapsuleRuntime:
    def __init__(self, swarm_id: str = "default-swarm"):
        ...
        self.swarm = SwarmSyncOrgan(
            ...
            coord_url=os.getenv("SWARM_COORD_URL", "https://swarm-coord.example.com"),
        )
        self.swarm.pull_policy()
