"""
Hive Guardian - Predictive Hive + Bandits + Rich Map (Single File)
------------------------------------------------------------------

This scaffold is an advanced, single-file prototype of a predictive, autonomous
"hive guardian" system.

Key features:
- Auto-elevation on Windows (run as Administrator)
- Cross-platform OS awareness (Windows / Linux / macOS)
- System-aware Node:
    - OS, hardware, processes, network connections
- Dual-drive Critical Cache with autonomous failover
    - Primary cache: user-selected folder at startup (subfolder: primary)
    - Secondary cache: user-selected folder at startup (subfolder: secondary)
- BehaviorStore (persistent brain per node):
    - Process sequences (what tends to follow what)
    - Time-of-day patterns (hour:weekday:process)
    - Prefetch reward memory (hits/misses)
    - Event timeline + n-gram prediction
    - Session/context fingerprints
- PrefetchEngine + PrefetchPolicy:
    - Uses behavior profiles, time patterns, event predictions,
      context fingerprints, global foresight hints
    - Bandit-style decision-making to choose which prefetches to execute
- AnomalyDetector:
    - Uses behavior fingerprints and novelty
- QueenState + QueenController:
    - Aggregates telemetry from nodes (in this file: one node)
    - Builds world connection graph (local node + external hosts)
    - Global foresight hints (top external endpoints)
    - Prediction quality metrics (hit rate, confidence)
- In-process async bus (telemetry, commands, foresight)
- Tkinter GUI:
    - Tab 1: Node Detail (system, cache, anomaly, prediction quality, bandit stats)
    - Tab 2: World Map (nodes + external hosts, risk-colored edges, thicker lines for heavy traffic)

Requirements:
    pip install psutil requests

Run:
    python hive_guardian_bandit.py
"""
...
# We will use Tkinter twice:
# - once at startup for folder picking (separate hidden root)
# - once later for the main GUI.
import tkinter as tk
from tkinter import ttk, filedialog

# =========================
# AUTO-ELEVATION CHECK (Best Version, Windows only)
# =========================
...
def ensure_admin():
    ...
    try:
        ...
        if platform.system().lower() != "windows":
            return
        # If not admin, relaunch elevated
        ...
        print(f"[Hive Guardian] Elevation failed: {e}")
        sys.exit()

ensure_admin()

# =========================
# Global cache base paths (set at startup via folder picker)
# =========================

PRIMARY_CACHE_BASE: Optional[str] = None
SECONDARY_CACHE_BASE: Optional[str] = None


def pick_cache_paths_at_startup():
    """
    Show two folder pickers at program startup:

    1) Primary cache base folder
    2) Secondary cache base folder (can be network/mapped drive/UNC)

    The actual cache paths will be:
      primary_path   = <primary_base>/primary
      secondary_path = <secondary_base>/secondary
    """
    global PRIMARY_CACHE_BASE, SECONDARY_CACHE_BASE

    # Use a temporary hidden root for folder dialogs
    root = tk.Tk()
    root.withdraw()

    # Primary cache base
    primary = filedialog.askdirectory(
        title="Select PRIMARY cache base folder (e.g. C:\\HiveCache)"
    )
    if not primary:
        # Fallback if user cancels
        primary = r"C:\HiveCache"
    PRIMARY_CACHE_BASE = primary

    # Secondary cache base
    secondary = filedialog.askdirectory(
        title="Select SECONDARY cache base folder (network drive/UNC allowed)"
    )
    if not secondary:
        # Fallback if user cancels
        secondary = r"Z:\HiveCache"
    SECONDARY_CACHE_BASE = secondary
...
class WindowsAdapter(OSAdapter):
    ...
    def get_cache_paths(self) -> Dict[str, str]:
        # Not used for placement anymore; retained for compatibility.
...
class SimpleBandit:
    """
    Very lightweight multi-armed bandit to decide:
    - which tasks to execute
    - how aggressively to prefetch

    For each "arm" (reason/type), we maintain stats and use UCB1-style scores.
    """

    def __init__(self):
        self.arms: Dict[str, Dict[str, float]] = {}  # arm -> {count, success, value_est}
        self.total_pulls: int = 0

    def record(self, arm: str, reward: float):
        stats = self.arms.setdefault(arm, {"count": 0, "success": 0.0, "value_est": 0.0})
        stats["count"] += 1
        self.total_pulls += 1
        stats["value_est"] += (reward - stats["value_est"]) / stats["count"]
        if reward > 0:
            stats["success"] += 1
    ...
    def score(self, arm: str) -> float:
        ...
        c = stats["count"]
        mu = stats["value_est"]
        if c == 0 or self.total_pulls == 0:
            return float("inf")
        return mu + 1.5 * math.sqrt(math.log(self.total_pulls) / c)

    def choose(self, candidates: List[Dict[str, Any]], max_choices: int) -> List[Dict[str, Any]]:
        if not candidates:
            return []
        scored = []
        for t in candidates:
            arm = t.get("reason", "unknown")
            bandit_score = self.score(arm)
            intrinsic = t.get("score", 0.5)
            final_score = intrinsic + 0.5 * bandit_score
            scored.append((final_score, t))
        scored.sort(key=lambda x: x[0], reverse=True)
        selected = [t for _, t in scored[:max_choices]]
        return selected
...
class PrefetchPolicy:
    ...
    Uses:
      ...
      - Simple bandit to choose which reasons/arms to act on
    """

    def __init__(self, behavior: BehaviorStore, bandit: SimpleBandit):
        self.behavior = behavior
        self.bandit = bandit
        self.process_url_hints: Dict[str, Set[str]] = defaultdict(set)
        self.prefetch_history: Dict[str, Dict[str, Any]] = {}
        self.global_hints: Set[str] = set()
    ...
    def suggest_tasks(self, telemetry: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Bandit chooses which tasks to actually execute
        selected = self.bandit.choose(unique, max_choices=20)
        return selected

    def record_prefetch_result(self, key: str, success: bool, reason: str) -> None:
        stats = self.prefetch_history.setdefault(key, {"tries": 0, "success": 0, "last": 0.0, "reason": reason})
        stats["tries"] += 1
        if success:
            stats["success"] += 1
        stats["last"] = time.time()
    ...
    def _worker_loop(self):
        while self.running:
            ...
            try:
                ...
                if key:
                    self.policy.record_prefetch_result(key, success, reason)
...
class NodeAgent:
    def __init__(self):
        ...
        # Use global cache bases selected at startup
        primary_base = PRIMARY_CACHE_BASE or r"C:\HiveCache"
        secondary_base = SECONDARY_CACHE_BASE or r"Z:\HiveCache"

        primary_path = os.path.join(primary_base, "primary")
        secondary_path = os.path.join(secondary_base, "secondary")
        ...
        behavior_path_dir = primary_base
        os.makedirs(behavior_path_dir, exist_ok=True)
        behavior_path = os.path.join(behavior_path_dir, "behavior.json")

        self.behavior = BehaviorStore(behavior_path)
        self.anomaly = AnomalyDetector(self.behavior)
        self.bandit = SimpleBandit()
        self.prefetch_policy = PrefetchPolicy(self.behavior, self.bandit)
    ...
    async def send_telemetry_loop(self):
        while self.running:
            ...
            telemetry["bandit_arms"] = self.bandit.arms
...
class QueenState:
    ...
    def update_from_telemetry(self, msg: Dict[str, Any]):
        ...
        bandit_arms = msg.get("bandit_arms", {})
...
class QueenController:
    ...
    async def foresight_loop(self):
        while True:
            ...
            if hosts:
                msg = {"type": "foresight", "hosts": hosts}
                await GLOBAL_BUS.publish("foresight", msg)
            await asyncio.sleep(15)


# =========================
# Tkinter GUI with richer world map
# =========================

class HiveGUI:
    """
    Two-tab GUI:
      - Node Detail: OS, CPU, RAM, anomaly, cache, prefetch stats, bandit arms, prediction
      - World Map: layout of local node + external endpoints, with risk-colored edges
    """
    ...
    def _build_node_detail_tab(self):
        ...
        self.info_frame.pack(side="left", fill="both", expand=True, padx=8, pady=4)
        ...
        self.bandit_frame = tk.Frame(self.node_frame)
        self.bandit_frame.pack(side="right", fill="both", expand=True, padx=8, pady=4)

        tk.Label(self.bandit_frame, text="Bandit Arms (Reason -> count / value_est):", anchor="w").pack(anchor="w")
        self.bandit_text = tk.Text(self.bandit_frame, height=20, width=45, bg="#111111", fg="#00ff00")
        self.bandit_text.pack(fill="both", expand=True)
    ...
    def update_node_view(self):
        ...
        bandit_arms = summary.get("bandit_arms", {})
        ...
        self.bandit_text.delete("1.0", tk.END)
        for arm, stats in bandit_arms.items():
            line = f"{arm}: count={int(stats.get('count', 0))}, value_est={stats.get('value_est', 0.0):.2f}\n"
            self.bandit_text.insert(tk.END, line)
    ...
    def _layout_world_graph(self, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]):
        ...
        radius_local = min(width, height) * 0.18
        ...
        radius_ext = min(width, height) * 0.40
        ...
        for e in edges:
            ...
            fade = max(0.2, min(1.0, 1.0 - age / 60.0))

            width_line = 1 + min(4, weight // 50)
            base_color = 0x55
            color_int = int(base_color * fade)
            color_hex = f"#{color_int:02x}{color_int:02x}ff"
            edge_id = self.canvas.create_line(x1, y1, x2, y2, fill=color_hex, width=width_line)
...
def main():
    # Step 1: ask user for primary and secondary cache folders BEFORE anything else
    pick_cache_paths_at_startup()
