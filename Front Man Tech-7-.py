...
def validate_libs():
    ...
    cache = os.path.expanduser("~/wheel_cache")
    status = {}
    for lib, pip in required.items():
        try: __import__(lib); status[lib] = "✓ Installed"
        except:
            wheel = next((f for f in os.listdir(cache) if pip in f and f.endswith(".whl")), None) if os.path.exists(cache) else None
            try:
                if wheel: subprocess.check_call([sys.executable, "-m", "pip", "install", os.path.join(cache, wheel)])
                else: subprocess.check_call([sys.executable, "-m", "pip", "install", pip])
                status[lib] = "✓ Auto-Installed"
            except: status[lib] = "✗ Failed"
    return status

def generate_persona():
    return {
        "persona_id": str(uuid.uuid4()),
        "role": "Idle Linux Node",
        "hostname": "node-" + uuid.uuid4().hex[:6],
        "os": "Linux 5.15.0",
        "behavior": "Low CPU, passive sync",
        "expires": (datetime.utcnow() + timedelta(minutes=60)).isoformat()
    }

def telemetry(real=True):
    base = {
    ...
    fake = {
        "IP": "10.0.0.99", "MAC": "00:FA:KE:00:00:00",
        "TTL": (datetime.utcnow() + timedelta(seconds=30)).isoformat()
    }
    persona = generate_persona()
    return {**(base if real else fake), "Persona": json.dumps(persona)}

def reverse_polarity(data):
    return {k[::-1]: str(v)[::-1] for k, v in data.items()}
...
def deploy():
    profile = {
        "persona": generate_persona(),
        "decoy_ip": "192.168.0.254",
        "decoy_ports": [8080,443],
        "decoy_processes": ["systemd","sshd","nginx"],
        "telemetry_signature": "MASKED",
        "timestamp": datetime.utcnow().isoformat(),
        "lineage": ["origin-node", "frontman-node"]
    }
    reversed = reverse_polarity(profile)
    s = zmq.Context().socket(zmq.PUB); s.bind("tcp://*:5555"); s.send_string(f"MASK_SYNC {json.dumps(reversed)}")
...
class ASIDash(QWidget):
    def __init__(s):
        ...
        for label,h in {
            ...
            "🧬 Mutation": ["Lineage","Status"], "🧠 Biometric": ["Node","Resonance"], "🕵️ Persona": ["Field","Value"],
            "🚨 Threats": ["Time","Type","Severity"]
        }.items():
    ...
    def refresh(s):
        ...
        s.fill("🧬 Mutation", {"origin-node → frontman-node → peer-1": "🟢"})
        s.fill("🧠 Biometric", {"frontman": "Stable", "peer-1": "Elevated"})
        s.fill("🕵️ Persona", generate_persona())
