...
class MemoryGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Hybrid Memory Overmind - Unified Hybrid Cortex")
    ...
    def _prewarm_cache(self):
        if not self.assets:
            messagebox.showwarning("Warning", "No assets present; scan game files or let the cortex learn first.")
            return
    ...
    def _refresh_ui(self):
        if self.assets and self.governor:
            ...
            self.status_strip.config(
                text=f"[HYBRID CORTEX] VRAM {self.tier_vram.used_mb:.0f}/{self.tier_vram.capacity_mb} MB | "
    ...
    def start_simulation(self):
        if self.simulator:
            ...
            self._log("AI CORE ENGAGED (auto-start or manual).")
