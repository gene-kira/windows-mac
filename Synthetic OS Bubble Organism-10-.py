...
- Linux (libvirt/KVM, firejail, iptables/nftables, swtpm hooks, wmctrl/xdotool)

Organs:
- Autoloader
- Persistence (SQLite + optional encrypted key blobs)
...
- LibvirtVtpmManager (libvirt XML + swtpm hooks)
...
- NetworkSandboxOrgan (per-VM NIC binding, Linux iptables/nftables)
- DriverCheckOrgan (Hyper-V, libvirt, firewall, firejail, driver metadata)
- NICEnumerator (host NICs, Hyper-V switches, Linux interfaces)
- VMHeartbeatMonitor (per-VM liveness + latency)
- GuestHeartbeatOrgan (host-side listener for guest agents)
- ResourceGovernorOrgan (CPU/GPU telemetry + soft throttling)
- WombController (multi-VM, Hyper-V/libvirt, network sandbox hooks, vTPM)
- SandboxManager (Windows Job Objects + Linux firejail + firewall)
- UIAutomationManager (Windows pywinauto + Linux wmctrl/xdotool)
- GuestAgent (process telemetry, lineage, sandbox + UI reactions)
- PluginManager (dynamic organs)
- PluginSandboxOrgan (plugins in separate processes)
- DistributedTrustOrgan (HTTP server for trust RPC)
- SecureStorageOrgan (encrypted TPM key blobs)
- Cockpit GUI (tabs: Overview, VMs, Windows, Logs, Timeline, Override, Plugins, Personas, Host/Drivers, Resources)
"""
...
# --- OPTIONAL WINDOWS ELEVATION CHECK (AUTO-RELAUNCH AS ADMIN) ---
if ON_WINDOWS:
    import ctypes
    ...
    def ensure_admin():
        try:
            ...
            print(f"[MAX++] Elevation failed: {e}")
            sys.exit()
...
def save_tpm_key_raw(key_id: str, data: bytes, required_trust: float, created: float):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""
...
def load_tpm_keys_raw() -> Dict[str, Dict[str, Any]]:
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
...
# ============================================================
#  Secure Storage Organ (simple symmetric encryption for TPM blobs)
# ============================================================

class SecureStorageOrgan:
    """
    Simple symmetric encryption for TPM key blobs.
    Not military-grade, but better than plain text.
    """
    def __init__(self, passphrase: Optional[str] = None):
        # Derive a key from passphrase or fallback to host-based hash
        if passphrase:
            seed = passphrase.encode("utf-8")
        else:
            seed = (platform.node() + platform.system()).encode("utf-8")
        self.key = hashlib.sha256(seed).digest()

    def encrypt(self, plaintext: str) -> bytes:
        data = plaintext.encode("utf-8")
        # XOR with key repeated; then base64
        out = bytearray()
...
class SyntheticTPM:
    def __init__(self, trust_core: BorgTrustCore, secure_store: SecureStorageOrgan, libvirt_conn=None):
        self.trust_core = trust_core
        self.secure_store = secure_store
        self.pcrs: Dict[int, str] = {i: "0" * 64 for i in range(24)}
        raw_keys = load_tpm_keys_raw()
        self.keys: Dict[str, Dict[str, Any]] = {}
        for key_id, meta in raw_keys.items():
            decrypted = self.secure_store.decrypt(meta["data"])
            self.keys[key_id] = {
                "data": decrypted,
                "required_trust": meta["required_trust"],
    ...
    def seal_key(self, key_id: str, data: str, required_trust: float = 0.7):
        ...
        enc = self.secure_store.encrypt(data)
        save_tpm_key_raw(key_id, enc, required_trust, meta["created"])
...
# ============================================================
#  Libvirt vTPM Manager (XML + swtpm hooks)
# ============================================================

class LibvirtVtpmManager:
    def __init__(self, telemetry: TelemetrySpine, conn):
        self.telemetry = telemetry
        self.conn = conn

    def _emit(self, level: str, msg: str, sev: float = 0.0):
        print(f"[LVTpm:{level}] {msg}")
        self.telemetry.publish({
            "node": "lvtpm",
            "severity": sev,
            "description": msg,
    ...
    def _domain(self, name: str):
        ...
        try:
            ...
            self._emit("WARN", f"lookupByName failed: {e}", sev=-0.05)
            return None

    def ensure_vtpm(self, vm_name: str) -> dict:
        if not (IS_LINUX and self.conn):
            return {"exists": False, "enabled": False, "errors": ["Not Linux or no libvirt"]}

        dom = self._domain(vm_name)
        if not dom:
            return {"exists": False, "enabled": False, "errors": ["Domain not found"]}

        try:
            xml = dom.XMLDesc()
        except Exception as e:
            self._emit("WARN", f"XMLDesc failed: {e}", sev=-0.05)
            return {"exists": True, "enabled": False, "errors": [str(e)]}

        if "<tpm" in xml:
            self._emit("INFO", f"Domain {vm_name} already has TPM device", sev=0.0)
            return {"exists": True, "enabled": True, "errors": []}

        # Minimal hint: we don't rewrite XML here, but we surface the need clearly.
        self._emit("WARN", f"Domain {vm_name} missing TPM device. Manual swtpm XML injection required.", sev=-0.02)
        return {"exists": True, "enabled": False, "errors": ["No <tpm> device in XML"]}
...
# ============================================================
#  Network Sandbox Organ (per-VM NIC binding + Linux iptables)
# ============================================================

class NetworkSandboxOrgan:
    def __init__(self, telemetry: TelemetrySpine, hv_nic: Optional[HyperVNetworkManager], win_fw: Optional[WindowsFirewallManager], libvirt_conn):
        self.telemetry = telemetry
        self.hv_nic = hv_nic
        self.win_fw = win_fw
        self.libvirt_conn = libvirt_conn
    ...
    def apply_vm_sandbox(self, vm_name: str, level: str) -> Tuple[bool, str]:
        if not HAS_ADMIN:
            msg = "No admin/root: cannot apply firewall rules"
            self._emit("WARN", msg, sev=-0.05)
            return False, msg

        if IS_WINDOWS and self.hv_nic and self.win_fw:
            adapters = self.hv_nic.get_vm_interfaces(vm_name)
            if not adapters:
        ...
        if IS_LINUX and self.libvirt_conn:
            try:
                dom = self.libvirt_conn.lookupByName(vm_name)
            except Exception as e:
                msg = f"libvirt domain lookup failed: {e}"
                self._emit("WARN", msg, sev=-0.05)
                return False, msg
            try:
                xml = dom.XMLDesc()
            except Exception as e:
                msg = f"XMLDesc failed: {e}"
                self._emit("WARN", msg, sev=-0.05)
                return False, msg

            # Very simple: look for <target dev='vnetX'> style tap devices
            import re
            devs = re.findall(r"<target dev='([^']+)'", xml)
            if not devs:
                msg = f"No tap devices found in XML for {vm_name}"
                self._emit("WARN", msg, sev=-0.05)
                return False, msg
            ...
            try:
                subprocess.check_output(
                    ["iptables", "-A", "FORWARD", "-o", dev, "-j", "DROP"],
                    stderr=subprocess.STDOUT,
                    text=True,
                )
                msg = f"Applied iptables DROP on interface {dev} for {vm_name}"
                self._emit("INFO", msg, sev=+0.02)
                return True, msg
            except Exception as e:
        ...
        msg = "Network sandbox not supported on this platform"
        self._emit("WARN", msg, sev=-0.05)
        return False, msg

# ============================================================
#  Driver Check Organ (extended)
# ============================================================
...
class DriverCheckOrgan:
    ...
    def check_firewall(self):
        if IS_WINDOWS:
            ...
            return {"available": True, "reason": "iptables/nftables assumed present (not fully verified)"}
    ...
    def check_driver_metadata(self):
        if IS_WINDOWS:
            # Minimal: check some core drivers presence via WMI/PowerShell
            import subprocess
            try:
                out = subprocess.check_output(
                    ["powershell", "-NoProfile", "-NonInteractive",
                     "-Command", "Get-WmiObject Win32_PnPSignedDriver | Select-Object -First 5 DeviceName,DriverVersion,IsSigned | ConvertTo-Json -Depth 3"],
                    stderr=subprocess.STDOUT,
                    text=True,
                )
            ...
            # Minimal: list loaded modules
            try:
                with open("/proc/modules", "r") as f:
                    lines = f.read().splitlines()[:10]
                return {"available": True, "sample": lines}
            except Exception as e:
                return {"available": False, "reason": str(e)}
        return {"available": False, "reason": "Unknown OS"}
    ...
    def run_full_check(self):
        ...
        meta = self.check_driver_metadata()
        status = {
            "hyperv": hv,
            "libvirt": lv,
            "firewall": fw,
            "firejail": fj,
            "driver_metadata": meta,
        }
...
# ============================================================
#  Guest Heartbeat Organ (simple TCP listener)
# ============================================================

class GuestHeartbeatOrgan:
    """
    Simple TCP listener for guest agents.
    Protocol: JSON lines: {"vm":"VM1","type":"heartbeat"} etc.
    """
    def __init__(self, telemetry: TelemetrySpine, hb_monitor: VMHeartbeatMonitor, port: int = 50051):
        self.telemetry = telemetry
        ...
        class Handler(socketserver.StreamRequestHandler):
            ...
            def handle(self):
                try:
                    ...
                    vm = data.get("vm")
                    if vm:
                        self.outer.hb_monitor.mark_heartbeat(vm)
                except Exception as e:
                    self.outer._emit("WARN", f"Guest HB parse error: {e}", sev=-0.01)

        try:
            with socketserver.TCPServer(("0.0.0.0", self.port), Handler) as srv:
                self._emit("INFO", f"Guest heartbeat listener on port {self.port}", sev=0.0)
                srv.serve_forever()
        except Exception as e:
            self._emit("WARN", f"Guest HB server failed: {e}", sev=-0.05)

# ============================================================
#  Resource Governor Organ (soft CPU/GPU telemetry)
# ============================================================

class ResourceGovernorOrgan:
    def __init__(self, telemetry: TelemetrySpine):
        self.telemetry = telemetry
        self.last_snapshot: Dict[str, Any] = {}

    def _emit(self, level: str, msg: str, sev: float = 0.0):
        print(f"[RES:{level}] {msg}")
        self.telemetry.publish({
            "node": "resource",
            "severity": sev,
            "description": msg,
    ...
    def snapshot(self):
        ...
        # GPU: best-effort via nvidia-smi if present
        gpu = None
        try:
            import subprocess
            out = subprocess.check_output(
        ...
        snap = {"cpu": cpu, "mem": mem, "gpu": gpu}
        self.last_snapshot = snap

        if cpu > 80 or mem > 90:
            self._emit("WARN", f"High resource usage: CPU={cpu} MEM={mem}", sev=-0.05)
        else:
            self._emit("INFO", f"Resource snapshot: {snap}", sev=0.0)
...
class WombController:
    def __init__(self, telemetry: TelemetrySpine, policy: PolicyEngine, libvirt_conn, net_sandbox: NetworkSandboxOrgan, hv_tpm: Optional[HyperVVtpmManager], lv_tpm: Optional[LibvirtVtpmManager]):
        self.telemetry = telemetry
        self.policy = policy
        self.vms: Dict[str, Dict[str, Any]] = {}
        self.libvirt_conn = libvirt_conn
        self.net_sandbox = net_sandbox
        self.hv_tpm = hv_tpm
        self.lv_tpm = lv_tpm
    ...
    def apply_network_sandbox(self, vm_name: str, level: str) -> (bool, str):
        return self.net_sandbox.apply_vm_sandbox(vm_name, level)
    ...
    def vtpm_status(self, name: str) -> dict:
        ...
        if IS_LINUX and self.lv_tpm:
            return self.lv_tpm.ensure_vtpm(name)
        return {"exists": False, "enabled": False, "errors": ["Unsupported platform"]}
    ...
    def configure_vtpm_for_vm(self, name: str) -> dict:
        ...
        return {"exists": False, "enabled": False, "errors": ["Unsupported platform"], "action": "none"}
...
# ============================================================
#  UI Automation Manager (Windows + Linux)
# ============================================================
...
class UIAutomationManager:
    def __init__(self, telemetry: TelemetrySpine):
        ...
        if self.is_linux:
            ...
            if self.wmctrl:
                self._emit_info("Init", f"wmctrl found at {self.wmctrl}", sev=0.0)
            else:
                self._emit_info("Init", "wmctrl not found", sev=0.0)
    ...
    def list_windows(self) -> List[dict]:
        if self.is_windows and self.Desktop:
            wins = []
            try:
                ...
                self._emit_error("ListWindowsWin", str(e))
            return wins

        if self.is_linux and self.wmctrl:
            import subprocess
            wins = []
            try:
                out = subprocess.check_output([self.wmctrl, "-l"], text=True)
                for line in out.splitlines():
                    parts = line.split(None, 3)
                    if len(parts) == 4:
                        _, _, _, title = parts
                        wins.append({"title": title, "class": "X11"})
            except Exception as e:
                self._emit_error("ListWindowsLinux", str(e))
            return wins
            ...
            try:
                for w in self.Desktop(backend="uia").windows():
                    try:
                        ...
                        continue
                if count:
                    self._emit_info("CloseWindowsWin", f"Closed {count} windows matching '{keyword}'", sev=+0.02)
            except Exception as e:
                self._emit_error("CloseWindowsWin", str(e))
            return count

        if self.is_linux and self.wmctrl and self.xdotool:
            import subprocess
            try:
                out = subprocess.check_output([self.wmctrl, "-l"], text=True)
                for line in out.splitlines():
                    ...
                    if len(parts) == 4:
                        ...
                        if kw in title.lower():
                            try:
                                subprocess.check_output([self.xdotool, "windowclose", wid], text=True)
                                count += 1
                            except Exception:
                                continue
                if count:
                    self._emit_info("CloseWindowsLinux", f"Closed {count} windows matching '{keyword}'", sev=+0.02)
            except Exception as e:
                self._emit_error("CloseWindowsLinux", str(e))
            return count
...
# ============================================================
#  Plugin Sandbox Organ (plugins as external processes)
# ============================================================

class PluginSandboxOrgan:
    """
    Very simple: run plugins as external commands with JSON stdin.
    """
    def __init__(self, telemetry: TelemetrySpine):
        self.telemetry = telemetry
    ...
    def register_plugin_command(self, name: str, cmd: List[str]):
        self.commands[name] = cmd
        self._emit("INFO", f"Plugin command registered: {name} -> {cmd}", sev=0.0)

    def invoke(self, name: str, event: Dict[str, Any]):
        import subprocess
        ...
        try:
            ...
            proc.communicate(json.dumps(event), timeout=2.0)
        except Exception as e:
            self._emit("WARN", f"Plugin {name} failed: {e}", sev=-0.05)
...
# ============================================================
#  Distributed Trust Organ (HTTP server)
# ============================================================

class DistributedTrustOrgan:
    """
    Tiny HTTP server for /trust/update and /trust/status.
    """
    def __init__(self, telemetry: TelemetrySpine, trust_core: BorgTrustCore, port: int = 50052):
        self.telemetry = telemetry
        self.trust_core = trust_core
        self.port = port
        self.thread = threading.Thread(target=self._run_server, daemon=True)
    ...
    def _run_server(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_POST(self):
                if self.path == "/trust/update":
                    ...
                    try:
                        ...
                        if node_id:
                            outer.trust_core.update_remote_trust(node_id, score)
                    except Exception as e:
                        outer._emit("WARN", f"Trust update parse error: {e}", sev=-0.01)
            ...
            def do_GET(self):
                if self.path == "/trust/status":
                    status = outer.trust_core.to_dict()
        ...
        try:
            srv = HTTPServer(("0.0.0.0", self.port), Handler)
            self._emit("INFO", f"Distributed trust server on port {self.port}", sev=0.0)
            srv.serve_forever()
        except Exception as e:
            self._emit("WARN", f"Distributed trust server failed: {e}", sev=-0.05)
...
class CockpitWindow(QtWidgets.QMainWindow):
    def __init__(self,
                 ...
                 resource_gov: ResourceGovernorOrgan):
        super().__init__()
        self.trust_core = trust_core
        self.tpm = tpm
        self.policy = policy
        ...
        self.hb_monitor = hb_monitor
        self.resource_gov = resource_gov
    ...
    def _build_resources_tab(self):
        tab = QtWidgets.QWidget()
        self.tabs.addTab(tab, "Resources")
        layout = QtWidgets.QVBoxLayout(tab)

        self.resource_view = QtWidgets.QPlainTextEdit()
        self.resource_view.setReadOnly(True)
        layout.addWidget(self.resource_view)
    ...
    def refresh_all(self):
        ...
        if cur_vm:
            ...
            if not st.get("exists", False):
                ...
                errs = "; ".join(st.get("errors", [])) if st.get("errors") else "OK"
                text = f"vTPM: {state} | {errs}"
            self.vtpm_status_label.setText(text)
        else:
            self.vtpm_status_label.setText("vTPM: n/a")

        self.win_list.clear()
        wins = self.ui_auto.list_windows()
        for w in wins:
        ...
        res = self.resource_gov.last_snapshot or {}
        self.resource_view.setPlainText("Resource Snapshot:\n" + json.dumps(res, indent=2))
...
def main():
    ...
    secure_store = SecureStorageOrgan()
    tpm = SyntheticTPM(trust_core, secure_store, libvirt_conn=libvirt_conn)

    hv_nic = HyperVNetworkManager(telemetry) if IS_WINDOWS else None
    win_fw = WindowsFirewallManager(telemetry) if IS_WINDOWS else None
    hv_tpm = HyperVVtpmManager(telemetry) if IS_WINDOWS else None
    lv_tpm = LibvirtVtpmManager(telemetry, libvirt_conn) if (IS_LINUX and libvirt_conn) else None
    net_sandbox = NetworkSandboxOrgan(telemetry, hv_nic, win_fw, libvirt_conn)

    womb = WombController(telemetry, policy, libvirt_conn, net_sandbox, hv_tpm, lv_tpm)
    sandbox = SandboxManager(telemetry)
    ui_auto = UIAutomationManager(telemetry)
    plugin_mgr = PluginManager(telemetry)
    plugin_sandbox = PluginSandboxOrgan(telemetry)
    driver_check = DriverCheckOrgan(telemetry)
    nic_enum = NICEnumerator(telemetry, hv_nic)
    hb_monitor = VMHeartbeatMonitor(telemetry, womb)
    resource_gov = ResourceGovernorOrgan(telemetry)

    DistributedTrustOrgan(telemetry, trust_core)
    GuestHeartbeatOrgan(telemetry, hb_monitor)
    ...
    win = CockpitWindow(trust_core, tpm, policy, womb, ui_auto, plugin_mgr, driver_check, nic_enum, hb_monitor, resource_gov)
    win.show()
    app.exec()
