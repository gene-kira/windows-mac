...
class AIEngine:
    """
    Backends:
    - RULE_BASED: simple pattern-based hardening
    - LOCAL_HTTP: OpenAI-style local server (LM Studio / GPT4All / many others)
    - REMOTE_OPENAI: OpenAI-compatible remote API (OpenAI / Azure / LM Studio server / Groq, etc.)
    """
    ...
    # ---- Local HTTP (LM Studio / GPT4All server / etc.) ----
    ...
    def rewrite_defensive(self, code: str) -> AIRewriteResult:
        ...
        elif self.backend == AIBackend.LOCAL_HTTP:
            rewritten = self._local_http_rewrite(code, lineage)
        elif self.backend == AIBackend.REMOTE_OPENAI:
...
class FileWatcherOrgan:
    """
    Auto-organ:
    - Watches a directory for *.py files
    - On new/changed file: forces Nuclear Mode, purifies, writes <name>.hardened.py
    - Logs everything and updates anomaly engine via CodePurifier
    """
    def __init__(self, logger: SecureLogger, purifier: CodePurifier,
                 anomaly: AnomalyEngine, dataguard: DataGuard,
    ...
    def _tick(self):
        ...
        for entry in self.watch_dir.iterdir():
            if entry.is_file() and entry.suffix == ".py" and not entry.name.endswith(".hardened.py"):
                mtime = entry.stat().st_mtime
                last = self._seen.get(entry)
                if last is None or mtime > last:
                    self._seen[entry] = mtime
...
class Cockpit(QWidget):
    ...
    def _build_ui(self):
        ...
        self.hardened_edit.setPlaceholderText("Hardened / AI-rewritten output will appear here...")
        ...
        # Automation page (UIAutomation + FileWatcher)
        auto_page = QWidget()
    ...
    def set_policy_mode(self, mode: PolicyMode):
        ...
        if mode == PolicyMode.NUCLEAR:
            ...
            # Baseline heat so overlay visibly pulses even before threats
    ...
    def on_purify(self):
        ...
        # If in nuclear, keep at least baseline heat
    ...
    def on_ai_rewrite(self):
        ...
        # Show AI-rewritten code directly
