"""
Compact Autonomous Engine + Adaptive Codex + Operator Registry + GUI + FastAPI + Autoloader
------------------------------------------------------------------------------------------
...
- FastAPI layer for remote access (optional, only if installed)
- Autoloader with safe checks to avoid crashes if libs are missing
"""
...
LIBS = autoload([
    ...
    "tkinter","psutil","fastapi","uvicorn"
])
...
class Engine:
    ...
    def _process(self, payload):
        ...
        bundle = {
            ...
            "answer": payload["text"],
            "system": snap,
            "solver": system_solver(snap),
            "codex": codex_state
        }
        for tier, funcs in self.registry.get_all().items():
            for f in funcs:
                try:
                    bundle.setdefault(tier, []).append(f(payload["text"]))
                except Exception as e:
                    bundle.setdefault("notes", []).append(str(e))
        return bundle
...
class EngineGUI:
    def __init__(self, engine):
        ...
        self.log = scrolledtext.ScrolledText(self.root, width=80, height=20, font=("Consolas", 10))
        self.log.pack()
    ...
    def update_loop(self):
        for b in self.engine.fetch():
            ...
            self.log.insert(tk.END, f"{b['id']} :: {b['answer']}\n"
                                    f" CPU={s['cpu']}% MEM={s['mem']}% DISK={s['disk']}% PROCS={s['procs']}\n")
            if sol["issues"]:
                self.log.insert(tk.END, f" Issues: {', '.join(sol['issues'])}\n")
            self.log.insert(tk.END, f" Actions: {sol['actions'][0]}\n"
                                    f" Codex purge_logic={codex['rules']['purge_logic']} ghost={codex['ghost_sync']}\n"
                                    f"{'-'*60}\n")
            self.log.see(tk.END)
...
if fastapi and uvicorn and FastAPI:
    ...
    def system():
        snap = system_snapshot()
        return {"snapshot": snap, "solver": system_solver(snap)}
else:
...
def main():
    ...
    if tk and scrolledtext and ttk:
        EngineGUI(engine).run()
    else:
        print("[WARNING] Tkinter not available, GUI layer disabled.")
