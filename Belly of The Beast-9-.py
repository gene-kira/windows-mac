...
from PyQt5.QtWidgets import (
    ...
    QSplitter, QGraphicsDropShadowEffect, QComboBox, QStatusBar
)
...
class ProcessesPanel(QWidget):
    ...
    def kill_selected(self):
        if DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "In Development Mode, process killing is disabled (shadow/hybrid only).")
            return
...
def save_snapshot(overrides):
    ensure_snapshot_dir()
    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    path = os.path.join(SNAPSHOT_DIR, f"shadow_snapshot_{ts}.json")
...
class UnknownSystemsPanel(QWidget):
    def __init__(self, parent=None):
        ...
        info = QLabel(
            ...
            "Conflicts are highlighted; edits are shadow-only."
        )
        ...
        self.btn_snapshot_save = QPushButton("⟒⧫ᛟ — Save Snapshot", self)
        self.btn_snapshot_load = QPushButton("ᛉ⧗⟒ — Load Snapshot", self)
        self.btn_snapshot_save.clicked.connect(self.save_snapshot_click)
        self.btn_snapshot_load.clicked.connect(self.load_snapshot_click)

        bottom_layout.addWidget(self.btn_apply)
        bottom_layout.addWidget(self.btn_snapshot_save)
        bottom_layout.addWidget(self.btn_snapshot_load)
    ...
    def update_banner(self):
        if DEV_MODE.enabled:
            self.banner.setText(
                f"DEVELOPMENT MODE ACTIVE — VIEW: {DEV_MODE.view_mode}\n"
                ...
                "Conflicts (Real ≠ Shadow) are highlighted."
            )
        else:
            self.banner.setText("Development Mode OFF — REAL view only, read-only hybrid display.")
    ...
    def update_editability(self):
        ...
        if not DEV_MODE.enabled:
            ...
            self.btn_snapshot_load.setEnabled(True)
            return
        ...
        if mode == "REAL":
            ...
            self.btn_snapshot_save.setEnabled(True)
        self.btn_snapshot_load.setEnabled(True)
    ...
    def _load_rows_into_table(self, rows):
        ...
        for r in rows:
            row = self.table.rowCount()
            self.table.insertRow(row)
            prop_item = QTableWidgetItem(r["property"])
            real_item = QTableWidgetItem(r.get("real_value", ""))
            shadow_item = QTableWidgetItem(r.get("shadow_value", ""))
            eff_item = QTableWidgetItem(r.get("effective_value", ""))
            type_item = QTableWidgetItem(r.get("type", "string"))
            src_item = QTableWidgetItem(r["source"])
            id_item = QTableWidgetItem(r["identifier"])

            if shadow_item.text() and shadow_item.text() != real_item.text():
                shadow_item.setBackground(QColor(80, 10, 10))
                eff_item.setBackground(QColor(120, 20, 20))

            self.table.setItem(row, 0, prop_item)
            self.table.setItem(row, 1, real_item)
            self.table.setItem(row, 2, shadow_item)
    ...
    def _count_conflicts(self):
        ...
        for row in range(rows):
            real_item = self.table.item(row, 1)
            shadow_item = self.table.item(row, 2)
            if not (real_item and shadow_item):
                continue
            if shadow_item.text() and shadow_item.text() != real_item.text():
                conflicts += 1
        return conflicts
    ...
    def apply_changes(self):
        ...
        conflicts_before = self._count_conflicts()
        ...
        delta_conflicts = conflicts_after - conflicts_before

        risky_sources = {"Registry", "Tasks", "AppX"}
        touched_risky = any(
            self.table.item(r, 5) and self.table.item(r, 5).text() in risky_sources
            for r in range(self.table.rowCount())
        )

        if touched_risky and delta_conflicts > 5 and mode == "AUTO":
            QMessageBox.warning(
                self,
                "Hybrid Negotiation",
                f"You just introduced {delta_conflicts} new conflicts in AUTO mode on risky sources.\n\n"
                "This is safe in shadow, but if ever applied to the real system it could be destructive.\n"
                "Treat this as an experimental branch of reality."
            )
        else:
    ...
    def save_snapshot_click(self):
        if not DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "Enable Development Mode to save shadow snapshots.")
            return
        path = save_snapshot(self.overrides)
        QMessageBox.information(self, "Snapshot Saved", f"Shadow snapshot saved:\n{path}")

    def load_snapshot_click(self):
        snaps = list_snapshots()
        if not snaps:
            QMessageBox.information(self, "Snapshots", "No snapshots found.")
            return
        snap_name, ok = QInputDialog.getItem(
            self,
            "Load Snapshot",
            "Choose snapshot:",
            snaps,
        ...
        if not ok or not snap_name:
            return
        data = load_snapshot_file(snap_name)
        self.overrides = data
        save_overrides(self.overrides)
        rows = self._build_view_rows()
        self._load_rows_into_table(rows)
        QMessageBox.information(self, "Snapshot Loaded", f"Snapshot '{snap_name}' loaded into shadow system.")
...
class DevModePanel(QWidget):
    def __init__(self, parent=None, unknown_panel: UnknownSystemsPanel = None, status_callback=None):
        super().__init__(parent)
        ...
        desc = QLabel(
            ...
            "AUTO: Hybrid — Effective column = Real overlaid with Shadow; edits go to Shadow.\n\n"
            "In AUTO, treat changes as experimental branches of reality."
        )
...
# ============================================================
# Organs registry (plugin-like panel registration)
# ============================================================

class OrgansRegistry:
    def __init__(self):
        self.organs = []

    def register(self, label, key, widget):
        self.organs.append((label, key, widget))

ORGANS = OrgansRegistry()

# ============================================================
# Main window with alien layout + pulsing nav + HUD
# ============================================================
...
class AlienAdminMainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        # FIX OPTION 1: status bar created first
        self.status = QStatusBar(self)
        self.setStatusBar(self.status)

        self.setWindowTitle("⟒⧫ᛟ  ALIEN ADMIN COCKPIT — HYBRID SHADOW SYSTEM  ⧖ᛉ⧗")
        ...
        self.devmode_panel = DevModePanel(self, unknown_panel=self.unknown_panel, status_callback=self.update_status_bar)

        ORGANS.register("⟒⧫ᛟ — Processes", "processes", self.processes_panel)
        ORGANS.register("⧖⟟ᛉ — Registry", "registry", self.registry_panel)
        ORGANS.register("⟟ᛞ⧗ — PowerShell", "powershell", self.powershell_panel)
        ORGANS.register("⧫ᛟ⟒ — Windows Apps", "apps", self.apps_panel)
        ORGANS.register("ᛉ⧗⟒ — Services", "services", self.services_panel)
        ORGANS.register("⧫⧖ᛉ — UNKNOWN SYSTEMS", "unknown", self.unknown_panel)
        ORGANS.register("⟒⧫⟒ — Development / Hybrid Mode", "devmode", self.devmode_panel)

        for label, key, widget in ORGANS.organs:
    ...
    def update_status_bar(self):
        ...
        organ_label = current_item.text() if current_item else "Unknown"
        dev_state = "ON" if DEV_MODE.enabled else "OFF"
        view = DEV_MODE.view_mode
        self.status.showMessage(
            f"Organ: {organ_label}   |   Dev Mode: {dev_state}   |   View: {view} (REAL / SHADOW / AUTO)"
        )
