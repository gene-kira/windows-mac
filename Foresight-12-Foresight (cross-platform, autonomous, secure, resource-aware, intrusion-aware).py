...
"""
Foresight: live assistant + forecaster with advanced prediction, explainability, encrypted persistence,
and intrusion detection — cross-platform with a compact Tk GUI.

New capabilities:
- Multi-step forecasting: next-3 step plan, iterative rollout.
- 3-step transition modeling (prev3→prev2→prev1→next) for deeper sequence learning.
- Context layering: hour, day-of-week, weekend flag, system load.
- Hour-of-day smoothing: neighbor-hour kernel to stabilize noisy hours.
- Recency boost: fresher habits have higher influence.
- Cluster generalization: browser/IDE/office/terminal boosts.
- Live assistant controls: Accept, Skip, Up/Down reorder, optional Execute using user-defined actions.
- Persistent encrypted cache including user action map.
- Hardened GUI: never breaks on tuple/sequence errors.
"""
...
# ----------------------------
# Next-gen predictor (multi-step + context + adaptive decay + clustering + recency)
# ----------------------------
class NextGenPredictor:
    ...
    def _tod_smoothing(self, hour: int, habits: Dict[str, HabitStats]) -> Dict[str, float]:
        # Neighbor-hour kernel: weights for h-1, h, h+1 (wrap-around)
    ...
    def _recency_scores(self, now_ts: float, habits: Dict[str, HabitStats]) -> Dict[str, float]:
        # Exponential decay over hours since last seen
        lam = 1.0 / 24.0  # half-life ~ 16.6 hours
        scores = {}
        for hkey, stats in habits.items():
    ...
    def _score_snapshot(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                        hour: int, dow: int, weekend: int, habits: Dict[str, HabitStats],
                        now_ts: float) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]]]:
        ...
        components = {"sequence1": {}, "sequence2": {}, "sequence3": {}, "context": {}, "tod": {}, "global": {}, "cluster": {}, "recency": {}}
    ...
    def predict(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                hour: int, dow: int, weekend: int, habits: Dict[str, HabitStats],
                system_load_bucket: int, now_ts: float) -> Tuple[Optional[str], float, Dict[str, float]]:
        try:
            scores, components = self._score_snapshot(prev1, prev2, prev3, hour, dow, weekend, habits, now_ts)
    ...
    def predict_plan(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                     hour: int, dow: int, weekend: int, habits: Dict[str, HabitStats],
                     system_load_bucket: int, now_ts: float, length: int = 3) -> List[Tuple[str, float]]:
        plan: List[Tuple[str, float]] = []
        p1, p2, p3 = prev1, prev2, prev3
        try:
            for _ in range(max(1, length)):
                scores, _ = self._score_snapshot(p1, p2, p3, hour, dow, weekend, habits, now_ts)
                nxt, conf = self._choose(scores)
                if not nxt:
                    break
                plan.append((nxt, conf))
                p3, p2, p1 = p2, p1, nxt  # advance context window
            return plan
        except Exception:
            return plan
...
# ----------------------------
# Foresight engine (encrypted cache + security + alerts + persistent predictor + actions)
# ----------------------------
class Foresight:
    def __init__(self, cache_file: str = "foresight_cache.bin"):
        ...
        # User-definable action map: habit_key -> command or OS-open hint
    ...
    def predict_next(self, when_ts: Optional[float] = None) -> Tuple[Optional[str], float, Dict[str, float]]:
        try:
            ...
            return self.predictor.predict(prev1, prev2, prev3, hour, dow, weekend, self.habits, bucket, now_ts)
        except Exception:
            return None, 0.0, {}

    def predict_next_plan(self, when_ts: Optional[float] = None, length: int = 3) -> List[Tuple[str, float]]:
        try:
            now_ts = when_ts if when_ts is not None else time.time()
            dt = datetime.datetime.fromtimestamp(now_ts)
            hour = dt.hour
            ...
            with self._lock:
                ...
                prev3 = prev3 if isinstance(prev3, str) and not prev3.startswith("advisory:") else None
                bucket = self._system_load_bucket()
            return self.predictor.predict_plan(prev1, prev2, prev3, hour, dow, weekend, self.habits, bucket, now_ts, length=length)
        except Exception:
            return []
    ...
    def reinforce(self) -> None:
        with self._lock:
            ...
            prev3 = prev3 if isinstance(prev3, str) and not prev3.startswith("advisory:") else None
            bucket = self._system_load_bucket()
            predicted, _, _ = self.predictor.predict(prev1, prev2, prev3, hour, dow, weekend, self.habits, bucket, last_ts)
    ...
    # -------- Live assistant: execution hooks --------
    def set_action(self, habit_key: str, action_type: str, value: str) -> None:
        # action_type: "cmd" to run subprocess, "open" to os.startfile (Windows) or "open" (macOS)
        with self._lock:
...
# ----------------------------
# GUI (live assistant + forecasting + hardened handling)
# ----------------------------
class ForesightGUI:
    ...
    def _build_layout(self):
        ...
        ttk.Label(top, textvariable=self.advisory_var, foreground="#b22222").grid(row=1, column=0, columnspan=5, sticky="w")
        ...
        # Middle: Frequency chart
        mid = ttk.LabelFrame(container, text="Frequency", padding=6)
        mid.grid(row=1, column=1, sticky="nsew", padx=(4, 4))
        mid.columnconfigure(0, weight=1)
        mid.rowconfigure(0, weight=1)
        if plt is not None and FigureCanvasTkAgg is not None:
            self.figure = plt.Figure(figsize=(3.8, 2.4), dpi=100)
        ...
        # Right: Next steps plan + controls
        right = ttk.LabelFrame(container, text="Next steps (live assistant)", padding=6)
        right.grid(row=1, column=2, sticky="nsew", padx=(4, 0))
        right.columnconfigure(0, weight=1)
        right.rowconfigure(0, weight=1)

        self.plan_tree = ttk.Treeview(right, columns=("step", "confidence"), show="headings", height=12)
        self.plan_tree.heading("step", text="Step")
        self.plan_tree.heading("confidence", text="Confidence")
        self.plan_tree.column("step", width=220, anchor="w")
        self.plan_tree.column("confidence", width=90, anchor="center")
        self.plan_tree.grid(row=0, column=0, columnspan=3, sticky="nsew")
        plan_scroll = ttk.Scrollbar(right, orient="vertical", command=self.plan_tree.yview)
        plan_scroll.grid(row=0, column=3, sticky="ns")
        self.plan_tree.config(yscrollcommand=plan_scroll.set)

        ttk.Button(right, text="Accept", command=self._accept_step).grid(row=1, column=0, sticky="ew", pady=(6, 0))
        ttk.Button(right, text="Skip", command=self._skip_step).grid(row=1, column=1, sticky="ew", pady=(6, 0))
        ttk.Button(right, text="Up", command=self._move_up).grid(row=1, column=2, sticky="ew", pady=(6, 0))

        ttk.Button(right, text="Down", command=self._move_down).grid(row=2, column=0, sticky="ew", pady=(4, 0))
        ttk.Button(right, text="Edit actions", command=self._edit_actions).grid(row=2, column=1, sticky="ew", pady=(4, 0))
        ttk.Button(right, text="Execute selected", command=self._execute_selected).grid(row=2, column=2, sticky="ew", pady=(4, 0))
    ...
    def _update_plan(self):
        try:
            plan = self.fs.predict_next_plan(length=3)
    ...
    def _accept_step(self):
        ...
        if not step:
            ...
            return
        # Record habit, optionally execute
    ...
    def _skip_step(self):
        ...
        if not step:
            self.status_var.set("No step selected.")
            return
        # Negative reinforcement to push predictor away
        self.fs.predictor.reward(matched=False)
        # Remove from plan view only; predictor will adapt over time
    ...
    def _edit_actions(self):
        # Simple inline editor: enter habit_key and action command/open target
        habit_key = simpledialog.askstring("Action editor", "Habit key to bind (e.g., app:Notepad, web:Example):")
        if not habit_key:
            return
