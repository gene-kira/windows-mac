#!/usr/bin/env python3
"""
Fused Guardian – Evolution Tier + Game Cortex (Whitelist/Blacklist + Manual Tabs)
...
- Tkinter GUI (Dashboard + Game Cortex Tactical HUD)
...
# ---------------------------------------------------------------------------
# UPDATED: Whitelist-aware helper
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Bloodhound Mode (UPDATED: No auto-kill, whitelist-aware)
# ---------------------------------------------------------------------------
...
class BloodhoundMode(BaseMode):
    ...
    def _capture_snapshot(self) -> Dict[str, Any]:
        suspicious = 0
        total = 0
        high_risk_terminated = 0  # No longer used, but kept for compatibility
        ...
        for c in conns:
            ...
            # Game processes are always safe
        ...
        return {
            ...
            "anomaly_count": anomaly_count,
            "high_risk_terminated": 0,  # Always 0 now
            "exe": "",
        }
...
# ---------------------------------------------------------------------------
# Software Interrogator Mode (UPDATED: Whitelist-aware, no auto-kill)
# ---------------------------------------------------------------------------
    ...
    def _capture_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            for p in psutil.process_iter(["pid", "name", "exe"]):
                ...
                # Windows directories are safe
                if self._is_under_windows_dirs(exe):
                    installed_count += 1
                    continue

                # Installed software name match
                known = False
...
# ---------------------------------------------------------------------------
# GuardianEngine (UPDATED: whitelist/blacklist support)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GUI – Tactical HUD styling and dashboard (UPDATED: manual tab switching)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Security Lists Panel (Whitelist / Blacklist GUI)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GuardianApp (UPDATED: manual tab switching, security lists)
# ---------------------------------------------------------------------------
...
class GuardianApp(tk.Tk):
    def __init__(self, engine: GuardianEngine):
        ...
        # Manual switching only — no auto-switch logic anywhere
...
# ---------------------------------------------------------------------------
# Game Cortex Tab (manual switching only)
# ---------------------------------------------------------------------------
...
class GameCortexFrame(ttk.Frame):
    ...
    # -------------------------------------------------------------------
    # REFRESH LOOP (manual tab switching only)
    # -------------------------------------------------------------------
    def refresh(self):
