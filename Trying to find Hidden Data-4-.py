...
class Finding:
    def __init__(self, ftype, description, snippet, source_type, source_id,
        ...
        self.snippet = snippet                # raw HTML snippet
        self.source_type = source_type        # "web", "email", "raw"
        self.source_id = source_id            # URL, filename, subject, etc.
        ...
        self.extras = extras or {}            # dictionary of additional info (domain, params, etc.)
...
class HiddenDataDetector:
    ...
    def _scan_links(self, soup, source_type, source_id):
        ...
        for a in soup.find_all("a"):
            ...
            # Visible text vs href domain mismatch
    ...
    def _scan_iframes_scripts(self, soup, source_type, source_id):
        ...
        for script in soup.find_all("script"):
            ...
            if src:
                ...
                # Inline script pattern scanning
    ...
    def _scan_auto_forms(self, soup, source_type, source_id):
        ...
        for form in soup.find_all("form"):
            ...
            # Many hidden inputs + tracking names
    ...
    def _analyze_query(self, url):
        """Return (tracking_keys, long_query, base64ish) for a URL."""
        if "?" not in url:
        ...
        for pair in qs.split("&"):
            ...
            for tk in self.tracking_param_keywords:
                if tk in k_lower:
                    tracking_keys.add(k)
            # crude base64-ish detector: long, only base64 charset, ends with = sometimes
            v_clean = re.sub(r"%[0-9a-fA-F]{2}", "", v)  # strip URL encoding
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Hidden Data & Behavior Inspector (Advanced single-file)")
        self.resize(1200, 750)

        central = QtWidgets.QWidget()
        main_layout = QtWidgets.QVBoxLayout(central)

        # Top row: buttons and summary
        ...
        # Summary label
        self.summary_label = QtWidgets.QLabel("No scan yet.")
    ...
    def show_findings(self, findings):
        ...
        self.apply_filters(update_summary=True)
        ...
        if total == 0:
            msg = "No hidden/suspicious items detected with current rules."
            ...
            if counts["suspicious"] or counts["critical"]:
    ...
    def clear_results(self):
        ...
        self.summary_label.setText("No scan yet.")

    # ---- filtering & table ----

    def apply_filters(self, update_summary=False):
        self.table.setRowCount(0)
        self.filtered_indices = []
        ...
        if update_summary and self.findings:
