...
"""
Foresight: anticipatory assistant with hierarchical workflows, ensemble confidence, branching forecasts,
...
Enhancements:
- Hierarchical workflows: discovers macro routines and nested chains.
- Ensemble confidence: blends sequence/context/seasonality scores with workflow and context votes.
- Branching forecasts: primary and alternative paths with probabilities (merged view in UI).
- Adaptive anomaly detection: rolling baselines per context/time.
- Context fusion: optional external context (deadlines/events) plus system telemetry.
- Feedback weighting: stronger adaptation on accept/skip to personalize faster.
- Habit clustering with semantic tags: work/play/comms/system, biases by time and context.

Base capabilities retained:
- Multi-step transitions, hour smoothing, recency boosts, cluster generalization.
- Encrypted cache of predictor, actions, calibration, safelists, and settings.
...
class ContextManager:
    ...
    def features(self) -> Dict[str, float]:
        ...
        # Normalize deadline: closer deadline => higher urgency
...
# ----------------------------
# Workflow chaining and hierarchy
# ----------------------------
class WorkflowChains:
    ...
    def _build_hierarchy(self) -> None:
        ...
        # Sort chains by length and count (longer and more frequent first)
        for k in hier:
    ...
    def next_in_chain(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str]) -> Optional[Tuple[str, float]]:
        if not prev1:
            return None
        # Prefer matching the longest chain starting at prev3/prev2/prev1 as prefix
        tails = [
            (3, (prev3, prev2, prev1)),
            (2, (prev2, prev1)),
            (1, (prev1,))
        ...
        for L, tail in tails:
            ...
            for chain, cnt in self.chain_counts.items():
                ...
                if tuple(chain[:L]) == tail:
                    candidate = chain[L]
                    score = cnt * (1.0 + 0.25 * (L-1)) * (1.0 + 0.05 * len(chain))
                    if score > best_score:
                        best = candidate
                        best_score = score
        if best:
...
class NextGenPredictor:
    def __init__(self):
        ...
        self.tag_bias = {
            ...
            "comms": 0.10,
            "system": 0.06,
            "play": -0.08  # slightly discouraged in typical work hours
        }
        ...
        # Rolling baselines per hour/dow for anomaly
    ...
    def _semantic_tag(self, habit_key: Optional[str], hour: int) -> Optional[str]:
        ...
        if cl == "system":
            return "system"
        if cl == "browser":
            # treat browser as work during 8-18, play for late night
    ...
    def _score_snapshot(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
        ...
        for h in habits.keys():
            ...
            # Context fusion: deadlines => work/comms priority; meeting_now => comms priority
            if ctx_features:
    ...
    def anomaly_score(self, habit: str, components: Dict[str, float], hour: int, dow: int) -> float:
        # Rolling baseline for that hour/dow vs current total contribution
        baseline = self.anomaly_baseline[(hour, dow)].get(habit, 0.0)
        total = components.get("total", 0.0)
        # normalize baseline
        norm_base = baseline / (baseline + 8.0)  # smooth
        # if total is large but baseline is tiny => anomaly high; or vice versa
...
class Foresight:
    ...
    def record_habit(self, habit: str, ts: Optional[float] = None) -> None:
        ...
        if (time.time() - self.workflows.last_build_ts) > 60.0:
            self.workflows.ingest_log(self.event_log[-800:])  # use recent slice
        self._save_cache()
    ...
    def _ensemble(self, base_pred: Optional[str], base_conf: float,
        ...
        # Simple ensemble: if chain suggests same as base, boost; if different, pick higher confidence but penalize anomaly via context urgency
        pred = base_pred
        conf = base_conf
        if chain_suggestion:
            c_step, c_conf = chain_suggestion
            if base_pred == c_step:
                conf = min(1.0, base_conf + 0.15 * c_conf)
            else:
                # Context urgency favors work/comms workflows
                urgency = ctx_features.get("deadline_urgency", 0.0) + ctx_features.get("meeting_now", 0.0)
                if c_conf > base_conf * (0.95 - 0.15 * urgency):
    ...
    def predict_branch_plans(self, when_ts: Optional[float] = None, length: int = 14) -> Tuple[List[Tuple[str, float]], List[Tuple[str, float]]]:
        # primary plan and an alternative branch starting from a deviation at step 2
        try:
            now_ts = when_ts if when_ts is not None else time.time()
            dt = datetime.datetime.fromtimestamp(now_ts)
            ctx_features = self.context.features()
            with self._lock:
                prev1, prev2, prev3 = self._prev_context()
            primary = self.predictor.predict_plan(prev1, prev2, prev3, dt, self.habits, now_ts, ctx_features, length=length)
            # Alternative: skip step 1 and recompute from prev1,prev2 with a synthetic deviation
            alt: List[Tuple[str, float]] = []
            if primary:
                dev_prev1 = prev1
                dev_prev2 = prev2
                dev_prev3 = prev3
                # pretend we chose the second-best at first decision by damping the top suggestion weight (approx via reward negative)
                self.predictor.reward(matched=False)
                alt = self.predictor.predict_plan(dev_prev1, dev_prev2, dev_prev3, dt, self.habits, now_ts, ctx_features, length=length-1)
            # Calibrate
            primary = [(s, self.calibrator.calibrate(c)) for s, c in primary]
            alt = [(s, self.calibrator.calibrate(c)) for s, c in alt]
            return primary, alt
        except Exception:
            return [], []
    ...
    def predict_next_plan(self, when_ts: Optional[float] = None, length: int = 14) -> List[Tuple[str, float]]:
        ...
        # Merge alt into primary view as labelled alternatives after step 2
        merged: List[Tuple[str, float]] = []
        for i, (s, c) in enumerate(primary):
            merged.append((s, c))
            if i == 1 and alt:
                # insert up to 3 alt steps with "(alt)" tag
