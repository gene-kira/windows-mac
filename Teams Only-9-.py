...
BEHAVIOR_DB = {}
# Stats DB: rolling system-level stats
STATS_DB = {
    "history": []  # list of {ts, proc_count, high_count, net_count, avg_score}
}
...
# ---------- STATS STORAGE (TEMPORAL ANOMALIES) ----------
...
class SentinelBrain:
    ...
    @staticmethod
    def compute_fingerprint(info, behavior_record):
        fp = []
        path = info["path"]
        name_l = info["name_l"]
        cpu = info["cpu"]
    ...
    @staticmethod
    def compute_score(info, behavior_record, bad_fingerprints):
        score = 0
        anomalies = []
        matched_bad_fp = False
        ...
        fp = SentinelBrain.compute_fingerprint(info, behavior_record)
        ...
        return score, threat, anomalies, fp, matched_bad_fp
...
class SentinelGUI:
    ...
    def add_selected_block(self):
        ...
        with BEHAVIOR_LOCK:
            ...
            # store current fingerprints as explicitly bad patterns
    ...
    # ---------- LOCKDOWN MODE / PROGRESSIVE ENGINE ----------
    ...
    def update_behavior(self, proc, info, bad_fingerprints):
        path = info["path"]
        name_l = info["name_l"]
        key = self._key_for_behavior(path, name_l)
        now = time.time()
        ...
        with BEHAVIOR_LOCK:
            rec = BEHAVIOR_DB.get(key, {})
            score, threat, anomalies, fp, matched_bad_fp = self.brain.compute_score(info, rec, bad_fingerprints)
        ...
        while self.running:
            ...
            for proc in psutil.process_iter():
                try:
                    ...
                    self.update_behavior(proc, info, bad_fps)
