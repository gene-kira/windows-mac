...
- Aggressive mode defaults (high-frequency sampling)
"""
...
class HiveIdentity:
    ...
    version: str = "1.7.0-wave2-aggressive"
            ...
            with self.lock:
                ...
                for key in sample_keys:
                    ...
                    try:
                        ...
                        print(f"[BORG_IO] Integrity error on {path} block {block_index}, reloading.")
...
class QueenBorg:
    ...
    def _score_current_config(self, stats: Dict[str, Any], pressure_trend: float) -> float:
        reads = stats["reads"]
    ...
    def _tune_borg_io(self):
        ...
        trend = self._pressure_trend()
        score = self._score_current_config(stats, trend)
...
def run_cockpit_telemetry(telemetry: TelemetryBus, seg_telemetry: SegmentedTelemetry,
    ...
    try:
        while True:
            ...
            if seg is not None:
                ov = {
                    "identity": {"hive_name": "The Borg Cache", "version": "segmented"},
                    "scan": {"gpu_mode": "unknown"},
                    "coolant": seg.get("coolant", {}),
                    "cache": seg.get("cache", {}),
                    "borg_io": seg.get("borg_io", {}),
                    "controls": seg.get("meta", {}).get("controls", {}),
                    "hive": seg.get("meta", {}).get("hive", {}),
                }
...
def main():
    parser = argparse.ArgumentParser(description="The Borg Cache - Model A, Wave 1 + Wave 2 (Aggressive)")
    parser.add_argument(
        "--mode",
