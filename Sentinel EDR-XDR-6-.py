...
# ============================================================
# DATA-LEAK / WINDOWS / APP HEURISTICS
# ============================================================

def is_windows_core(path: str) -> bool:
    if not path or path.startswith("<no-path:"):
        return False
    system_root = os.environ.get("SystemRoot", r"C:\Windows")
    try:
        return norm_path(path).startswith(norm_path(system_root))
    except Exception:
        return False

def is_browser_name(name_l: str) -> bool:
    browsers = ["chrome", "msedge", "edge", "firefox", "brave", "opera", "vivaldi", "safari"]
    return any(b in name_l for b in browsers)

def is_game_process(name_l: str, path: str) -> bool:
    if "steam" in name_l or "epic" in name_l or "battle.net" in name_l:
        return True
    if path:
        p = path.lower()
        if "\\games\\" in p or "\\steamapps\\" in p or "\\riot games\\" in p:
            return True
    return False

def is_office_like(name_l: str) -> bool:
    office = ["winword", "excel", "powerpnt", "outlook"]
    return any(o in name_l for o in office)

# ============================================================
# CHAMELEON / MIRROR / GLYPH – ENCRYPTION HOOKS
# ============================================================

def chameleon_encrypt(meta: dict):
    """
    Chameleon: adaptive encryption wrapper – conceptually encrypts outbound PII.
    """
    log_event(f"CHAMELEON: Encrypting outbound PII for {meta.get('name')} pid={meta.get('pid')}")

def mirror_obfuscate(meta: dict):
    """
    Mirror: structural obfuscation – reshapes identifiers, tokens, and IDs.
    """
    log_event(f"MIRROR: Obfuscating identifiers for {meta.get('name')} pid={meta.get('pid')}")

def glyph_mask(meta: dict):
    """
    Glyph: glyph-based masking – replaces visible PII with non-reversible glyphs.
    """
    log_event(f"GLYPH: Masking visible PII for {meta.get('name')} pid={meta.get('pid')}")
...
class SentinelBrain:
    def classify(self, proc):
        ...
        # Windows grey-zone: allowed but telemetry/data-leak prone
        if is_windows_core(exe):
            windows_grey = True
            # Keep status as ALLOW/TRUSTED if already, otherwise treat as ALLOW
            if status not in ("ALLOW", "TRUSTED", "BLOCKED", "RADIOACTIVE"):
                status = "ALLOW"

        # Auto radioactive classification for browsers / games / office-like
        if status not in ("BLOCKED", "RADIOACTIVE"):
            if is_browser_name(name_l) or is_game_process(name_l, exe) or is_office_like(name_l):
                status = "RADIOACTIVE"
                data_leak = True
                pii_guard = True
...
class EngineCore:
    ...
    def _monitor_loop(self):
        while self.running:
            ...
            # Outbound PII interception hook
            self.inspect_pii_leaks(net_rows, pid_to_key)
    ...
    def update_behavior(self, proc, info, bad_fingerprints, cluster_stats, campaign_stats, now):
        ...
        with BEHAVIOR_LOCK:
            ...
            # Persist grey/data-leak flags from classification
            rec["windows_grey"] = info.get("windows_grey", False)
            # Heuristic data-leak: network + suspicious/radioactive/office/browser
            if rec.get("has_network", False) or info["status"] in ("RADIOACTIVE", "SUSPICIOUS"):
                if is_browser_name(name_l) or is_office_like(name_l) or is_game_process(name_l, path):
                    rec["data_leak"] = True
                    rec["pii_guard"] = True
        ...
        info["windows_grey"] = rec.get("windows_grey", False)
        info["data_leak"] = rec.get("data_leak", False)
        info["pii_guard"] = rec.get("pii_guard", False)
    ...
    def auto_update_lists(self, info):
        ...
        # Auto-radioactive for suspicious paths (data-leak prone)
    ...
    def inspect_pii_leaks(self, net_rows, pid_to_key):
        """
        Outbound PII interception hook:
        - Any process marked data_leak/pii_guard gets Chameleon/Mirror/Glyph stack applied.
        """
        with BEHAVIOR_LOCK:
            for conn in net_rows:
                ...
                if not rec:
                    continue
                if rec.get("data_leak") or rec.get("pii_guard"):
                    meta = {
                        "pid": pid,
                        "name": conn.get("name"),
                    ...
                    chameleon_encrypt(meta)
                    mirror_obfuscate(meta)
                    glyph_mask(meta)
                    log_event(
                        f"PII-GUARD: Outbound connection protected for PID {pid} "
                        f"{conn.get('name')} {conn.get('laddr')} -> {conn.get('raddr')}"
                    )

    def ai_data_leak_detection(self):
        """
        AI-driven data-leak detection stub:
        - Periodically sends a snapshot of high-risk, networked processes to LLM.
        - Today: used for analyst narrative + future tuning, not hard decisions.
        """
        with BEHAVIOR_LOCK:
            candidates = []
            for key, rec in BEHAVIOR_DB.items():
                if rec.get("has_network") and (rec.get("data_leak") or rec.get("last_score", 0) >= 60):
                    candidates.append({
                        "key": key,
                        "last_score": rec.get("last_score", 0),
                        "parent_type": rec.get("parent_type"),
                        "cluster_key": rec.get("cluster_key"),
                        "data_leak": rec.get("data_leak"),
                    })
            if not candidates:
                return

        prompt = (
            "You are an EDR/XDR analyst LLM focused on data-leak and PII exfiltration.\n"
            "Given these high-risk, networked processes, identify which ones are most likely leaking PII "
            "such as user IDs, phone numbers, biometrics, machine IDs, MAC addresses, or IPs.\n"
            "Return a short narrative for the analyst; do NOT change policy directly.\n\n"
            f"Candidates: {json.dumps(candidates, indent=2)}\n"
        )
        results = self.llm_manager.ask_role("anomaly", prompt)
        if results:
            for name_engine, resp in results.items():
                log_event(f"AI DATA-LEAK ({name_engine}): {resp[:400]}")
    ...
    def _watchdog_loop(self):
        while self.running:
            ...
            # AI-driven data-leak detection every ~60s
            if now - self._last_data_leak_ai_ts > 60:
                self._last_data_leak_ai_ts = now
                self.ai_data_leak_detection()
...
class EDRConsole(QMainWindow):
    ...
    def on_proc_double_click(self, row, col):
        ...
        windows_grey = row_info.get("windows_grey", False)
        data_leak = row_info.get("data_leak", False)
        pii_guard = row_info.get("pii_guard", False)
        ...
        text += f"Windows grey-zone: {'YES' if windows_grey else 'NO'}\n"
        text += f"Data-leak risk: {'YES' if data_leak else 'NO'}\n"
        text += f"PII guard active: {'YES' if pii_guard else 'NO'}\n\n"
    ...
    def on_explain_selected(self):
        ...
        if len(rows) == 1:
            ...
            prompt = (
                ...
                "5) Whether it is likely leaking PII (IDs, phone, bio, machine IDs, MAC, IP) and why.\n\n"
                f"Name: {name}\n"
                f"PID: {pid}\n"
                ...
                f"Windows_grey: {windows_grey}\n"
                f"Data_leak_flag: {data_leak}\n"
                f"PII_guard: {pii_guard}\n"
            ...
            if self.llm_fuse_checkbox.isChecked() and len(results) > 1:
                fusion = self.llm_manager.fuse(
                    results,
                    task_description="Explain single process with kill-chain, prediction, and data-leak risk."
                )
                text += "===== FUSION SUMMARY =====\n"
                text += fusion
            ...
            prompt = (
                ...
                "behavior, likely kill-chain stages, and data-leak/PII exfiltration risk.\n\n"
                "For each process, consider:\n"
                "- anomalies\n"
                "- score and threat label\n"
                "- cluster and campaign context\n"
                "- predicted next behaviors\n"
                "- likelihood of leaking PII (IDs, phone, bio, machine IDs, MAC, IP)\n\n"
            ...
            if self.llm_fuse_checkbox.isChecked() and len(results) > 1:
                fusion = self.llm_manager.fuse(
                    results,
                    task_description="Compare two processes, map to kill-chain, and assess data-leak risk."
                )
                text += "===== FUSION SUMMARY =====\n"
                text += fusion
    ...
    def _build_row_explanation(self, info):
        parts = []
        if info.get("windows_grey"):
            parts.append("Windows core process (grey zone: allowed but may leak telemetry).")
        if info["status"] == "ALLOW":
            parts.append("Explicitly allowed.")
        if info["status"] == "BLOCKED":
            parts.append("Explicitly blocked.")
        if info["status"] == "RADIOACTIVE":
            parts.append("Radioactive: potential data-leak / PII exfil path.")
        if info.get("data_leak"):
            parts.append("Data-leak guard active: outbound PII will be encrypted/obfuscated.")
        if info.get("pii_guard"):
            parts.append("Chameleon/Mirror/Glyph stack attached to outbound connections.")
        anomalies = info.get("anomalies", [])
        if anomalies:
            parts.append("Anomalies: " + "; ".join(anomalies[:4]))
        return "\n".join(parts) if parts else "No special notes."

    def _color_for_row(self, info):
        status = info["status"]
        windows_grey = info.get("windows_grey", False)
        data_leak = info.get("data_leak", False)

        # Windows grey-zone: light grey
        if windows_grey:
        ...
        # Radioactive / data-leak: red
        if status == "RADIOACTIVE" or data_leak:
            return QColor(255, 200, 200)

        # Suspicious: light yellow
        if status == "SUSPICIOUS":
    ...
    def refresh_process_view(self):
        ...
        for i in rows:
            ...
            explanation = self._build_row_explanation(i)
            row_color = self._color_for_row(i)
            for c, v in enumerate(vals):
                item = QTableWidgetItem(v)
                item.setToolTip(explanation)
                # Color-coded rows
                item.setBackground(QBrush(row_color))
                # For black background, make text white
                if row_color == QColor(0, 0, 0):
