...
class HiveMindGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Unified Hive-Mind Controller (Predictive, Live Telemetry)")
        ...
        ttk.Button(button_frame, text="Activate Hive-Mind", command=self.activate_hive)\
            .grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(button_frame, text="Trigger Ghost Boost", command=self.trigger_boost)\
            .grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(button_frame, text="Run Predictive Foresight", command=self.run_foresight)\
        ...
        ttk.Button(button_frame, text="Restore from Backup", command=self.restore_backup)\
        ...
        self.activity_counter = 0  # cumulative activity bytes
        self.state = {}
        ...
        # Predictive layer: store last 60 seconds of telemetry
        self.history = deque(maxlen=60)  # each entry: dict with ram_pct, vram_pct, combined_pct, activity
    ...
    def activate_hive(self):
        ...
        if not self.telemetry_running:
            ...
            self.log("Telemetry + predictive loop started (RAM/VRAM/Disk/Net/CPU).")
    ...
    def trigger_boost(self):
        ...
        self.log(
            f"L4 Ghost Cache ({self.mode}) boosting {self.cores} cores and "
    ...
    def run_foresight(self):
        ...
        self.log(
            f"Foresight {'succeeded' if success else 'failed'} → policy updated: "
            f"{self.learner.policy}"
        )
    ...
    def update_usage_and_predict(self, ram_percent, vram_percent, combined_percent, activity):
        # Update bars
        ...
        # Autosave after enough activity
    ...
    def run_predictive_analysis(self):
        ...
        activity_series = [h["activity"] for h in self.history]

        # Simple anomaly detection via z-score on combined load
        try:
            ...
            if stdev_combined > 0:
                ...
                if abs(z) >= 3:
                    self.log(
                        f"[Glitch Detection] Anomalous combined load detected (z={z:.2f}). "
                        f"Mean={mean_combined:.1f}%, Latest={latest:.1f}%"
                    )
        except Exception:
            pass

        # EWMA forecast for next few seconds
        ...
        # Rough trend estimation: difference between last and first, per second
        ...
        if t_comb_80 is not None and t_comb_80 <= 60:
            msg_parts.append(f"Combined load may cross 80% in ~{t_comb_80:.1f}s.")
        if t_comb_90 is not None and t_comb_90 <= 60:
            msg_parts.append(f"Combined load may cross 90% in ~{t_comb_90:.1f}s.")
