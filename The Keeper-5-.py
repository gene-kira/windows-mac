...
- FIRST-LAUNCH GUI PICKER FOR MEMORY LOCATION
- WINDOWS-SPECIFIC TRUST MODEL
- STEAM + EPIC GAME LAUNCHER TRUST (AND CHILD GAMES)
- MANUAL WHITELIST/BLACKLIST OVERRIDES
- LEAK MONITORING + NON-BLOCKING GUI POPUP ALERTS
...
- Steam/Epic + all child games:
  - Trusted to run, monitored for leaks
  - Attempts to touch vault or sensitive categories => leak alert popup
...
- Persistent memory with:
  ...
  - Backup memory path (local or SMB, optional)
  - First-launch GUI picker for user to choose paths
  - Automatic fallback to backup path if primary save fails
- Windows-specific trust model with:
  - Whitelist/blacklist registry
  - Game launcher category
  - Game child category
- Live GUI controls to whitelist/blacklist EXE paths
- GUI console (tkinter) with mode indicator, alerts, and status panels
"""
...
class MemoryConfig:
    ...
    If not present, it:
    - If tkinter is available: shows a small GUI dialog for the user to pick
      primary and optional backup paths (local or SMB).
    - Otherwise: defaults to a local file in the home directory.
    """
    ...
    def _load_or_init(self):
        if os.path.exists(self.config_path):
            try:
                ...
                log(f"MemoryConfig load error, falling back to picker/defaults: {e}")
                traceback.print_exc()

        if not self.primary_memory_path:
            if tk is not None and filedialog is not None:
                self._run_gui_picker()
            else:
                self.primary_memory_path = os.path.join(self.home, "guardian_state.json")
                self.backup_memory_path = None
                log("MemoryConfig: tkinter not available, using default local path.")

        self._save()

    def _run_gui_picker(self):
        root = tk.Tk()
        root.title("Guardian Memory Location Setup")
        root.geometry("600x220")
        root.resizable(False, False)

        primary_var = tk.StringVar(value=os.path.join(self.home, "guardian_state.json"))
        backup_var = tk.StringVar(value="")

        def browse_primary():
            path = filedialog.asksaveasfilename(
                title="Select primary memory location",
                initialdir=self.home,
                defaultextension=".json",
                filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
            )
            if path:
                primary_var.set(path)

        def browse_backup():
            path = filedialog.asksaveasfilename(
                title="Select backup memory location (optional)",
                initialdir=self.home,
                defaultextension=".json",
                filetypes=[("JSON files", "*.json"), ("All files", "*.*")]
            )
        ...
        def on_ok():
            p = primary_var.get().strip()
            if not p:
                if messagebox is not None:
                    messagebox.showerror("Error", "Primary memory path is required.")
                return
            self.primary_memory_path = p
            b = backup_var.get().strip()
            self.backup_memory_path = b if b else None
            root.destroy()

        frm = ttk.Frame(root, padding=10)
        frm.pack(fill="both", expand=True)

        ttk.Label(frm, text="Primary memory path (local or SMB):").grid(row=0, column=0, sticky="w")
        entry_primary = ttk.Entry(frm, textvariable=primary_var, width=65)
        entry_primary.grid(row=1, column=0, sticky="we", padx=(0, 5))
        btn_primary = ttk.Button(frm, text="Browse...", command=browse_primary)
        btn_primary.grid(row=1, column=1, sticky="e")

        ttk.Label(frm, text="Backup memory path (optional):").grid(row=2, column=0, sticky="w", pady=(10, 0))
        entry_backup = ttk.Entry(frm, textvariable=backup_var, width=65)
        entry_backup.grid(row=3, column=0, sticky="we", padx=(0, 5))
        btn_backup = ttk.Button(frm, text="Browse...", command=browse_backup)
        btn_backup.grid(row=3, column=1, sticky="e")

        ttk.Label(
            frm,
            text="Tip: You can use SMB paths like \\\\SERVER\\Share\\guardian_state.json\n"
                 "If the primary path fails later, the guardian will try the backup path."
        ).grid(row=4, column=0, columnspan=2, sticky="w", pady=(10, 5))

        btn_ok = ttk.Button(frm, text="Save and Continue", command=on_ok)
        btn_ok.grid(row=5, column=0, columnspan=2, pady=(10, 0))
        ...
        if not self.primary_memory_path:
            self.primary_memory_path = os.path.join(self.home, "guardian_state.json")
            self.backup_memory_path = None
            log("MemoryConfig: user did not complete picker, using default local path.")
    ...
    def _save(self):
        try:
            ...
            log(f"MemoryConfig saved to {self.config_path}")
        except Exception as e:
...
class GuardianCore:
    def __init__(self, system_profile: SystemProfile, event_bus: EventBus, memory_store: MemoryStore):
        ...
        #   "trusted": bool,
        #   "reason": str,
        #   "source": "whitelist"/"blacklist"/"whitelist_game_launcher"/"game_launcher_child"/"auto"
        # }
        ...
        if self.system.os_family == "windows":
            ...
            # Seed Steam + Epic as game launchers (if present or not, registry still stands)
            game_launchers = [
            ...
            for path in game_launchers:
                norm = os.path.normcase(path)
                if norm not in self.trust_registry:
                    self.trust_registry[norm] = {
                        "trusted": True,
                        "reason": "trusted_to_run_but_monitored_for_leaks",
                        "source": "whitelist_game_launcher"
                    }
    ...
    def add_game_child_trust(self, exe_path):
        exe_norm = os.path.normcase(exe_path)
        if exe_norm in self.trust_registry:
            return
        self.trust_registry[exe_norm] = {
            "trusted": True,
            "reason": "launched_by_trusted_game_launcher",
            "source": "game_launcher_child"
        }
        log(f"Game child trusted: {exe_norm}")
        self.event_bus.emit("trust_update", {
            "exe": exe_norm,
            "trusted": True,
            "source": "game_launcher_child",
    ...
    def is_process_trusted(self, proc_info):
        """
        Returns:
        (trusted: bool, trust_reason: str, trust_source: str or None)
        """
        exe = (proc_info.get("exe") or "").strip()
        if not exe:
            return False, "no_exe_path", None

        exe_norm = os.path.normcase(exe)
        reg_entry = self.trust_registry.get(exe_norm)

        if reg_entry is not None:
            src = reg_entry.get("source")
            reason = reg_entry.get("reason", "")
            if src == "blacklist":
                return False, reason or "blacklisted", src
            return bool(reg_entry.get("trusted", True)), reason or "whitelisted", src

        if self.system.os_family == "windows":
            for root in self.win_system_roots:
                try:
                    if exe_norm.startswith(root.lower()):
                        return False, "windows_system_path_unwhitelisted", None
                except Exception:
                    continue
            return False, "windows_non_system_path_unwhitelisted", None

        elif self.system.os_family == "linux":
            return False, "linux_zero_trust_default", None

        else:
            return False, "unknown_os_zero_trust_default", None
    ...
    # ------------ Presidential stability enforcement ------------
    ...
    def on_sensitive_access_attempt(self, process_info, data_category):
        """
        - If untrusted: block, escalate if needed.
        - If trusted game launcher or game child: still block vault access,
          and raise a leak alert popup (non-blocking).
        - If trusted non-game: allow (for now).
        """
        trusted, trust_reason, trust_source = self.is_process_trusted(process_info)
        decision = "allow"
        reason = "trusted_process"

        is_game_related = trust_source in ("whitelist_game_launcher", "game_launcher_child")
        # Any sensitive category is guarded; you can specialize later
        sensitive_categories = {"SSN", "EMAIL", "MIXED_ID", "GENERIC"}
        ...
        if not trusted:
            ...
            # Trusted, but if it's a game launcher or game child touching sensitive data -> leak alert
            if is_game_related and data_category in sensitive_categories:
                decision = "block"
                reason = f"game_leak_block_{data_category}"
                self.event_bus.emit("leak_alert", {
                    "process": process_info,
                    "data_category": data_category,
            ...
            try:
                if psutil and self.system.supports_process_inspection:
                    for proc in psutil.process_iter(['pid', 'name', 'exe', 'username', 'ppid']):
                        ...
                        # Game child detection: if parent is a trusted game launcher, mark child trusted
                        if ppid and self.system.os_family == "windows":
                            try:
                                parent_proc = psutil.Process(ppid)
                                parent_exe = parent_proc.exe() or ""
                                parent_norm = os.path.normcase(parent_exe)
                                reg = self.guardian.trust_registry.get(parent_norm)
                                if reg and reg.get("source") == "whitelist_game_launcher":
                                    if exe:
                                        self.guardian.add_game_child_trust(exe)
                            except Exception:
                                pass
...
# ============================================================
# GUI console (with trust control tab and leak popups)
# ============================================================
...
class GuardianGUI:
    ...
    def show_leak_popup(self, proc, data_category, trust_source, trust_reason):
        """
        Non-blocking popup: does not freeze the game.
        Lets user choose to block (blacklist), whitelist, or ignore.
        """
        exe = proc.get("exe") or ""
        ...
        popup.title("Leak-like behavior detected")
        popup.geometry("420x180")
        popup.resizable(False, False)

        msg = (
            f"Leak-like behavior detected.\n\n"
            f"Process: {name or '<unknown>'}\n"
            f"PID: {pid}\n"
            f"EXE: {exe}\n"
            f"Data category: {data_category}\n"
            f"Trust source: {trust_source} ({trust_reason})\n\n"
            f"Action taken: Blocked access to sensitive data.\n"
            f"(Game continues running; this popup is advisory.)"
        )

        lbl = tk.Label(popup, text=msg, justify="left", wraplength=400)
        ...
        def on_block():
            if exe:
                self.guardian.add_to_blacklist(exe, reason="user_blocked_after_leak_alert")
            popup.destroy()

        def on_allow():
            if exe:
                self.guardian.add_to_whitelist(exe, reason="user_whitelisted_after_leak_alert")
            popup.destroy()

        def on_ignore():
            popup.destroy()

        btn_block = tk.Button(btn_frame, text="Block (Blacklist)", command=on_block, fg="red")
        btn_block.pack(side="left", padx=5)

        btn_allow = tk.Button(btn_frame, text="Allow (Whitelist)", command=on_allow, fg="green")
        btn_allow.pack(side="left", padx=5)

        btn_ignore = tk.Button(btn_frame, text="Ignore", command=on_ignore)
    ...
    def process_events(self):
        while True:
            ...
            if etype == "mode_change":
                ...
                trust_source = payload.get("trust_source")
                line = (
                    f"[SensitiveAccess] proc={proc.get('name')} pid={proc.get('pid')} exe={proc.get('exe')} "
                    f"category={cat} decision={decision} reason={reason} trust={trust_reason} src={trust_source}\n"
                )
            ...
            elif etype == "leak_alert":
                proc = payload.get("process", {})
                cat = payload.get("data_category")
                trust_source = payload.get("trust_source")
                trust_reason = payload.get("trust_reason")
                line = (
                    f"[LeakAlert] proc={proc.get('name')} pid={proc.get('pid')} exe={proc.get('exe')} "
                    f"category={cat} trust_source={trust_source} trust_reason={trust_reason}\n"
                )
                self.append_log("Presidential Mode", line)
                self.show_leak_popup(proc, cat, trust_source, trust_reason)
