...
def trainer_main():
    ...
    # Threading config: always reserve at least 2 threads for background
    try:
...
if IS_GUI:
    ...
    class SystemMonitor(QtCore.QObject):
        ...
        def __init__(self, parent=None):
            ...
            # Faster polling for hot-plug detection (2 seconds)
    ...
    def main_gui():
        # Pin GUI/background to first 2 cores (or as many as available)
        try:
            p = psutil.Process()
            total_cores = psutil.cpu_count(logical=False) or psutil.cpu_count(logical=True) or 1
            if total_cores >= 2:
