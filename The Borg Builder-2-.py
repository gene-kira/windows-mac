...
# with a basic Prediction Organ and fixed Qt usage.
...
class PredictionEngine:
    """
    Very simple v0 prediction engine:
    ...
    def _tokenize(self, text: str):
        # crude: split on spaces and symbols, lowercase, ignore very short tokens
        delimiters = ".,;:!?()[]{}<>\"'\\/-_"
        tmp = text.lower()
        for d in delimiters:
            tmp = tmp.replace(d, " ")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, thought_model: ThoughtStreamModel):
        ...
        self._start_prediction_log_watcher()

    def _init_ui(self):
        central = QtWidgets.QWidget()
        layout = QtWidgets.QHBoxLayout(central)

        # Left: organs + vitals + prediction
        ...
        self.organs_list.setFixedHeight(170)
    ...
    def _start_prediction_log_watcher(self):
        def watch():
            last_size = 0
            while True:
                if PREDICTION_LOG.exists():
                    size = PREDICTION_LOG.stat().st_size
                    if size != last_size:
                        with open(PREDICTION_LOG, "r", encoding="utf-8") as f:
                            lines = f.readlines()
                        last_size = size
                        last_line = lines[-1].strip() if lines else ""
                        self._set_prediction_text(last_line)
                time.sleep(2)
    ...
    @QtCore.Slot(str)
    def _set_prediction_text(self, text: str):
        def update():
            # Show just the topics part if present
            if "topics=" in text:
                label = text.split("topics=", 1)[-1]
            else:
                label = text
            self.prediction_text.setPlainText(label)
        QtCore.QTimer.singleShot(0, update)
