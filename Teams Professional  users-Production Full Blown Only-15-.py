...
class LineageGraph:
    ...
    def propagate_risk(self, pid, base_score):
        """
        Simple graph propagation: boost score if ancestors/descendants are hot.
        """
        with self.lock:
            boost = 0
            node = self.nodes.get(pid)
            ...
            # Look at parents and children up to depth 2
            visited = set()
            queue = deque([(pid, 0)])
            while queue:
                current, d = queue.popleft()
...
class AsyncInferenceManager:
    """
    Thin wrapper around a thread pool for async ONNX inference.
    We still call result() synchronously in this version, but the
    infrastructure is ready for future decoupling.
    """
...
class ModelUpdateManager:
    ...
    def download_model_to(self, dest_path):
        ...
        Enforce a simple sandbox: auto-downloaded models must live under APPDATA.
        """
...
class ScoringPipeline:
    ...
    def load_model(path: str):
        ...
        # Sandbox: only allow loading from APPDATA or explicit user path
    ...
    def ml_model_predict(feature_vector):
        ...
        try:
            # Async inference via thread pool (still awaited here, but off UI thread)
    ...
    def compute_score(info, behavior_record, bad_fingerprints, cluster_stats, campaign_stats, now):
        ...
        if status == "ALLOW":
            return 0, "LOW", [], [], False, 0.0, 0.0, None, 0.0
        ...
        # Multi-stage chain scoring via lineage graph propagation
        ...
        return score, threat, anomalies, fp, matched_bad_fp, momentum, features["age_seconds"], features["cluster_key"], chain_boost
...
class SentinelGUI:
    ...
    def _build_ui(self):
        ...
        # Timeline view (multi-stage chain / temporal view)
    ...
    def update_behavior(self, proc, info, bad_fingerprints, cluster_stats, campaign_stats, now):
        ...
        with BEHAVIOR_LOCK:
            rec = BEHAVIOR_DB.get(key, {})
            score, threat, anomalies, fp, matched_bad_fp, momentum, age_seconds, cluster_key, chain_boost = \
