...
COMM_STATUS_OK = "OK"
COMM_STATUS_WARN = "WARN"
COMM_STATUS_ERROR = "ERROR"
...
# ============================================================
# BOARD COMMUNICATION ENGINE (ENCRYPTION + PROTOCOL)
# ============================================================

class BoardCommEngine:
    """
    Internal board communication engine:
    - Generates a random symmetric key per run (or from persisted seed)
    - Encrypts/decrypts state before writing/after reading from shared drives
    - Exposes communication health and error stats

    This doesn't send real network traffic; it just ensures that
    the state files and board messages are opaque to outsiders.
    """

    def __init__(self, config):
        self._lock = threading.Lock()
        self.errors_encrypt = 0
        self.errors_decrypt = 0
        self.total_messages = 0

        # Load or create a persistent seed to derive a key
        seed = config.get("board_comm_seed")
        if not seed:
            seed = base64.b64encode(secrets.token_bytes(32)).decode("utf-8")
            config["board_comm_seed"] = seed
            save_config(config)

        # Derive key from seed (simple hash-like transform)
        seed_bytes = base64.b64decode(seed.encode("utf-8"))
        self.key = self._derive_key(seed_bytes)

        # Build a substitution map (simple protocol flavor)
        self.sub_map, self.inv_sub_map = self._build_substitution_maps(self.key)

    def _derive_key(self, seed_bytes):
        # Simple derivation: XOR fold into 32 bytes
        key = bytearray(32)
        for i, b in enumerate(seed_bytes):
            key[i % 32] ^= b
        return bytes(key)

    def _build_substitution_maps(self, key):
        # Build a substitution alphabet based on key
        alphabet = list(range(256))
        rnd = list(key)
        # Fisher-Yates style shuffle
        j = 0
        for i in range(255, -1, -1):
            j = (j + rnd[i % len(rnd)]) % (i + 1)
            alphabet[i], alphabet[j] = alphabet[j], alphabet[i]
        sub = {i: alphabet[i] for i in range(256)}
    ...
    def _substitute(self, data, forward=True):
        mapping = self.sub_map if forward else self.inv_sub_map
        out = bytearray()
        for b in data:
            out.append(mapping[b])
        return bytes(out)

    def encrypt_state(self, state_obj):
        """
        Serialize + encrypt + encode to base64 text.
        """
        try:
            with self._lock:
            ...
            # Apply substitution + XOR
            step1 = self._substitute(raw, forward=True)
            step2 = self._xor_bytes(step1)
            enc = base64.b64encode(step2).decode("utf-8")
            return {"enc": enc}
        except Exception as e:
            with self._lock:
                self.errors_encrypt += 1
            print(f"[BoardComm] Encryption error: {e}")
            return {"enc": ""}

    def decrypt_state(self, enc_obj):
        """
        Reverse of encrypt_state. Returns dict or None on failure.
        """
        try:
            enc = enc_obj.get("enc", "")
            if not enc:
            ...
            step1 = self._xor_bytes(data)
            raw = self._substitute(step1, forward=False)
            state = json.loads(raw.decode("utf-8"))
            return state
        except Exception as e:
            with self._lock:
                self.errors_decrypt += 1
            print(f"[BoardComm] Decryption error: {e}")
            return None

    def get_status(self):
        with self._lock:
            if self.errors_decrypt > 0 or self.errors_encrypt > 0:
                status = COMM_STATUS_WARN if (self.errors_decrypt + self.errors_encrypt) < 5 else COMM_STATUS_ERROR
            else:
                status = COMM_STATUS_OK
            return {
                "status": status,
                "errors_encrypt": self.errors_encrypt,
                "errors_decrypt": self.errors_decrypt,
...
# ============================================================
# SHARED STATE SYNC (USES BOARD COMM ENGINE)
# ============================================================

class SharedStateSync(threading.Thread):
    """
    - Writes node state to active drive (primary preferred, secondary fallback)
    - Encrypts state using BoardCommEngine before writing
    - Decrypts back (for verification) using the same engine
    - Tracks drive health + latency
    - As Queen: compares states to detect differences
    - When a drive transitions OFFLINE/FAILING -> ONLINE, resyncs it
    """
    def __init__(self, node, get_primary_dir_func, get_secondary_dir_func, shared_registry, comm_engine):
        super().__init__(daemon=True)
        self.node = node
    ...
    def get_drive_latency_stats(self):
        with self._lat_lock:
            ...
            p_avg, p_recent = stats(self._primary_latencies)
            s_avg, s_recent = stats(self._secondary_latencies)
            return {
                "primary_avg": p_avg,
    ...
    def _write_state_to_dir(self, shared_dir, primary=True):
        try:
            ...
            enc_wrapper = self.comm_engine.encrypt_state(state)
            fpath = self._state_file_path(shared_dir, self.node.name)
            with open(fpath, "w", encoding="utf-8") as f:
                json.dump(enc_wrapper, f, indent=2)

            # Verify by reading and decrypting
            try:
                with open(fpath, "r", encoding="utf-8") as f:
                    enc_back = json.load(f)
                dec_state = self.comm_engine.decrypt_state(enc_back)
                if not dec_state:
        ...
        try:
            ...
            for fname in files:
                ...
                try:
                    with open(fpath, "r", encoding="utf-8") as f:
                        enc_obj = json.load(f)
                    state = self.comm_engine.decrypt_state(enc_obj)
                    if state:
                        states.append(state)
                except Exception:
                    continue
    ...
    def _resync_drive_from_registry(self, target_dir):
        ...
        try:
            ...
            for s in states:
                ...
                enc_wrapper = self.comm_engine.encrypt_state(s)
...
class PredictiveAIEngine(threading.Thread):
    ...
    def _compute_anomaly_risk(self):
        summary = self.node.threat_detector.get_summary()
        new_delta = summary["new_endpoints"] - self._last_anomaly_counts["new"]
        rare_delta = summary["rare_endpoints"] - self._last_anomaly_counts["rare"]
        unusual_delta = summary["unusual_time_endpoints"] - self._last_anomaly_counts["unusual"]

        self._last_anomaly_counts["new"] = summary["new_endpoints"]
        self._last_anomaly_counts["rare"] = summary["rare_endpoints"]
        self._last_anomaly_counts["unusual"] = summary["unusual_time_endpoints"]
        ...
        if unusual_delta > 0:
            score += 40
            notes.append("Increase in unusual-time endpoints.")
        if rare_delta > 0:
            score += 20
            notes.append("Increase in rare endpoints.")
        if new_delta > 0:
            score += 10
            notes.append("New endpoints appearing.")

        if summary["unusual_time_endpoints"] > 0:
            score += 30
            notes.append("Existing high-severity anomalies present.")
    ...
    def _compute_drive_risk(self):
        ...
        if p_status in (DRIVE_STATUS_OFFLINE, DRIVE_STATUS_FAILING):
            score += 40
            notes.append(f"Primary drive {p_status}.")
        if s_status in (DRIVE_STATUS_OFFLINE, DRIVE_STATUS_FAILING):
            score += 20
            notes.append(f"Secondary drive {s_status}.")

        for label, avg, recent in (
            ("primary", lat_stats["primary_avg"], lat_stats["primary_recent"]),
            ("secondary", lat_stats["secondary_avg"], lat_stats["secondary_recent"])
        ):
            if avg > 0.3:
                score += 20
                notes.append(f"{label.capitalize()} drive average latency high ({avg:.3f}s).")
            elif recent > 0.5:
                score += 15
                notes.append(f"{label.capitalize()} drive recent latency spike ({recent:.3f}s).")
    ...
    def _compute_collective_risk(self, anomaly_risk, drive_risk):
        with self.registry_lock:
            states = list(self.shared_registry["nodes"].values())

        if not states:
            return 100, ["No shared state yet; collective appears idle."]

        queen_state = None
        for s in states:
            if s.get("name") == "BorgNet-Queen":
                queen_state = s
                break

        if not queen_state:
            return 70, ["Queen state missing; collective visibility reduced."]

        q_mem = queen_state.get("memory_summary", {})
        q_conn = q_mem.get("connection_count", 0)
        q_endpoints = q_mem.get("known_endpoints", 0)
        ...
        for s in states:
            ...
            if q_conn > 0:
                conn_gap = abs(conn - q_conn) / float(q_conn)
            else:
                conn_gap = 0.0
            if q_endpoints > 0:
                ep_gap = abs(endpoints - q_endpoints) / float(q_endpoints)
            else:
                ep_gap = 0.0

            if conn_gap > 0.5:
                divergence_score += 15
                notes.append(f"{s.get('name')} connection count diverges from Queen.")
            if ep_gap > 0.5:
                divergence_score += 15
                notes.append(f"{s.get('name')} endpoint set diverges from Queen.")
        ...
        if not notes:
            notes.append("Nodes appear consistent with Queen.")

        return base, notes

    def _compute_health_trend_and_forecast(self, collective_score, anomaly_risk, drive_risk):
        ...
        if len(self._risk_history) >= 5:
            ...
            if trend == "DETERIORATING":
                forecast = "Collective stability trending down; expect increased risk if pattern continues."
            elif trend == "IMPROVING":
                forecast = "Collective stability improving; risk likely to decrease if pattern holds."
            else:
                forecast = "Collective stability stable in recent window."

        if anomaly_risk == "HIGH":
            forecast += " Anomaly pressure is high; prepare for more unusual activity."
        elif anomaly_risk == "MEDIUM":
            forecast += " Anomaly pressure moderate; watch for escalation."

        if drive_risk == "HIGH":
            forecast += " Drive subsystem at high risk; resync and redundancy are critical."
        elif drive_risk == "MEDIUM":
            forecast += " Drive subsystem shows early warning signs."

        return trend, forecast.strip()
    ...
    def run(self):
        while self.running and self.node.is_running():
            try:
                anomaly_level, anomaly_notes = self._compute_anomaly_risk()
                drive_level, drive_notes = self._compute_drive_risk()
                collective_score, collective_notes = self._compute_collective_risk(
                    anomaly_level, drive_level
                )

                health_trend, forecast = self._compute_health_trend_and_forecast(
                    collective_score, anomaly_level, drive_level
                )
                gaming_condition = self._compute_gaming_condition(anomaly_level, drive_level)

                all_notes = anomaly_notes + drive_notes + collective_notes
...
class BorgNode:
    def __init__(
        ...
        self.shared_sync = SharedStateSync(
            self, get_primary_dir_func, get_secondary_dir_func, shared_registry, self.comm_engine
        )
...
class BorgGUI:
    def __init__(self, root, queen, node_list, shared_dirs_ref, config_ref, shared_registry, comm_engine):
        self.root = root
        self.queen = queen
        ...
        self.root.title("BorgNet Guardian (Encrypted Board Comm)")
    ...
    def _build_layout(self):
        ...
        self.pred_trend_var = tk.StringVar(value="STABLE")
        ...
        self.pred_collective_label = ttk.Label(predictive_frame, textvariable=self.pred_collective_var)
        self.pred_collective_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Health Trend:").pack(anchor="w")
        self.pred_trend_label = ttk.Label(predictive_frame, textvariable=self.pred_trend_var)
        self.pred_trend_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Gaming Condition:").pack(anchor="w")
        self.pred_gaming_label = ttk.Label(predictive_frame, textvariable=self.pred_gaming_var)
        self.pred_gaming_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Forecast:").pack(anchor="w")
        self.pred_forecast_label = ttk.Label(predictive_frame, textvariable=self.pred_forecast_var, wraplength=300)
        self.pred_forecast_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Notes:").pack(anchor="w")
        self.pred_notes_label = ttk.Label(predictive_frame, textvariable=self.pred_notes_var, wraplength=300)
        self.pred_notes_label.pack(anchor="w")

        comm_frame = ttk.LabelFrame(main, text="Board Communication Status")
        comm_frame.pack(fill=tk.X, pady=5)

        self.comm_status_var = tk.StringVar(value="OK")
        self.comm_detail_var = tk.StringVar(value="")

        ttk.Label(comm_frame, text="Status:").grid(row=0, column=0, sticky="w")
        self.comm_status_label = ttk.Label(comm_frame, textvariable=self.comm_status_var)
        self.comm_status_label.grid(row=0, column=1, sticky="w", padx=5)

        ttk.Label(comm_frame, text="Details:").grid(row=1, column=0, sticky="w")
        self.comm_detail_label = ttk.Label(comm_frame, textvariable=self.comm_detail_var, wraplength=600)
        self.comm_detail_label.grid(row=1, column=1, sticky="w", padx=5)
    ...
    def _update_predictive_panel(self):
        ...
        self.pred_trend_var.set(summary.get("health_trend", "UNKNOWN"))
        ...
        self.pred_anomaly_label.config(foreground=color_for_risk(self.pred_anomaly_var.get()))
        self.pred_drive_label.config(foreground=color_for_risk(self.pred_drive_var.get()))

    def _update_comm_panel(self):
        status = self.comm_engine.get_status()
        s = status["status"]
        self.comm_status_var.set(s)
        detail = (
            f"Total messages: {status['total_messages']}, "
            f"Encrypt errors: {status['errors_encrypt']}, "
            f"Decrypt errors: {status['errors_decrypt']}"
        )
        self.comm_detail_var.set(detail)

        color = "black"
        if s == COMM_STATUS_OK:
            color = "green"
        elif s == COMM_STATUS_WARN:
            color = "orange"
        elif s == COMM_STATUS_ERROR:
            color = "red"
        self.comm_status_label.config(foreground=color)
...
def main():
    ...
    # Board communication engine (shared by all nodes)
    comm_engine = BoardCommEngine(cfg)
    ...
    root = tk.Tk()
    gui = BorgGUI(root, queen, node_list, shared_dirs_ref, cfg, shared_registry, comm_engine)
