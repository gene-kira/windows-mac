# defense_daemon_lcars_policies_restore.py
...
# ===================== Obfuscation (reversible for admins) =====================
def reverse_mirror(text: str) -> str:
    return text[::-1]
...
# ===================== Enhanced detection: Bernoulli + CUSUM =====================
...
# ===================== Containment (manual + auto reversible with expiry) =====================
CONTAINMENT_MANIFEST = []  # applied actions
STAGED_ACTIONS = []        # recommendations and staged actions (pending or auto-applied with expiry)

QUAR_DIR = Path.home() / "Quarantine"
...
def restore_quarantine(dst: Path):
    # restore file from quarantine back to original location
    try:
        if not dst.exists(): return False
        src_name = dst.name[:-5] if dst.name.endswith(".quar") else dst.name
        orig = dst.parent.parent / src_name  # assumes same parent folder (Quarantine is one level)
        shutil.move(str(dst), str(orig))
        log("contain", f"♻️ Restored: {dst} -> {orig}")
        return True
    except Exception as e:
        log("contain", f"❌ Restore failed: {e}")
        return False
...
def firewall_remove_rule(rule_name: str):
    ...
    try:
        subprocess.run(cmd, check=True, capture_output=True)
        log("contain", f"🧹 Firewall rule removed: {rule_name}")
        return True
    except Exception as e:
        log("contain", f"❌ Remove rule failed: {e}")
        return False
...
class FileSensor:
    ...
    def _loop(self):
        ...
        while True:
            cur = set(p.name for p in self.path.glob("*"))
            new = cur - self.seen
            self.seen = cur
            for name in new:
                ...
                if any(p.lower().endswith(e) for e in exts):
                    ...
                    DETECTOR.update(1)  # threaty signal
            time.sleep(5)
...
class ProcessSensor:
    ...
    def _loop(self):
        while True:
            ...
            self.last = cur
            for pid in new:
                BUS.publish("proc.start", proc_context(pid))
            time.sleep(7)
        ...
        while True:
            ...
            for c in psutil.net_connections(kind="inet"):
                ...
                if hostname and is_suspicious_domain(hostname):
                    ...
                    DETECTOR.update(1)
            # CUSUM over unique endpoints in window
            for pid, ev in list(self.hist.items()):
                ...
                if CUSUM.update(float(uniq_count)):
                    BUS.publish("net.burst_pred", {"pid": pid, "count": uniq_count})
                    DETECTOR.update(1)
            time.sleep(5)
...
class Bot:
    ...
    def switch(self):
        ...
        log("bot", f"🔺 Mode={self.mode}")
    def _loop(self):
        while True:
            if self.mode == "guardian":
                phrase = f"LCARS audit {rand_hex(8)}"
                camo = chameleon_camouflage(phrase, "guardian")
                log("bot", f"🔱 {reverse_mirror(phrase)} | camo={camo}")
                self.agent.mutate("ghost_sync"); CIPHER.escalate("guardian")
                DETECTOR.update(0)
            else:
                raw = f"{reverse_read_encrypt(str(int(time.time()) % 2048), 'rogue-key')}-{rand_hex(8)}"
                camo = chameleon_camouflage(raw, "rogue")
                log("bot", f"🜏 pattern={raw} | camo={camo}")
                self.agent.mutate("rogue_escalation"); CIPHER.escalate("rogue")
            time.sleep(10)
BOT = Bot(AGENT)

# ===================== Ethical autonomous devourer mode =====================
class DevourerEthical:
    """
    Autonomous analysis with guardrails:
    - Reads Bernoulli probability + CUSUM bursts
    - If policy auto.enabled and reversible_only:
        - Auto-apply reversible actions (soft quarantine for new executables, program-scoped firewall block)
        - Each auto action has expiry and is restorable
    - Otherwise, stage actions for admin approval
    """
    def __init__(self):
        self.running = False
        self.policy = POLICIES.get()
    def start(self):
        self.running = True
        threading.Thread(target=self._loop, daemon=True).start()
    def _stage(self, action: dict):
        action["ts"] = now_ts()
        STAGED_ACTIONS.append(action)
        BUS.publish("devourer.stage", action)
    def _auto_apply_with_expiry(self, apply_fn, revert_fn, meta: dict, expiry_min: int):
        applied = apply_fn()
        if applied:
            expire_at = datetime.utcnow() + timedelta(minutes=expiry_min)
            meta["expire_at"] = expire_at.strftime("%Y-%m-%dT%H:%M:%SZ")
            STAGED_ACTIONS.append({**meta, "applied": True})
            BUS.publish("devourer.auto_applied", meta)
            # schedule expiry
            def expiry_worker():
                while datetime.utcnow() < expire_at:
                    time.sleep(5)
                # Try revert
                ok = revert_fn()
                BUS.publish("devourer.expired", {"meta": meta, "reverted": ok})
    ...
    def _loop(self):
        while self.running:
            ...
            BUS.publish("detector.status", {"p": round(p, 4), "decision": ("block_ready" if p >= bern_block else ("warn" if p >= bern_warn else "normal"))})
            # Inspect staged candidates from recent bus signals (simple polling of STAGED_ACTIONS scope)
            # Ethical auto behavior: recommendations on high probability, auto reversible if enabled
            # Example: pre-stage quarantine for last new executable path in feed (admin apply)
            # In practice, the daemon would maintain a small cache of last events; here, we just sleep
            time.sleep(5)
...
class DefenseGUI:
    def __init__(self, root):
        ...
        right = tk.Frame(main, bg=LCARS_PANEL, width=520); right.pack(side=tk.RIGHT, fill=tk.Y)

        # Event feed
        tk.Label(left, text="Event feed", bg=LCARS_PANEL, fg=LCARS_TEXT, font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=8, pady=(8, 0))
        self.feed = scrolledtext.ScrolledText(left, wrap=tk.WORD, height=28, bg="#0F0F0F", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        ...
        self.restore_quar = tk.Entry(rp, width=36, bg="#101010", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        self.restore_quar.grid(row=0, column=0, padx=(0,8))
        tk.Button(rp, text="Restore quarantine", command=self.do_restore_quarantine, bg=LCARS_ACCENT_B).grid(row=0, column=1)
        self.restore_rule = tk.Entry(rp, width=24, bg="#101010", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        self.restore_rule.grid(row=1, column=0, padx=(0,8), pady=(8,0))
        ...
        self.auto_enabled = tk.BooleanVar(value=POLICIES.get()["auto"]["enabled"])
        tk.Checkbutton(pp, variable=self.auto_enabled, bg=LCARS_PANEL).grid(row=0, column=1)
        tk.Label(pp, text="Expiry minutes", bg=LCARS_PANEL, fg=LCARS_TEXT).grid(row=1, column=0, sticky="w")
        self.expiry_min = tk.Entry(pp, width=6, bg="#101010", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        self.expiry_min.insert(0, str(POLICIES.get()["auto"]["expiry_minutes"]))
        self.expiry_min.grid(row=1, column=1)
        ...
        self.camo_blob = tk.Entry(tools, width=36, bg="#101010", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        self.camo_blob.grid(row=1, column=0, padx=(0,8))
        ...
        self.rev_blob = tk.Entry(tools, width=36, bg="#101010", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        self.rev_blob.grid(row=3, column=0, padx=(0,8))
        self.rev_key = tk.Entry(tools, width=20, bg="#101010", fg=LCARS_TEXT, insertbackground=LCARS_TEXT)
        self.rev_key.grid(row=3, column=1)
    ...
    def add_event(self, evt):
        ...
        if t == "detector.status":
            status = evt["payload"].get("decision")
            p = evt["payload"].get("p")
            self.incidents.insert(tk.END, f"detector :: p={p} decision={status}")
    ...
    def do_quarantine(self):
        ...
        if not str(path):
            messagebox.showinfo("Quarantine", "Enter a valid path."); return
        ok = quarantine(path)
        messagebox.showinfo("Quarantine", "File moved to Quarantine." if ok else "Quarantine failed.")
    ...
    def do_decode_camo(self):
        blob = self.camo_blob.get().strip()
        decoded = chameleon_decode(blob)
        messagebox.showinfo("Camouflage decode", decoded or "Unable to decode")

    def do_reverse_decrypt(self):
        blob = self.rev_blob.get().strip()
        key = self.rev_key.get().strip() or "admin-key"
        try:
            plain = reverse_read_decrypt(blob, key)
            messagebox.showinfo("Reverse decrypt", plain)
        except Exception as e:
            messagebox.showerror("Reverse decrypt", f"Error: {e}")

    def do_restore_quarantine(self):
        qpath = Path(self.restore_quar.get().strip())
        ok = restore_quarantine(qpath)
        messagebox.showinfo("Restore", "Restored." if ok else "Restore failed.")

    def do_restore_rule(self):
        rule = self.restore_rule.get().strip()
        ok = firewall_remove_rule(rule)
        messagebox.showinfo("Firewall", "Rule removed." if ok else "Remove failed.")
        ...
        try:
            POLICIES.update({
                "auto": {
                    ...
                    "reversible_only": True,
                    "expiry_minutes": int(self.expiry_min.get().strip() or POLICIES.get()["auto"]["expiry_minutes"]),
                }
            })
            messagebox.showinfo("Policies", "Saved.")
        except Exception as e:
            messagebox.showerror("Policies", f"Error: {e}")

# ===================== Entrypoint orchestration =====================
def start_all():
    FileSensor().start(); ProcessSensor().start(); NetSensor().start()
    BOT.start(); DEVOURER.start()

def main():
    log("boot", "Starting LCARS ASI Defense Interface (ethical autonomous mode, manual-only irreversible actions)")
    start_all()
