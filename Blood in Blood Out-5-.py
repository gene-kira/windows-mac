...
class DeepAnomalyOrgan:
    ...
    def _run_clustering(self):
        ...
        X = self._vectors[-50:]  # last window
        n_samples = len(X)
    ...
    def current_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        prev = self.history[-2] if len(self.history) > 1 else {c.name: 0 for c in Category}

        for c in Category:
            name = c.name
            series = [h.get(name, 0) for h in self.history]
            mean = sum(series) / len(series)
            var = sum((x - mean) ** 2 for x in series) / max(1, len(series) - 1)
            std = var ** 0.5
            last = series[-1]
    ...
    def predictive_horizon(self) -> Dict[str, Any]:
        ...
        total_series = [sum(h.values()) for h in self.history]
...
class Cockpit(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Defensive Code Purifier – Autonomous Nuclear Cockpit")
    ...
    def _auto_tick(self):
        ...
        if "Risk: SAFE" not in result.policy_summary:
            ...
            self.log_viewer.add_log(
                f"[AI-AUTO] Rewrite applied; confidence={ai_result.confidence:.2f}, flags={ai_result.safety_flags}"
            )
