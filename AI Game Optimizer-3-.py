...
# Optional SMB / network path (UNC or mapped drive)
# This is just the initial value; can be changed live in the GUI.
...
class ProfileStore:
    def __init__(self, local_root: Path, network_root: str | None):
        ...
        # network_root can be changed at runtime by the core
...
class GameProfile:
    ...
    scenario_diversity: float = 0.0  # 0–1
    pattern_noise: float = 0.0       # 0–1 (higher = more chaotic)

    learning_percent: int = 0        # 0–100
    confidence_percent: int = 0      # 0–100

    successful_predictions: int = 0
    total_predictions: int = 0
    recent_anomalies: int = 0        # last N events
    stability_score: float = 0.0     # 0–100-ish
...
class MovidiusInferenceEngine(InferenceEngine):
    ...
    def predict_risk(self, features: dict) -> float:
        if not self.available:
            return 0.0
        # TODO: convert features to tensor, run inference, return delta risk
        return 0.0
...
class ForesightEngine:
    ...
    def compute_risk(self) -> float:
        ...
        # inertia on risk itself
        ine = self.inertia
        a_r = ine["alpha"]
        ine["risk"] = a_r * base_risk + (1 - a_r) * ine["risk"]
        base_risk = ine["risk"]

        # optional Movidius adjustment
    ...
    def get_confidence(self) -> float:
        ...
        v = self.volatility
        if lt["samples"] < 50:
            return 20.0  # barely learned

        vol = (v["cpu"] + v["ram"]) / 2.0
        sample_term = min(lt["samples"] / 500.0, 1.0)  # up to 1
...
class PowerPostureEngine:
    def __init__(self):
        # Fill these with actual GUIDs from `powercfg /L` if you want automatic plan switching
        self.plan_balanced = None   # e.g. "381b4222-f694-41f0-9685-ff5bb260df2e"
        self.plan_high_perf = None  # e.g. "8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c"
...
class OptimizerCore(QtCore.QObject):
    ...
    # --- Network / SMB management ---

    def set_network_root(self, path: str | None, enabled: bool):
        """
        Enable/disable network-backed storage and update the ProfileStore.
...
class OptimizerGUI(QtWidgets.QWidget):
    def __init__(self, core: OptimizerCore):
        super().__init__()
        self.core = core
    ...
    def _build_layout(self):
        ...
        # Foresight
        self.lbl_risk = QtWidgets.QLabel("Foresight Risk: 0% (C:0%)")
        ...
        # Network / SMB section (Option B - Advanced)
        layout.addWidget(QtWidgets.QLabel("Network / SMB storage:"))

        net_row = QtWidgets.QHBoxLayout()
        self.txt_network_path = QtWidgets.QLineEdit()
        initial_net = self.core.network_root or ""
        self.txt_network_path.setText(initial_net)

        self.btn_browse_net = QtWidgets.QPushButton("Browse…")
        self.btn_browse_net.clicked.connect(self._on_browse_network)

        net_row.addWidget(self.txt_network_path, 1)
        net_row.addWidget(self.btn_browse_net)
        layout.addLayout(net_row)

        net_ctrl_row = QtWidgets.QHBoxLayout()
        self.chk_network_enable = QtWidgets.QCheckBox("Enable network/SMB storage")
        self.chk_network_enable.setChecked(self.core.network_enabled)
        self.chk_network_enable.stateChanged.connect(self._on_network_enable_changed)

        self.btn_test_net = QtWidgets.QPushButton("Test connection")
        self.btn_test_net.clicked.connect(self._on_test_network)

        net_ctrl_row.addWidget(self.chk_network_enable)
        net_ctrl_row.addWidget(self.btn_test_net)
        layout.addLayout(net_ctrl_row)

        self.lbl_net_status = QtWidgets.QLabel("Network: Disabled")
        self.lbl_net_status.setStyleSheet("color: gray;")
        layout.addWidget(self.lbl_net_status)
    ...
    def _on_browse_network(self):
        current = self.txt_network_path.text().strip() or ""
        path = QtWidgets.QFileDialog.getExistingDirectory(
            self,
            "Select network / SMB folder",
            current
        )
        if path:
            self.txt_network_path.setText(path)

    def _on_test_network(self):
        path = self.txt_network_path.text().strip()
        if not path:
            self.lbl_net_status.setText("Network: No path specified.")
            self.lbl_net_status.setStyleSheet("color: orange;")
            return
        ok, msg = self.core.test_network_root(path)
        self.lbl_net_status.setText(f"Network test: {msg}")
        self.lbl_net_status.setStyleSheet("color: lightgreen;" if ok else "color: red;")

    def _on_network_enable_changed(self, state):
        enabled = (state == QtCore.Qt.Checked)
        path = self.txt_network_path.text().strip()
        ok, msg = self.core.set_network_root(path, enabled)
        color = "lightgreen" if ok and enabled else ("gray" if ok and not enabled else "red")
        self.lbl_net_status.setText(f"Network: {msg}")
        self.lbl_net_status.setStyleSheet(f"color: {color};")
    ...
    def update_stats(self, stats: dict):
        ...
        self.lbl_risk.setText(f"Foresight Risk: {risk:.0f}% (C:{foresight_conf:.0f}%)")
        ...
        if net_status.startswith("Error"):
            self.lbl_net_status.setStyleSheet("color: red;")
        elif net_status == "Connected":
            self.lbl_net_status.setStyleSheet("color: lightgreen;")
        elif net_status == "Disabled":
            self.lbl_net_status.setStyleSheet("color: gray;")
        else:
            self.lbl_net_status.setStyleSheet("color: orange;")
...
def main():
    ...
    store = ProfileStore(LOCAL_ROOT, initial_network_root)

    app = QtWidgets.QApplication(sys.argv)
    core = OptimizerCore(store)
    gui = OptimizerGUI(core)
    overlay = OverlayHUD()
