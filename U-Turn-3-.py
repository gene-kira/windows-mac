...
# --------------------------------------------------------------------
# Baseline learning: require ~1 week of observed presence before we allow
# suspending a process based on behavior.
BASELINE_AGE_SECONDS = 7 * 24 * 3600
# How many consecutive "idle" observations before we consider a process suspendable.
IDLE_STRIKES_THRESHOLD = 5

# AI mode tuning (Option A: ultra-fast reaction)
AI_MODE_POLL_MS = 200          # 0.2 seconds
AI_MODE_STABLE_TICKS = 3       # require 3 consecutive same predictions before switching mode

# --------------------------------------------------------------------
# Memory core (primary + backup JSON, dual-queen + forecast + presidential)
# --------------------------------------------------------------------
...
class MemoryCore:
    ...
    - Primary + optional backup (network/USB)
    - Network, process, sidebrain, queen, baseline
    - Mode, booster config
    ...
    def __init__(
        ...
        self.state: Dict[str, Any] = {
            ...
            "queen": {},
            "baseline": {
                # name -> { first_seen: ts, last_seen: ts, total_observed: int }
                "process_names": {}
            },
            "modes": {
                "current": "idle",      # idle / game / browse / work
            ...
            "forecast": {
                "transitions": {},   # event -> {next_event: count}
                "last_event": None,  # last observed event
            },
            "presidential": {
                "enabled": False,
            ...
            "guardian": {
                "override_enabled": False,  # True = manual override (relaxed)
            },
        }
    ...
    def load(self):
        ...
        if chosen:
            self.state = chosen
            print(
                "[MEMORY] Loaded state from",
                "primary" if chosen is primary else "backup",
            )
        else:
    ...
    def save(self):
        ...
        print("[MEMORY] State saved to primary", "and backup" if self.backup_file else "")
    ...
    def get_auto_mode_enabled(self) -> bool:
        return self.state.get("modes", {}).get("auto_mode_enabled", False)
    ...
    # Baseline process info (for week-long learning before suspending)
...
# --------------------------------------------------------------------
# System scanning (hardware + processes)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Sidebrain (ad / irrelevant classifier for proxy)
# --------------------------------------------------------------------
...
class SideBrain:
    """
    Sidebrain that classifies requests as AD/IRRELEVANT vs NORMAL.
    Extendable with real AI later.
    """
...
# --------------------------------------------------------------------
# Booster (adaptive 3–10 second CPU afterburner, smart wind-down)
# --------------------------------------------------------------------
...
class Booster:
    """
    Booster that starts with a base 3s boost but can extend up to 10s
    when the system remains congested.

    Smart wind-down:
    - When stopping, it stops creating new boosts.
    - Existing boosts are allowed to expire naturally.
    """
    ...
    def stop(self):
        """
        Smart stop:
        - Stop creating new boosts.
        - Let existing boosts expire and restore themselves in the normal cycle.
        - After a short grace, force-restore any leftover just in case.
        """
        print("[BOOSTER] Smart stopping boost controller (no new boosts, letting existing expire)...")
...
# --------------------------------------------------------------------
# Optimizer (baseline-aware, smart, OS-protective, no blind shutdown)
# --------------------------------------------------------------------
...
class TrackedProcess:
    ...
    idle_strikes: int = 0  # how many consecutive idle checks
...
class OptimizerAI:
    def __init__(
        ...
        # Critical patterns expanded to strongly protect OS and shell/start menu
...
class SystemOptimizer:
    """
    Process optimizer with:
    - Baseline learning over time (via MemoryCore) before suspending.
    - Smart, gradual resume on disable.
    - Presidential mode calls force-freeze, but still respects OS safety and
      baseline data so we don't blindly kill anything.

    It NEVER touches OS-critical components (protected by name patterns)
    and it NEVER changes startup configuration; it only pauses/resumes
    non-critical processes based on observed behavior over time.
    """
    ...
    def __init__(self, memory: Optional[MemoryCore] = None, scan_interval=2.0):
        ...
        self._thread = threading.Thread(target=self._loop, daemon=True)
        self._enabled = False
        self._force_freeze = False  # used by PresidentialMode
        self._winding_down = False  # smart shutdown flag
    ...
    def disable(self):
        """
        Smart disable:
        - Stop freezing *new* processes.
        - Gradually resume suspended processes in the normal tick loop.
        - Once everything is resumed, tracked state is cleared.
        """
        ...
        self._winding_down = True
        print(
            "[OPTIMIZER] Smart disabling: no new freezes, resuming suspended processes gradually..."
        )
    ...
    def _tick(self):
        ...
        if self._winding_down and not self._enabled:
            ...
            if not any_suspended:
                ...
                self._winding_down = False
                print(
                    "[OPTIMIZER] Smart disable complete. All previously suspended processes resumed."
                )
        ...
        for snap in snaps:
            ...
            if self._force_freeze:
                ...
                if (
                    ...
                    tp.suspended
                    and (tp.classification is ProcClass.ACTIVE)
                ):
                    self._resume_process(pid)
                    tp.suspended = False
            else:
...
# --------------------------------------------------------------------
# Presidential Controller (Hybrid: manual + forecast auto, OS-protective)
# --------------------------------------------------------------------
...
class PresidentialController:
    ...
    - Protects the OS / core shell stability.
    - Uses the optimizer to reduce non-essential system functions ONLY
      when they are not needed, and ONLY after baseline learning.
    - Never touches startup configuration; it just pauses/resumes processes.
    """
    ...
    def activate(self, source: str = "manual"):
        if self._active:
            return
        ...
        print(f"[PRESIDENTIAL] ACTIVATED ({source}).")
        self._active = True
        ...
        if self.booster is not None:
            print(
                "[PRESIDENTIAL] Booster remains active for continuous high-priority bursts."
            )
    ...
    def deactivate(self, source: str = "manual"):
        if not self._active:
            return
        pres = self.memory.get_presidential_state()
        pres["enabled"] = False
        ...
        print(f"[PRESIDENTIAL] DEACTIVATED ({source}).")
        self._active = False
...
# --------------------------------------------------------------------
# Look-ahead forecasting engine (preemptive strike / guessing)
# --------------------------------------------------------------------
...
class LookAheadEngine:
    ...
    - Observes events (modes, heavy processes)
    - Learns transitions between events (simple Markov-style)
    - Forecasts likely next events
    - Can suggest/trigger Presidential Mode preemptively
    """
    ...
    def _maybe_trigger_presidential(self):
        ...
        preds = self._get_predictions(current_event)
        if preds:
            print(f"[FORECAST] From {current_event}, likely next: {preds}")

        if current_mode in ("game",) and not self.presidential_ref.is_active():
            system_cpu = psutil.cpu_percent(interval=0.0)
            if system_cpu > 50.0:
                print(
                    "[FORECAST] Auto-triggering Presidential Mode based on mode=game and high CPU."
                )
                self.presidential_ref.activate(source="forecast")
...
# --------------------------------------------------------------------
# Network scanning (Prometheus-style)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Hive (Queen + workers) tied into MemoryCore
# --------------------------------------------------------------------
...
class Queen:
    """
    Queen that supervises all IP workers and talks to MemoryCore.
    """
...
# --------------------------------------------------------------------
# Guardian Mode (always-on shield + emergency redirect with override)
# --------------------------------------------------------------------
...
class GuardianMode:
    """
    Always-on guardian:
    - Watches requests at the proxy edge.
    - If unusual/suspicious and no override, performs a "U-turn":
      sends a 302 redirect back, does not forward to target.
    - Manual override (red) relaxes behavior.
    """
    ...
    def is_override_enabled(self) -> bool:
        return self.memory.get_guardian_state().get(
            "override_enabled", False
        )
    ...
    def _is_suspicious(
        ...
        if ":" in host:
            ...
            host_name = host
            port_num = 80
        if port_num not in (80, 443, 8080):
            return True
    ...
    def handle_connection(
        ...
        if self.is_override_enabled():
            return False
        try:
            if self._is_suspicious(method, url, headers):
                print(
...
# --------------------------------------------------------------------
# Stealth proxy (HTTP) integrating SideBrain + Queen + Guardian
# --------------------------------------------------------------------
...
class StealthProxy:
    ...
    def handle_client(self, client_socket: socket.socket, addr):
        try:
            ...
            if self.guardian.handle_connection(
                method, url, headers, client_socket
            ):
                return

            if self.is_irrelevant(url):
    ...
    def is_irrelevant(self, url: str) -> bool:
        ad_substrings = [
            "doubleclick",
            "adservice",
            "/ads?",
        ...
        return any(s in url for s in ad_substrings)
...
# --------------------------------------------------------------------
# Tkinter GUI + AI Mode + LookAhead + Presidential + Guardian + Learning Meter
# --------------------------------------------------------------------
...
class StealthGUI:
    ...
    def toggle_boost(self):
        ...
        if desired and not self.boost_enabled:
            ...
            self._log(
                "Adaptive system boost ENABLED (3–10s bursts, smart wind-down on stop)."
            )
        elif not desired and self.boost_enabled:
            if self.booster:
                self.booster.stop()
            self.boost_enabled = False
            self._log("Adaptive system boost DISABLED (smart wind-down complete).")
    ...
    def toggle_optimizer(self):
        ...
        if desired and not self.optimizer._enabled:
            self.optimizer.enable()
            self._log(
                "Optimizer ENABLED (baseline-aware, slowly freezing clearly idle junk/occasional processes)."
            )
        elif not desired and self.optimizer._enabled:
            self.optimizer.disable()
            self._log(
                "Optimizer DISABLED (smart resume: suspended processes will thaw gradually)."
            )

    # ---------- Mode control (manual + AI) ----------
    ...
    def toggle_ai_mode(self):
        ...
        self._update_mode_controls_state()
        if enabled:
            self._log("AI Mode ENABLED: system will auto-switch modes based on real-time load.")
        else:
            self._log("AI Mode DISABLED: manual mode selection active.")
    ...
    def _auto_detect_mode(self) -> str:
        """
        Ultra-fast heuristic mode detector based on:
        - CPU load
        - Per-process load and names
        - Network activity
        """
        try:
            cpu = psutil.cpu_percent(interval=None)  # last sample
        except Exception:
            cpu = 0.0
        ...
        # Track net delta over time using instance variable
        ...
        # Heuristics: game / browse / work / idle
        is_browser = any(
            x in name
            for x in [
                "chrome",
        ...
        # Use CPU + net + name signals
        if is_game or (cpu > 60 and top_cpu > 50):
            return "game"
        if is_browser or net_delta > 200000:  # ~200 KB per tick
            return "browse"
    ...
    def toggle_auto_lan(self):
        ...
        if self.auto_lan:
            self.lan_cidrs = detect_lan_cidrs()
            self.lan_label.config(
                text=f"LAN: {', '.join(self.lan_cidrs)}"
            )
            self._log("Auto LAN detection ENABLED.")
        else:
            self._log("Auto LAN detection DISABLED. Using manual list.")
    ...
    def toggle_presidential_manual(self):
        ...
        if desired and not self.presidential.is_active():
            self.presidential.activate(source="manual")
            self._log("Presidential Mode ACTIVATED manually (OS-protective).")
        elif not desired and self.presidential.is_active():
            self.presidential.deactivate(source="manual")
            self._log("Presidential Mode DEACTIVATED manually.")
        self._update_presidential_color()

    def _update_presidential_color(self):
        if self.pres_var.get():
            self.pres_check.config(
                bg="red",
    ...
    def toggle_guardian_override(self):
        ...
        if enabled:
            self._log(
                "Guardian Override ENABLED (RED) - Guardian relaxed, admin in control."
            )
        else:
            self._log(
                "Guardian Override DISABLED (GREEN) - Guardian fully automatic."
            )

    def _update_guardian_color(self):
        if self.guard_var.get():
            self.guard_check.config(
                bg="red",
                activebackground="red",
                fg="white",
                selectcolor="red",
    ...
    def _update_learning_meter(self):
        ...
        self.learn_label.config(
            text=f"{total_events} events ({blended}%)"
        )
...
def on_close(app: StealthGUI, root):
    if app.booster and app.boost_enabled:
        app.booster.stop()
    if app.optimizer:
        app.optimizer.stop()
    if app.proxy and app.running_proxy:
        app.stop_proxy()
    if app.lookahead:
        app.lookahead.stop()
    root.destroy()
