...
- Queen brain + workers (process scanner, system metrics, network scanner, file activity scanner)
...
- Security signals: basic network activity, trust tier from exe path, simple file activity spikes
...
- Security cockpit (Decision Panel GUI, admin-only):
    - Toggle enforcement (dry run vs real)
    - Adjust paranoia level
    - Shutdown queen
    - Change admin password via popup
...
class QueenBrain:
    ...
    def __init__(self, persistence: PersistenceManager):
        ...
        # Security signals
        self.process_network_activity: Dict[str, Dict[str, int]] = {}  # pid_str -> {"outgoing", "listening"}
        self.listening_ports: List[Tuple[str, int]] = []               # (pid_str, port)
        self.file_spike_events: List[Tuple[str, str, int, int]] = []   # (ts, root, created, deleted)
    ...
    def update_network_signals(self, proc_conn_counts: Dict[str, Dict[str, int]], listening_ports: List[Tuple[str, int]]):
        with self.lock:
            self.process_network_activity = proc_conn_counts
            self.listening_ports = listening_ports

    def update_file_signals(self, events: List[Tuple[str, str, int, int]]):
        with self.lock:
            ts = time.strftime("%H:%M:%S")
    ...
    def _classify_anomaly(self, node: NodeInfo) -> str:
        ...
        # Security amplification from network signals
        with self.lock:
        ...
        if (profile is None or profile.count < 5) and (outgoing > 20 or listening > 0):
            if base_action == "WATCH":
                base_action = "QUARANTINE"
            elif base_action == "QUARANTINE" and self.paranoia_level > 0.7:
                base_action = "KILL"

        # Signature / trust tier adjustment
...
class NetworkScannerWorker(BaseWorker):
    """
    Worker that scans network connections and listening ports.
    Feeds security signals into the queen.
    """
    ...
    def step(self):
        ...
        self.queen.update_worker_status(self.name, f"Scanned net activity for {total_procs} procs")


class FileActivityWorker(BaseWorker):
    """
    Worker that samples file system changes in critical directories.
    Lightweight polling approximation (not real-time kernel hooks).
    """
    ...
    def step(self):
        ...
        if suspicious_events:
            ...
            self.queen.update_worker_status(self.name, f"Spike detected: {msg}")
        else:
...
class DecisionPanelGUI:
    ...
    ADMIN_PASSWORD = "admin"  # initial default; can be changed at runtime
    ...
    def _build_ui(self):
        ...
        # Password change link / button
        change_pass_btn = ttk.Button(btn_frame1, text="Change Admin Password", command=self._on_change_password)
        change_pass_btn.pack(side=tk.LEFT, padx=4)
    ...
    def _on_change_password(self):
        current = simpledialog.askstring("Change Password", "Enter current admin password:", show="*")
        if current is None:
            return
        ...
        new1 = simpledialog.askstring("Change Password", "Enter new admin password:", show="*")
        if new1 is None or new1.strip() == "":
            messagebox.showerror("Error", "New password cannot be empty.")
            return

        new2 = simpledialog.askstring("Change Password", "Confirm new admin password:", show="*")
        if new2 is None:
            return
        ...
        DecisionPanelGUI.ADMIN_PASSWORD = new1
        self.queen.log("[ADMIN] Admin password changed (in-memory for this session).")
