...
class HiveMindGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Unified Hive-Mind Controller (L6 Temporal)")
    ...
    def start_telemetry_if_needed(self):
        if not self.telemetry_running:
            ...
            self.log("[Telemetry] Predictive + physics + temporal engine is now always on.")
    ...
    def update_usage_and_predict(self, ram_percent, vram_percent, combined_percent):
        ...
        # L6 temporal memory update + save
        if wp:
