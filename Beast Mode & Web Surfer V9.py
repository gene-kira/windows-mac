...
# ============================================================
#  GUI AUTOLOADER (PyQt5 + psutil only)
# ============================================================
...
# ============================================================
#  WEB SURFER AUTOLOADER (Flask stack)
# ============================================================
...
# ============================================================
#  TRAINER AUTOLOADER (TensorFlow stack)
# ============================================================
...
# ============================================================
#  WEB SURFER APP (runs only in --websurfer mode)
# ============================================================
...
# ============================================================
#  TRAINER TOP-LEVEL AUGMENT FUNCTION (for multiprocessing)
# ============================================================
...
# ============================================================
#  NPU / GPU / MOVIDIUS / CORAL DETECTION HELPERS
# ============================================================
    ...
    try:
        ...
        try:
            ...
            for dev in core.available_devices:
                devices.append(f"OpenVINO device: {dev}")
        except Exception:
            pass
        # Coral / Edge TPU (tflite_runtime)
        try:
...
def detect_movidius_status():
    """
    Best-effort Movidius (MYRIAD) detection via OpenVINO.
...
def detect_coral_tpu_status():
    """
    Best-effort Coral USB / Edge TPU detection via tflite_runtime.
    Returns (detected: bool, description: str)
    """
    try:
        import tflite_runtime.interpreter as tflite
        # If import works, runtime is present; we don't need to open a model here.
...
# ============================================================
#  TRAINER IMPLEMENTATION (runs only in --trainer mode)
# ============================================================

def trainer_main():
    # CPU affinity: Beast Mode on all cores except first 2 (reserved for background)
    try:
        p = psutil.Process()
        total_cores = psutil.cpu_count(logical=False) or psutil.cpu_count(logical=True) or 1
        if total_cores > 2:
            cores = list(range(2, total_cores))
            p.cpu_affinity(cores)
            print(f"[Trainer] Affinity set to cores: {cores}")
        else:
            print("[Trainer] Not enough cores to reserve 2 for background; using all cores.")
    except Exception as e:
        print(f"[Trainer] Failed to set CPU affinity: {e}")

    # GPU / threading config: reserve 4 threads for background
    try:
        total_threads = multiprocessing.cpu_count()
        beast_threads = max(1, total_threads - 4)
    ...
    # GPU selection: require at least ~2048 MB free VRAM to use
    use_gpu = False
    try:
    ...
    # NPU detection (Movidius + Coral + others)
    ...
    # Movidius OpenVINO Core (for inference offload)
    ov_core = None
    ...
    if movidius_detected:
        try:
            ...
            for dev in ov_core.available_devices:
                if "MYRIAD" in dev.upper():
                    movidius_device_name = dev
                    break
    ...
    # Coral TPU interpreter placeholder (for inference offload)
    coral_interpreter = None
    if coral_detected:
        try:
            import tflite_runtime.interpreter as tflite
            # NOTE: you must provide a real Edge TPU–compiled model path here.
            # For now we just show how it would be wired.
...
# ============================================================
#  GUI-SIDE CONTROLLERS (subprocess wrappers)
# ============================================================
...
if IS_GUI:
    ...
    class WebSurferController(QtCore.QObject):
        ...
        def stop(self):
            ...
            if self._proc and self._proc.poll() is None:
                self.logEvent.emit("Stopping Web Surfer...")
                try:
    ...
    class SystemMonitor(QtCore.QObject):
        ...
        def _get_system_thread_count(self):
            total = 0
        ...
        def _update(self):
            ...
            system_threads = self._get_system_thread_count()
            ...
            hw["npu"] = npu
            hw["movidius_detected"] = movidius_detected
            hw["movidius_desc"] = movidius_desc
            hw["coral_detected"] = coral_detected
            hw["coral_desc"] = coral_desc
            hw["gpus"] = gpus
            hw["gpu_free_vram_mb"] = free_vram_mb
            hw["usb"] = usb
            self.hardwareChanged.emit(hw)
    ...
    class MainWindow(QtWidgets.QMainWindow):
        def __init__(self, websurfer: WebSurferController, trainer: TrainerController, monitor: SystemMonitor, parent=None):
            ...
            self.setWindowTitle("Beast Mode + Web Surfer – Unified GUI")
            ...
            self.ws_status_label = QtWidgets.QLabel("Mode: stopped")
            ...
            self.ws_start_btn = QtWidgets.QPushButton("Start Web Surfer")
            self.ws_stop_btn = QtWidgets.QPushButton("Stop Web Surfer")
            ...
            ws_layout.addWidget(self.ws_start_btn, 3, 0)
            ws_layout.addWidget(self.ws_stop_btn, 3, 1)
            ...
            self.ws_start_btn.clicked.connect(self.websurfer.start)
            self.ws_stop_btn.clicked.connect(self.websurfer.stop)
        ...
        def update_global_status(self):
            ws_running = self.ws_status_label.text().startswith("Mode: running")
            tr_running = self.tr_status_label.text().startswith("Mode: running")
        ...
        def on_hardware(self, hw: dict):
            npu = hw.get("npu", [])
            movidius_detected = hw.get("movidius_detected", False)
            movidius_desc = hw.get("movidius_desc", "Movidius: status unknown")
            coral_detected = hw.get("coral_detected", False)
            coral_desc = hw.get("coral_desc", "Coral / Edge TPU: status unknown")
            gpus = hw.get("gpus", [])
            free_vram_mb = hw.get("gpu_free_vram_mb", None)
            usb = hw.get("usb", [])

            if npu:
                self.npu_label.setText("NPU: " + "; ".join(npu))
            else:
            ...
            if gpus:
                self.gpu_label.setText("GPU: " + "; ".join(gpus))
            else:
            ...
            if usb:
                for line in usb:
                    self.usb_list.addItem(line)
            else:
                self.usb_list.addItem("No USB summary available")

    def main_gui():
        # Pin GUI/background to first 2 cores
        try:
            p = psutil.Process()
            total_cores = psutil.cpu_count(logical=False) or psutil.cpu_count(logical=True) or 1
            if total_cores > 1:
                bg_cores = [0, 1] if total_cores > 2 else [0]
                p.cpu_affinity(bg_cores)
                print(f"[GUI] Affinity set to cores: {bg_cores}")
            else:
                print("[GUI] Only one core available; cannot reserve 2 cores.")
        except Exception as e:
