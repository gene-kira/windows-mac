...
class ThreatDetector(threading.Thread):
    ...
    def _check_sequence_anomalies(self, snap):
        # Very simple rule: transitions with count == 1 and involving endpoints
        # that are otherwise common can be flagged as "sequence anomalies".
        ...
        for key, count in transition_counts.items():
            if count == 1:
                a, b = key.split("|", 1)
                # Only consider if endpoints themselves are not ultra-rare
    ...
    def run(self):
        while self.running:
            try:
                ...
                # Update interval anomaly event history
...
class PredictiveAIEngine(threading.Thread):
    ...
    def _scale_score(self, base, factor):
        return int(max(0, min(100, base * factor)))
    ...
    def _compute_anomaly_prediction(self):
        ...
        avg_new = total_new / length
        avg_rare = total_rare / length
...
class BorgGUI:
    ...
    def _build_layout(self):
        ...
        self.pred_anomaly_trend_label = ttk.Label(predictive_frame, textvariable=self.pred_anomaly_trend_var)
        self.pred_anomaly_trend_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Anomaly Forecast:").pack(anchor="w")
        self.pred_anomaly_forecast_label = ttk.Label(predictive_frame, textvariable=self.pred_anomaly_forecast_var, wraplength=300)
        ...
        self.pred_drive_trend_label = ttk.Label(predictive_frame, textvariable=self.pred_drive_trend_var)
        self.pred_drive_trend_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Drive Forecast:").pack(anchor="w")
        self.pred_drive_forecast_label = ttk.Label(predictive_frame, textvariable=self.pred_drive_forecast_var, wraplength=300)
        ...
        self.pred_hive_anomaly_label = ttk.Label(predictive_frame, textvariable=self.pred_hive_anomaly_var)
        self.pred_hive_anomaly_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Hive Drive Risk:").pack(anchor="w")
        self.pred_hive_drive_label = ttk.Label(predictive_frame, textvariable=self.pred_hive_drive_var)
        self.pred_hive_drive_label.pack(anchor="w")

        ttk.Label(predictive_frame, text="Hive Trend:").pack(anchor="w")
        self.pred_hive_trend_label = ttk.Label(predictive_frame, textvariable=self.pred_hive_trend_var)
