...
class PolicyAIOrgan:
    def __init__(
        ...
        self._capsule_stats: Dict[str, Dict[str, Any]] = {}
    ...
    def record_outcome(
        ...
        stats = self._capsule_stats.setdefault(capsule_id, {"allowed": 0, "denied": 0, "last_ts": time.time()})
        if allowed:
            stats["allowed"] += 1
        else:
            stats["denied"] += 1
        ...
        if allowed and "risk_above" in reason:
            stats.setdefault("near_miss", 0)
            stats["near_miss"] += 1
            if stats["near_miss"] >= 3:
                send_beacon(
                    {
                        "capsule_id": capsule_id,
                        "species": {},
                        "reason": "auto_tighten_policy_suggested",
                        "env": env,
                        "policy": {},
                        "destroyed": False,
                        "timestamp": time.time(),
        ...
        if stats["allowed"] >= 20 and stats["denied"] == 0:
            send_beacon(
                {
                    "capsule_id": capsule_id,
                    "species": {},
                    "reason": "auto_relax_policy_suggested",
                    "env": env,
                    "policy": {},
                    "destroyed": False,
                    "timestamp": time.time(),
...
class PredictionOrgan:
    ...
    def _slope_and_volatility(self, series: List[Dict[str, float]]) -> (float, float, float):
        if len(series) < 2:
            return 0.0, 0.0, 0.0
        t0 = series[0]["t"]
        t1 = series[-1]["t"]
    ...
    def _health_score(self, species_id: str, last_risk: float, volatility: float, age_seconds: float) -> float:
        species_weight = 1.0
        if species_id == "species-oxygen":
            species_weight = 1.2
        elif species_id == "species-silver":
            species_weight = 1.0

        risk_penalty = last_risk
        vol_penalty = min(1.0, volatility)
        age_factor = min(1.0, age_seconds / (7 * 24 * 3600))

        raw = 1.0 - (0.6 * risk_penalty + 0.2 * vol_penalty + 0.2 * age_factor * species_weight)
        return max(0.0, min(1.0, raw))

    def _confidence(self, n_points: int, span: float) -> str:
        if n_points < 3 or span < 60:
            return "low"
        if n_points < 10 or span < 600:
            return "medium"
        return "high"
    ...
    def predict_for_capsule(
        ...
        if not series:
            return {
                "capsule_id": capsule_id,
                "predicted_risk_1m": 0.0,
                "predicted_risk_5m": 0.0,
                "predicted_risk_30m": 0.0,
                "slope": 0.0,
                "volatility": 0.0,
        ...
        slope, vol, span = self._slope_and_volatility(series)
        last_risk = series[-1]["r"]

        horizons = [60.0, 300.0, 1800.0]
        preds = []
        for h in horizons:
            pr = last_risk + slope * h
            preds.append(max(0.0, min(1.0, pr)))

        age_seconds = max(0.0, time.time() - created_at)
        health = self._health_score(species_id, last_risk, vol, age_seconds)
        next_label = self._predict_next_event_label(last_risk, slope)
        conf = self._confidence(len(series), span)

        trajectory = series[-50:]

        return {
            "capsule_id": capsule_id,
            "predicted_risk_1m": preds[0],
            "predicted_risk_5m": preds[1],
            "predicted_risk_30m": preds[2],
            "slope": slope,
            "volatility": vol,
...
# ============================================================
#  ACCESS-DRIVEN CAPSULE CREATION ORGAN (INTELLIGENT)
# ============================================================
...
class AccessDrivenCapsuleOrgan:
    def __init__(self, coordinator: SpeciesCoordinator, interval: float = 5.0, max_capsules: int = 500) -> None:
        ...
        self.interval = interval
        self.base_max_capsules = max_capsules
        self.enabled = HAS_PSUTIL
        self._stop = False
        self._thread: Optional[threading.Thread] = None
    ...
    def _file_interest_score(self, path: str) -> float:
        lower = path.lower()
        score = 0.0
        if any(ext in lower for ext in [".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx", ".pdf"]):
            score += 0.6
        if any(ext in lower for ext in [".zip", ".7z", ".rar"]):
            score += 0.4
        if any(ext in lower for ext in [".txt", ".md", ".cfg", ".conf", ".ini", ".json", ".yaml", ".yml"]):
            score += 0.3
        if any(folder in lower for folder in ["documents", "desktop", "downloads", "projects", "code"]):
            score += 0.3
        if any(folder in lower for folder in ["temp", "tmp", "cache", "logs"]):
            score -= 0.4
        return max(0.0, min(1.0, score))

    def _effective_max_capsules(self) -> int:
        if not HAS_PSUTIL:
            return self.base_max_capsules
        try:
            cpu = psutil.cpu_percent(interval=0.0)
            if cpu > 80:
                return max(50, int(self.base_max_capsules * 0.3))
            if cpu > 50:
                return max(100, int(self.base_max_capsules * 0.6))
            return self.base_max_capsules
        except Exception:
            return self.base_max_capsules

    def _wrap_file(self, path: str, pid: int, pname: str):
        max_capsules = self._effective_max_capsules()
        if len(self._seen_files) >= max_capsules:
            return
        ...
        interest = self._file_interest_score(path)
        if interest < 0.4:
            return

        self._seen_files.add(path)
        self._create_capsule_for_path(path, pid, pname, interest)

    def _create_capsule_for_path(self, path: str, pid: int, pname: str, interest: float):
...
def refresh_species_state(sender=None, app_data=None, user_data=None):
    ...
    for r in rows:
        ...
        events = GUI_STATE["capsule_events"].get(cid, [])
        if pred_organ:
            prediction = pred_organ.predict_for_capsule(cid, events, get_environment_fingerprint(), sid, created_at)
            GUI_STATE["prediction_cache"][cid] = prediction
            GUI_STATE["health_cache"][cid] = prediction["health"]
        else:
            GUI_STATE["health_cache"][cid] = 1.0
    ...
    for r in rows:
        ...
        if GUI_STATE["filter_risky_only"] and risk < 0.5:
            continue
        display_rows.append(r)

    if GUI_STATE["sort_by_risk"]:
        display_rows.sort(key=lambda r: GUI_STATE["risk_cache"].get(r.get("capsule_id"), 0.0), reverse=True)
    ...
    for r in display_rows:
        ...
        risk = GUI_STATE["risk_cache"].get(cid, 0.0)
        health = GUI_STATE["health_cache"].get(cid, 1.0)
        ...
        if dpg.does_item_exist("capsule_table"):
            with dpg.table_row(parent="capsule_table", tag=row_tag):
                ...
                dpg.add_text(f"{risk:.2f}")
                dpg.add_progress_bar(default_value=health, overlay=f"{health:.2f}", width=-1)
    ...
    prev = GUI_STATE.get("last_metrics_snapshot")
    GUI_STATE["last_metrics_snapshot"] = metrics
    degraded = False
    if prev is not None:
        if metrics["disk_read_bytes"] == prev["disk_read_bytes"] and metrics["disk_write_bytes"] == prev["disk_write_bytes"] and metrics["net_sent_bytes"] == prev["net_sent_bytes"] and metrics["net_recv_bytes"] == prev["net_recv_bytes"]:
            if time.time() - metrics["last_update_ts"] > 30:
                degraded = True
    GUI_STATE["telemetry_degraded"] = degraded
    if dpg.does_item_exist("telemetry_banner"):
        if degraded:
            dpg.set_value("telemetry_banner", "Telemetry degraded: predictions may be unreliable.")
        else:
            dpg.set_value("telemetry_banner", "")
...
def update_prediction_for_capsule(capsule_id: str):
    ...
    rows = GUI_STATE["capsules"]
    meta = next((m for m in rows if m.get("capsule_id") == capsule_id), None)
    if not meta:
        return
    sid = meta.get("species", {}).get("species_id", "unknown")
    created_at = meta.get("created_at", time.time())
    events = GUI_STATE["capsule_events"].get(capsule_id, [])
    env = get_environment_fingerprint()
    prediction = pred_organ.predict_for_capsule(capsule_id, events, env, sid, created_at)
    GUI_STATE["prediction_cache"][capsule_id] = prediction
    GUI_STATE["health_cache"][capsule_id] = prediction["health"]

    if dpg.does_item_exist("prediction_text"):
        txt = (
            f"Capsule: {capsule_id}\n"
            f"Predicted Risk 1m: {prediction['predicted_risk_1m']:.2f}\n"
            f"Predicted Risk 5m: {prediction['predicted_risk_5m']:.2f}\n"
            f"Predicted Risk 30m: {prediction['predicted_risk_30m']:.2f}\n"
            f"Slope: {prediction['slope']:.6f}\n"
            f"Volatility: {prediction['volatility']:.3f}\n"
            f"Health: {prediction['health']:.2f}\n"
    ...
    top_risers = sorted(
        GUI_STATE["prediction_cache"].values(),
        key=lambda p: p.get("slope", 0.0),
        reverse=True,
    )[:5]
    lines = []
    for p in top_risers:
        lines.append(
            f"{p['capsule_id']}: slope={p['slope']:.6f}, risk_5m={p['predicted_risk_5m']:.2f}, health={p['health']:.2f}"
        )
    if dpg.does_item_exist("top_risers_text"):
        dpg.set_value("top_risers_text", "\n".join(lines))
...
def filter_risky_toggled(sender, app_data, user_data):
    GUI_STATE["filter_risky_only"] = dpg.get_value("filter_risky_checkbox")


def sort_by_risk_toggled(sender, app_data, user_data):
    GUI_STATE["sort_by_risk"] = dpg.get_value("sort_by_risk_checkbox")
...
def update_lineage_text():
    ...
    for cid, evs in GUI_STATE["capsule_events"].items():
        ...
        for reason, group in grouped.items():
            count = len(group)
            ts = group[-1].get("timestamp", 0)
            prefix = ""
            if "denied" in reason or "forbidden" in reason or "destruct" in reason or "export" in reason:
                prefix = "!! "
            if count == 1:
                lines.append(f"  - {prefix}{reason} @ {ts:.0f}")
            else:
                lines.append(f"  - {prefix}{reason} x{count} (last @ {ts:.0f})")
...
def build_gui():
    ...
    with dpg.window(label="Dark Nebula Command Deck — Access-Driven", tag="main_window", width=1800, height=950):
        ...
        dpg.add_separator()
        dpg.add_text("", tag="telemetry_banner", color=(255, 180, 80, 255))
        dpg.add_separator()
        ...
        with dpg.group(horizontal=True):
            with dpg.child_window(width=800, height=800):
                ...
                dpg.add_checkbox(label="Show only risky (risk >= 0.5)", tag="filter_risky_checkbox", callback=filter_risky_toggled)
                dpg.add_checkbox(label="Sort by risk (desc)", tag="sort_by_risk_checkbox", default_value=True, callback=sort_by_risk_toggled)
                dpg.add_separator()
                ...
                with dpg.table(
                    ...
                    dpg.add_table_column(label="Health")
            ...
            with dpg.child_window(width=500, height=800):
                with dpg.tab_bar():
                    ...
                    with dpg.tab(label="Predictive Intelligence"):
                        ...
                        dpg.add_spacer(height=5)
                        dpg.add_text("Top 5 Rising Risk Capsules")
                        dpg.add_input_text(
                            multiline=True,
                            readonly=True,
                            tag="top_risers_text",
