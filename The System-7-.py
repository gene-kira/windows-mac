#!/usr/bin/env python3
"""
Fused Guardian – Evolution Tier + Game Cortex (Corrected Full Version)
    ...
    def _capture_snapshot(self) -> Dict[str, Any]:
        ...
        for c in conns:
            ...
            if game_pid is not None and pid == game_pid:
                is_non_private = False
            else:
                is_non_private = not self._is_private_ip(ip)

            pid_unregistered = pid in unreg_pids if pid is not None else False

            if is_non_private:
                suspicious += 1

            if pid_unregistered and is_non_private and pid in pid_to_proc:
                suspicious += 3
                try:
                    pid_to_proc[pid].terminate()
        ...
        try:
            for p in psutil.process_iter(["pid", "name", "exe"]):
                ...
                if game_exe and name == game_exe:
                    known = True
                elif self._is_under_windows_dirs(exe):
                    known = True
                else:
...
class GuardianApp(tk.Tk):
    ...
    def _auto_switch_to_game_tab_if_needed(self):
        if not self.game_cortex_frame:
            return
        game_info = self.engine.memory.get("current_game_info", None)
        if game_info and game_info.get("is_game"):
            current_tab = self.notebook.index(self.notebook.select())
            game_tab_index = self.notebook.index(self.game_cortex_frame)
            if current_tab != game_tab_index:
                self.notebook.select(game_tab_index)
...
class GameCortexFrame(ttk.Frame):
    def __init__(self, parent, engine: GuardianEngine, auto_switch_callback):
        super().__init__(parent, style="TFrame", padding=8)
        self.engine = engine
        self.auto_switch_callback = auto_switch_callback
...
def add_game_cortex_tab(app: GuardianApp):
    app.game_cortex_frame = GameCortexFrame(
        app.notebook,
        app.engine,
        auto_switch_callback=lambda: app._auto_switch_to_game_tab_if_needed()
    )
