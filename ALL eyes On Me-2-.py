...
# Aliases
queue, random, threading, time, uuid, socket = LIBS["queue"], LIBS["random"], LIBS["threading"], LIBS["time"], LIBS["uuid"], LIBS["socket"]
tk, psutil, fastapi, uvicorn = LIBS["tkinter"], LIBS["psutil"], LIBS["fastapi"], LIBS["uvicorn"]
...
def main():
    # Start engine and system feed immediately
    engine.start(3)
    feed.start()
