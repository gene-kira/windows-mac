...
# ---------- CONFIG ----------
APPDATA_DIR = os.getenv("APPDATA") or os.getcwd()
...
def log_event(msg: str):
    ...
    try:
        ...
        print("Permission denied writing to log file.")
...
class HoundDog:
    ...
    def find_suspects(self):
        with self.lock:
            ...
            before_ts, before_label, before_procs = self.snapshots[-2]
            after_ts, after_label, after_procs = self.snapshots[-1]
...
class TimeBossGUI:
    ...
    def _build_ui(self):
        ...
        self.auto_kill_var = tk.BooleanVar(value=self.settings["auto_kill"])

        ttk.Label(stf, text="Warning:").grid(row=0, column=0, sticky="w")
        ttk.Entry(stf, textvariable=self.warn_var, width=10).grid(row=0, column=1, sticky="w")
        ttk.Label(stf, text="Severe:").grid(row=1, column=0, sticky="w")
        ttk.Entry(stf, textvariable=self.severe_var, width=10).grid(row=1, column=1, sticky="w")
        ttk.Label(stf, text="Critical:").grid(row=2, column=0, sticky="w")
        ttk.Entry(stf, textvariable=self.critical_var, width=10).grid(row=2, column=1, sticky="w")

        optf = ttk.LabelFrame(settings_tab, text="Options")
        optf.pack(fill="x", padx=10, pady=10)
        ttk.Checkbutton(optf, text="Auto-correct (force sync on critical drift)",
                        variable=self.auto_correct_var).pack(anchor="w")
        ttk.Checkbutton(optf, text="Auto-kill suspicious processes on severe/critical drift",
                        variable=self.auto_kill_var).pack(anchor="w")
    ...
    def update_sync_view(self):
        ...
        if checked:
            self.sync_checked_var.set(f"Last Checked: {checked.strftime('%Y-%m-%d %H:%M:%S')}")
        else:
            self.sync_checked_var.set("Last Checked: Never")

    def update_hound_view(self, suspects):
        self.hound_tree.delete(*self.hound_tree.get_children())
        for s in suspects:
            self.hound_tree.insert("", "end",
                                   values=(s["name"], s["pid"], s["score"], s["reasons"], s["exe"]))
            if self.settings["auto_kill"] and s["score"] >= 3:
                try:
                    p = psutil.Process(s["pid"])
                    p.terminate()
                    log_event(f"AUTO-KILL: Terminated {s['name']} ({s['pid']})")
                except Exception as e:
                    log_event(f"AUTO-KILL FAILED: {s['name']} ({s['pid']}): {e}")
    ...
    def monitor_loop(self):
        while self.running:
            ...
            if severity in ("SEVERE", "CRITICAL") or clock_fight:
                ...
                if suspects:
                    log_event(f"HOUND DOG: {len(suspects)} suspects around drift event.")
            def gui_update():
