...
VAULT = os.path.expanduser("~\\Documents\\CodexVault")
...
class CodexDevourer:
    ...
    def isolate_files(self, path):
        for proc in psutil.process_iter(['pid', 'open_files', 'name']):
            try:
                files = proc.open_files()
                for f in files:
                    if f.path.startswith(path) and proc.info['name'] in self.sigs:
                        self.purge(proc.info['name'], proc, os.path.splitdrive(f.path)[0])
                        self.log(f"🛡️ Isolated file access blocked: {f.path}")
                        self.retain({"type": "file_isolation", "target": f.path})
