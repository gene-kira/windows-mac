...
def open_regedit_at(path_str):
    # path_str like HKEY_LOCAL_MACHINE\SOFTWARE\...
    subprocess.Popen(["regedit.exe"])
    # No official way to jump directly; user can navigate with path shown.
...
class RegistryEditorWindow(QMainWindow):
    ...
    def edit_value(self):
        ...
        data_item = self.table.item(row, 2)
        if not name_item:
            return
        name = name_item.text()
        vtype_name = type_item.text()
        # re-read actual data from registry
        values = enum_reg_values(self.root, self.subkey)
        data = None
        vtype = None
        for n, d, t in values:
