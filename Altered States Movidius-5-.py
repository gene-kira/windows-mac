...
# ============================================================
# ASI USB Movidius Turbo Backbone – Part 1
# Core brain, Movidius turbo organ, schedulers, runtime
# (Paste Part 2 immediately after this in the same .py file)
# ============================================================
...
import psutil  # system telemetry
...
# ============================================================
# 2. Movidius/OpenVINO engine + controller (batch + governor)
# ============================================================

class MovidiusEngine:
    """
    OpenVINO backend preferring USB MYRIAD.
    - USB-aware latency/jitter
    - Batch inference support
    - Simple load governor via latency / queue stats
    """
    ...
    def _raw_infer(self, compiled_model, input_name: str, input_array: np.ndarray):
        infer_request = compiled_model.create_infer_request()
        infer_request.infer({input_name: input_array})
        return infer_request.get_output_tensor().data

    def run_inference(self, graph_id: str, input_array: np.ndarray, batch_mode: bool = False):
        with self.lock:
            if not self.device_connected or self.core is None or self.backend_device == "NONE":
                raise RuntimeError("No device connected")
        ...
        if batch_mode and input_array.ndim == 2 and input_array.shape[0] > 1:
            outputs = []
            for i in range(input_array.shape[0]):
                out_i = self._raw_infer(compiled_model, input_name, input_array[i:i + 1])
                outputs.append(out_i)
            output = np.concatenate(outputs, axis=0)
        else:
            output = self._raw_infer(compiled_model, input_name, input_array)
        ...
        while self.engine.running:
            ...
            try:
                if cmd == "connect_device":
                    ...
                    graph_id, input_arr, batch_mode = args
                    output, latency = self.engine.run_inference(graph_id, input_arr, batch_mode=batch_mode)
...
# ============================================================
# 3. MovidiusInferenceEngine + Turbocharger (precompute + routing)
# ============================================================
...
class MovidiusInferenceEngine:
    ...
    High-level organ:
    - USB-aware accel
    - Accepts (state, forecast) and returns anomaly, risk_delta, intent, flow.
    """
    ...
    def accelerate(self, state: dict, forecast: dict, batch_mode: bool = False) -> dict:
        ...
        try:
            ok, result = self._safe_run_inference(features, batch_mode=batch_mode)
            if not ok:
    ...
    def _safe_run_inference(self, features: np.ndarray, batch_mode: bool = False, timeout: float = 3.0):
        res_queue = queue.Queue(maxsize=1)
        self.controller.cmd_queue.put(("run_inference", (self.model_graph_id, features, batch_mode), res_queue))
        ok, payload = res_queue.get(timeout=timeout)
        return ok, payload
...
class Turbocharger:
    """
    Turbocharger on top of MovidiusInferenceEngine.
    - Caching
    - CPU fallback
    - Precomputation worker (next-state forecasts)
    """
    ...
    def __init__(self, mv_engine: MovidiusInferenceEngine | None):
        ...
        self.precompute_queue: queue.Queue = queue.Queue(maxsize=64)
    ...
    def _should_use_movidius(self, cost_estimate: float, mv_status: dict) -> bool:
        ...
        if metrics.get("queue_depth", 0) > 5:
            return False
        if metrics.get("avg_latency_ms", 0.0) > 30.0:
            return False
        return True
    ...
    def accelerated_infer(self,
        ...
        try:
            self.precompute_queue.put_nowait((op_type, state, forecast, cost_estimate))
        except queue.Full:
            pass
    ...
    def _precompute_worker(self):
        while True:
            try:
                op_type, state, forecast, cost_estimate = self.precompute_queue.get(timeout=1.0)
            except queue.Empty:
                continue
            ...
            status = self.mv_engine.get_status()
            if not self._should_use_movidius(cost_estimate, status):
                continue

            next_state = dict(state)
            next_state["risk_score"] = max(0.0, min(1.0, state.get("risk_score", 0.0) * 0.9))
            next_state["opportunity_score"] = max(0.0, min(1.0, state.get("opportunity_score", 0.0) * 1.05))

            key_payload = {
                "op_type": op_type + "_pre",
                "state": {
                    "risk_score": next_state["risk_score"],
                    "opportunity_score": next_state["opportunity_score"],
                    "input_profile": next_state.get("input_profile"),
                },
                "forecast": forecast,
            }
            cache_key = self._make_key(op_type + "_pre", key_payload)

            with self.lock:
                if cache_key in self.cache:
                    continue

            mv_res = self.mv_engine.accelerate(next_state, forecast)
            result = {
                "backend": mv_res.get("backend", "MYRIAD"),
                "anomaly_score": mv_res.get("anomaly_score"),
                "risk_adjustment": mv_res.get("risk_adjustment"),
...
# ============================================================
# 4. Brain organs, system telemetry, extra organs, autoloader
# ============================================================
...
class ASIDialogueInterface:
    ...
    def why_did_you_choose_this_mission(self) -> DialogueAnswer:
        ...
        return DialogueAnswer(
            intent=f"I chose mission {mission} based on risk, opportunity, system load, device health, and your overrides.",
            confidence=0.78,
    ...
    def what_do_you_need_from_me(self) -> DialogueAnswer:
        return DialogueAnswer(
            ...
            confidence=0.9,
            alternatives=["You can also tune learning window bias and per-app policies."],
...
class SensoryOrgan:
    def derive_state(self, state: dict, flow: dict, mv_result: dict) -> dict:
        ip = state.get("input_profile", {}) or {}
        keys = ip.get("keys_per_sec", 0.0)
        mouse = ip.get("mouse_moves_per_sec", 0.0)
        burst = ip.get("burst_factor", 0.0)
...
class SystemTelemetryOrgan:
    def sample(self) -> dict:
        try:
            ...
            print(f"[SystemTelemetry] ERROR: {e}")
            return {
                "cpu_percent": 0.0,
                "ram_percent": 0.0,
                "ram_used_mb": 0.0,
...
class HybridBrainCore:
    ...
    def _apply_commands(self):
        ...
        for cmd in cmds:
            if cmd.type == CommandType.STABILIZE_SYSTEM:
                ...
                self.situational.set_risk_tolerance(max(0.0, self.situational.state.risk_tolerance - 0.2))
            elif cmd.type == CommandType.HIGH_ALERT_MODE:
                self.situational.force_protect()
                self.situational.set_risk_tolerance(max(0.0, self.situational.state.risk_tolerance - 0.1))
    ...
    def decide(self,
        ...
        return {"action": action, "mission": effective_mission.name, "reason": self.last_explanation}
...
class BrainRuntime:
    def __init__(self, loader: BrainAutoloader):
        ...
        self.system_telemetry = SystemTelemetryOrgan()

        self.last_system_sample: dict = {}
        self.tick_interval_sec = 0.7  # dynamic governor can adjust this
    ...
    def _dynamic_tick_adjust(self, risk: float, anomaly: float, system_cpu: float):
        base = 0.7
        if risk < 0.3 and anomaly < 0.3 and system_cpu < 50:
            self.tick_interval_sec = min(1.2, base * 1.5)
        elif risk > 0.7 or anomaly > 0.7:
            self.tick_interval_sec = max(0.2, base * 0.4)
        else:
            self.tick_interval_sec = base
    ...
    def tick(self, raw_telemetry: dict) -> dict:
        ...
        self.last_system_sample = self.system_telemetry.sample()

        self._dynamic_tick_adjust(risk_score, risk_score, self.last_system_sample.get("cpu_percent", 0.0))
        ...
        state_for_brain = {
            ...
            "system": self.last_system_sample,
        }
...
# ============================================================
# ASI USB Movidius Turbo Backbone – Part 2
# GUI cockpit and entry point
# (Append this directly after Part 1 in the same file)
# ============================================================
...
class HivePanel(QGroupBox):
    def __init__(self, brain: BrainRuntime):
        ...
        for lbl in (self.collective_label, self.density_label,
                    self.agreement_label, self.divergence_label):
            lbl.setStyleSheet("color: #EEEEEE;")
...
class SystemPanel(QGroupBox):
    def __init__(self):
        super().__init__("System Telemetry (Backbone View)")
        self.setStyleSheet("QGroupBox { border: 1px solid #444444; border-radius: 10px; }")
        layout = QVBoxLayout(self)
        layout.setSpacing(6)
        ...
        self.ram_mb_label = QLabel("0.0")
        self.disk_read_label = QLabel("0.0")
        self.disk_write_label = QLabel("0.0")
        self.net_sent_label = QLabel("0.0")
        self.net_recv_label = QLabel("0.0")

        for lbl in (self.cpu_label, self.ram_label, self.ram_mb_label,
                    self.disk_read_label, self.disk_write_label,
                    self.net_sent_label, self.net_recv_label):
            lbl.setStyleSheet("color: #EEEEEE;")

        layout.addWidget(make_kv_row("CPU %:", self.cpu_label))
        layout.addWidget(make_kv_row("RAM %:", self.ram_label))
        layout.addWidget(make_kv_row("RAM used (MB):", self.ram_mb_label))
        layout.addWidget(make_kv_row("Disk read (MB):", self.disk_read_label))
        layout.addWidget(make_kv_row("Disk write (MB):", self.disk_write_label))
        layout.addWidget(make_kv_row("Net sent (MB):", self.net_sent_label))
        layout.addWidget(make_kv_row("Net recv (MB):", self.net_recv_label))

    def update_from_snapshot(self, snap: dict):
        self.cpu_label.setText(f"{snap.get('cpu_percent', 0.0):.1f}")
        self.ram_label.setText(f"{snap.get('ram_percent', 0.0):.1f}")
        self.ram_mb_label.setText(f"{snap.get('ram_used_mb', 0.0):.1f}")
        self.disk_read_label.setText(f"{snap.get('disk_read_mb', 0.0):.1f}")
        self.disk_write_label.setText(f"{snap.get('disk_write_mb', 0.0):.1f}")
        self.net_sent_label.setText(f"{snap.get('net_sent_mb', 0.0):.1f}")
        self.net_recv_label.setText(f"{snap.get('net_recv_mb', 0.0):.1f}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("ASI USB Movidius Turbo Backbone Cockpit")
        ...
        # TAB 2 – Turbo / USB / System / Safety / Sensory
        mv_tab = QWidget()
        ...
        mv_layout.addWidget(self.watchdog_panel, 2, 0)
        mv_layout.addWidget(self.system_panel, 2, 1)

        tabs.addTab(mv_tab, "Turbo / USB / System")
    ...
    def on_tick(self):
        ...
        self.system_panel.update_from_snapshot(snapshot["system"])

        # Optional: adjust timer interval based on brain tick governor
        interval_ms = int(max(200.0, min(1500.0, snapshot.get("tick_interval_sec", 0.7) * 1000.0)))
