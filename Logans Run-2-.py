"""
Hive Guardian - Full Updated Scaffold with World Map
----------------------------------------------------
...
- BehaviorStore (persistent brain per node):
    - Process sequences
    - Time-of-day patterns (hour x weekday x process)
    - Prefetch reward memory
    - Event timeline + n-gram prediction
- PrefetchEngine + PrefetchPolicy:
    - Uses behavior profiles, time patterns,
      event predictions, and global foresight hints
- AnomalyDetector using learned behavior + novelty
...
- Tkinter GUI:
    - Tab 1: Node Detail (system, cache, anomaly, prefetch)
    - Tab 2: World Map (simple force-style layout of local node + external endpoints)

Requirements:
    pip install psutil requests

Run:
    python hive_guardian_full.py
"""
...
class BehaviorStore:
    ...
    Adds:
      - process_sequences (per-process transitions)
      - proc_resource_usage
      - time_patterns  (hour:weekday:process)
      - prefetch_reward
      - event_ngrams: n-gram event model over high-level events
    """
    ...
    def __init__(self, path: str, ngram_order: int = 2, history_len: int = 200):
        ...
        self.data = {
            ...
            "time_patterns": {},
            "prefetch_reward": {},
            "event_ngrams": {},  # context_tuple(str) -> {next_event -> count}
        }
    ...
    def add_event(self, event: str):
        """
        event is a high-level tag like:
          PROC:chrome.exe
          CONN:1.2.3.4:443
          FILE:C:\\path...
        """
    ...
    def predict_next_events(self, top_k: int = 5) -> List[Tuple[str, float]]:
        """
        Return list of (event, probability-like score).
        """
...
class PrefetchPolicy:
    """
    Uses:
      - BehaviorStore (event model, time patterns, rewards)
    ...
    def suggest_tasks(self, telemetry: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # 4) Time-of-day patterns: processes that are likely now
        now = time.localtime()
        hour = now.tm_hour
        weekday = now.tm_wday
...
class QueenState:
    ...
    def world_graph_snapshot(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        Returns (nodes, edges) for world graph.
...
class HiveGUI:
    ...
    Two-tab GUI:
      ...
      - World Map: simple layout of local node + external endpoints
    """
    ...
    def _layout_world_graph(self, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]):
        """
        Simple radial / random layout:
        - Local nodes near center
        - External nodes in ring
        """
        ...
        # Place local nodes around center
        ...
        # Place external nodes in a bigger ring
        ...
        for n in nodes:
            ...
            label = self.canvas.create_text(x, y - r - 5, text=n["label"], fill="#ffffff", font=("Arial", 8))
            self.world_node_objects[nid] = oval
            # label is not tracked individually, that's fine for now
    ...
    def refresh_ui(self):
        # Update node dropdown and detail
