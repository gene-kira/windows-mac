...
# --- 4D projection illusion: project W into Z offset ---
def project_4d_to_3d(pos4, w_scale=0.4):
    # pos4 = (x, y, z, w)
    x, y, z, w = pos4
    return (x, y, z + w * w_scale)

# --- 3D mirror sim with 4D illusion ---
class Mirror4DSimulation:
    def __init__(self, size=(20, 20, 20), mirror_plane_z=10, exposed_bounds=((6, 13), (6, 13))):
        self.size = size
        self.grid = np.zeros(size, dtype=int)
        self.viewer_pos4 = None   # (x,y,z,w)
        self.object_pos4 = None   # (x,y,z,w)
    ...
    def place_viewer(self, pos4):
        self.viewer_pos4 = pos4
        x, y, z, _ = [int(round(v)) for v in pos4]
        self.grid[x, y, z] = VIEWER

    def place_object(self, pos4):
        self.object_pos4 = pos4
        x, y, z, _ = [int(round(v)) for v in pos4]
        self.grid[x, y, z] = OBJECT
    ...
    def visualize(self, ax):
        ...
        # Viewer render (base + ghost depending on W)
        if self.viewer_pos4:
            vx, vy, vz, vw = self.viewer_pos4
            ax.scatter(vx, vy, vz, c='green', s=50, label='Viewer')
            # subtle ghost for viewer W
            ghost_v = project_4d_to_3d(self.viewer_pos4, w_scale=0.25)
            ax.scatter(*ghost_v, c='lime', s=30, alpha=0.35)

        # Object render (base + multiple ghosts to suggest layers)
        if self.object_pos4:
            ox, oy, oz, ow = self.object_pos4
            ax.scatter(ox, oy, oz, c='red', s=50, label='Object')
            # three ghost layers scaled by W to simulate 4D bleed-through
            for factor, color, alpha in [(0.4, 'magenta', 0.35), (0.8, 'violet', 0.28), (1.2, 'orchid', 0.22)]:
                ghost_o = project_4d_to_3d(self.object_pos4, w_scale=factor)
                ax.scatter(*ghost_o, c=color, s=36, alpha=alpha)

        ax.set_xlim(0, self.size[0]); ax.set_ylim(0, self.size[1]); ax.set_zlim(0, self.size[2])
        ax.set_xlabel("X"); ax.set_ylabel("Y"); ax.set_zlabel("Z")
        ax.legend(loc='upper left', fontsize=8)
        ax.set_title("3D Mirror with 4D Projection Illusion", fontsize=10)
...
class MirrorGUI:
    def __init__(self, root):
        ...
        # Positions include W component for 4D illusion
        self.viewer_pos4 = [5, 5, 2, 0.0]
        self.object_pos4 = [15, 15, 18, 0.0]
        ...
        self.fig = plt.Figure(figsize=(3.6, 2.6))  # compact ~50%
        ...
        # Viewer controls (XYZW)
        self._section_label("Viewer position (XYZW)")
        for i, label in enumerate(["X", "Y", "Z"]):
            self._slider_row(self.ctrl_inner, label, 0, self.size[i]-1, self.viewer_pos4[i],
                             lambda v, i=i: self._set_viewer_xyz(i, int(v)), resolution=1)
        # Viewer W
        self._slider_row(self.ctrl_inner, "W", -10, 10, int(self.viewer_pos4[3]),
                         lambda v: self._set_viewer_w(float(v)), resolution=1)

        # Object controls (XYZW)
        self._section_label("Object position (XYZW)")
        for i, label in enumerate(["X", "Y", "Z"]):
            self._slider_row(self.ctrl_inner, label, 0, self.size[i]-1, self.object_pos4[i],
                             lambda v, i=i: self._set_object_xyz(i, int(v)), resolution=1)
        # Object W
        self._slider_row(self.ctrl_inner, "W", -10, 10, int(self.object_pos4[3]),
                         lambda v: self._set_object_w(float(v)), resolution=1)
        ...
        for i, label in enumerate(labels):
            self._slider_row(self.ctrl_inner, label, ranges[i][0], ranges[i][1],
                             self.exposed_bounds[i], lambda v, i=i: self._set_exposed(i, int(v)), resolution=1)
    ...
    def _slider_row(self, parent, label, frm, to, init, cmd, resolution=1):
        ...
        s = tk.Scale(row, from_=frm, to=to, orient=tk.HORIZONTAL, command=lambda v: cmd(v),
                     resolution=resolution, length=160, showvalue=True, font=("Arial", 9))
        s.set(init)
    ...
    # Slider callbacks
    def _set_viewer_xyz(self, i, val):
        self.viewer_pos4[i] = val
        self.update_scene()

    def _set_viewer_w(self, val):
        self.viewer_pos4[3] = float(val)
        self.update_scene()

    def _set_object_xyz(self, i, val):
        self.object_pos4[i] = val
        self.update_scene()

    def _set_object_w(self, val):
        self.object_pos4[3] = float(val)
        self.update_scene()
    ...
    def tick(self):
        ...
        # Map telemetry to W illusion:
        # Viewer W from CPU (centered around 0), Object W from disk+net throughput.
        cpu = metrics["cpu_percent"]            # 0..100
        net_total = metrics["net_recv_bps"] + metrics["net_sent_bps"]
        disk_total = metrics["disk_read_bps"] + metrics["disk_write_bps"]
        gpu = metrics.get("gpu_load", 0.0)
        lat = metrics.get("latency_ms", None)

        # Normalize to [-10, 10] scale for W sliders
        viewer_w = (cpu - 50.0) / 5.0           # approx map: 0..100 -> -10..10
        object_w = min(10.0, max(-10.0, (net_total + disk_total) / 5_000_000.0))  # 0.. ~50MB/s -> ~0..10

        # Bias W with GPU and latency effects
        viewer_w += (gpu / 50.0)
        if lat is not None and lat > 150.0:
            object_w += 2.0

        # Smoothly approach values to avoid jitter
        self.viewer_pos4[3] = 0.7 * self.viewer_pos4[3] + 0.3 * viewer_w
        self.object_pos4[3] = 0.7 * self.object_pos4[3] + 0.3 * object_w
    ...
    def update_scene(self, metrics=None):
        ...
        sim = Mirror4DSimulation(size=self.size, mirror_plane_z=self.mirror_plane_z, exposed_bounds=bounds)
        sim.place_mirror()
        sim.place_viewer(tuple(self.viewer_pos4))
        sim.place_object(tuple(self.object_pos4))
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Compact 3D Mirror with 4D Projection Illusion")
    root.geometry("980x560")  # compact window with room for scrollable controls
