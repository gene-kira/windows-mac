#!/usr/bin/env python3
# ============================================================
# SYSTEM NERVE CENTER – HybridBrain + Tri-Stance + MultiGame v5.0 "Event Horizon Full"
...
class PredictionEngine:
    ...
    def _engine_votes(self) -> Tuple[float, Dict[str, float], str, Dict[str, float]]:
        ...
        # Horizon cluster classification
        cluster, conf = self._classify_horizon_cluster(horizon_vote)
    ...
    def _update_meta_state_momentum(self, regime: str):
        ...
        if cluster in ("DANGER", "CRITICAL") or risk > 0.8 or integrity < 0.6:
...
class TriStanceEngine:
    ...
    def tick(self):
        ...
        if cluster in ("DANGER", "CRITICAL") and cluster_conf > 0.6:
            new = "Conservative"
...
class NerveCenterApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("System Nerve Center – HybridBrain Tri-Stance (Event Horizon Full)")
    ...
    def _build_gui(self):
        ...
        self.lbl_top_status = tk.Label(
            top,
            text="Nerve Center Online (Event Horizon Full)",
            fg="#00ff99",
            bg="#111111",
    ...
    def cmd_save_reboot_memory(self):
        ...
        meta = {"ts": time.time(), "version": "5.0-event-horizon-full"}
    ...
    def _save_settings(self):
        ...
        path = os.path.join(os.path.expanduser("~"), ".nerve_center_settings_event_horizon_full.json")
        try:
