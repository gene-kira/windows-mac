...
class DataGuard:
    ...
    def protect_endpoint(self, ip: str, port: int) -> str:
        # Mask IP but keep port visible
        return f"{self._encode(ip)}:{port}"
...
# ========================= NETWORK THREAT ORGAN (TIERED MODES) =========================

class NetworkMode(Enum):
    AUTO = auto()        # detect + auto-terminate
    SEMI_AUTO = auto()   # detect + queue for manual decision
    MANUAL = auto()      # detect + log only
    RADIOACTIVE = auto() # detect + hard-mask, keep “hot” but no auto-kill
...
class SuspiciousConnection:
    ...
    remote_port: int
    status: str
    mode_at_detection: NetworkMode


class NetworkThreatOrgan:
    """
    Tiered network defense:
    - AUTO: detect + auto-terminate suspicious processes
    - SEMI_AUTO: detect + queue as High Suspense, no auto-kill
    - MANUAL: detect + log only
    - RADIOACTIVE: detect + hard-mask endpoints, no auto-kill
    """

    def __init__(self, logger: SecureLogger, dataguard: DataGuard,
                 anomaly: Optional[DeepAnomalyOrgan] = None):
        self.logger = logger
        self.dg = dataguard
        self.anomaly = anomaly
        self.enabled = True
        self.mode: NetworkMode = NetworkMode.AUTO
        self._lock = threading.Lock()
        self._suspicious_ports = {4444, 1337, 31337, 5555}
        self._high_port_threshold = 50000
        self._suspense_queue: List[SuspiciousConnection] = []
    ...
    def set_mode(self, mode: NetworkMode):
        self.mode = mode
        self.logger.info(f"NetworkThreatOrgan mode={mode.name}")

    def get_suspense_queue(self) -> List[SuspiciousConnection]:
        with self._lock:
            return list(self._suspense_queue)

    def clear_suspense_item(self, index: int):
        with self._lock:
            if 0 <= index < len(self._suspense_queue):
                self._suspense_queue.pop(index)
    ...
    def _is_suspicious(self, rport: int) -> bool:
        return rport in self._suspicious_ports or rport > self._high_port_threshold
    ...
    def scan(self):
        ...
        findings: List[str] = []
        auto_kill_pids = set()
        new_suspense: List[SuspiciousConnection] = []
        ...
        for c in conns:
            ...
            if not self._is_suspicious(rport):
                continue
            ...
            findings.append(
                f"Suspicious outbound connection to {rip}:{rport} "
                f"(status={status}, pid={pid}, proc={proc_name}, category: NETWORK)"
            )

            conn = SuspiciousConnection(
                pid=pid or -1,
                proc_name=proc_name,
                remote_ip=rip,
                remote_port=rport,
                status=status,
                mode_at_detection=self.mode,
            )

            if self.mode == NetworkMode.AUTO:
                if pid:
                    auto_kill_pids.add(pid)
            elif self.mode == NetworkMode.SEMI_AUTO:
                new_suspense.append(conn)
            elif self.mode == NetworkMode.MANUAL:
                pass
            elif self.mode == NetworkMode.RADIOACTIVE:
                new_suspense.append(conn)
        ...
        if findings:
            self.logger.warn(
                f"NetworkThreatOrgan: {len(findings)} suspicious connections detected (mode={self.mode.name})"
            )

        if new_suspense:
            with self._lock:
                self._suspense_queue.extend(new_suspense)

        if self.mode == NetworkMode.AUTO and auto_kill_pids:
            for pid in auto_kill_pids:
                try:
                    p = psutil.Process(pid)
                    name = p.name()
                    self.logger.warn(f"Active defense (AUTO): terminating suspicious process PID={pid}, name={name}")
                    p.terminate()
                except Exception as e:
...
class Cockpit(QWidget):
    def __init__(self):
        ...
        self.network_organ = NetworkThreatOrgan(logger=self.logger,
        ...
        self.setWindowTitle("Defensive Code Purifier – Autonomous Nuclear Cockpit (Network Override)")
    ...
    def _build_ui(self):
        ...
        nav = QListWidget()
        for name in [
            "Code", "Threats", "Logs", "AI Engine",
            "Memory", "Nuclear", "Lineage", "Automation",
            "Network Threats"
        ]:
        ...
        auto_layout.addWidget(QLabel("Network Threat Organ Mode"))
        self.net_mode_combo = QComboBox()
        self.net_mode_combo.addItems([m.name for m in NetworkMode])
        self.net_mode_combo.setCurrentText(self.network_organ.mode.name)
        self.net_mode_combo.currentTextChanged.connect(self.on_set_network_mode)
        auto_layout.addWidget(self.net_mode_combo)
        ...
        auto_layout.addWidget(QLabel("Note: AUTO = auto-terminate, SEMI_AUTO = High Suspense queue, MANUAL = log only, RADIOACTIVE = masked, hot."))

        self.stack.addWidget(auto_page)

        # Network Threats page (manual override GUI)
        net_page = QWidget()
        net_layout = QVBoxLayout()
        net_page.setLayout(net_layout)

        net_layout.addWidget(QLabel("Network Threats – High Suspense / Manual Override"))

        self.net_table = QTableWidget(0, 6)
        self.net_table.setHorizontalHeaderLabels(
            ["PID", "Process", "Remote (masked)", "Port", "Status", "Mode"]
        )
        self.net_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        net_layout.addWidget(self.net_table)

        btn_bar = QHBoxLayout()
        self.btn_net_refresh = QPushButton("Refresh Suspense Queue")
        self.btn_net_refresh.clicked.connect(self.refresh_network_table)
        btn_bar.addWidget(self.btn_net_refresh)

        self.btn_net_allow = QPushButton("Allow Selected")
        self.btn_net_allow.clicked.connect(self.on_net_allow)
        btn_bar.addWidget(self.btn_net_allow)

        self.btn_net_terminate = QPushButton("Terminate Selected")
        self.btn_net_terminate.clicked.connect(self.on_net_terminate)
        btn_bar.addWidget(self.btn_net_terminate)

        self.btn_net_radioactive = QPushButton("Mark Selected as Radioactive")
        self.btn_net_radioactive.clicked.connect(self.on_net_radioactive)
        btn_bar.addWidget(self.btn_net_radioactive)

        net_layout.addLayout(btn_bar)
    ...
    def on_set_network_mode(self, name: str):
        try:
            mode = NetworkMode[name]
            self.network_organ.set_mode(mode)
        except KeyError:
            self.log_viewer.add_log(f"[ERROR] Invalid NetworkMode: {name}")
        ...
        try:
            ...
            for c in conns:
                if not c.raddr:
                    continue
                rport = c.raddr.port
                if self.network_organ._is_suspicious(rport):
                    suspicious += 1
    ...
    def refresh_network_table(self):
        queue = self.network_organ.get_suspense_queue()
        self.net_table.setRowCount(len(queue))
        for row, conn in enumerate(queue):
            pid_item = QTableWidgetItem(str(conn.pid))
            proc_item = QTableWidgetItem(conn.proc_name or "?")
            masked = self.dataguard.protect_endpoint(conn.remote_ip, conn.remote_port)
            remote_item = QTableWidgetItem(masked)
            port_item = QTableWidgetItem(str(conn.remote_port))
            status_item = QTableWidgetItem(conn.status)
            mode_item = QTableWidgetItem(conn.mode_at_detection.name)

            for item in [pid_item, proc_item, remote_item, port_item, status_item, mode_item]:
                item.setFlags(item.flags() ^ Qt.ItemIsEditable)

            self.net_table.setItem(row, 0, pid_item)
            self.net_table.setItem(row, 1, proc_item)
            self.net_table.setItem(row, 2, remote_item)
            self.net_table.setItem(row, 3, port_item)
            self.net_table.setItem(row, 4, status_item)
            self.net_table.setItem(row, 5, mode_item)

    def _get_selected_suspense_indices(self) -> List[int]:
        rows = set()
        for idx in self.net_table.selectedIndexes():
            rows.add(idx.row())
        return sorted(rows)

    def on_net_allow(self):
        indices = self._get_selected_suspense_indices()
        if not indices:
            return
        indices.reverse()
        for i in indices:
            self.network_organ.clear_suspense_item(i)
        self.refresh_network_table()
        self.log_viewer.add_log(f"[NET] Operator allowed {len(indices)} suspense item(s)")

    def on_net_terminate(self):
        if not HAS_PSUTIL:
            return
        queue = self.network_organ.get_suspense_queue()
        indices = self._get_selected_suspense_indices()
            ...
            if 0 <= i < len(queue):
                ...
                if conn.pid > 0:
                    try:
                        ...
                        name = p.name()
                        self.logger.warn(
                            f"Operator terminate: PID={conn.pid}, name={name}, remote={conn.remote_ip}:{conn.remote_port}"
                        )
                        p.terminate()
                    except Exception as e:
                        self.logger.error(f"Operator terminate failed PID={conn.pid}: {e}")
        indices.reverse()
        for i in indices:
            self.network_organ.clear_suspense_item(i)
        self.refresh_network_table()

    def on_net_radioactive(self):
        indices = self._get_selected_suspense_indices()
        if not indices:
            return
        self.log_viewer.add_log(
            f"[NET] Operator marked {len(indices)} suspense item(s) as RADIOACTIVE (kept hot, masked)."
        )
        # We keep them in the queue; they remain visible but masked.
        # No extra action needed here beyond logging.
