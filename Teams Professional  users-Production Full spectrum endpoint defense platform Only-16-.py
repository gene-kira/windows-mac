...
TRAINING_DATA_FILE = os.path.join(APPDATA, "sentinel_training_data.json")
NODE_SYNC_FILE = os.path.join(APPDATA, "sentinel_node_sync.json")
...
# Distributed node sync config (simple JSON-based)
NODE_SYNC_CONFIG = {
    "enabled": False,
    "mode": "file",  # "file" or "http"
    "file_path": NODE_SYNC_FILE,
    "http_url": "",
...
class LineageGraph:
    ...
    def propagate_risk(self, pid, base_score):
        with self.lock:
            boost = 0
            node = self.nodes.get(pid)
            if not node:
            ...
            return base_score + boost, boost

    def get_kill_chain_view(self, max_nodes=200):
        with self.lock:
            nodes = dict(list(self.nodes.items())[-max_nodes:])
            edges = []
...
class AsyncInferenceManager:
    ...
    def submit(fn, *args, **kwargs):
        return AsyncInferenceManager._executor.submit(fn, *args, **kwargs)


# ============================================================
# MODEL UPDATE MANAGER (AUTO + MANUAL DOWNLOAD + SIGNATURE)
# ============================================================
...
class ModelUpdateManager:
    ...
    def set_advanced_config(self, primary, fallbacks, manifest, directory, github_repo, auto_update, expected_sha256=None):
        with self.lock:
            ...
            if expected_sha256 is not None:
                self.cfg["expected_sha256"] = expected_sha256.strip()
        self.save_cfg()
    ...
    def _verify_signature(self, path, expected_hash):
        if not expected_hash:
            return True
        actual = compute_file_hash(path)
        if not actual:
            log_event("MODEL: Failed to compute hash for signature verification.")
            return False
        if actual.lower() != expected_hash.lower():
            log_event(f"MODEL: Signature mismatch. Expected {expected_hash}, got {actual}")
            return False
        log_event("MODEL: Signature verification passed.")
        return True
    ...
    def _try_manifest(self, dest_path):
        ...
        expected_hash = data.get("sha256") or cfg.get("expected_sha256", "")
        if not model_url:
        ...
        if not self._validate_onnx(dest_path):
            return False
        if not self._verify_signature(dest_path, expected_hash):
            return False
    ...
    def _try_direct_url(self, url, dest_path):
        ...
        if not self._validate_onnx(dest_path):
            return False
        cfg = self.get_cfg()
        if not self._verify_signature(dest_path, cfg.get("expected_sha256", "")):
            return False
    ...
    def download_model_to(self, dest_path):
        ...
        if not dest_dir.startswith(os.path.abspath(APPDATA)):
            log_event(f"MODEL: Sandbox violation: refusing to auto-download to {dest_dir}")
            return False
...
# ============================================================
# TRAINING PIPELINE (DATA COLLECTION + EXPORT)
# ============================================================

class TrainingPipeline:
    """
    Simple training data collector/exporter.
    Not doing actual training here, but preparing data for offline training.
    """

    @staticmethod
    def collect_sample(info, feature_vector):
        label = None
        status = info.get("status")
        score = info.get("score", 0)
        if status in ("BLOCKED", "RADIOACTIVE") or score >= 80:
            label = 1
        elif status in ("ALLOW", "TRUSTED") and score <= 20:
            label = 0
        ...
        sample = {
            "ts": time.time(),
            "label": label,
            "features": {k: float(v) for k, v in feature_vector.items()},
            "path": info.get("path"),
            "name": info.get("name"),
        }
        ...
        tmp = TRAINING_DATA_FILE + ".tmp"
        try:
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2)
            os.replace(tmp, TRAINING_DATA_FILE)
        except OSError as e:
            log_event(f"TRAINING: Failed to save training data: {e}")
            try:
                if os.path.exists(tmp):
                    os.remove(tmp)
            except OSError:
    ...
    def export_training_data(path):
        try:
            if not os.path.exists(TRAINING_DATA_FILE):
                messagebox.showinfo("Training Data", "No training data collected yet.")
                return False
            with open(TRAINING_DATA_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            tmp = path + ".tmp"
            ...
            log_event(f"TRAINING: Export failed: {e}")
            return False


# ============================================================
# DISTRIBUTED NODE SYNC (SIMPLE FILE/HTTP)
# ============================================================
...
class NodeSyncManager:
    ...
    def set_cfg(self, enabled, mode, file_path, http_url, interval):
        with self.lock:
            self.cfg["enabled"] = bool(enabled)
            self.cfg["mode"] = mode
            self.cfg["file_path"] = file_path
            self.cfg["http_url"] = http_url
            self.cfg["sync_interval"] = int(interval)
        log_event(f"NODE SYNC: Updated config: {self.cfg}")

    def _collect_local_state(self):
        with BEHAVIOR_LOCK, STATS_LOCK, CAMPAIGN_LOCK:
            return {
    ...
    def _merge_remote_state(self, remote):
        if not isinstance(remote, dict):
            return
        # Very simple merge: we only merge campaigns and stats; behavior is local.
        with STATS_LOCK:
            rh = remote.get("stats", {}).get("history", [])
            lh = STATS_DB.get("history", [])
            combined = lh + rh
            combined = sorted(combined, key=lambda x: x.get("ts", 0))[-300:]
            STATS_DB["history"] = combined
        with CAMPAIGN_LOCK:
            rc = remote.get("campaigns", {})
            for k, v in rc.items():
                local = CAMPAIGN_DB.get(k, {})
                if v.get("last_seen", 0) > local.get("last_seen", 0):
                    CAMPAIGN_DB[k] = v
        save_stats()
        save_campaigns()

    def _sync_file(self):
        path = self.cfg.get("file_path") or NODE_SYNC_FILE
        state = self._collect_local_state()
        try:
            if os.path.exists(path):
                with open(path, "r", encoding="utf-8") as f:
                    remote_all = json.load(f)
            else:
                remote_all = {}
        except (OSError, json.JSONDecodeError):
            remote_all = {}

        remote_all[state["node_id"]] = state

        tmp = path + ".tmp"
        try:
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump(remote_all, f, indent=2)
            os.replace(tmp, path)
        except OSError as e:
            log_event(f"NODE SYNC: Failed to write sync file: {e}")
            try:
                if os.path.exists(tmp):
                    os.remove(tmp)
            except OSError:
        ...
        for nid, rstate in remote_all.items():
            if nid == state["node_id"]:
                continue
            self._merge_remote_state(rstate)

    def _sync_http(self):
        if requests is None:
            log_event("NODE SYNC: requests not available, cannot HTTP sync.")
            return
        url = self.cfg.get("http_url")
        if not url:
            return
        state = self._collect_local_state()
        try:
            resp = requests.post(url, json=state, timeout=10)
            if resp.status_code != 200:
                log_event(f"NODE SYNC: HTTP sync failed: {resp.status_code}")
                return
            remote_all = resp.json()
        except Exception as e:
            log_event(f"NODE SYNC: HTTP error: {e}")
            return

        if isinstance(remote_all, dict):
            for nid, rstate in remote_all.items():
                if nid == state["node_id"]:
                    continue
                self._merge_remote_state(rstate)
    ...
    def sync_loop(self):
        while True:
            ...
            if not cfg.get("enabled", False):
                continue
            now = time.time()
            if now - self.last_sync < cfg.get("sync_interval", 60):
                continue
            self.last_sync = now
...
NODE_SYNC_MANAGER = NodeSyncManager(NODE_SYNC_CONFIG)


# ============================================================
# MODULAR SCORING PIPELINE + ML HOOKS + GRAPH PROPAGATION
# ============================================================
...
class ScoringPipeline:
    ...
    def load_model(path: str):
        ...
        if not abs_path.startswith(os.path.abspath(APPDATA)):
            log_event(f"ML: Loading model outside sandbox: {abs_path} (manual load assumed)")
        try:
            sess = ort.InferenceSession(path, providers=["CPUExecutionProvider"])
    ...
    @staticmethod
    def _onnx_predict_internal(feature_vector):
        ...
        return float(outputs[0].ravel()[0])
    ...
    def ml_model_predict(feature_vector):
        ...
        try:
            future = AsyncInferenceManager.submit(ScoringPipeline._onnx_predict_internal, feature_vector)
            ml_score = future.result(timeout=2.0)
            if ml_score is None:
                return None, None
    ...
    def compute_score(info, behavior_record, bad_fingerprints, cluster_stats, campaign_stats, now):
        ...
        if status == "ALLOW":
            return 0, "LOW", [], [], False, 0.0, 0.0, None, 0.0, None
        ...
        propagated_score, chain_boost = LINEAGE_GRAPH.propagate_risk(info["pid"], score)
        if chain_boost > 0:
            anomalies.append(f"Graph propagation boost: +{chain_boost:.1f} from ancestor/descendant risk")
        ...
        return score, threat, anomalies, fp, matched_bad_fp, momentum, features["age_seconds"], features["cluster_key"], chain_boost, feature_vector
...
class SentinelGUI:
    def __init__(self, root):
        ...
        self.node_sync_enabled_var = tk.BooleanVar(value=NODE_SYNC_CONFIG.get("enabled", False))
        self.node_sync_mode_var = tk.StringVar(value=NODE_SYNC_CONFIG.get("mode", "file"))
        self.node_sync_file_var = tk.StringVar(value=NODE_SYNC_CONFIG.get("file_path", NODE_SYNC_FILE))
        self.node_sync_http_var = tk.StringVar(value=NODE_SYNC_CONFIG.get("http_url", ""))
        self.node_sync_interval_var = tk.IntVar(value=NODE_SYNC_CONFIG.get("sync_interval", 60))
    ...
    def start_node_sync_thread(self):
        t = threading.Thread(target=NODE_SYNC_MANAGER.sync_loop, daemon=True)
        self.node_sync_thread = t
        t.start()
        log_event("Node sync thread started.")
    ...
    def _build_ui(self):
        ...
        self.timeline_tree = ttk.Treeview(
            timeline_frame,
            columns=("ts", "proc_count", "high_count", "avg_score"),
            show="headings",
        )
        for c in ("ts", "proc_count", "high_count", "avg_score"):
            self.timeline_tree.heading(c, text=c.upper())
        self.timeline_tree.pack(fill="both", expand=True)

        killchain_frame = ttk.Frame(notebook)
        notebook.add(killchain_frame, text="Kill Chain")

        self.killchain_text = tk.Text(killchain_frame, wrap="word")
        self.killchain_text.pack(fill="both", expand=True)

        heatmap_frame = ttk.Frame(notebook)
        notebook.add(heatmap_frame, text="Threat Heatmap")
        ...
        ttk.Label(training_frame, text="Export collected training data for offline model training.").pack(anchor="w", padx=5, pady=5)
        ttk.Button(training_frame, text="Export Training Data...", command=self.on_export_training_data).pack(anchor="w", padx=5, pady=5)

        sync_frame = ttk.Frame(notebook)
        notebook.add(sync_frame, text="Node Sync")

        ttk.Checkbutton(sync_frame, text="Enable Node Sync", variable=self.node_sync_enabled_var,
                        command=self.on_node_sync_changed).grid(row=0, column=0, sticky="w", padx=5, pady=5)

        ttk.Label(sync_frame, text="Mode:").grid(row=1, column=0, sticky="w", padx=5, pady=3)
        mode_combo = ttk.Combobox(sync_frame, textvariable=self.node_sync_mode_var,
                                  values=["file", "http"], state="readonly", width=10)
        mode_combo.grid(row=1, column=1, sticky="w", padx=5, pady=3)
        mode_combo.bind("<<ComboboxSelected>>", lambda e: self.on_node_sync_changed())

        ttk.Label(sync_frame, text="File path:").grid(row=2, column=0, sticky="w", padx=5, pady=3)
        file_entry = ttk.Entry(sync_frame, textvariable=self.node_sync_file_var, width=40)
        file_entry.grid(row=2, column=1, sticky="we", padx=5, pady=3)

        ttk.Label(sync_frame, text="HTTP URL:").grid(row=3, column=0, sticky="w", padx=5, pady=3)
        http_entry = ttk.Entry(sync_frame, textvariable=self.node_sync_http_var, width=40)
        http_entry.grid(row=3, column=1, sticky="we", padx=5, pady=3)

        ttk.Label(sync_frame, text="Sync interval (sec):").grid(row=4, column=0, sticky="w", padx=5, pady=3)
        interval_entry = ttk.Entry(sync_frame, textvariable=self.node_sync_interval_var, width=10)
        interval_entry.grid(row=4, column=1, sticky="w", padx=5, pady=3)

        ttk.Button(sync_frame, text="Apply", command=self.on_node_sync_changed).grid(row=5, column=1, sticky="e", padx=5, pady=10)
    ...
    def on_download_model_clicked(self):
        ...
        if not ok:
            self.model_status_var.set("Model: none")
            messagebox.showerror("Download Failed", "Failed to find/download a model using configured sources (or sandbox blocked path).")
            return
    ...
    def on_advanced_model_clicked(self):
        ...
        ttk.Label(win, text="Expected SHA-256 (optional):").grid(row=5, column=0, sticky="w", padx=5, pady=3)
        sha_var = tk.StringVar(value=cfg.get("expected_sha256", ""))
        sha_entry = ttk.Entry(win, textvariable=sha_var, width=50)
        sha_entry.grid(row=5, column=1, sticky="we", padx=5, pady=3)
        ...
        auto_check.grid(row=6, column=1, sticky="w", padx=5, pady=5)
    ...
    def on_export_training_data(self):
        path = filedialog.asksaveasfilename(
        ...
        if not path:
            return
        ok = TrainingPipeline.export_training_data(path)
        if ok:
            messagebox.showinfo("Training Data", f"Exported training data to:\n{path}")
        else:
            messagebox.showerror("Training Data", "Failed to export training data. See log for details.")

    # ---------------- NODE SYNC UI ----------------

    def on_node_sync_changed(self):
        enabled = self.node_sync_enabled_var.get()
        mode = self.node_sync_mode_var.get()
        file_path = self.node_sync_file_var.get().strip() or NODE_SYNC_FILE
        http_url = self.node_sync_http_var.get().strip()
        interval = self.node_sync_interval_var.get() or 60
        NODE_SYNC_MANAGER.set_cfg(enabled, mode, file_path, http_url, interval)
    ...
    def show_explanation_popup(self, event):
        ...
        chain_boost = row.get("chain_boost", 0.0)
        ...
        text.insert("end", f"Chain boost: {chain_boost:.1f}\n")
    ...
    def update_behavior(self, proc, info, bad_fingerprints, cluster_stats, campaign_stats, now):
        ...
        with BEHAVIOR_LOCK:
            rec = BEHAVIOR_DB.get(key, {})
            score, threat, anomalies, fp, matched_bad_fp, momentum, age_seconds, cluster_key, chain_boost, feature_vector = \
        ...
        info["chain_boost"] = chain_boost
        info["feature_vector"] = feature_vector

        LINEAGE_GRAPH.update_process(info["pid"], info["ppid"], info["name"], info["path"], score, now)

        if feature_vector is not None:
            TrainingPipeline.collect_sample(info, feature_vector)
    ...
    def refresh_timeline_view(self):
        with STATS_LOCK:
            hist = list(STATS_DB.get("history", []))
    ...
    def refresh_killchain_view(self):
        nodes, edges = LINEAGE_GRAPH.get_kill_chain_view()
        self.killchain_text.config(state="normal")
        self.killchain_text.delete("1.0", "end")
        self.killchain_text.insert("end", "Kill Chain View (recent processes):\n\n")
        self.killchain_text.insert("end", "Nodes:\n")
        for pid, n in nodes.items():
            self.killchain_text.insert(
                "end",
                f"  PID {pid}: {n['name']} [{n['path']}] score={n['score']}\n"
            )
        self.killchain_text.insert("end", "\nEdges (parent -> child):\n")
        for p, c in edges:
            self.killchain_text.insert("end", f"  {p} -> {c}\n")
        self.killchain_text.config(state="disabled")

    # ---------------- THREAT HEATMAP ----------------

    def refresh_heatmap_view(self):
        ...
        height = self.heatmap_canvas.winfo_height() or 200
        ...
        for i, count in enumerate(buckets):
            x0 = i * bar_width
            x1 = x0 + bar_width - 2
            h_ratio = count / max_count
            y1 = height - 5
            y0 = y1 - h_ratio * (height - 20)
    ...
    def update_ui(self, rows, net_rows, updated):
        ...
        self.refresh_killchain_view()
        self.refresh_heatmap_view()
        if updated:
            self.refresh_lists_ui()
...
def main():
    ...
    print("Training data file:", os.path.abspath(TRAINING_DATA_FILE))
    print("Node sync file:", os.path.abspath(NODE_SYNC_FILE))
