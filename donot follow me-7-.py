...
import winreg, json, threading, time, os, ctypes, sys, subprocess, requests
...
SYNC_ENDPOINT = "https://codex-sync.example.com/api/purge"
...
def purge_vector(vector_name):
    ...
    sync_to_node(vector_name)

def sync_to_node(vector_name):
    try:
        payload = {"vector": vector_name, "timestamp": time.ctime()}
        requests.post(SYNC_ENDPOINT, json=payload, timeout=5)
        log_event(f"🌐 Synced purge of {vector_name} to external node")
    except Exception as e:
        log_event(f"❌ Sync failed: {e}")
    ...
    def run_watchdog():
        while True:
            try:
                ...
                suspicious = [s for s in ["CompatTelRunner.exe", "wsqmcons.exe"] if s in output]
                for proc in suspicious:
                    log_event(f"🛰️ Watchdog alert: {proc} is running")
            except Exception as e:
                log_event(f"Watchdog scan failed: {e}")
            time.sleep(60)
