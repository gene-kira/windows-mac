...
- Unlimited drive selection (local + network) with per-drive stats and rates
...
def is_network_fs(fstype, device, mountpoint):
    ...
    if platform.system() == "Windows" and device and device.startswith("\\\\"):
        return True
    # macOS SMB can appear as "smbfs" or fuse mounts
    return False
...
def basename_device(dev):
    ...
    # Normalize Windows drive letters like 'C:\\' to 'C:'
...
class DiskCollector(CollectorBase):
    ...
    def __init__(self, get_selection_fn):
        # get_selection_fn returns a list of selected drive keys (device or basename)
        self.get_selection_fn = get_selection_fn

    def collect(self):
        data = {"perdisk": {}, "aggregate": {}}
        selected = set(self.get_selection_fn())  # user-selected set
        if HAS_PSUTIL:
            import psutil
            # perdisk counters keyed by device name (platform-specific)
            perdisk = safe_get(lambda: psutil.disk_io_counters(perdisk=True), {}) or {}
            # collect only selected, with flexible key matching (full and basename)
            ...
            # Aggregate total counters too (across all disks, for context)
...
class TelemetryEngine:
    def __init__(self, interval=1.0, adaptive=True, enabled_plugins=None, thresholds=None):
        ...
        self.drive_inventory = []   # list of dicts: {device, mountpoint, fstype, is_network, key}
        self.selected_drives = set()  # keys chosen by user (device or basename)
        ...
        self._prev_disk_per = {}  # prev per-drive counters
    ...
    def _enumerate_drives(self):
        ...
        if HAS_PSUTIL:
            ...
            # Default select: all discovered drives
    ...
    def update_selected_drives(self, selection_keys):
        # Accept any number of drives (unlimited)
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            # Enqueue snapshot with deltas + alerts + drive inventory
...
# ---------------------------
# Settings dialog (plugins, thresholds, overlay, drive selection)
# ---------------------------
class SettingsDialog(tk.Toplevel):
    def __init__(self, master, engine: TelemetryEngine, on_apply):
        ...
        # Scrollable area for many drives
        ...
        # Populate drive checkboxes (unlimited count)
...
class Dashboard(tk.Frame):
    ...
    def update_metrics_tree(self, s):
        ...
        # Per-drive stats (selected only if present)
        ...
        if perdisk:
            ...
            for dev_key, stats in perdisk.items():
                ...
                self._add_metric(f"{tag} read", f"{fmt_bytes(rb)} ({fmt_bytes(rbps)}/s)")
                self._add_metric(f"{tag} write", f"{fmt_bytes(wb)} ({fmt_bytes(wbps)}/s)")

        # Network aggregate & rates
