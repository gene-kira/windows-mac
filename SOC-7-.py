#!/usr/bin/env python3
"""
Hive Guardian - ASI Queen v9 (Admin Tab + Predictive Engine)
...
- Threat timeline:
    - Per-process / per-identity chain of events (risk over time, actions, signals)
...
- Persistence (JSON brain file):
    ...
    - Paranoia level, strategy, profiles, signatures, trust, cluster labels, network backup path, swarm config
- Optional network backup of brain file
- Memory+disk-aware behavior
- Swarm sync (simple JSON push/pull to a configurable endpoint)
- ML & GPU hooks:
    - Optional PyTorch-based risk refinement if installed and enabled
    - Placeholder function you can wire to a real model
...
# Optional ML / GPU library (PyTorch)
try:
    import torch
...
class QueenBrain:
    ...
    def _load_brain(self):
        ...
        self.ml_enabled = data.get("ml_enabled", False) and TORCH_AVAILABLE
        if self.ml_enabled:
            print("[ML] ML-enhanced risk enabled; load your model into self.ml_model if desired.")
    ...
    def set_ml_enabled(self, enabled: bool):
        if enabled and not TORCH_AVAILABLE:
            self.log("[ML] PyTorch not available; cannot enable ML-enhanced risk.")
            return
        self.ml_enabled = enabled
        self._save_brain()
        self.log(f"[ADMIN] ML-enhanced risk set to: {self.ml_enabled}")
    ...
    def _ml_refine_risk(self, features: List[float], base_risk: float) -> float:
        if not (self.ml_enabled and TORCH_AVAILABLE and self.ml_model is not None):
            return base_risk
        try:
            with torch.no_grad():
                x = torch.tensor(features, dtype=torch.float32).unsqueeze(0)
                delta = self.ml_model(x).item()
            return max(0.0, min(20.0, base_risk + delta))
        except Exception as e:
            self.log(f"[ML] Error during ML risk refinement: {e}")
            return base_risk
    ...
    def _compute_risk_score(self, node: NodeInfo) -> float:
        ...
        risk += signature_anomaly * 1.2
        risk += cluster_anomaly * 1.0

        trust_factor = 1.5 - trust_score
        risk *= trust_factor

        risk *= (0.5 + self.paranoia_level)
        risk *= (0.5 + self.current_strategy.enforcement_aggressiveness)
        ...
        risk = self._ml_refine_risk(features, risk)
        return risk
    ...
    def _record_action(self, node: NodeInfo, risk: float, action: str, signals: Dict):
        identity = self._get_identity_for_node(node)
        name = node.extra.get("name", node.label)
    ...
    def _ingest_snapshot(self, snapshot: SystemSnapshot):
        ...
        for node in hot_nodes:
            ...
            self.log(msg)
            self._record_action(node, risk, action, signals)
...
class HiveGUI:
    ...
    def _build_admin_tab(self):
        ...
        self.chk_ml = tk.Checkbutton(
            self.admin_controls_frame,
            text="Enable ML-enhanced risk (requires PyTorch)",
        ...
        # Network & swarm settings
        settings_frame = ttk.Frame(self.admin_controls_frame)
        settings_frame.pack(fill=tk.X, pady=6)

        ttk.Button(settings_frame, text="Network Backup Path", command=self._on_set_network_backup, state=tk.DISABLED).pack(side=tk.LEFT, padx=4)
        ttk.Button(settings_frame, text="Swarm Endpoint", command=self._on_set_swarm, state=tk.DISABLED).pack(side=tk.LEFT, padx=4)
        ttk.Button(settings_frame, text="Change Admin Password", command=self._on_change_password, state=tk.DISABLED).pack(side=tk.LEFT, padx=4)

        # Save & shutdown
        bottom_frame = ttk.Frame(self.admin_controls_frame)
        bottom_frame.pack(fill=tk.X, pady=8)

        ttk.Button(bottom_frame, text="Save Brain Now", command=self._on_save, state=tk.DISABLED).pack(side=tk.LEFT, padx=4)
        ttk.Button(bottom_frame, text="Shutdown Queen (exit program)", command=self._on_shutdown, state=tk.DISABLED).pack(side=tk.RIGHT, padx=4)

        self.admin_controls_widgets = [
            self.chk_enforce,
            self.paranoia_scale,
            self.chk_ml,
            *settings_frame.winfo_children(),
    ...
    def _update_admin_status_text(self):
        ...
        lines = [
            ...
            f"ML-enhanced risk: {'ENABLED' if self.queen.ml_enabled else 'DISABLED'}",
        ]
    ...
    def _update_overview(self):
        ...
        lines.append(f"ML risk: {'ENABLED' if self.queen.ml_enabled else 'DISABLED'}")
        lines.append("")
