...
# Try to import real ML clustering; fall back gracefully if unavailable
try:
...
# ============================================================
# ROUTE HISTORY, ANOMALY, VOLATILITY & FINGERPRINTS + ML CLUSTERING
# ============================================================
class RouteHistory:
    def __init__(self):
        ...
        self.last_ml_cluster_time: float = 0.0
        self.ml_cluster_interval: float = 30.0  # seconds
    ...
    def add_sample(self, status: RouteStatus):
        ...
        # Periodically run ML clustering on fingerprints
        now = time.time()
        if now - self.last_ml_cluster_time > self.ml_cluster_interval:
            self._run_ml_clustering()
            self.last_ml_cluster_time = now
    ...
    def _run_ml_clustering(self):
        """
        Real ML clustering on anomaly fingerprints:
        - If sklearn is available, use KMeans and DBSCAN.
        - Otherwise, fall back to simple bucket clustering.
        """
        clusters_result: Dict[str, List[Dict[str, Any]]] = {}
        for route_name, fps in self.anomaly_fingerprints.items():
            if len(fps) < 5:
                # Not enough data, fallback to simple buckets
                clusters_result[route_name] = self._simple_bucket_clusters(fps)
                continue
            ...
            # Build feature matrix: [latency, loss, congestion, anomaly]
            X = [
                [fp["latency_ms"], fp["loss"], fp["congestion"], fp["anomaly"]]
                for fp in fps
            ]

            # KMeans for coarse structure
            try:
                k = min(4, max(2, len(X) // 10))
                kmeans = KMeans(n_clusters=k, n_init=5, random_state=42)
                labels_k = kmeans.fit_predict(X)
            except Exception:
                labels_k = [0] * len(X)

            # DBSCAN for dense anomaly regions
            try:
            ...
            combined_clusters: Dict[str, int] = {}
            for idx, fp in enumerate(fps):
                lk = labels_k[idx]
                ld = labels_d[idx]
                key = f"k{lk}_d{ld}"
                combined_clusters[key] = combined_clusters.get(key, 0) + 1

            clusters_result[route_name] = [
                {"cluster_id": cid, "count": cnt} for cid, cnt in combined_clusters.items()
            ]

        self.anomaly_clusters = clusters_result

    def _simple_bucket_clusters(self, fps: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        buckets = {"low": 0, "medium": 0, "high": 0}
        for fp in fps:
            score = fp["latency_ms"] * 0.4 + fp["loss"] * 0.3 + fp["congestion"] * 0.3
            if score < 40:
                buckets["low"] += 1
            elif score < 70:
                buckets["medium"] += 1
...
# ============================================================
# QUEEN BRAIN, DECISION LOG, STANCE & RL (WITH PERSISTENCE)
# ============================================================
class QueenDecisionLog:
    def __init__(self):
        self.entries: List[Any] = []

    def record_decision(self, chosen: RouteStatus, all_routes: Dict[str, RouteStatus], explanation: str):
        self.entries.append(
            DecisionEntry(
                timestamp=time.time(),
...
class RLMemoryManager:
    def __init__(self):
        ...
        self.path = os.path.join(base_dir, "rl_memory.json")

    def load(self) -> Dict[str, RouteArm]:
        if not os.path.exists(self.path):
            return {}
        try:
            with open(self.path, "r") as f:
                raw = json.load(f)
            arms: Dict[str, RouteArm] = {}
            for name, data in raw.get("arms", {}).items():
                arms[name] = RouteArm(
    ...
    def save(self, arms: Dict[str, RouteArm]):
        data = {
            "arms": {
...
class QueenBrain:
    def __init__(self):
        ...
        self.rl_memory = RLMemoryManager()
        self._load_rl_memory()

    def _load_rl_memory(self):
        self.arms = self.rl_memory.load()

    def save_rl_memory(self):
        self.rl_memory.save(self.arms)
...
# ============================================================
# MULTI-ROUTE PARETO SCORING + VOLATILITY-WEIGHTED RL
# ============================================================
def route_metrics(
    route: RouteStatus,
    queen: QueenBrain,
    ...
    """
    Multi-dimensional metrics for Pareto optimization:
    (latency, loss, congestion, risk, volatility, -rl_score)
    Lower is better on all except rl_score (we invert it).
    """
    hist = queen.history.get_history(route.name)
    lat_vals = [s.latency_ms for s in hist] if hist else [route.latency_ms]
    loss_vals = [s.loss for s in hist] if hist else [route.loss]
    cong_vals = [s.congestion for s in hist] if hist else [route.congestion]
    ...
    if projections and route.name in projections:
        risk = projections[route.name].risk_score

    arm = queen.arms.get(route.name, RouteArm())
    base_rl = bandit_score(arm)
    # Volatility-weighted RL: penalize high volatility
    rl_effective = base_rl * (1.0 - 0.7 * vol)
    rl_effective = max(0.0, rl_effective)
...
def is_dominated(a: Tuple[float, ...], b: Tuple[float, ...]) -> bool:
    """
    Returns True if vector a is dominated by vector b (b is <= a in all dims and < in at least one).
    """
    assert len(a) == len(b)
    less_or_equal_all = all(bi <= ai for ai, bi in zip(a, b))
    strictly_less_any = any(bi < ai for ai, bi in zip(a, b))
    return less_or_equal_all and strictly_less_any
...
def pareto_front(
    ...
    queen: QueenBrain,
    projections: Optional[Dict[str, RouteThreatProjection]] = None,
) -> List[Tuple[RouteStatus, Tuple[float, ...]]]:
    scored = [(r, route_metrics(r, queen, projections)) for r in routes]
    front: List[Tuple[RouteStatus, Tuple[float, ...]]] = []
    for r, m in scored:
        dominated = False
        for _, m2 in scored:
            if m is m2:
                continue
            if is_dominated(m, m2):
                dominated = True
                break
        if not dominated:
            front.append((r, m))
...
def choose_best_route_with_stance(
    ...
    # From Pareto front, pick the one with lowest latency as tie-breaker
    front.sort(key=lambda x: x[1][0])
    best_route, _ = front[0]
    explanation = (
        f"Chosen via Pareto front: latency={front[0][1][0]:.1f}, "
        f"loss={front[0][1][1]:.1f}, cong={front[0][1][2]:.1f}, "
        f"risk={front[0][1][3]:.1f}, vol={front[0][1][4]:.1f}, "
        f"rl_score={-front[0][1][5]:.1f}"
    )
    queen.log.record_decision(best_route, route_map, explanation)
    return best_route
...
def update_reinforcement(queen: QueenBrain, mission: BackupMission):
    ...
    if mission.success:
        ...
        arm.failures += 1
    queen.save_rl_memory()


# ============================================================
# PREEMPTIVE REROUTE BRAIN (COOLDOWN + STANCE THRESHOLDS + PARETO)
# ============================================================
    ...
    def maybe_preemptive_reroute(
        ...
        for name, status in snapshot.items():
            ...
            if p.risk_score < cur_proj.risk_score and p.projected_color_30s in (
                ...
                metrics = route_metrics(status, self.queen, projections)
                safer_candidates.append((metrics, status, p))

        if not safer_candidates:
            return None

        # Pareto front among safer candidates
        routes_only = [c[1] for c in safer_candidates]
        front = pareto_front(routes_only, self.queen, projections)
        if not front:
            return None
        front.sort(key=lambda x: x[1][0])
        new_status, new_metrics = front[0]
        new_proj = projections[new_status.name]

        self.last_reroute_time = now
        reason = (
            f"Preemptive reroute via Pareto front due to high risk on {current_best.name} "
            f"(risk={cur_proj.risk_score:.2f}, anomaly={cur_proj.anomaly:.2f}, "
            f"stance={self.queen.stance.value}) -> "
            f"{new_status.name} [lat={new_metrics[0]:.1f}, loss={new_metrics[1]:.1f}, "
            f"cong={new_metrics[2]:.1f}, risk={new_metrics[3]:.1f}, vol={new_metrics[4]:.1f}]"
        )
...
class ExportImportPanel(QWidget):
    ...
    def _export_state(self):
        ...
        data = {
            ...
            "rl_arms": {
                name: {"successes": arm.successes, "failures": arm.failures}
                for name, arm in self.queen.arms.items()
            },
        }
        try:
            with JSON_LOCK:
    ...
    def _import_state(self):
        ...
        try:
            ...
            rl_arms_data = data.get("rl_arms", {})
            for name, rd in rl_arms_data.items():
                self.queen.arms[name] = RouteArm(
                    successes=rd.get("successes", 0),
                    failures=rd.get("failures", 0),
                )
            self.queen.save_rl_memory()
