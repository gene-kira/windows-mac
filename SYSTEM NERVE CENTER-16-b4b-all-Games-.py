#!/usr/bin/env python3
# ============================================================
# SYSTEM NERVE CENTER – HybridBrain + Tri-Stance + MultiGame v6.0 "Event Horizon + Glitch"
# - Event Horizon prediction + Horizon patterns + clustering
# - Glitch detection (drift + integrity + horizon + risk)
# - Glitch memory + Glitch trace + GUI tab
# - Glitch affects stance, meta-state, restoration
# - Multi-game + Game AI ↔ System AI conversation + organs
# ============================================================
...
class HybridBrain:
    ...
    horizon_cluster: str = "UNKNOWN"
    horizon_cluster_conf: float = 0.0
    # Glitch fields
    glitch_active: bool = False
    glitch_score: float = 0.0
    glitch_reason: str = ""
    glitch_trace: List[GlitchEvent] = field(default_factory=list)
...
GAME_PROFILES: Dict[str, Dict[str, float]] = {
    "b4b": {"baseline_risk": 0.3},
    "worldwarz": {"baseline_risk": 0.35},
    "other_game": {"baseline_risk": 0.35},
    "generic": {"baseline_risk": 0.3},
}
...
class PredictionEngine:
    ...
    def _classify_horizon_cluster(self, horizon_vote: float) -> Tuple[str, float]:
        if horizon_vote < 0.25:
            return "SAFE", 1.0 - horizon_vote * 4.0
        if horizon_vote < 0.5:
            return "ALERT", 1.0 - abs(horizon_vote - 0.4) * 5.0
        if horizon_vote < 0.75:
            return "DANGER", (horizon_vote - 0.5) * 4.0
        return "CRITICAL", min(1.0, (horizon_vote - 0.75) * 4.0 + 0.5)
    ...
    def _engine_votes(self) -> Tuple[float, Dict[str, float], str, Dict[str, float]]:
        ...
        cluster, conf = self._classify_horizon_cluster(votes["event_horizon"])
        self.brain.horizon_cluster = cluster
        self.brain.horizon_cluster_conf = conf
    ...
    def _update_meta_state_momentum(self, regime: str):
        ...
        cluster = self.brain.horizon_cluster
        glitch = self.brain.glitch_active

        target = self.brain.meta_state.name

        if glitch or cluster in ("DANGER", "CRITICAL") or risk > 0.8 or integrity < 0.6:
            target = "Recovery-Flow"
        elif regime in ("rising", "stable") and risk < 0.4 and integrity > 0.8 and alignment > 0.6 and cluster in ("SAFE", "ALERT"):
            target = "Hyper-Flow"
        elif regime == "cooling" and risk < 0.3 and cluster == "SAFE":
            target = "Sentinel"
        else:
            target = self.brain.meta_state.name
    ...
    def _update_glitch_state(self, turbulence: float):
        drift = self.brain.integrity.prediction_drift
        integrity = self.brain.integrity.integrity_score
        risk = self.brain.cortex.risk
        cluster = self.brain.horizon_cluster
        ...
        score = 0.0
        score += clamp01((drift - 0.3) / 0.5)
        score += clamp01((0.6 - integrity) / 0.6)
        score += clamp01((risk - 0.5) / 0.5)
        score += clamp01(turbulence)
        ...
        reason_parts = []
        if drift > 0.4:
            reason_parts.append("high drift")
        if integrity < 0.6:
            reason_parts.append("low integrity")
        if risk > 0.7:
            reason_parts.append("high risk")
        if cluster == "CRITICAL":
            reason_parts.append("critical horizon")
        if turbulence > 0.6:
            reason_parts.append("turbulent")

        reason = ", ".join(reason_parts) if reason_parts else "none"

        glitch_now = (score > 0.55 and cluster in ("DANGER", "CRITICAL"))
        if glitch_now and not self.brain.glitch_active:
            ge = GlitchEvent(
                ts=utc_ts(),
                risk=risk,
                drift=drift,
                integrity=integrity,
                horizon_cluster=cluster,
                reason=reason,
            )
            self.brain.glitch_trace.append(ge)
            if len(self.brain.glitch_trace) > 50:
                self.brain.glitch_trace.pop(0)

        self.brain.glitch_active = glitch_now
        self.brain.glitch_score = score
        self.brain.glitch_reason = reason
    ...
    def update_forecast(self):
        ...
        cluster = self.brain.horizon_cluster
        if overload_p > 0.7 or cluster in ("DANGER", "CRITICAL"):
            self.brain.cortex.environment = "DANGER"
            self.brain.cortex.anticipation = f"Imminent overload ({cluster})"
        elif beast_p > 0.6 and self.brain.cortex.risk < 0.5 and cluster in ("SAFE", "ALERT"):
        ...
        self._update_glitch_state(turbulence)
...
class TriStanceEngine:
    ...
    def tick(self):
        ...
        stance = self.brain.stance
        cluster = self.brain.horizon_cluster
        cluster_conf = self.brain.horizon_cluster_conf
        glitch = self.brain.glitch_active
        glitch_score = self.brain.glitch_score

        new = stance

        if glitch and glitch_score > 0.6:
            new = "Conservative"
        elif cluster in ("DANGER", "CRITICAL") and cluster_conf > 0.6:
            new = "Conservative"
        elif risk > 0.8 or integrity < 0.6 or regime == "chaotic":
            new = "Conservative"
        elif cluster in ("SAFE", "ALERT") and cluster_conf > 0.6 and risk < 0.4 and integrity > 0.8:
            new = "Beast"
...
class RestorationProtocol:
    ...
    def tick(self):
        if self.brain.glitch_active and not self.active and self.brain.glitch_score > 0.7:
            self.engage()
        ...
        if self.brain.cortex.risk < 0.3 and self.brain.integrity.integrity_score > 0.8 and not self.brain.glitch_active:
            self.active = False
...
# ==================== PART 2: GUI, TABS, MEMORY ====================

class NerveCenterApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("System Nerve Center – Event Horizon + Glitch")
    ...
    def _build_gui(self):
        ...
        self.lbl_top_status = tk.Label(
            top,
            text="Nerve Center Online (Event Horizon + Glitch)",
            fg="#00ff99",
            bg="#111111",
        ...
        self.tab_glitch = ttk.Frame(notebook)
        ...
        notebook.add(self.tab_glitch, text="Glitch Trace")
        ...
        self._build_glitch_tab(self.tab_glitch)
    ...
    def _build_cortex_tab(self, parent):
        ...
        self.lbl_horizon = ttk.Label(frame_top, text="Horizon: UNKNOWN (0.00)")
        self.lbl_horizon.grid(row=3, column=0, sticky="w", padx=5, pady=2)

        self.lbl_glitch = ttk.Label(frame_top, text="Glitch: none (0.00) reason: -")
        self.lbl_glitch.grid(row=3, column=1, sticky="w", padx=5, pady=2)

        frame_chart = ttk.LabelFrame(parent, text="Prediction / Glitch Micro-chart")
        frame_chart.pack(fill="both", expand=True, padx=10, pady=10)
        self.canvas_chart = tk.Canvas(frame_chart, bg="#111111", height=220)
        self.canvas_chart.pack(fill="both", expand=True, padx=5, pady=5)
    ...
    def _build_glitch_tab(self, tab):
        frame = ttk.LabelFrame(tab, text="Glitch History")
        frame.pack(fill="both", expand=True, padx=10, pady=10)

        self.txt_glitch = tk.Text(
            frame,
            bg="#050505",
            fg="#d0d0d0",
            font=("Consolas", 9)
        )
        self.txt_glitch.pack(fill="both", expand=True, padx=5, pady=5)

        ttk.Label(
            frame,
            text="Glitches are triggered by high drift, low integrity, high risk, and critical horizon clusters."
        ).pack(anchor="w", padx=5, pady=5)
    ...
    def cmd_save_reboot_memory(self):
        ...
        meta = {"ts": time.time(), "version": "6.0-event-horizon-glitch"}
    ...
    def _save_settings(self):
        ...
        path = os.path.join(os.path.expanduser("~"), ".nerve_center_settings_event_horizon_glitch.json")
        try:
...
# ==================== PART 3: LOOP, REFRESH, MAIN ====================
    ...
    def _schedule_gui_refresh(self):
        self._refresh_gui()
        self.after(300, self._schedule_gui_refresh)
    ...
    def _refresh_gui(self):
        ...
        self.lbl_horizon.config(
            text=f"Horizon: {b.horizon_cluster} ({b.horizon_cluster_conf:.2f})"
        )

        glitch_text = f"Glitch: {'ACTIVE' if b.glitch_active else 'none'} ({b.glitch_score:.2f}) reason: {b.glitch_reason or '-'}"
        color = "#ff4444" if b.glitch_active else "#00ff99"
        self.lbl_glitch.config(text=glitch_text, foreground=color)

        top_color = "#00ff99"
        top_text = "Nerve Center Online (Event Horizon + Glitch)"
        if self.restoration.active:
            top_color = "#ffdd55"
            top_text = "Serve Again Mode: Restoration Active"
        elif b.glitch_active:
            top_color = "#ff4444"
            top_text = "⚠ GLITCH STATE – unstable prediction zone"
        elif b.cortex.risk > 0.8:
            top_color = "#ff4444"
        elif b.cortex.risk > 0.5:
            top_color = "#ffaa00"
        self.lbl_top_status.config(fg=top_color, text=top_text)
    ...
    def _draw_micro_chart(self):
        ...
        if self.brain.glitch_active:
            canvas.create_rectangle(0, 0, w, h, fill="#330000", outline="")
        else:
        ...
        y_base = h - int(baseline_val * (h - 40)) - 20
        ...
        def bar(x, value, color, label):
            bar_h = int(value * (h - 60))
            canvas.create_rectangle(x, h - bar_h - 30, x + 40, h - 30, fill=color, outline=color)
            canvas.create_text(x + 20, h - 20, text=label, fill="#ffffff", font=("TkDefaultFont", 8))
            canvas.create_text(x + 20, h - bar_h - 40, text=f"{value:.2f}", fill="#ffffff",
                               font=("TkDefaultFont", 8))

        bar(50, risk, "#ff8800", "risk")
        bar(120, drift, "#ff00ff", "drift")
        bar(190, integrity, "#00ff99", "integ")
        bar(260, glitch_score, "#ff4444", "glitch")

        canvas.create_text(
            w - 10, 10,
            text=f"Horizon: {self.brain.horizon_cluster}",
            anchor="ne",
            fill="#ffffff",
    ...
    def _update_glitch_panel(self):
        self.txt_glitch.delete("1.0", "end")
        trace = self.brain.glitch_trace
        if not trace:
            self.txt_glitch.insert("end", "No glitches recorded yet.\n")
            return

        self.txt_glitch.insert("end", "Glitch Events (latest first):\n\n")
        for ge in reversed(trace[-20:]):
            ts_str = time.strftime("%H:%M:%S", time.localtime(ge.ts))
            self.txt_glitch.insert(
                "end",
                f"[{ts_str}] GLITCH – "
                f"risk={ge.risk:.2f}, drift={ge.drift:.2f}, "
                f"integrity={ge.integrity:.2f}, horizon={ge.horizon_cluster}, "
                f"reason={ge.reason}\n"
            )
