...
- CLI commands: switch | mutate | escalate | cipher on | cipher off | quit
...
yaml = optional_import("yaml")                    # codex sync (optional)
...
class CrossOSCollectors:
    ...
    def monitor_processes(self):
        if not psutil:
            ...
            return
        SignalThreat = self.bridge.Signal("threat", self.bridge.ui)
        SignalIngest = self.bridge.Signal("ingest", self.bridge.ui)
        while self.running:
            try:
    ...
    def monitor_network(self):
        if not psutil:
            ...
            return
        SignalSync = self.bridge.Signal("sync", self.bridge.ui)
        while self.running:
            try:
                for conn in psutil.net_connections(kind='inet'):
                    if conn.status == 'ESTABLISHED' and conn.raddr:
                        ...
                        SignalSync.emit(f"{EMBLEMS['globe']} Outbound: {masked}")
        ...
        class Handler(watchdog_events.FileSystemEventHandler):
            def on_modified(_, event):
                if not event.is_directory:
                    ...
                    SignalIngest.emit(f"{EMBLEMS['lock']} File modified: {self.ace.anonymize_event(e)['src_path']}")
...
class Orchestrator:
    def __init__(self, use_ui=True):
        ...
        self.bot = DualPersonalityBot(lambda m: self.bridge.Signal("ingest", self.ui).emit(m), self.codex, self.trust, self.ace)
    ...
    def start(self):
        ...
        self.bridge.Signal("ingest", self.ui).emit(
            f"{EMBLEMS['dna']} {EMBLEMS['monitor']} System online | OS={CAPS['os']} | GPU={CAPS['gpu']} | UI={self.ui is not None}"
        )
        self.bridge.Signal("ingest", self.ui).emit(
    ...
    def _governance_pulse(self):
        SignalSync = self.bridge.Signal("sync", self.ui)
        while self.running:
            try:
                manifest = self.codex.export_manifest()
                SignalSync.emit(f"{EMBLEMS['trident']} Codex manifest: {manifest}")
                time.sleep(30)
            except Exception as e:
    ...
    def mutate_again(self):
        # Trigger codex mutation with a ghost sync hint and obfuscation spike
        self.codex.mutate({"ghost_sync": True, "obfuscation_spike": True})
        self.bridge.Signal("ingest", self.ui).emit(f"{EMBLEMS['dna']} mutate again — Codex v{self.codex.version}")

    def escalate_defense(self):
        self.codex.mutate({"escalate_defense": True})
        self.bridge.Signal("threat", self.ui).emit(f"{EMBLEMS['alchemical']} escalate defense — strict mode enabled")

    def cipher_toggle(self, on: bool):
        self.ace.toggle(on)
        state = "ON" if on else "OFF"
        self.bridge.Signal("ingest", self.ui).emit(f"{EMBLEMS['mask']} Autonomous Cipher Engine {state}")
