...
- Surfaces transparent reasoning through a GUI
...
class TelemetryCollector:
    ...
    def _gpu_snapshot(self) -> Dict[str, Any]:
        gpu = {"vendor": None, "util": None, "vram_total_mb": None, "vram_free_mb": None}
        if NVML_AVAILABLE:
            try:
                pynvml.nvmlInit()
            ...
            try:
                ...
                if devs:
                    ...
                    gpu.update({
                        "vendor": "AMD",
                        "util": None,
                        "vram_total_mb": dev.get("vram_total"),
                        "vram_free_mb": None
                    })
            except Exception:
                pass
...
# ---------------------------
# Capability profiler (real metrics → scores)
# ---------------------------
...
class CapabilityProfiler:
    def __init__(self, logger: Logger, telemetry: TelemetryCollector):
        ...
        self.profile = CapabilityProfile(
            scores={"cpu": 0.7, "gpu": 0.6, "mem": 0.7, "disk": 0.8, "net": 0.6, "power": 0.6},
    ...
    def run_probes(self):
        ...
        gpu_score = 0.5 if snap["gpu"]["vendor"] is None else 0.7
        ...
        self.logger.log("capability", "Profile updated", {"profile": asdict(self.profile)})
...
# ---------------------------
# Port & device scanner (connections + USB)
# ---------------------------
...
# ---------------------------
# Network scanner (LAN recognition)
# ---------------------------
...
class Planner:
    ...
    def synthesize(self, intents: List[Intent]) -> Plan:
        ...
        if top.id == "Open Email":
            ...
            steps.append(PlanStep("Preload Steam Runtime", deps=["Steam.exe"], cost={"cpu": 0.18, "disk": 180e6, "mem": 512.0}, expected_gain_ms=1400))
...
class AutoLoader:
    def __init__(self, logger: Logger, dep_graph: DependencyGraph, capability: CapabilityProfiler):
        self.logger = logger
        self.dep_graph = dep_graph
        self.capability = capability

    def _strategy(self, plan: Plan) -> str:
        prof = self.capability.get_profile()
        disk = prof.scores.get("disk", 0.8)
        mem = prof.scores.get("mem", 0.7)
        gpu = prof.scores.get("gpu", 0.7)
        if plan.total_cost.get("gpu_vram", 0.0) > 0 and gpu > 0.65:
    ...
    def execute(self, plan: Plan) -> Result:
        ...
        return Result(success=True, resources=resources, verified_hashes=verified, notes=f"Preload via {strategy}")
...
class Orchestrator:
    ...
    def _loop(self, interval_sec: float):
        while not self.stop_flag:
            ...
            if decision.allow:
                result = self.autoloader.execute(plan)
                success = True
...
# ---------------------------
# GUI manager (transparent reasoning + persona/cooldown/accuracy)
# ---------------------------
...
class GUIManager:
    def __init__(self, predictor: Predictor, telemetry: TelemetryCollector,
        ...
        if not TK_AVAILABLE:
            raise RuntimeError("GUI environment not available")
        self.predictor = predictor
        self.telemetry = telemetry
...
def build_system(project_root: str = ".", iface_cidr: Optional[str] = None) -> Dict[str, Any]:
    ...
    autoloader = AutoLoader(logger, dep_graph, capability)
...
def main(cli_mode: bool = False, iface_cidr: Optional[str] = None):
    ...
    if not cli_mode and TK_AVAILABLE:
        ...
        logger.log("system", "Running Fortune Teller (autonomous CLI)")
        try:
            for _ in range(5):
                time.sleep(3.0)
...
if __name__ == "__main__":
    ...
    # Set iface_cidr to your LAN (e.g., "192.168.1.0/24") to enable ARP scanning
    main(cli_mode=False, iface_cidr=None)
