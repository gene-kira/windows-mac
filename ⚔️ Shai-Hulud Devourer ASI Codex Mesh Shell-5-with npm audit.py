...
import sys, os, time, threading, subprocess, hashlib, platform, random
...
SWARM_AGENTS = [{"id": f"agent_{i}", "signal": random.uniform(0.8, 1.2)} for i in range(8)]
...
def quarantine_npm():
    try:
        ...
        if result.returncode != 0: log("[NPM] Audit failed"); return
        import json
        audit = json.loads(result.stdout)
        for pkg in audit.get("advisories", {}).values():
            name = pkg.get("module_name")
            log(f"[QUARANTINE] {name} flagged: {pkg.get('title')}")
            schedule(encrypt(name), 60, f"NPM:{name}")
    except Exception as e:
        log(f"[ERROR] NPM quarantine failed: {e}")

def detect_anomalies():
    avg = sum(a["signal"] for a in SWARM_AGENTS) / len(SWARM_AGENTS)
    for agent in SWARM_AGENTS:
        deviation = abs(agent["signal"] - avg)
        if deviation > 0.3:
            log(f"[ANOMALY] {agent['id']} deviated ({deviation:.2f}) 🔥")
            schedule(encrypt(agent["id"]), 45, f"Anomaly:{agent['id']}")
        else:
            log(f"[SYNC] {agent['id']} aligned 🧬")
...
class Guardian(QWidget):
    def __init__(self):
        ...
        for text, func in [
            ...
            ("Simulate Biometrics", simulate),
            ("🔍 Scanning for Shai-Hulud", self.scan_update),
            ("🧪 Quarantine NPM", quarantine_npm),
            ("🧿 Swarm Anomaly Scan", detect_anomalies)
        ]:
