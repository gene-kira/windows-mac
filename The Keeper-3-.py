...
MANUAL WHITELIST/BLACKLIST OVERRIDES, AND EXTRA LOGGING
...
- Extra logging when non-whitelisted binaries touch vault or trigger suspicion
...
class MemoryStore:
    ...
    def __init__(self, system_os_family: str):
        ...
        self.data = {
            ...
            "baseline_data": {},
            "trust_registry": {},  # exe_path_normcased: {"trusted": bool, "reason": str, "source": "whitelist"/"blacklist"/"auto"}
        }
        self._load()
...
class GuardianCore:
    def __init__(self, system_profile: SystemProfile, event_bus: EventBus, memory_store: MemoryStore):
        ...
        # Trust registry with whitelist / blacklist info
        # exe_norm: { "trusted": bool, "reason": str, "source": "whitelist"/"blacklist"/"auto" }
        ...
        if self.system.os_family == "windows":
            ...
            # You can extend this manually later.
    ...
    # ------------ Whitelist/blacklist management (manual override hooks) ------------
    ...
    # ------------ Zero-trust process trust model (whitelist/blacklist + Windows-specific logic) ------------
    ...
    def is_process_trusted(self, proc_info):
        ...
        3. Windows system paths (C:\Windows, C:\Windows\System32) -> default untrusted
        ...
        # Not in explicit registry yet
        if self.system.os_family == "windows":
            for root in self.win_system_roots:
                try:
                    if exe_norm.startswith(root.lower()):
                        # Windows system path but not whitelisted -> still untrusted by default
            ...
            # Extend with explicit Linux whitelists later
    ...
    # ------------ Policy hooks (zero-tolerance, extra logging) ------------
    ...
    def on_sensitive_access_attempt(self, process_info, data_category):
        ...
        - Extra logging for non-whitelisted binaries.
        """
        ...
        if not trusted:
            ...
            # Extra logging for non-whitelisted touching vault
...
class NetworkGatekeeper(BaseOrgan):
    def run_loop(self):
        ...
        while self.running:
            try:
                # For now, we don't have per-process net events,
                # but trust model is ready to apply once we inspect sockets.
    ...
    def process_events(self):
        while True:
            ...
            if etype == "mode_change":
                ...
                line = f"[SensitiveAccess] proc={proc.get('name')} pid={proc.get('pid')} exe={proc.get('exe')} category={cat} decision={decision} reason={reason} trust={trust_reason}\n"
                ...
                cat = payload.get("data_category")
                trust_reason = payload.get("trust_reason")
                line = f"[NonWhitelistSensitiveTouch] proc={proc.get('name')} pid={proc.get('pid')} exe={proc.get('exe')} category={cat} trust={trust_reason}\n"
