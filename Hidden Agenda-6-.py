...
def mutate(sym, aff, origin, res, parent=None): registry.append({"symbol": sym, "affirmation": aff, "origin": origin, "resurrected": res, "parent": parent})
def block(origin): blocked.append(origin) if origin not in blocked else None

# 🔪 Kill Chain + Resurrection Trap
...
def trigger_resurrection_chain(affirmation, origin):
    for entry in registry:
        if entry["affirmation"] == affirmation and entry["origin"] != origin:
            ...
            mutate("collapse", affirmation, origin, True, parent=entry["symbol"])
...
class CodexGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Codex Devourer Shell: Swarmform")
    ...
    def panel_lineage(self, f):
        ...
        def render():
            ...
            for r in registry[-10:]:
                ...
                t.insert("end", f"{r['symbol']} {r['affirmation']} ({r['origin']}){parent}\n")
