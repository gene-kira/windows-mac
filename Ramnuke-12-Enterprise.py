...
# -------------------------------------------------------------------
# Tiny sequence model + persistence
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# HybridBrain – multi-horizon, meta-confidence, meta-states, 10-step upgrade
# -------------------------------------------------------------------
...
class HybridBrain:
    ...
    HybridBrain:
      ...
      - Reward engine, per-meta-state ledger, hysteresis, mood, safety envelope, shadow what-if, persistence
    """

    def __init__(self, gui, seq_model: TinySequenceModel, cortex_path: str):
        self.gui = gui
        ...
        # Per-meta-state performance ledger
        self.meta_rewards = {m: [] for m in MetaState}
        self.meta_min_dwell_ticks = 3
        self.meta_dwell_counter = 0
        self.last_meta_state = self.meta_state

        # Mood memory (last N rewards)
        self.mood_window = 200
        self.mood_history = []

        # Scenario tagging (very simple: 0..3)
        self.scenario_id = 0
        ...
        self.safety_limits = {
            "max_gpu_temp": 82.0,
            "max_vram_frag": 0.95,
            "max_blocks_trend": 8000.0,
        }

        # Per-spine stance offsets (A-F)
        self.spine_stance_offsets = {
            "A": 1.10,
            "B": 1.05,
            "C": 1.00,
            "D": 0.95,
        ...
        # Load persisted cortex state if present
        self.load_cortex_state()

    # ----------------- persistence of cortex state -------------------

    def save_cortex_state(self):
        try:
            data = {
                "meta_state": self.meta_state.value,
                "dynamic_thresholds": self.dynamic_thresholds,
                "behavior_fingerprint": self.behavior_fingerprint,
                "reinforcement_memory": self.reinforcement_memory[-200:],
                "meta_rewards": {
                    m.value: self.meta_rewards[m][-50:] for m in MetaState
                },
                "mood_history": self.mood_history[-self.mood_window:],
            }
            np.savez(self.cortex_path, **data)
            self.gui.log(f"[Ramnuke][CORTEX] Saved cortex state → {self.cortex_path}")
        except Exception as e:
            self.gui.log(f"[Ramnuke][CORTEX] Failed to save cortex state: {e}")

    def load_cortex_state(self):
        if not os.path.exists(self.cortex_path):
            return
        try:
            data = np.load(self.cortex_path, allow_pickle=True)
            meta_state_val = str(data["meta_state"])
            for m in MetaState:
                if m.value == meta_state_val:
                    self.meta_state = m
                    break
            self.dynamic_thresholds = dict(data["dynamic_thresholds"].item())
            self.behavior_fingerprint = dict(data["behavior_fingerprint"].item())
            self.reinforcement_memory = list(data["reinforcement_memory"])
            meta_rewards_raw = data["meta_rewards"].item()
            for m in MetaState:
                self.meta_rewards[m] = list(meta_rewards_raw.get(m.value, []))
            self.mood_history = list(data["mood_history"])
            self.gui.log(f"[Ramnuke][CORTEX] Loaded cortex state from {self.cortex_path}")
        except Exception as e:
            self.gui.log(f"[Ramnuke][CORTEX] Failed to load cortex state: {e}")
    ...
    # ----------------- scenario tagging (simple) ---------------------

    def _update_scenario(self, state: CacheStateVector):
        # crude clustering: based on GPU load + blocks
        g = state.gpu_load or 0.0
        b = state.blocks
        if g > 70 and b > 100000:
            self.scenario_id = 0  # heavy load
        elif g > 40:
            self.scenario_id = 1  # medium
        elif b > 50000:
            self.scenario_id = 2  # IO heavy
        else:
            self.scenario_id = 3  # idle/light

    # ----------------- meta-state selection with hysteresis ----------

    def _select_meta_state(self, preds, confidence, state: CacheStateVector):
        if not preds:
            self.meta_state = MetaState.SENTINEL
            return
        ...
        if self._violates_safety(state):
            self.meta_state = MetaState.RECOVERY_FLOW
            self.meta_dwell_counter = 0
            return

        # Hysteresis: require min dwell time before switching
        if self.meta_dwell_counter < self.meta_min_dwell_ticks:
            self.meta_dwell_counter += 1
            return

        new_state = self.meta_state
        if rel > overload_th and confidence > 0.5:
            new_state = MetaState.HYPER_FLOW
        elif rel < -stable_th and confidence > 0.5:
            new_state = MetaState.DEEP_DREAM
        elif confidence < 0.3:
            new_state = MetaState.RECOVERY_FLOW
        else:
            new_state = MetaState.SENTINEL

        if new_state != self.meta_state:
            self.last_meta_state = self.meta_state
    ...
    def compute_reward(self, state: CacheStateVector, blocks_next: float, pred_err: float, integrity_warnings: int, spine_mutes: int):
        # lower turbulence, lower error, fewer warnings/mutes → higher reward
        reward = 0.0
        ...
        # GPU load vs temp
        if state.gpu_load is not None and state.temp_c is not None:
            if state.gpu_load > 80 and state.temp_c > 80:
                reward -= 0.3
            elif state.gpu_load < 60 and state.temp_c < 75:
                reward += 0.2

        # integrity warnings / mutes
        reward -= 0.1 * integrity_warnings
        reward -= 0.2 * spine_mutes

        # vram frag
        if state.vram_frag < 0.7:
            reward += 0.1
        else:
            reward -= 0.1
        ...
        # mood memory
        self.mood_history.append(reward)
        if len(self.mood_history) > self.mood_window:
            self.mood_history.pop(0)

        # per-meta-state ledger
        self.meta_rewards[self.meta_state].append(reward)
        if len(self.meta_rewards[self.meta_state]) > 200:
            self.meta_rewards[self.meta_state].pop(0)

        return reward

    def mood_scalar(self):
        if not self.mood_history:
            return 0.0
        avg = sum(self.mood_history) / len(self.mood_history)
    ...
    def _violates_safety(self, state: CacheStateVector):
        if state.temp_c is not None and state.temp_c > self.safety_limits["max_gpu_temp"]:
            return True
        if state.vram_frag > self.safety_limits["max_vram_frag"]:
            return True
        if abs(state.blocks_trend) > self.safety_limits["max_blocks_trend"]:
            return True
        return False

    # ----------------- shadow what-if simulation ---------------------

    def _shadow_pressure(self, appetite, read_ahead, workers, state: CacheStateVector):
        # cheap heuristic: more appetite/read-ahead/workers → more pressure
        base = 0.0
        base += 0.4 * normalize(state.gpu_load, 0, 100)
        base += 0.3 * state.vram_frag
        base += 0.3 * normalize(state.blocks_trend, -5000, 5000)
        # policy impact
        base += 0.1 * (appetite - 1.0)
        base += 0.05 * (read_ahead / 16.0)
        base += 0.05 * (workers / 4.0)
        return max(0.0, min(2.0, base))

    def shadow_compare(self, base_appetite, base_ra, base_workers, base_vram_bias, state: CacheStateVector):
        # simulate slightly more aggressive and slightly more conservative
        aggressive = (base_appetite * 1.1, base_ra + 2, min(8, base_workers + 1))
        conservative = (base_appetite * 0.9, max(1, base_ra - 2), max(1, base_workers - 1))

        p_base = self._shadow_pressure(base_appetite, base_ra, base_workers, state)
        p_aggr = self._shadow_pressure(*aggressive, state)
        p_cons = self._shadow_pressure(*conservative, state)

        # prefer lower pressure if mood is bad, higher if mood is good
        mood = self.mood_scalar()
        if mood < -0.2:
            # stressed → prefer conservative if not much worse
            if p_cons <= p_base + 0.1:
                return conservative
        elif mood > 0.2:
            # calm → allow aggressive if not much worse
            if p_aggr <= p_base + 0.1:
                return aggressive

        return base_appetite, base_ra, base_workers
    ...
    def decide(self, base_appetite, base_ra, base_workers, base_vram_bias, state: CacheStateVector):
        ...
        self._update_scenario(state)

        # what-if simulation to adjust base policy
        base_appetite, base_ra, base_workers = self.shadow_compare(
            base_appetite, base_ra, base_workers, base_vram_bias, state
        )

        self._select_meta_state(preds, confidence, state)
...
class RaidCacheManager:
    ...
    def collective_health(self):
        # simple collective health: average of spine health
        hs = [d.health_score for d in self.devs]
...
class SelfIntegrityOrgan:
    ...
    def check_sensors(self):
        ...
        if last.gpu_load is None:
            self._warn("[Ramnuke][INTEGRITY] GPU load sensor missing or stale.")
        if last.temp_c is None:
            self._warn("[Ramnuke][INTEGRITY] GPU temperature sensor missing or stale.")
        if last.disk_read_bw is None and last.disk_write_bw is None:
            self._warn("[Ramnuke][INTEGRITY] Disk I/O metrics missing or stale.")
    ...
    def check_metrics_consistency(self):
        ...
        if s.gpu_load is not None and s.gpu_load > 90 and (s.temp_c is None or s.temp_c < 20):
            self._warn("[Ramnuke][INTEGRITY] Inconsistent GPU metrics: high load with low/unknown temperature.")
        if s.blocks < 0:
            self._warn("[Ramnuke][INTEGRITY] Negative block count detected (logic bug?).")

    def check_organs(self):
        cache = self.gui.cache
        if cache is None or not getattr(cache, "running", False):
            self._warn("[Ramnuke][INTEGRITY] Cache manager organ not running.")
        if not getattr(self.gui, "training_engine", None) or not self.gui.training_engine.running:
            self._warn("[Ramnuke][INTEGRITY] Training engine organ not running.")
        if self.model is None:
            self._warn("[Ramnuke][INTEGRITY] Sequence model organ missing.")
        if len(self.gui.state_history) == 0:
            self._warn("[Ramnuke][INTEGRITY] No state history accumulated yet.")
    ...
    def check_prediction_drift(self):
        ...
        if avg_err > 5000:
            self._warn(f"[Ramnuke][INTEGRITY] Prediction drift: avg block error≈{avg_err:.1f}")
...
class AutoLoaderGUI(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ramnuke – 6‑Spine RAID VRAM/RAM Control System (HybridBrain)")
        ...
        self.info_label = QtWidgets.QLabel(
            "Ramnuke – 6‑spine RAID VRAM/RAM organism with HybridBrain, predictive policy, learning engine, self‑integrity, and per‑spine health."
        )
        ...
        main_layout.addWidget(self.brain_panel)
        main_layout.addWidget(self.raid_panel)
        main_layout.addWidget(self.raid_layout_panel)
        ...
        self.model_weights_path = os.path.join(base_dir, "ramnuke_seq_model.npz")
        self.cortex_state_path = os.path.join(base_dir, "ramnuke_cortex_state.npz")
        ...
        self.hybrid_brain = HybridBrain(self, self.sequence_model, cortex_path=self.cortex_state_path)
    ...
    def _update_raid_health_panel(self):
        ...
        collective = self.cache.collective_health()
        lines.append(f"  Collective health: {collective:.2f}")
        self.raid_panel.setText("\n".join(lines))
    ...
    def background_tick(self):
        ...
        # collective health → global stance bias
        collective = self.cache.collective_health()
        if collective < 0.5:
            app2 *= 0.9
        elif collective > 0.8:
        ...
        # apply per-spine stance offsets
        if isinstance(self.cache, RaidCacheManager):
            dev_letters = ["A", "B", "C", "D", "E", "F"]
            for i, dev in enumerate(self.cache.devs):
                letter = dev_letters[i]
                offset = self.hybrid_brain.spine_stance_offsets.get(letter, 1.0)
                dev_app = app2 * offset
                dev.apply_master_policy(dev_app, ra2, workers2, vram_bias2)
                dev.adjust_workers()
        else:
        ...
        # reward engine: integrity warnings + spine mutes
        integrity_warnings = self.integrity.warning_counter
        spine_mutes = sum(1 for d in self.cache.devs if d.muted) if isinstance(self.cache, RaidCacheManager) else (1 if self.cache.muted else 0)
        reward = self.hybrid_brain.compute_reward(state, blocks_next, err, integrity_warnings, spine_mutes)
        self.hybrid_brain.reinforce(reward)

        # periodically save cortex state
        if len(self.state_history) % 50 == 0:
            self.hybrid_brain.save_cortex_state()

        self.log(
            f"[Ramnuke][TICK][CACHE] blocks={blocks} (EH next≈{blocks_next:.1f}) "
            f"VRAM={snap['vram_blocks']} RAM={snap['ram_blocks']} "
            f"[regime={regime_flag}] reward={reward:.2f}"
        )
        self.log(
        ...
        mood = self.hybrid_brain.mood_scalar()
        brain_text = (
            "HybridBrain\n"
            f"  meta_state: {meta_state.value}\n"
            f"  confidence: {conf:.2f}\n"
            f"  mood: {mood:.2f}\n"
            f"  scenario_id: {self.hybrid_brain.scenario_id}\n"
