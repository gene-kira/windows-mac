...
"""
Foresight: predictive assistant + forecaster with workflow chaining, anomaly detection, multi-horizon planning,
confidence calibration, encrypted persistence, intrusion detection, and safe auto-play — cross-platform Tk GUI.

Key upgrades:
- Workflow chaining: automatic discovery of frequent macro-sequences and chain-aware suggestions.
- Anomaly detection: contextual-seasonal outlier scoring with UI warnings.
- Multi-horizon plans: 14-step rollout + branch preview.
- Confidence shaping: bin calibration + Bayesian reliability smoothing.
- Safe auto-play: adaptive delay; executes only approved clusters/habits.

Base capabilities kept:
- 3-step transitions, context layering, hour smoothing, recency boosts, cluster generalization.
- Encrypted cache of predictor state, actions, calibration, safelists, and settings.
- Intrusion scoring and advisory banners.
"""
...
class ConfidenceCalibrator:
    def __init__(self, bins: int = 10, history_max: int = 2000):
        self.bins = bins
        ...
        self.global_beta = [1.0, 1.0]  # Beta(a,b) for Bayesian smoothing (correct, incorrect)
...
# ----------------------------
# Workflow chaining: discover macro-sequences
# ----------------------------
class WorkflowChains:
    def __init__(self, min_len: int = 3, max_len: int = 6, top_k: int = 50):
    ...
    def ingest_log(self, events: List[Tuple[float, str]]) -> None:
        # Build chains from recent slice for performance
        ...
        for L in range(self.min_len, self.max_len + 1):
            for i in range(0, max(0, n - L + 1)):
                ...
                self.chain_counts[window] += 1
        self.last_build_ts = time.time()
        # Keep only top_k
        self.chain_counts = Counter(dict(self.chain_counts.most_common(self.top_k)))

    def next_in_chain(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str]) -> Optional[Tuple[str, float]]:
        if not prev1:
            return None
        # Try matching tails of chains; prefer longer
        tails = [
            (3, (prev3, prev2, prev1)),
            (2, (prev2, prev1)),
            (1, (prev1,))
        ...
        for L, tail in tails:
            ...
            for chain, cnt in self.chain_counts.items():
                ...
                if tuple(chain[:L]) == tail:
                    candidate = chain[L]
                    score = cnt * (1.0 + 0.2 * (L-1))
                    if score > best_score:
                        best = candidate
                        best_score = score
        if best:
            # Normalize score into confidence-like value
...
# ----------------------------
# Next-gen predictor with seasonality/context/recency/clustering + anomaly scoring
# ----------------------------
class NextGenPredictor:
    ...
    def _score_snapshot(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                        dt: datetime.datetime, habits: Dict[str, HabitStats], now_ts: float) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]]]:
        hour = dt.hour
        dow = dt.weekday()
        weekend = 1 if dow >= 5 else 0
        month = dt.month
    ...
    def anomaly_score(self, habit: str, components: Dict[str, float]) -> float:
        # Compare context+season vs global/tod; large deviation is anomalous
        ctx = components.get("context", 0.0) + components.get("season_month", 0.0) + components.get("season_wom", 0.0)
        base = components.get("global", 0.0) + components.get("tod", 0.0)
        # If base dominates but context-season is tiny, or vice versa, flag
        total = components.get("total", ctx + base + 1e-9)
        ratio = abs(ctx - base) / (total + 1e-9)
        # Map ratio to 0..1 (higher = more anomalous)
        return max(0.0, min(1.0, ratio))

    def predict(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                dt: datetime.datetime, habits: Dict[str, HabitStats], now_ts: float) -> Tuple[Optional[str], float, Dict[str, float]]:
        try:
            scores, components = self._score_snapshot(prev1, prev2, prev3, dt, habits, now_ts)
            ...
            if best_h:
                ...
                explain["anomaly"] = self.anomaly_score(best_h, explain)
            return best_h, conf, explain
        except Exception:
            return None, 0.0, {}

    def predict_plan(self, prev1: Optional[str], prev2: Optional[str], prev3: Optional[str],
                     dt: datetime.datetime, habits: Dict[str, HabitStats], now_ts: float, length: int = 14) -> List[Tuple[str, float]]:
        plan: List[Tuple[str, float]] = []
        p1, p2, p3 = prev1, prev2, prev3
        try:
            for _ in range(max(1, length)):
                scores, _ = self._score_snapshot(p1, p2, p3, dt, habits, now_ts)
...
# ----------------------------
# Foresight engine: encrypted cache, security, calibration, workflows, auto-play
# ----------------------------
class Foresight:
    def __init__(self, cache_file: str = "foresight_cache.bin"):
        ...
        self.auto_safe_clusters: Set[str] = {"browser", "office"}
    ...
    def record_habit(self, habit: str, ts: Optional[float] = None) -> None:
        ...
        # Refresh workflow chains occasionally
        if (time.time() - self.workflows.last_build_ts) > 60.0:
            self.workflows.ingest_log(self.event_log[-500:])  # use recent slice
        self._save_cache()
    ...
    def predict_next(self, when_ts: Optional[float] = None) -> Tuple[Optional[str], float, Dict[str, float]]:
        try:
            ...
            # Chain-aware suggestion
            chain_suggestion = self.workflows.next_in_chain(prev1, prev2, prev3)
            raw_pred, raw_conf, explain = self.predictor.predict(prev1, prev2, prev3, dt, self.habits, now_ts)
            # Blend chain suggestion if stronger
            if chain_suggestion and chain_suggestion[1] > raw_conf * 0.95:
                raw_pred, raw_conf = chain_suggestion
                # explanation placeholder
                explain = {"sequence1": 0.0, "sequence2": 0.0, "sequence3": 0.0, "context": 0.0,
                           "season_month": 0.0, "season_wom": 0.0, "tod": 0.0, "global": 0.0, "cluster": 0.0,
                           "recency": 0.0, "total": raw_conf, "anomaly": 0.0}
    ...
    def predict_next_plan(self, when_ts: Optional[float] = None, length: int = 14) -> List[Tuple[str, float]]:
        try:
            ...
            with self._lock:
                prev1, prev2, prev3 = self._prev_context()
            plan = self.predictor.predict_plan(prev1, prev2, prev3, dt, self.habits, now_ts, length=length)
            # If workflow chain available, prepend its next step if distinct
            chain_suggestion = self.workflows.next_in_chain(prev1, prev2, prev3)
            if chain_suggestion:
                step, conf = chain_suggestion
                if not plan or plan[0][0] != step:
                    plan.insert(0, (step, conf))
            return [(step, self.calibrator.calibrate(conf)) for step, conf in plan]
        except Exception:
            return []
    ...
    def reinforce(self) -> None:
        with self._lock:
            ...
            prev2 = prev2 if isinstance(prev2, str) and not prev2.startswith("advisory:") else None
            prev3 = prev3 if isinstance(prev3, str) and not prev3.startswith("advisory:") else None
            predicted, raw_conf, _ = self.predictor.predict(prev1, prev2, prev3, dt, self.habits, last_ts)
            correct = (predicted == last_habit)
            self.calibrator.update(raw_conf or 0.0, correct)
            self.predictor.reward(correct)
...
# ----------------------------
# GUI (live assistant + forecasting + auto-play + safe lists + anomaly banner)
# ----------------------------
class ForesightGUI:
    ...
    def _build_layout(self):
        ...
        ttk.Checkbutton(top, text="Learning", variable=self.learning_var, command=self._toggle_learning).grid(row=0, column=3, sticky="e")
        ttk.Checkbutton(top, text="Execute actions", variable=self.execute_enabled).grid(row=0, column=4, sticky="e")

        ttk.Checkbutton(top, text="Auto-play", variable=self.auto_enabled_var, command=self._apply_auto).grid(row=0, column=5, sticky="e")
        ttk.Label(top, text="Threshold").grid(row=0, column=6, sticky="e")
        ...
        self.threshold_spin.grid(row=0, column=7, sticky="e")

        ttk.Label(top, text="Delay (s)").grid(row=1, column=6, sticky="e")
        ...
        self.delay_spin.grid(row=1, column=7, sticky="e")

        ttk.Button(top, text="Cancel auto", command=self._cancel_auto).grid(row=1, column=5, sticky="e")
        ttk.Button(top, text="Safe lists", command=self._edit_safe_lists).grid(row=1, column=4, sticky="e")
        ...
        self.plan_tree.column("step", width=280, anchor="w")
    ...
    def _edit_safe_lists(self):
        clusters_str = simpledialog.askstring("Safe clusters", "Comma-separated clusters to auto-execute (e.g., browser,office):")
    ...
    def _skip_step(self):
        ...
        if not step:
            self.status_var.set("No step selected.")
            return
        self.fs.predictor.reward(matched=False)
    ...
    def _execute_selected(self):
        ...
        if not step:
            self.status_var.set("No step selected.")
            return
        ok = self.fs.execute_action(step)
