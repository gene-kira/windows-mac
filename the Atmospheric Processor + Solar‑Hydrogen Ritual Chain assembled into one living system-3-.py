...
# --- Autoloader for required and optional libraries ---
...
# Core + optional visualization libraries
required_libs = ["tkinter"]  # tkinter is built-in, but listed for clarity
optional_libs = ["matplotlib"]

ensure_libs(required_libs + optional_libs)
...
class HydrogenCycleGUI:
    def __init__(self, root):
        ...
        self.create_panels()
        self.log("🌞 System initialized. Continuous phoenix loop ready.")
    ...
    def create_panels(self):
        ...
        self.log_box = tk.Text(self.root, height=15, width=100, bg="#2e2e3e", fg="#ffffff", font=("Courier", 10))
        ...
        self.ax.set_title("Cycle Metrics")
        self.ax.set_xlabel("Cycle")
        self.ax.set_ylabel("Values")
        self.line_h2, = self.ax.plot([], [], label="Hydrogen (kg)", color="cyan")
        self.line_o2, = self.ax.plot([], [], label="Oxygen (kg)", color="orange")
        self.line_energy, = self.ax.plot([], [], label="Energy (kWh)", color="lime")
        self.ax.legend()
    ...
    def simulate_cycle(self, cycle_num):
        ...
        self.history_cycles.append(cycle_num)
        self.history_h2.append(h2)
        self.history_o2.append(o2)
        self.history_energy.append(self.energy_available)

        # Reset storage after rebirth
        self.h2_storage = 0
        self.o2_storage = 0
        self.update_status()

        # Update chart
        self.line_h2.set_data(self.history_cycles, self.history_h2)
        self.line_o2.set_data(self.history_cycles, self.history_o2)
        self.line_energy.set_data(self.history_cycles, self.history_energy)
        self.ax.relim()
