...
# -*- coding: utf-8 -*-
"""
Autonomous Cipher Engine – Nerve Center (Augmented + Micro-Chart + Memory Save)

Includes:
- Auto-loader for optional libs (numpy, onnxruntime, psutil, pynvml)
- HybridBrain:
  * Multi-horizon prediction (EWMA + trend + regime + MovidiusInferenceEngine stub)
  * Meta-confidence fusion (variance, trend, noise, turbulence, reinforcement, model_conf)
  * Pattern memory & fingerprints
  * Dynamic stance thresholds (+ auto-tuning, auto-calibration)
  ...
  * Best-guess via multi-engine voting
- Organs:
  * DeepRamOrgan, BackupEngineOrgan, NetworkWatcherOrgan, GPUCacheOrgan,
    ThermalOrgan, DiskOrgan, VRAMOrgan, AICoachOrgan, SwarmNodeOrgan, Back4BloodAnalyzer
- SelfIntegrityOrgan
- AugmentationEngine with hooks:
  * before_prediction / after_prediction
  * before_stance / after_stance
  * before_organs / after_organs
- Tkinter GUI “System Nerve Center”:
  * Tab 1: Nerve Center (Brain, Tri-Stance, micro-chart, Cortex, Collective, Integrity, Organs, Reasoning, Heatmap, Augmentations, Commands)
  * Tab 2: Altered States (meta-states, Tri-Stance mapping)
  * Commands:
      - Stabilize, High-Alert, Learn, Optimize
      - Purge Mem, Rebuild Model, Reset Cortex
      - Snapshot (placeholder), Meta-State cycle
      - Train ONNX (stub)
      - Save Memory (Local) → JSON + BIN snapshot
      - Save Memory (SMB)   → JSON + BIN snapshot (no auth; uses OS context)
  * Micro-chart:
      - Short, Medium, Long predictions
      - Baseline line
      - Stance-colored medium
      - WHITE Best-Guess line
"""
...
# ------------------------------------------------------------
# 9. HybridBrain + Tri-Stance Decision Engine
# ------------------------------------------------------------
...
class HybridBrain:
    ...
    def update(self, snap: TelemetrySnapshot, organs: Dict[str, BaseOrgan]) -> Tuple[float, float, float, float]:
        ...
        # Best-guess: vote of medium engines
...
# ------------------------------------------------------------
# 11. GUI – System Nerve Center with Altered States, Augmentation, Micro-Chart, Memory Save
# ------------------------------------------------------------
...
class NerveCenterGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("Autonomous Cipher Engine – Nerve Center (Augmented)")
    ...
    def cmd_save_memory_local(self):
        # Ask for base filename; we'll save .json and .bin
    ...
    def cmd_save_memory_smb(self):
        # For SMB, we still use the same picker; user can enter UNC manually or use mapped drive
        path = filedialog.asksaveasfilename(
            title="Save Memory Snapshot to SMB or UNC (JSON+BIN)",
            defaultextension=".json",
    ...
    def _save_memory_to_paths(self, json_path: str, bin_path: str):
        ...
        try:
            ...
            messagebox.showerror("Save Memory JSON", f"Failed to save JSON:\n{e}")
        ...
        try:
            ...
            messagebox.showerror("Save Memory BIN", f"Failed to save BIN:\n{e}")
    ...
    def _update_loop(self):
        ...
        # Store predictions including best-guess
        self.pred_history.append((pred_s, pred_m, pred_l, self.brain.best_guess))
        if len(self.pred_history) > self.chart_max_points:
            self.pred_history.pop(0)

        self.log_reason(
            f"Tick {self.tick} | stance={self.brain.tri_stance} mode={self.brain.mode} med={pred_m:.2f} best={self.brain.best_guess:.2f} meta={meta_conf:.2f}"
        )
    ...
    def _refresh_gui(self):
        ...
        text_brain.append(
            f"Pred S/M/L: {self.brain.pred_short:.2f}/{self.brain.pred_medium:.2f}/{self.brain.pred_long:.2f}  Best={self.brain.best_guess:.2f}  MetaConf={self.brain.meta_confidence:.2f}"
        )
        text_brain.append(
            f"Stance thresholds: stability<{self.brain.stability_threshold:.2f}, reflex>{self.brain.reflex_threshold:.2f}"
        )
        text_brain.append(
            f"Meta biases: H {meta.prediction_horizon_short}/{meta.prediction_horizon_medium}/{meta.prediction_horizon_long} "
            f"Agg {meta.stance_aggressiveness:.2f} RAM {meta.deep_ram_appetite_bias:.2f} Thr {meta.thread_expansion_bias:.2f} Cache {meta.cache_behavior_bias:.2f}"
        )
        ...
        # Cortex / collective / integrity label
        text_ctx = []
        ...
        text_ctx.append(
            f"Collective risk={coll.collective_risk_score:.2f}  HiveMode={coll.hive_sync_mode}  Consensus={coll.consensus_weighting:.2f}"
        )
        text_ctx.append(
            f"Integrity={si.integrity_score:.2f}  SafeMode={si.safe_mode}  Drift={si.prediction_drift:.2f}  ModelDegr={si.model_degradation:.2f}"
        )
        ...
        for name, m in self.brain.meta_states.items():
            ...
            text_states.append(
                f"    Horizons: short={m.prediction_horizon_short}s, med={m.prediction_horizon_medium}s, long={m.prediction_horizon_long}s"
            )
            text_states.append(
                f"    Agg={m.stance_aggressiveness:.2f} DeepRAM={m.deep_ram_appetite_bias:.2f} Threads={m.thread_expansion_bias:.2f} Cache={m.cache_behavior_bias:.2f}"
            )
            text_states.append("")
    ...
    def _draw_micro_chart(self):
        ...
        c.create_text(
            5, 5, anchor="nw",
            text="S/M/L/Best (short/med/long/best); baseline line; stance color on M",
            fill="#bbbbbb",
            font=("Consolas", 7),
        )
