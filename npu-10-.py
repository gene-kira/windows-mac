...
class PersistenceManager:
    ...
    Handles persistence of:
      ...
      - Foresight / Cortex / State metadata
    ...
    def save_backbone(self, backbone):
        ...
        try:
            ...
            meta = {
                ...
                "state_mode": backbone.state_mode,
                "dream_intensity": backbone.dream_intensity,
            }
            self.save_json("backbone_meta.json", meta)

            foresight_state = backbone.foresight.to_dict()
            self.save_json("foresight.json", foresight_state)

            cortex_state = backbone.cortex.to_dict()
            self.save_json("cortex.json", cortex_state)
    ...
    def load_backbone(self, backbone):
        ...
        try:
            ...
            if isinstance(meta, dict):
                ...
                backbone.state_mode = meta.get("state_mode", backbone.state_mode)
                backbone.dream_intensity = meta.get("dream_intensity", backbone.dream_intensity)

            foresight_state = self.load_json("foresight.json")
            if isinstance(foresight_state, dict):
                backbone.foresight.from_dict(foresight_state)

            cortex_state = self.load_json("cortex.json")
            if isinstance(cortex_state, dict):
                backbone.cortex.from_dict(cortex_state)
...
class RaidCacheOrgan:
    def __init__(self, npu: ReplicaNPU,
        ...
        self.entropy_history = deque(maxlen=64)
        self.util_history = deque(maxlen=64)
        self.latency_history = deque(maxlen=64)
    ...
    def from_dict(self, d):
        try:
            ...
            self.entropy_history = deque(d.get("entropy_history", []), maxlen=64)
            self.util_history = deque(d.get("util_history", []), maxlen=64)
            self.latency_history = deque(d.get("latency_history", []), maxlen=64)
        except Exception as e:
...
# ============================================================
# FORESIGHT ENGINE (PREDICTIVE)
# ============================================================

class ForesightEngine:
    """
    Predicts near-future utilization and pressure using simple trend analysis.
    Also classifies scenarios (chaotic, stable, stressed).
    """
    ...
    def update(self, organs_snap):
        self.forecasts = []
        util_trends = []
        entropy_trends = []
        latency_trends = []

        for s in organs_snap:
            ent_hist = s.get("entropy_history", [])
            util_hist = s.get("util_history", [])
            lat_hist = s.get("latency_history", [])

            ent_trend = self._linear_trend(ent_hist)
            util_trend = self._linear_trend(util_hist)
            lat_trend = self._linear_trend(lat_hist)

            util_trends.append(util_trend)
            entropy_trends.append(ent_trend)
            latency_trends.append(lat_trend)

            current_util = s["utilization"]
            forecast_util = current_util + util_trend * 10.0
            forecast_util = max(0.0, min(1.0, forecast_util))
        ...
        avg_util_trend = sum(util_trends) / len(util_trends)
        avg_ent_trend = sum(entropy_trends) / len(entropy_trends)
        avg_lat_trend = sum(latency_trends) / len(latency_trends)

        if avg_ent_trend > 0.01 and avg_util_trend > 0.01:
            self.scenario = "Chaotic Ingestion"
        elif avg_ent_trend < -0.01 and avg_util_trend < 0.01 and avg_lat_trend < 0.0:
            self.scenario = "Stable Workload"
        elif avg_lat_trend > 0.01 or avg_util_trend > 0.02:
            self.scenario = "Stressed"
        else:
            self.scenario = "Neutral"

        mag = abs(avg_util_trend) + abs(avg_ent_trend) + abs(avg_lat_trend)
        self.confidence = max(0.0, min(1.0, mag * 10.0))
    ...
    def from_dict(self, d):
        self.forecasts = d.get("forecasts", [])
        self.scenario = d.get("scenario", "Unknown")
...
class BestGuessCortex:
    """
    Fuses global signals into a single best-guess about future pressure and stance.
    """

    def __init__(self):
        self.best_guess = "Balanced"
        self.best_guess_confidence = 0.0
        self.predicted_global_util = 0.0

    def update(self, global_util, foresight: ForesightEngine, movidius_decision, movidius_conf):
        if foresight.forecasts:
            fut_utils = [f["forecast_util"] for f in foresight.forecasts]
            self.predicted_global_util = sum(fut_utils) / len(fut_utils)
        else:
            self.predicted_global_util = global_util

        score = 0.5 * movidius_conf + 0.5 * foresight.confidence

        if self.predicted_global_util > 0.85:
            self.best_guess = "Conservative"
        elif self.predicted_global_util > 0.6:
            self.best_guess = "Balanced"
        else:
            self.best_guess = "Beast"

        if movidius_decision == self.best_guess:
            score = min(1.0, score + 0.2)

        self.best_guess_confidence = score

    def to_dict(self):
        return {
            "best_guess": self.best_guess,
            "best_guess_confidence": self.best_guess_confidence,
            "predicted_global_util": self.predicted_global_util,
        }

    def from_dict(self, d):
        self.best_guess = d.get("best_guess", "Balanced")
        self.best_guess_confidence = d.get("best_guess_confidence", 0.0)
        self.predicted_global_util = d.get("predicted_global_util", 0.0)
...
class AICoachBay:
    def __init__(self, bay: RaidBay, movidius: MovidiusEngine, foresight: ForesightEngine, cortex: BestGuessCortex):
        self.bay = bay
        ...
        self.last_state_mode = "NORMAL"

    def update(self, load_score: float, global_util: float, state_mode: str):
        healths = []
        hits = []
        utils = []
        ...
        organs_snap = []
        for idx, (_, _, organ) in enumerate(self.bay.sets):
            organs_snap.append({
                "index": idx,
                "type": "RAM",
                "utilization": organ.utilization,
                "entropy_history": list(organ.entropy_history),
                "util_history": list(organ.util_history),
                "latency_history": list(organ.latency_history),
            })
        organs_snap.append({
            "index": 6,
            "type": "VRAM",
            "utilization": vram_organ.utilization,
            "entropy_history": list(vram_organ.entropy_history),
            "util_history": list(vram_organ.util_history),
            "latency_history": list(vram_organ.latency_history),
        })

        self.foresight.update(organs_snap)
        self.cortex.update(global_util, self.foresight, decision, conf)

        scenario = self.foresight.scenario
        f_conf = self.foresight.confidence
        best_guess = self.cortex.best_guess
        best_conf = self.cortex.best_guess_confidence

        if state_mode == "ALERT":
            prefix = "ALERT"
        elif state_mode == "DREAM":
            prefix = "DREAM"
        elif state_mode == "RECOVERY":
            prefix = "RECOVERY"
        else:
            prefix = "NORMAL"
        ...
        if decision == "Beast":
            if avg_health > 0.55:
                msg = "Beast stance sustained. Backbone thriving under load."
            else:
                msg = "Beast stance active, but health dipping. Monitor closely."
        elif decision == "Balanced":
            msg = "Balanced stance. Throughput and safety in equilibrium."
        else:
            msg = "Conservative stance. System under stress, backing off deep cycles."

        msg += f" Scenario: {scenario} (Foresight {f_conf:.2f}). "
        msg += f"Best Guess: {best_guess} ({best_conf:.2f})."

        self.last_message = f"{prefix}: {msg}"
        self.last_state_mode = state_mode


# ============================================================
# NPU RAID BACKBONE (WITH STATES + DREAM MODE)
# ============================================================
...
class NpuRaidBackbone:
    def __init__(self):
        ...
        self.cortex = BestGuessCortex()
        self.coach = AICoachBay(self.bay, self.movidius, self.foresight, self.cortex)
        ...
        self.state_mode = "NORMAL"   # NORMAL, ALERT, DREAM, RECOVERY
        self.dream_intensity = 0.3   # 0..1
    ...
    def _update_state_mode(self, load, global_util):
        if load > 0.85 or global_util > 0.9:
            self.state_mode = "ALERT"
        elif load < 0.2 and global_util < 0.3:
            self.state_mode = "DREAM"
        elif self.state_mode == "ALERT" and load < 0.5:
            self.state_mode = "RECOVERY"
        elif self.state_mode == "RECOVERY" and load < 0.3:
            self.state_mode = "NORMAL"
        elif self.state_mode not in ("ALERT", "DREAM", "RECOVERY"):
            self.state_mode = "NORMAL"

    def _apply_state_to_interval(self, load):
        if self.state_mode == "ALERT":
            t_min, t_max = 800, 2000
        elif self.state_mode == "DREAM":
            t_min, t_max = 100, 300
        elif self.state_mode == "RECOVERY":
            t_min, t_max = 400, 900
        else:
            t_min, t_max = 200, 600
    ...
    def _dream_cycle(self):
        ...
        _, _, vram_organ = self.bay.vram
        for _ in range(int(3 * self.dream_intensity)):
            synthetic = f"DREAM|{time.time()}|{random.random()}".encode("utf-8")
            vram_organ.cache_write_bytes(synthetic)

        for _, npu, _ in self.bay.sets:
            npu.micro_recovery(0.005 * self.dream_intensity)
        _, vram_npu, _ = self.bay.vram
        vram_npu.micro_recovery(0.005 * self.dream_intensity)
    ...
    def maybe_deep_cycle(self):
        ...
        self._update_state_mode(load, global_util)
        self._apply_state_to_interval(load)

        if self.state_mode == "DREAM":
            self._dream_cycle()

        self.coach.update(load, global_util, self.state_mode)
    ...
    def snapshot(self):
        ...
        return {
            ...
            "state_mode": self.state_mode,
            "foresight_scenario": self.foresight.scenario,
            "foresight_confidence": self.foresight.confidence,
            "cortex_best_guess": self.cortex.best_guess,
            "cortex_best_guess_conf": self.cortex.best_guess_confidence,
            "cortex_predicted_global_util": self.cortex.predicted_global_util,
        }
...
# ============================================================
# TKINTER CONSOLE (COMPACT NUMERIC COCKPIT + PERSIST + PREDICTIVE)
# ============================================================
...
class NpuRaidWindow:
    def __init__(self, root, backbone: NpuRaidBackbone, mem_layer: MemoryCacheLayer):
        ...
        self.root.title("NPU RAID – Event Horizon – Predictive Organism")
    ...
    def _build_ui(self):
        ...
        title = tk.Label(
            self.main_frame,
            text="EVENT HORIZON – NPU RAID BACKBONE – ALTERED STATES",
            fg="#00FFAA",
            bg="#05050A",
        ...
        self.lbl_persist.pack(anchor="w", pady=(0, 2))
        ...
        lbl_debug = tk.Label(
            debug_frame,
            text="COCKPIT LINE – ALL SETS (RAM/VRAM, MB+GB, UTIL, HIT, LAT, PREDICTED)",
            fg="#8888FF",
            bg="#05050A",
            font=("Consolas", 10, "bold")
        )
        ...
        self.lbl_cortex = tk.Label(
            coach_frame,
            text="CORTEX BEST GUESS: -",
            fg="#00DDFF",
            bg="#05050A",
            font=("Consolas", 10, "bold")
        )
        self.lbl_cortex.pack(anchor="w")

        self.lbl_foresight = tk.Label(
            coach_frame,
            text="FORESIGHT: -",
            fg="#AAAAFF",
            bg="#05050A",
        ...
        self.lbl_foresight.pack(anchor="w")
    ...
    def _update_gui(self):
        try:
            ...
            self.lbl_state.config(text=f"STATE: {snap['state_mode']}")
            ...
            g_pred_util = snap["cortex_predicted_global_util"] * 100.0
            ...
            header = (
                ...
                f"{g_util:5.1f}% UTIL | "
                f"FORECAST UTIL {g_pred_util:5.1f}%\n"
            )
            self.txt_debug.insert(tk.END, header)
            self.txt_debug.insert(tk.END, "-" * 110 + "\n")

            foresight_map = {f["index"]: f for f in self.backbone.foresight.forecasts}
            ...
            for s in snap["sets"]:
                ...
                f_entry = foresight_map.get(idx)
                if f_entry:
                    fut_util_pct = f_entry["forecast_util"] * 100.0
                else:
                    fut_util_pct = util_pct
            ...
            self.lbl_cortex.config(
                text=f"CORTEX BEST GUESS: {snap['cortex_best_guess']} ({snap['cortex_best_guess_conf']:.2f})"
            )
            self.lbl_foresight.config(
                text=f"FORESIGHT: {snap['foresight_scenario']} ({snap['foresight_confidence']:.2f})"
            )
