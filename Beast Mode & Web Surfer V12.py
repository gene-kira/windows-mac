...
class ResourcePolicy:
    ...
    def choose_engine(self, beast_avg, gpu_free_mb, movidius_ok, coral_ok):
        # Simple heuristic: prefer GPU when Beast is hot and VRAM is available,
        # then NPUs, otherwise CPU.
        if gpu_free_mb is not None and gpu_free_mb >= 2048 and beast_avg is not None and beast_avg > 50:
            decision = "GPU"
        elif movidius_ok and beast_avg is not None and beast_avg > 60:
            decision = "MOVIDIUS"
        elif coral_ok and beast_avg is not None and beast_avg > 60:
            decision = "CORAL"
        else:
            decision = "CPU"
...
if IS_GUI:
    ...
    class SystemWorker(QtCore.QObject):
        ...
        def __init__(self, parent=None):
            ...
            self._last_slow = 0.0
            self._last_hw = {}
            self._cpu_history = []          # list of (timestamp, [per_core_usage])
        ...
        def _compute_beast_metrics(self):
            ...
            for _, per_core in recent:
                ...
                if beast_slice:
                    beast_avgs.append(sum(beast_slice) / len(beast_slice))
            ...
            past = beast_avgs[0]
            slope = current - past  # crude trend over window

            # Predictive rules
            active = False
            if current >= 70.0:
                active = True  # hard spike
            elif current >= 40.0 and slope is not None and slope > 5.0:
                active = True  # rising into heavy load
        ...
        def run(self):
            while self._running:
                ...
                if now - self._last_slow >= self._slow_interval:
                    ...
                    self._last_gpu_free_mb = free_vram_mb
                    self._last_movidius = movidius_detected
                    self._last_coral = coral_detected
    ...
    class MainWindow(QtWidgets.QMainWindow):
        ...
        def on_metrics(self, metrics: dict):
            ...
            self._set_beast_led(beast_active)
