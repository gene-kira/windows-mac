...
class Back4BloodPhysicsOrgan:
    ...
    Uses analyzer.last_metrics as source, with fallback if no JSON.
    """
    ...
    def tick(self):
        ...
        # Fallback: if no metrics JSON but Back4Blood is running, approximate from system
    ...
    def _bg_loop(self):
        while True:
            try:
                ...
                # 2) Update B4B analyzer + physics
                self.back4blood_analyzer.tick()
                self.back4blood_physics.tick()

                # 3) World + game + live pattern ingestion (uses latest risk)
                hour, weekday = local_time_components()
                cpu_mean = self.predictor.cpu.mean()
                net_mean = self.predictor.net.mean()
                cpu_band = int(cpu_mean * 10)
                net_band = int(net_mean * 10)
                # world outcome based on last risk (previous tick)
                if self.brain.cortex.risk > 0.8:
                ...
                # 5) Prediction engine (now sees all updated signals)
                self.predictor.ingest_sample()
                self.predictor.update_forecast()

                # 6) Integrity, stance, restoration
