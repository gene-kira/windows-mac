#!/usr/bin/env python3
# guardian_hybrid_edr_ai_encoding_thermal_peltier_data_auto.py
...
# ---------------------------------------------------------
# NORMAL AUTOLOADER (NO AGGRESSIVE RESTART)
# ---------------------------------------------------------
...
def load_state(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if "auto_move_enabled" not in st:
            st["auto_move_enabled"] = False
        if "auto_move_threshold" not in st:
            st["auto_move_threshold"] = 0.7
        if "auto_move_cooldown_sec" not in st:
            st["auto_move_cooldown_sec"] = 30
        return st
    except Exception:
        return {
            "ai_enabled": False,
...
# -----------------------------
# Thermal control thread (hardware Peltier)
# -----------------------------
...
# -----------------------------
# Serial auto-detect for thermal device
# -----------------------------
...
class GuardianApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Guardian Hybrid EDR - SYSTEM + AI + Encoding + Thermal + Data Peltier")
    ...
    # ---------- Threads / watchdog / scanner / quarantine ----------
    ...
    # ---------- Controls / AI / Encoding / Storage path / Auto-move ----------
    ...
    # ---------- Scanner → auto-suggest + AI + Encoding + Auto-move ----------
    ...
    # ---------- File move suggestions / auto-move / rollback ----------
