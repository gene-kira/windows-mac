# PART 1 — CORE ENGINE + DATA MODELS + HELPERS
# ============================================
...
from PyQt5.QtWidgets import (
    ...
    QListWidget, QListWidgetItem, QGridLayout, QTabWidget,
    QComboBox, QInputDialog, QSplitter
)
...
# PART 2 — HISTORY, FORECAST, THREAT CONE, QUEEN, SETTINGS, BACKUP, WORKERS
# =========================================================================
...
class ShortHorizonForecaster:
    ...
    def forecast_routes(self, route_map: Dict[str, RouteStatus]) -> Dict[str, RouteForecast]:
        ...
        for name, status in route_map.items():
            ...
            health = f_lat * 0.4 + f_loss * 0.4 + f_cong * 0.2
...
class ThreatCone:
    ...
    def project_30s(self) -> List[RouteThreatProjection]:
        ...
        for name, status in snapshot.items():
            ...
            score = smoothed_lat * 0.4 + smoothed_loss * 0.4 + smoothed_cong * 0.2
            if score < 20:
                projected = RouteColor.GREEN
                risk = 0.2 + anomaly * 0.3
            elif score < 40:
                projected = RouteColor.YELLOW
                risk = 0.4 + anomaly * 0.3
            elif score < 60:
                projected = RouteColor.ORANGE
                risk = 0.7 + anomaly * 0.2
            else:
                projected = RouteColor.RED
                risk = 0.9 + anomaly * 0.1
...
# -----------------------------
class SettingsStore:
    def __init__(self, path: str = "./heavy_data_settings.json"):
        self.path = path
...
class BackupEngine:
    ...
    def run_mission(self, mission: BackupMission) -> Tuple[BackupMission, List[Dict[str, Any]]]:
...
# PART 3 — PANELS (HEATMAP, LANES, REROUTE, TIMELINE, DECISIONS, BACKUP MANAGER)
# ===============================================================================
...
# PART 4 — UNIFIED TABLE, ROUTE CATEGORIES SUPPORT, ADVANCED PANELS
# =================================================================
...
class UnifiedRouteTable(QTableWidget):
    def __init__(self, queen: QueenBrain, history: RouteHistory,
        ...
        header.setSectionResizeMode(0, QHeaderView.Stretch)   # Route
        header.setSectionResizeMode(1, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(2, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(3, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(4, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(5, QHeaderView.ResizeToContents)
        header.setSectionResizeMode(6, QHeaderView.Stretch)   # Threat Projection
    ...
    def refresh(self):
        ...
        for row, (name, status) in enumerate(routes.items()):
            ...
            if proj:
                proj_text = proj.projected_color_30s.value
...
# -----------------------------
# LIVE SYSTEM FEEDBACK PANEL (4 SUB-TABS)
# -----------------------------
...
class ThreatConePanel(QWidget):
    def __init__(self, threat_cone: ThreatCone):
        ...
        self.table = QTableWidget(0, 5)
        self.table.setHorizontalHeaderLabels([
            "Route", "Current Color", "Projected (30s)", "Risk", "Anomaly"
        ])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table.verticalHeader().setDefaultSectionSize(22)
    ...
    def refresh(self):
        ...
        for row, p in enumerate(projections):
            ...
            self.table.setItem(row, 3, QTableWidgetItem(f"{p.risk_score:.2f}"))
            self.table.setItem(row, 4, QTableWidgetItem(f"{p.anomaly:.2f}"))
...
# -----------------------------
# NEURAL FUSION PANEL (FUTURE UPGRADE SHELL)
# -----------------------------
...
# -----------------------------
# ADAPTIVE WEIGHTS PANEL (FUTURE UPGRADE SHELL)
# -----------------------------
...
# -----------------------------
# SCHEDULER PANEL (FUTURE UPGRADE SHELL)
# -----------------------------
...
# -----------------------------
# AUTO-REROUTE PANEL (FUTURE UPGRADE SHELL)
# -----------------------------
...
# -----------------------------
# SWARM SYNC PANEL (FUTURE UPGRADE SHELL)
# -----------------------------
...
# -----------------------------
# MISSION REPLAY PANEL (FUTURE UPGRADE SHELL)
# -----------------------------
...
# PART 5 — MAIN WINDOW, TABS, THEMES, REFRESH, WORKERS, BOOTSTRAP
# ===============================================================
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Reroute banner (only visible when reroute happens)
        self.reroute_banner = QLabel("REROUTE ACTIVE")
        self.reroute_banner.setAlignment(Qt.AlignCenter)
        self.reroute_banner.setStyleSheet("background-color: #803000; color: white; font-weight: bold; padding: 4px;")
        self.reroute_banner.setVisible(False)
        root_layout.addWidget(self.reroute_banner)
        ...
        # --- Tab 1: Traffic Monitor (Unified Table) ---
        ...
        # --- Tab 2: Route Categories (Browser / Game / System / Network) ---
        ...
        # --- Live System Feedback (separate tab for real-time events) ---
    ...
    def _on_status_ready(self, status: RouteStatus):
        ...
        # Detect best route and reroute events
        snapshot = self.queen.get_snapshot()
        ...
        if best:
            if self.last_best_route is None:
                ...
                self.last_best_route = best.name
                # Show banner briefly
                self.reroute_banner.setVisible(True)
                QTimer.singleShot(4000, lambda: self.reroute_banner.setVisible(False))
    ...
    def _refresh_all(self):
        ...
        # Route Categories filtered tables
        ...
        # Update queen route_map temporarily for each filtered table, then refresh
        # (They share the same queen/history/forecaster/threat_cone, so we just swap route_map view)
        ...
        # Mission timeline (placeholder: could be wired to real missions)
        # self.mission_timeline_panel.set_events(...)

        # Live system feedback is fed externally via add_event(category, text)
