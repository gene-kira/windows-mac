...
# -*- coding: utf-8 -*-
"""
Autonomous Cipher Engine – Nerve Center GUI
Single-file organism with:
...
- HybridBrain:
  ...
  * Meta-states (Hyper-Flow, Deep-Dream, Sentinel, Recovery-Flow) with momentum transitions
...
- Tkinter GUI “System Nerve Center”:
  * HybridBrain panel
  * Tri-Stance panel
  * Cortex / Collective / Integrity panel
  * Organs health/risk table
  * Reasoning tail
  * Reasoning heatmap (textual)
  * Command buttons (Stabilize, High-alert, Learn, Optimize, etc.)
"""
...
# ------------------------------------------------------------
# 8. HybridBrain + Tri-Stance Decision Engine
# ------------------------------------------------------------
...
class HybridBrain:
    def __init__(self, judgment: JudgmentEngine, cortex: SituationalCortex,
        ...
        # Tri-Stance: Conservative / Balanced / Beast
    ...
    def decide_tri_stance(self, snap: TelemetrySnapshot, pred_short: float,
        ...
        # Simple bottleneck test: if any organ risk > 0.8, consider pressured
...
class NerveCenterGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        root.title("Autonomous Cipher Engine – Nerve Center")

        self.judgment = JudgmentEngine()
        self.cortex = SituationalCortex()
        self.collective = CollectiveHealth()
        self.pattern_memory = PatternMemory()
        self.brain = HybridBrain(self.judgment, self.cortex, self.collective, self.pattern_memory)
    ...
    def _build_layout(self):
        ...
        for col in columns:
            ...
            self.tree_organs.column(col, width=90, anchor="center")
        ...
        self.txt_tail = tk.Text(self.frame_tail, height=15, width=40, font=("Consolas", 8))
        ...
        self.txt_heatmap = tk.Text(self.frame_heatmap, height=12, width=40, font=("Consolas", 8))
    ...
    def _update_loop(self):
        ...
        self.log_reason(
            f"Tick {self.tick} | stance={self.brain.tri_stance} mode={self.brain.mode} med={pred_m:.2f} meta={meta_conf:.2f}"
        )
    ...
    def _sparks(self, v: float) -> str:
        bars = "▁▂▃▄▅▆▇█"
        idx = int(clamp(v, 0.0, 0.999) * len(bars))
        return bars[idx]
    ...
    def _refresh_gui(self):
        ...
        text_brain.append(
            f"Volatility={self.brain.volatility:.2f}  Trust={self.brain.trust:.2f}  Load={self.brain.cognitive_load:.2f}"
        )
        text_brain.append(
            f"Pred S/M/L: {self.brain.pred_short:.2f}/{self.brain.pred_medium:.2f}/{self.brain.pred_long:.2f}  MetaConf={self.brain.meta_confidence:.2f}"
        )
        text_brain.append(
            f"Stance thresholds: stability<{self.brain.stability_threshold:.2f}, reflex>{self.brain.reflex_threshold:.2f}"
        )
        text_brain.append(
            f"Meta biases: H {meta.prediction_horizon_short}/{meta.prediction_horizon_medium}/{meta.prediction_horizon_long} Agg {meta.stance_aggressiveness:.2f}"
        )
        ...
        for line in self.reasoning_tail[-20:]:
