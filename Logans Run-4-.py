...
- QueenState + QueenController:
    - Aggregates telemetry from nodes
...
class SimpleBandit:
    ...
    def record(self, arm: str, reward: float):
        ...
        stats["count"] += 1
        self.total_pulls += 1
        # incremental average for value estimate
    ...
    def score(self, arm: str) -> float:
        ...
        if not stats:
            return float("inf")  # force exploration for new arms
        c = stats["count"]
        mu = stats["value_est"]
    ...
    def choose(self, candidates: List[Dict[str, Any]], max_choices: int) -> List[Dict[str, Any]]:
        """
        candidates: tasks with 'reason' field (arm name) and 'score' (intrinsic score).
        We blend bandit score with intrinsic score.
        """
        if not candidates:
            return []
        scored = []
...
class HiveGUI:
    ...
    def _layout_world_graph(self, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]):
        ...
        for e in edges:
            ...
            fade = max(0.2, min(1.0, 1.0 - age / 60.0))  # fade over 60s
