# ============================================================
# LINE COMPUTING SYSTEM v13
# Autonomous, predictive, adaptive, defensive, self-tuning + evolutionary guardian
#
# Layers:
# - Line Engine (expected sequence, anomalies, folding)
# - Predictive Engine (temporal, rarity, idle, behavior, entropy)
# - Profiling Manager (baseline learning)
# - Process Behavior Model (fingerprints)
# - Policy Engine (allow/log/alert/kill/ignore)
# - System Event Collector (process/CPU/network/FS)
# - Swarm Manager (multi-node sync + shared risk)
# - Watchdog (stall detection)
# - Risk History + Replay (simulation sandbox)
# - Threat Glyph Renderer (visual threat symbols)
# - GUI Cockpit (control & visibility)
# - Tuning Manager (online gradient-like self-tuning)
# - Simulation Manager + Evolution Engine (offline config evolution)
# ============================================================
...
required_libs = [
    ...
    "subprocess", "socket", "math", "hashlib", "copy"
]
...
def load_settings():
    ...
    return {
        ...
        # Per-process policies: allow | log | alert | kill | ignore
        "process_policies": {
        ...
        "predictive": {
            ...
            "weight_rarity": 1.0,
            "weight_cpu_behavior": 1.0,
            "weight_cmd_entropy": 1.0,
            "idle_multiplier": 1.3
        },
...
class RiskHistoryManager:
    ...
    def record(self, ts, event, source, is_anomaly, risk, tags=None):
        with self.lock:
            self.history.append({
                "ts": ts,
                "event": event,
...
class ReplayController:
    ...
    def start(self, history, config_override=None):
        with self.lock:
            if self.running:
                return
            self.running = True
        self.thread = threading.Thread(target=self._run, args=(history, config_override), daemon=True)
        self.thread.start()
    ...
    def _run(self, history, config_override):
        cfg = settings.get("predictive", {}).copy()
        if config_override:
            cfg.update(config_override)
        replay_engine = PredictiveEngine(cfg)
        last_ts = None
        for entry in history:
            ...
            risk = replay_engine.observe_event(event, source, is_anomaly, behavior_score=0.0, tags=entry.get("tags"))
...
class PredictiveEngine:
    def __init__(self, config):
        ...
        self.weight_rarity = config.get("weight_rarity", 1.0)
        self.weight_cpu_behavior = config.get("weight_cpu_behavior", 1.0)
        self.weight_cmd_entropy = config.get("weight_cmd_entropy", 1.0)
        self.idle_multiplier = config.get("idle_multiplier", 1.3)
    ...
    def observe_event(self, event_type, source, is_anomaly=False, behavior_score=0.0, tags=None):
        now = time.time()
        ...
        with self.lock:
            ...
            rarity_score = self._compute_rarity(event_type, hour) * self.weight_rarity
            behavior_contrib = behavior_score * self.weight_cpu_behavior

            base = 30.0 if is_anomaly else 5.0
            delta = base + rarity_score + behavior_contrib

            if tags and "high_entropy" in tags:
                delta += 10.0 * self.weight_cmd_entropy

            tod_weight = self._time_of_day_weight(hour)
            delta *= tod_weight

            if self.is_operator_idle():
                delta *= self.idle_multiplier
        ...
        risk_history.record(now, event_type, source, is_anomaly, risk, tags=tags)
...
class LineComputer:
    ...
    def process(self, event, source="line", behavior_score=0.0, tags=None):
        with self.lock:
...
class ProcessPolicyEngine:
    ...
    def apply_action(self, pid: str, name: str, action: str, event_type: str, source: str,
        ...
        if psutil and pid not in (None, "?", ""):
            ...
            ent_score = compute_cmd_entropy(pid)
            if ent_score > 0:
                tags.append("high_entropy")
                behavior_score += ent_score
            if behavior_score > 0:
                log.add("INFO", f"{SYMBOL_FP} Behavior+CLI deviation for {name} (pid={pid}) score={behavior_score:.1f}")

        match = self.engine.process(event_type, source=source, behavior_score=behavior_score, tags=tags)
        ...
        regret_tags = []
        if action_effective in ("log", "allow") and risk > predictive_engine.risk_threshold_strict:
            regret_tags.append("false_negative_candidate")
        if action_effective in ("alert", "kill") and risk < predictive_engine.risk_threshold_warn:
            regret_tags.append("false_positive_candidate")
        if regret_tags:
            risk_history.record(time.time(), event_type, source, not match, risk, tags=regret_tags)
...
# -----------------------------
# TUNING MANAGER (ONLINE SELF-TUNING)
# -----------------------------
class TuningManager:
    def __init__(self, policy_engine: ProcessPolicyEngine):
        self.policy_engine = policy_engine
        self.running = False
        self.thread = None
        self.lock = threading.Lock()
        ...
        self.cfg = settings.get("tuning", {})
        self.interval = self.cfg.get("interval_seconds", 600)
        self.max_step = self.cfg.get("max_step", 0.2)
        self.policy_adjust_threshold = self.cfg.get("policy_adjust_threshold", 5)
    ...
    def start(self):
        ...
        self.thread = threading.Thread(target=self._run, daemon=True)
        self.thread.start()
        log.add("INFO", f"{SYMBOL_TUNE} TuningManager started (interval={self.interval}s).")

    def stop(self):
        self.running = False
        log.add("INFO", f"{SYMBOL_TUNE} TuningManager stopped.")
    ...
    def _tune_once(self):
        ...
        for entry in hist[-2000:]:
            tags = entry.get("tags") or []
            if "false_positive_candidate" in tags:
                fp_count += 1
                src = entry.get("source", "")
                pname = self._extract_process_from_source(src)
                if pname:
                    process_fp[pname] = process_fp.get(pname, 0) + 1
            if "false_negative_candidate" in tags:
                fn_count += 1
                src = entry.get("source", "")
                pname = self._extract_process_from_source(src)
                if pname:
                    process_fn[pname] = process_fn.get(pname, 0) + 1

        if fp_count == 0 and fn_count == 0:
            return

        log.add("INFO", f"{SYMBOL_TUNE} Tuning pass: FP={fp_count}, FN={fn_count}")

        direction = 1 if fn_count > fp_count else -1
        balance_ratio = abs(fn_count - fp_count) / max(fp_count + fn_count, 1)

        step = self.max_step * balance_ratio
        self._adjust_predictive_weights(direction, step)
        self._adjust_thresholds(direction, step)
        self._adjust_folding_threshold(direction, step)
        self._adjust_policies(process_fp, process_fn)

        self._save_tuning_history()
        save_settings(settings)

    def _extract_process_from_source(self, source):
        if ":" in source:
            return source.split(":", 1)[1]
        return None

    def _adjust_predictive_weights(self, direction, step):
        pred = settings.get("predictive", {})
        before = pred.copy()

        for key in ["weight_rarity", "weight_cpu_behavior", "weight_cmd_entropy", "idle_multiplier"]:
            val = pred.get(key, 1.0)
            new_val = val + direction * step
            if key == "idle_multiplier":
                new_val = max(1.0, min(new_val, 2.5))
            else:
                new_val = max(0.2, min(new_val, 3.0))
            pred[key] = new_val

        settings["predictive"] = pred
        self._record_tune("predictive_weights", before, pred)
        log.add("INFO", f"{SYMBOL_TUNE} Adjusted predictive weights: direction={direction}, step={step:.3f}")

    def _adjust_thresholds(self, direction, step):
        pred = settings.get("predictive", {})
        before = pred.copy()

        rt_warn = pred.get("risk_threshold_warn", 40)
        rt_strict = pred.get("risk_threshold_strict", 70)

        if direction > 0:
            rt_warn = max(10, rt_warn - step * 5)
            rt_strict = max(rt_warn + 5, rt_strict - step * 5)
        else:
            rt_warn = min(80, rt_warn + step * 5)
            rt_strict = min(95, rt_strict + step * 5)

        pred["risk_threshold_warn"] = rt_warn
        pred["risk_threshold_strict"] = rt_strict
        settings["predictive"] = pred

        self._record_tune("risk_thresholds", before, pred)
        log.add("INFO", f"{SYMBOL_TUNE} Adjusted thresholds: warn={rt_warn:.1f}, strict={rt_strict:.1f}")

    def _adjust_folding_threshold(self, direction, step):
        folding = settings.get("folding", {})
        before = folding.copy()
        n = folding.get("unknown_threshold", 5)

        if direction > 0:
            n = min(15, n + int(max(1, step * 3)))
        else:
            n = max(1, n - int(max(1, step * 3)))

        folding["unknown_threshold"] = n
        settings["folding"] = folding
        if fold_manager:
            fold_manager.unknown_threshold = n

        self._record_tune("folding_threshold", before, folding)
        log.add("INFO", f"{SYMBOL_TUNE} Adjusted folding threshold N={n}")

    def _adjust_policies(self, process_fp, process_fn):
        if not process_fp and not process_fn:
            return
        policies = settings.get("process_policies", {}).copy()
        before = policies.copy()

        for pname, count in process_fp.items():
            if count < self.policy_adjust_threshold:
                continue
            key = pname.upper()
            cur = policies.get(key)
            if cur in ("kill", "alert"):
                new = "log" if cur == "alert" else "alert"
                policies[key] = new
                log.add("INFO", f"{SYMBOL_TUNE} Policy softened for {pname}: {cur} -> {new} (FP count={count})")

        for pname, count in process_fn.items():
            if count < self.policy_adjust_threshold:
                continue
            key = pname.upper()
            cur = policies.get(key, "log")
            if cur in ("log", "allow"):
                new = "alert"
            elif cur == "alert":
                new = "kill"
            ...
            if new != cur:
                policies[key] = new
                log.add("INFO", f"{SYMBOL_TUNE} Policy hardened for {pname}: {cur} -> {new} (FN count={count})")

        settings["process_policies"] = policies
        if self.policy_engine:
            self.policy_engine.update_policies(policies)
        self._record_tune("process_policies", before, policies)

    def _record_tune(self, kind, before, after):
        entry = {
            "ts": time.time(),
            "kind": kind,
            "before": before,
    ...
    def _save_tuning_history(self):
        try:
            with open(TUNING_HISTORY_FILE, "w") as f:
                json.dump(self.tuning_history, f, indent=4)
        except Exception:
            pass
...
# -----------------------------
# SIMULATION MANAGER + EVOLUTION ENGINE
# -----------------------------
class SimulationManager:
    """
    Offline evaluator:
    - Takes a slice of risk history.
    - Runs a simulated PredictiveEngine with candidate config.
    - Scores configuration based on FP/FN tags and risk profile.
    """
    def __init__(self):
        pass

    def score_config(self, history, candidate_predictive_cfg):
        if not history:
            return {"score": 0.0, "fp": 0, "fn": 0, "avg_risk": 0.0}

        engine = PredictiveEngine(candidate_predictive_cfg)
        fp = 0
        fn = 0
        risk_sum = 0.0
        count = 0
        ...
        for entry in history:
            event = entry.get("event", "UNKNOWN")
            source = entry.get("source", "sim")
            is_anomaly = entry.get("is_anomaly", False)
            tags = entry.get("tags") or []
            risk = engine.observe_event(event, source, is_anomaly, behavior_score=0.0, tags=tags)
            risk_sum += risk
            count += 1
        ...
        avg_risk = risk_sum / max(count, 1)
        penalty_fp = fp * 1.0
        penalty_fn = fn * 2.0
        penalty_risk = max(0.0, avg_risk - 30) * 0.1

        score = - (penalty_fp + penalty_fn + penalty_risk)
        return {
            "score": score,
            "fp": fp,
            "fn": fn,
...
class EvolutionEngine:
    """
    Evolutionary tuner:
    - Periodically generates candidate predictive configs around current.
    - Evaluates via SimulationManager on recent history.
    - Promotes best candidate if it beats current with margin.
    - Logs and persists evolution choices.
    """
    def __init__(self):
        self.running = False
        self.thread = None
        ...
        self.cfg = settings.get("evolution", {})
        self.enabled = self.cfg.get("enabled", True)
        self.interval = self.cfg.get("interval_seconds", 3600)
        self.candidate_count = self.cfg.get("candidate_count", 4)
        self.history_window = self.cfg.get("history_window", 5000)
        self.explore_strength = self.cfg.get("explore_strength", 0.4)
        self.keep_top_k = self.cfg.get("keep_top_k", 1)
    ...
    def start(self):
        ...
        self.thread = threading.Thread(target=self._run, daemon=True)
        self.thread.start()
        log.add("INFO", f"{SYMBOL_EVOL} EvolutionEngine started (interval={self.interval}s).")

    def stop(self):
        self.running = False
        log.add("INFO", f"{SYMBOL_EVOL} EvolutionEngine stopped.")
    ...
    def _evolve_once(self):
        ...
        if not hist:
            return
        hist_slice = hist[-self.history_window:]

        base_pred = settings.get("predictive", {})
        base_score_info = self.sim.score_config(hist_slice, base_pred)
        base_score = base_score_info["score"]

        candidates = []
        for i in range(self.candidate_count):
            cand = self._mutate_config(base_pred, self.explore_strength)
            info = self.sim.score_config(hist_slice, cand)
            candidates.append((cand, info))

        candidates.sort(key=lambda x: x[1]["score"], reverse=True)
        best_cand, best_info = candidates[0]

        margin = best_info["score"] - base_score
        if margin > 0.5:
            before = base_pred.copy()
            settings["predictive"] = best_cand
            self._record_evolution(before, best_cand, base_score_info, best_info)
            save_settings(settings)
            log.add("INFO", f"{SYMBOL_EVOL} Evolution promoted new predictive config (margin={margin:.2f}, score={best_info['score']:.2f})")
        else:
            log.add("INFO", f"{SYMBOL_EVOL} Evolution kept existing config (best margin={margin:.2f})")

    def _mutate_config(self, base_cfg, strength):
        ...
        for key in ["weight_rarity", "weight_cpu_behavior", "weight_cmd_entropy", "idle_multiplier"]:
            ...
            if key == "idle_multiplier":
                new_val = max(1.0, min(val + delta, 2.5))
            else:
                new_val = max(0.2, min(val + delta, 3.0))
            cand[key] = new_val

        rt_warn = cand.get("risk_threshold_warn", 40)
        rt_strict = cand.get("risk_threshold_strict", 70)
        delta_warn = (random.random() * 2 - 1) * (strength * 5)
        delta_strict = (random.random() * 2 - 1) * (strength * 5)
        rt_warn = max(10, min(rt_warn + delta_warn, 80))
        rt_strict = max(rt_warn + 5, min(rt_strict + delta_strict, 95))
        cand["risk_threshold_warn"] = rt_warn
        cand["risk_threshold_strict"] = rt_strict

        return cand

    def _record_evolution(self, before, after, base_info, best_info):
        entry = {
            "ts": time.time(),
            "before": before,
            "after": after,
    ...
    def _save_evolution_history(self):
        try:
            with open(EVOLUTION_HISTORY_FILE, "w") as f:
...
class LineGUI:
    def __init__(self, root, engine: LineComputer, swarm: SwarmManager,
        ...
        self.root.title("Line Computing Cockpit v13 (Self-Tuning + Evolution)")
    ...
    def _build_layout(self):
        ...
        tune_frame = tkinter.Frame(top_frame)
        tune_frame.pack(side=tkinter.LEFT, padx=5, pady=5)

        evol_frame = tkinter.Frame(top_frame)
        evol_frame.pack(side=tkinter.LEFT, padx=5, pady=5)
        ...
        self.btn_force_tune = tkinter.Button(tune_frame, text="Force Tuning Pass", command=self.force_tune)
        self.btn_force_tune.pack(side=tkinter.LEFT, padx=3)

        self.btn_force_evol = tkinter.Button(evol_frame, text="Force Evolution Pass", command=self.force_evolution)
        self.btn_force_evol.pack(side=tkinter.LEFT, padx=3)
    ...
    def force_tune(self):
        if tuning_manager:
            tuning_manager._tune_once()
            self._append_log(f"{SYMBOL_TUNE} Manual tuning pass executed.")

    def force_evolution(self):
        if evolution_engine:
            evolution_engine._evolve_once()
            self._append_log(f"{SYMBOL_EVOL} Manual evolution pass executed.")

# -----------------------------
# MAIN
# -----------------------------
def main():
    global fold_manager, tuning_manager, evolution_engine
    ...
    tuning_manager_local = TuningManager(policy_engine)
    tuning_manager_local.start()
    tuning_manager = tuning_manager_local

    evolution_engine_local = EvolutionEngine()
    evolution_engine_local.start()
    evolution_engine = evolution_engine_local
    ...
    try:
        ...
        sys_collector.stop()
        replay_controller.stop()
        tuning_manager_local.stop()
        evolution_engine_local.stop()
        save_settings(settings)
