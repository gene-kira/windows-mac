...
- Habit learning (per-process name, with simple time-of-day profiling)
- Predictive-ish anomaly logic (habits + time + trust + network)
...
- Persistence layer (JSON brain file):
    - Admin password (salted hash + salt)
    - Paranoia level, strategy, profiles, network backup path
- Optional network backup of the brain file
- Memory+disk-aware behavior, backs off under load
- Main GUI: Overview / Workers / Events / Process Map
- Decision Mode GUI (admin-only):
    - Password-gated
    - Toggle enforcement (dry run vs real)
    - Adjust paranoia level
    - Save brain now
    - Configure network backup path
    - Change admin password (persisted)
    - Shutdown queen

Usage:
    python hive_guardian_queen.py          # GUI mode
    python hive_guardian_queen.py --service  # service mode (no GUI, console only)
"""
...
class QueenBrain:
    ...
    # -------- Anomaly classification / predictive logic -------- #
    ...
    def _classify_anomaly(self, node: NodeInfo) -> str:
        ...
        hour_activity = profile.activity_at_hour(now_hour) if profile else 0
        if profile and profile.count > 30 and hour_activity < (profile.count * 0.02):
...
class DecisionPanelGUI:
    """
    Small admin-only control panel:
    - Password-gated
    - Toggle enforcement
    - Adjust paranoia
