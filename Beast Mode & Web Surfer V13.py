...
class ResourcePolicy:
    ...
    def choose_engine(self, beast_avg, beast_trend, gpu_free_mb, movidius_ok, coral_ok):
        if beast_avg is None:
            decision = "CPU"
        else:
            hot = beast_avg > 60
            rising = beast_trend is not None and beast_trend > 5.0

            if gpu_free_mb is not None and gpu_free_mb >= 2048 and (hot or rising):
                decision = "GPU"
            elif movidius_ok and hot:
                decision = "MOVIDIUS"
            elif coral_ok and hot:
                decision = "CORAL"
            else:
                decision = "CPU"
...
if IS_GUI:
    ...
    class PredictionGraph(QtWidgets.QWidget):
        """
        Crash-proof embedded prediction graph.
        Handles None, NaN, empty history, bad forecast, and zero-range scaling.
        """
        def __init__(self, parent=None):
            super().__init__(parent)
            self._history = []
    ...
    class SystemWorker(QtCore.QObject):
        ...
        def _compute_beast_metrics(self):
            ...
            forecast = current + 0.5 * slope
            return current, slope, active, forecast
        ...
        def run(self):
            while self._running:
                ...
                try:
                    ...
                    beast_avg, beast_trend, beast_active, beast_forecast = self._compute_beast_metrics()
                    ...
                    metrics = {
                        ...
                        "system_threads": int(system_threads),
