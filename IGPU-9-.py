# golden_star_event_horizon_swarm_civilization.py
...
COLLECTIVE_LEADER_LOG = os.path.join(DIRS["collective"], "leader_log.jsonl")
COLLECTIVE_LEADER_STATE = os.path.join(DIRS["collective"], "leader_state.json")
...
def ensure_directories():
    ...
    if not os.path.exists(POLICY_FILE):
        default_policy = {
            ...
            "last_tune": None,
            "epochs": [],
            "rl_q_table": {},  # swarm-level RL Q-table (state_bucket -> mode -> value)
        }
    ...
    for logname in ["mpu.log", "encode.log", "hash.log", "swarm.log", "policy.log", "collective.log"]:
...
# -------------------------------
# Collective Memory / Policy / Reputation / Behavior
# -------------------------------
...
# -------------------------------
# Consensus-based Swarm Consciousness + Leader + RL
# -------------------------------
class SwarmConsciousness(QThread):
    ...
    def __init__(self, node_id, parent=None):
        ...
        self.rl_alpha = 0.2
        self.rl_gamma = 0.9
        self.last_state_action = None  # (state_bucket, mode)
    ...
    def _load_q_table(self):
        ...
        return policy.get("rl_q_table", {}), policy

    def _save_q_table(self, q_table, policy):
        policy["rl_q_table"] = q_table
        save_policy(policy)
    ...
    def _choose_mode_rl(self, swarm_c):
        ...
        # epsilon-greedy using exploration_rate
    ...
    def _compute_reward(self, swarm_c):
        # simple reward: high stability, low pressure, moderate arousal
        r = 0.0
    ...
    def _update_q_table(self, new_swarm_c):
        ...
        self._save_q_table(q_table, policy)
        log_message("policy", f"RL update: state={state}, mode={mode}, reward={reward:.3f}")
    ...
    def _read_leader_state(self):
        return load_json(COLLECTIVE_LEADER_STATE, {"leader_id": None, "term": 0, "timestamp": None})

    def _write_leader_state(self, leader_id, term):
        data = {"leader_id": leader_id, "term": term, "timestamp": datetime.now().isoformat()}
        save_json(COLLECTIVE_LEADER_STATE, data)

    def _append_leader_log(self, entry):
        try:
            with open(COLLECTIVE_LEADER_LOG, "a", encoding="utf-8") as f:
                f.write(json.dumps(entry) + "\n")
        except Exception:
            pass

    def _run_election_if_needed(self, nodes):
        now = datetime.now()
        leader_state = self._read_leader_state()
        leader_id = leader_state.get("leader_id")
        ts = leader_state.get("timestamp")
        leader_alive = False
        if leader_id and ts:
            try:
        ...
        self.term = leader_state.get("term", 0) + 1
        ...
        self._append_leader_log(entry)

        votes = 1  # vote for self
        total_nodes = max(1, len(nodes))
        # naive: count any vote entry for us in this term
        try:
            with open(COLLECTIVE_LEADER_LOG, "r", encoding="utf-8") as f:
                for line in f:
                    try:
                        e = json.loads(line.strip())
                    except Exception:
                        continue
                    if e.get("type") == "vote" and e.get("term") == self.term and e.get("vote_for") == self.node_id:
                        votes += 1
            # cast our own vote entry
            self._append_leader_log({
                "type": "vote",
                "term": self.term,
                "voter": self.node_id,
        ...
        if votes > total_nodes / 2:
            ...
            self._write_leader_state(self.node_id, self.term)
            self.leader_changed.emit(self.node_id)
            log_message("swarm", f"Consensus leader elected: {self.node_id} (term {self.term})")
        else:
            self.state = "follower"
    ...
    def run(self):
        ...
        while self._running:
            ...
            self._run_election_if_needed(nodes)
            ...
            if leader_id != self.current_leader:
                ...
                if leader_id:
                    ...
                    log_message("swarm", f"Leader changed (consensus): {leader_id}")

            if self.current_leader == self.node_id and self.state == "leader":
...
# -------------------------------
# Task Scheduler (with predictive routing + reputation)
# -------------------------------
class TaskScheduler(QObject):
    ...
    def update_from_telemetry(self, telemetry):
        ...
        # predictive routing: choose route with lowest predicted cost
        enc_cost_mpu = self._predict_cost("encoder", "MPU")
        enc_cost_dgpu = self._predict_cost("encoder", "dGPU")
        if not mpu_enabled:
            encoder_target = "dGPU"
        else:
            encoder_target = "MPU" if enc_cost_mpu <= enc_cost_dgpu else "dGPU"

        comp_cost_mpu = self._predict_cost("compute", "MPU")
        comp_cost_cpu = self._predict_cost("compute", "CPU")
        if not mpu_enabled:
            compute_target = "CPU"
        else:
            compute_target = "MPU" if comp_cost_mpu <= comp_cost_cpu else "CPU"
...
class BorgControlTower(QWidget):
    def __init__(self, telemetry_engine, scheduler, encoder_worker, compute_worker,
                 mpu_manager, swarm_node, swarm_jobs, swarm_consciousness, tray_icon):
        super().__init__()
        ...
        self.setWindowTitle("Borg Collective Control Tower – Golden Star Event Horizon")
        ...
        swarm_consciousness.leader_changed.connect(self.on_leader_changed)
...
def main():
    ...
    scheduler = TaskScheduler(node_id=node_id)
    mpu = MPUManager()
    ...
    swarm_consciousness = SwarmConsciousness(node_id=node_id)
    ...
    window = BorgControlTower(
        ...
        swarm, swarm_jobs, swarm_consciousness, tray_dummy
    )
