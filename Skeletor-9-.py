...
# Holt-Winters-lite (additive smoothing)
...
# =========================
# Scenario planner with adaptive weights (integrated)
# =========================
@dataclass
...
# =========================
# Feature extraction (slope, curvature, seasonality hint, burst density)
# =========================
@dataclass
class Features:
    slope: float = 0.0
...
# =========================
# Counterfactual simulator & policy search
# =========================
...
# =========================
# Autonomous engine (ultra-predictive v2)
# =========================
class AutonomousLightDark:
    def __init__(self, channels: List[str], state_path: str = "light_dark_state.json", log_path: str = "light_dark_telemetry.jsonl"):
        ...
        self.history: Dict[str, RingBuffer] = {ch: RingBuffer(22000) for ch in channels}
        ...
        self.causal_links: List[Tuple[str, str, int, float, float]] = []
        ...
        try:
            if os.path.exists(self.state_path):
                with open(self.state_path, "r", encoding="utf-8") as f:
                    ...
                    ps.version = data.get("version", 11)
    ...
    def _schedule_forecasts(self):
        for ch in self.channels:
            series = self.history[ch].tail(2400)
            if series:
            ...
            if regime == "spike":
                wts.kalman = 0.5; wts.ewma_linear = 0.35; wts.holt_winters = 0.15
            elif regime == "drift":
                wts.kalman = 0.35; wts.ewma_linear = 0.40; wts.holt_winters = 0.25
            else:
                wts.kalman = 0.4; wts.ewma_linear = 0.35; wts.holt_winters = 0.25
            ...
            def f_short():
                ...
                fh, loh, hih = holt_winters(series, steps=self.pred_horizons["S"])
                ff = ensemble_blend([fk, fS, fh], wts)
                nmin = min(len(lok), len(loS), len(loh))
                lo2 = [min(1.0, max(0.0, wts.kalman*lok[i] + wts.ewma_linear*loS[i] + wts.holt_winters*loh[i])) for i in range(nmin)]
                nmin2 = min(len(hik), len(hiS), len(hih))
                hi2 = [min(1.0, max(0.0, wts.kalman*hik[i] + wts.ewma_linear*hiS[i] + wts.holt_winters*hih[i])) for i in range(nmin2)]
                return ff, lo2, hi2
            def f_mid():
    ...
    def _schedule_causality(self):
        pairs = [("cpu","load"), ("net","trust"), ("disk","mem"), ("cpu","mem"), ("load","mem"), ("cpu","context"), ("mem","context")]
        ...
        while self.running:
            ...
            try:
                ...
                for ch in self.channels:
                    ...
                    if loS and hiS:
                        n = min(12, len(loS))
                        tightness_list.append(sum(hiS[i]-loS[i] for i in range(n))/max(1,n))
                band_tightness = min(1.0, (sum(tightness_list)/max(1, len(tightness_list))) if tightness_list else 0.0)
                ...
                self.pred_anomaly_risk = fuse_risk(volatility, band_tightness, causality_gain, self.calibrator.precision, self.calibrator.recall)
                ...
                if eta_pack:
                    ...
                    prop = self.leadlag.propagate_eta("cpu", self.thresholds["load"].high_on, self.bands, hz=self.rate_hz)
                    if prop:
                ...
                cf_regime = self.profiles["load"].regime
                loM, hiM = self.bands.get("load", {}).get("M", ([],[]))
                base_risk = self.pred_anomaly_risk
                risk_mut = expected_risk(self.history["load"].tail(200), loM, hiM, base_risk, "mutate")
                risk_rev = expected_risk(self.history["load"].tail(200), loM, hiM, base_risk, "revert")
                risk_hold = expected_risk(self.history["load"].tail(200), loM, hiM, base_risk, "hold")
                policies = sorted([
                    PolicyChoice("mutate", self.pred_strategy_eta_s or 6.0, risk_mut, 0.0),
                    PolicyChoice("revert", 6.0, risk_rev, 0.0),
                    PolicyChoice("hold", 0.0, risk_hold, 0.0)
                ], key=lambda p: p.expected_risk)
                if len(policies) >= 2:
                    policies[0].margin_vs_next = max(0.0, policies[1].expected_risk - policies[0].expected_risk)
                ...
                payload = {
                    ...
                    "counterfactuals": {"mutate_risk": risk_mut, "revert_risk": risk_rev, "hold_risk": risk_hold},
    ...
    def _refresh_plans(self, now: float):
        ...
        dwell_ok = (now - self.last_strategy_change_ts) > (14.0 if high_risk else 10.0)
        if high_risk and eta is not None and eta_conf > 0.60 and not has_mutate and dwell_ok:
            self.plan_stack.append(StrategyPlan("mutate", eta, "hazard_eta_high+dual_trigger", now))
        elif low_risk and not has_revert and dwell_ok:
            self.plan_stack.append(StrategyPlan("revert", 6.0, "risk_low+dual_decay", now))
    ...
    def _judge_repair_and_think(self, protected: ProtectedState) -> List[Dict]:
        ...
        for a,b,lag,corr,imp in self.causal_links[:6]:
        ...
        if len(self.thought_log) > 3400:
            self.thought_log = self.thought_log[-3400:]
        try:
...
class ScientificGUI:
    def __init__(self, engine: AutonomousLightDark):
        self.engine = engine
        self.root = tk.Tk()
        self.root.title("Predictive Skeleton Monitor (Ensemble, Lead-Lag, Counterfactual Policy)")
        ...
        self.summary_policy = tk.StringVar(value="Policy: —")
        ...
        ttk.Label(self.summary, textvariable=self.summary_policy, font=("Consolas", 8)).pack(anchor="w")
        ...
        for i, ch in enumerate(engine.channels):
            c_series = tk.Canvas(self.chart_grid, width=320, height=150, bg="#1b1b1b", highlightthickness=0)
            c_series.grid(row=(i // 3)*3, column=i % 3, padx=3, pady=3, sticky="nsew"); self.series_canvases[ch] = c_series
            c_conf = tk.Canvas(self.chart_grid, width=320, height=50, bg="#151515", highlightthickness=0)
            c_conf.grid(row=(i // 3)*3+1, column=i % 3, padx=3, pady=3, sticky="nsew"); self.conf_canvases[ch] = c_conf
            c_ag = tk.Canvas(self.chart_grid, width=320, height=16, bg="#121212", highlightthickness=0)
        ...
        self.timeline_canvas = tk.Canvas(self.analytics, width=1080, height=60, bg="#1b1b1b", highlightthickness=0)
        self.timeline_canvas.pack(fill="x", padx=4, pady=4)
        self.heat_canvas = tk.Canvas(self.analytics, width=1080, height=160, bg="#1b1b1b", highlightthickness=0)
        self.heat_canvas.pack(fill="x", padx=4, pady=4)
        self.anomaly_canvas = tk.Canvas(self.analytics, width=1080, height=45, bg="#151515", highlightthickness=0)
        ...
        self.quality_canvas = tk.Canvas(self.analytics, width=1080, height=40, bg="#151515", highlightthickness=0)
        ...
        self.graph_canvas = tk.Canvas(self.graph_tab, width=1080, height=420, bg="#1b1b1b", highlightthickness=0)
        ...
        for col in ("label","conclusion","confidence","evidence","notes","ts"):
            ...
            self.th_tree.column(col, width=120 if col!="evidence" else 260, stretch=True)
    ...
    def refresh(self):
        ...
        if payload:
            ...
            policy_str = " | ".join(f"{p['action']}→risk≈{p['expected_risk']:.2f}{' Δ'+str(round(p.get('margin_vs_next',0.0),2)) if p.get('margin_vs_next',0.0)>0 else ''}" for p in pol[:3])
            self.summary_cf.set(f"Counterfactuals: mutate≈{cf.get('mutate_risk', float('nan')):.2f} | revert≈{cf.get('revert_risk', float('nan')):.2f} | hold≈{cf.get('hold_risk', float('nan')):.2f}")
            self.summary_policy.set(f"Policy choice: {policy_str or '—'}")
    ...
    def draw_quality(self, canvas: tk.Canvas, cal: Dict[str, float]):
        ...
        canvas.delete("all")
        w = int(canvas.winfo_width() or 1080); h = int(canvas.winfo_height() or 40)
    ...
    def draw_series(self, canvas: tk.Canvas, series: List[float], th: Thresholds,
        ...
        w = int(canvas.winfo_width() or 320); h = int(canvas.winfo_height() or 150)
        canvas.delete("all")
    ...
    def draw_confidence(self, canvas: tk.Canvas, conf_series: List[float]):
        w = int(canvas.winfo_width() or 320); h = int(canvas.winfo_height() or 50)
        canvas.delete("all")
        canvas.create_rectangle(0, 0, w, h, fill="#151515", outline="")
        if conf_series:
            n = len(conf_series)
    ...
    def draw_agreement(self, canvas: tk.Canvas, preds: Dict[str, List[float]]):
        w = int(canvas.winfo_width() or 320); h = int(canvas.winfo_height() or 16)
        canvas.delete("all")
        ...
        canvas.create_text(w-86, 2, text=f"agree≈{agree:.2f}", anchor="nw", fill="#cccccc", font=("Consolas", 8))

    def draw_timeline(self, canvas: tk.Canvas, events: List[Tuple[float, str, str]]):
        w = int(canvas.winfo_width() or 1080); h = int(canvas.winfo_height() or 60)
        canvas.delete("all")
        canvas.create_rectangle(0, 0, w, h, fill="#1b1b1b", outline="")
    ...
    def draw_heatmap(self, canvas: tk.Canvas, series_map: Dict[str, List[float]]):
        canvas.delete("all")
        w = int(canvas.winfo_width() or 1080); h = int(canvas.winfo_height() or 160)
        canvas.create_rectangle(0, 0, w, h, fill="#1b1b1b", outline="")
    ...
    def draw_anomaly_bar(self, canvas: tk.Canvas, anomaly_series: List[float]):
        w = int(canvas.winfo_width() or 1080); h = int(canvas.winfo_height() or 45)
        canvas.delete("all")
        canvas.create_rectangle(0, 0, w, h, fill="#151515", outline="")
    ...
    def draw_leadlag(self, canvas: tk.Canvas, graph: Dict[str, Dict[str, Dict]]):
        canvas.delete("all")
        w = int(canvas.winfo_width() or 1080); h = int(canvas.winfo_height() or 420)
        canvas.create_rectangle(0, 0, w, h, fill="#1b1b1b", outline="")
