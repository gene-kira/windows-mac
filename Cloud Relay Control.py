...
def relay_server_main():
    print(f"[RELAY] Starting TCP relay on {RELAY_HOST}:{RELAY_PORT}")
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.bind((RELAY_HOST, RELAY_PORT))
    s.listen(50)
    print("[RELAY] Ready. Waiting for connections...")
    while True:
        conn, addr = s.accept()
        t = threading.Thread(target=handle_relay_client, args=(conn, addr), daemon=True)
        t.start()

def start_relay_server_in_background():
    t = threading.Thread(target=relay_server_main, daemon=True)
    t.start()
    return t
...
# Default local endpoints (now backed by real relay)
LOCAL_TCP_HOST = "127.0.0.1"
LOCAL_TCP_PORT = RELAY_PORT

LOCAL_WS_URL = "ws://localhost:8765"
LOCAL_HTTP_URL = "http://localhost:5000/relay"

# Example cloud defaults (point to same relay for now)
CLOUD_TCP_HOST = "127.0.0.1"
CLOUD_TCP_PORT = RELAY_PORT
CLOUD_WS_URL = "ws://127.0.0.1:8765"
CLOUD_HTTP_URL = "http://127.0.0.1:5000/relay"

# Internet candidates (include our local relay as a “cloud” candidate)
INTERNET_CANDIDATES = [
    (f"tcp://127.0.0.1:{RELAY_PORT}", Protocol.TCP),
]

# Congestion thresholds (ms)
CONGESTION_MED = 80.0
...
# Default auto re‑optimizer config
DEFAULT_AUTO_REOPT = {
    "enabled": True,
    "interval_minutes": 5,
...
DEFAULT_DISCOVERY_CONFIG = {
    "dns_srv_domain": "localhost",  # harmless default
    "api_directory_url": "",        # empty by default
}
...
class RelayClient(QtCore.QObject):
    ...
    def _contact_http(self, url: str):
        headers = {}
        r = find_known_relay(self.relays_db, url, Protocol.HTTP)
        if r and r.get("auth_token"):
            headers["Authorization"] = f"Bearer {r['auth_token']}"
        resp = requests.get(url, timeout=1.0, headers=headers)
        if not resp.ok:
            raise RuntimeError(f"HTTP status {resp.status_code}")

    def _contact_ws(self, url: str):
        async def ping_ws():
            async with websockets.connect(url, ping_timeout=1.0) as ws:
                await ws.send("PING")
                await ws.recv()
...
class AutoSearchWorker(QtCore.QObject):
    ...
    def run(self):
        ...
        try:
            ...
            self.progress.emit("Probing internet candidates...")
    ...
    def _scan_local_tcp(self):
        ...
        for port in ports_to_try:
            ...
            try:
                ...
                score, trust = self._score_candidate(endpoint, Protocol.TCP, latency, loss, congestion, hist, secure=False)
                discovered.append(DiscoveredRelay(endpoint, Protocol.TCP, latency, loss, congestion, score, trust, "LOCAL"))
    ...
    def _probe_internet_candidates(self):
        ...
        for endpoint, protocol in INTERNET_CANDIDATES:
            ...
            try:
                ...
                secure = endpoint.startswith("https://") or endpoint.startswith("wss://")
    ...
    def _dns_srv_discovery(self):
        ...
        for rdata in answers:
            ...
            try:
                ...
                hist = get_historical_stability(self.relays_db, endpoint, Protocol.TCP)
                score, trust = self._score_candidate(endpoint, Protocol.TCP, latency, loss, congestion, hist, secure=False)
                discovered.append(DiscoveredRelay(endpoint, Protocol.TCP, latency, loss, congestion, score, trust, "DNS-SRV"))
        ...
        for host in hosts:
            for port in ports:
                ...
                try:
                    ...
                    hist = get_historical_stability(self.relays_db, endpoint, Protocol.TCP)
                    score, trust = self._score_candidate(endpoint, Protocol.TCP, latency, loss, congestion, hist, secure=False)
                    discovered.append(DiscoveredRelay(endpoint, Protocol.TCP, latency, loss, congestion, score, trust, "PROMETHEUS"))
...
class AutoReoptimizer(QtCore.QObject):
    ...
    def __init__(self, relays_db, status: RelayStatus, parent=None):
        ...
        self.interval_minutes = int(cfg.get("interval_minutes", 5))
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Relay Control Cockpit")
    ...
    def _switch_with_validation(self, mode: RelayMode, protocol: Protocol, endpoint: str, source: str):
        ...
        if not ok:
            ...
            return False
        self.client.set_mode_and_protocol(mode, protocol, endpoint)
        self.hud_overlay.show_alert(f"Connected to: {endpoint}", QtGui.QColor(0, 255, 0), 1200, "green")
    ...
    def on_custom_clicked(self):
        ...
        if protocol == Protocol.TCP:
            ...
            text, ok2 = QtWidgets.QInputDialog.getText(self, "Custom WebSocket URL", "ws://...", text="ws://localhost:8765")
        else:
            text, ok2 = QtWidgets.QInputDialog.getText(self, "Custom HTTP URL", "http://...", text="http://localhost:5000/relay")
...
def main():
    ...
    start_relay_server_in_background()
