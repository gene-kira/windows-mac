#!/usr/bin/env python3
# ============================================================
# SYSTEM NERVE CENTER – HybridBrain + Tri-Stance + MultiGame v4.0 "Event Horizon"
...
class PredictionEngine:
    ...
    def _engine_votes(self) -> Tuple[float, Dict[str, float], str, Dict[str, float]]:
        ...
        for k, v in votes.items():
            w = weights.get(k, 0.0)
            # Drift-aware fusion: dampen chaotic features when drift is high, trust horizon/meta more
...
class NerveCenterApp(tk.Tk):
    ...
    def cmd_save_reboot_memory(self):
        ...
        meta = {"ts": time.time(), "version": "4.0-event-horizon"}
    ...
    def _try_load_reboot_memory(self):
        ...
        try:
            ...
            # pattern + horizon memory restore
    ...
    def _save_settings(self):
        ...
        path = os.path.join(os.path.expanduser("~"), ".nerve_center_settings_event.json")
        try:
    ...
    def _bg_loop(self):
        while True:
            try:
                ...
                # Event Horizon update: previous context gets current risk as "future"
