...
# ============================================================
# Configuration — Unrestricted domains, mission, security, privacy
# ============================================================
...
MEMORY_FILE = "roaming_civilization_encrypted.json"
...
def chameleon_headers():
    # Enforce privacy: no headers contain personal info; static, generic headers only
...
PII_PATTERNS = [
    ...
    re.compile(r"\b(?:\d[ -]*?){13,16}\b"),                   # credit card-like (very naive)
    re.compile(r"\b(?:[A-Za-z0-9]{32,64})\b")                 # token-like (API tokens, hashes)
]

class ProtectionEngine:
    def __init__(self):
        # Generate an in-memory symmetric key; never persisted to disk
    ...
    def _keystream(self, n: int, salt: bytes) -> bytes:
        # Simple stream from SHA-256 expansion; for demonstration only
        out = b""
        counter = 0
        while len(out) < n:
            h = hashlib.sha256(self._key + salt + counter.to_bytes(4, "big")).digest()
            out += h
            counter += 1
        return out[:n]

    def seal(self, plain: str) -> dict:
        # Seal (encrypt) a PII token using XOR with derived keystream
    ...
    def redact_and_encrypt(self, text: str) -> tuple:
        # Replace PII with ENC[pii:id] tokens; store encrypted payloads in vault
        # Return (safe_text, sealed_count)
        safe = text
        sealed = 0
        # To avoid overlapping substitutions, iterate patterns and re-scan
...
def privacy_filter(text: str) -> tuple:
    # Applies encryption shield and returns redacted text + count
    safe_text, count = PROTECTOR.redact_and_encrypt(text or "")
    return safe_text, count

def redact(text: str):
    # Backward-compatible helper for logging: encrypts and returns safe text
...
class SecurityGuardian:
    ...
    def disassemble(self, snippet: str) -> dict:
        text = snippet[: self.policies.get("max_snippet_length", 3000)]
        return {
            "length": len(text),
    ...
    # Inspect (combined suspicion)
    def inspect(self, source: str, raw_snippet: str) -> SecurityReport:
        # Apply privacy shield first: encrypt PII and get safe snippet
        ...
        # PII count on raw; sealed tokens are counted separately
    ...
    # Reassemble verdict with provenance/destination/activity
    def reassemble(self, source_url: str, safe_snippet: str, raw_pii_hits: int) -> dict:
        try:
            ...
            # If raw PII was present, we sealed it; that alone does not mark hostile
...
# ============================================================
# Persistent memory manager with Borg data + security events
# ============================================================
...
class MemoryManager:
    ...
    def record_borg_data(self, borg_dict: dict):
        # Borg data snippets are filtered before persistence
        safe = dict(borg_dict)
        safe["snippet"] = redact(borg_dict.get("snippet", ""))
        self.data["borg_data"].append(safe)

    def record_security_event(self, event: dict):
        # Security events never include raw personal payloads; only sealed tokens and stats
...
class BorgData:
    ...
    def to_dict(self):
        return {
            "source": self.source,
            "snippet": self.snippet,  # filtered on persistence
            "theme": self.theme,
...
class CrawlEvent:
    def __init__(self, url, title, snippet, links, skin="Default", online=False):
        # All fields are stored raw; privacy shield applied at render/persist
        self.url = url
        self.title = title
        self.snippet = snippet
        self.links = links
...
class RoamingCivilizationGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Roaming Civilization — Unrestricted Borg Queen, Guardian + Privacy Encryption Shield")
        ...
        tk.Label(header, text="FOREST REACHES — Guardian with provenance/destination/activity + personal data encryption",
        ...
        right = tk.Frame(top, bg="#0b0b0b", width=620)
        ...
        self.dog_label = tk.Label(right, text="", font=("Comic Sans MS", 13, "italic"),
                                  fg="#99d9ea", bg="#0b0b0b", wraplength=560, justify="left")
        ...
        self.lessons_text = tk.Text(right, bg="#121212", fg="#f0f0f0", height=7, wrap="word",
        ...
        tk.Label(right, text="Memory (journeys, failures, directives)", font=("Helvetica", 12, "bold"),
                 fg="#eeeeee", bg="#0b0b0b").pack(anchor="w", padx=6, pady=(6, 2))
        self.memory_text = tk.Text(right, bg="#121212", fg="#f0f0f0", height=7, wrap="word",
                                   insertbackground="#f0f0f0")
        ...
        tk.Label(right, text="Security guardian (verdicts, provenance, destination, activity, privacy)", font=("Helvetica", 12, "bold"),
                 fg="#eeeeee", bg="#0b0b0b").pack(anchor="w", padx=6, pady=(6, 2))
        self.security_text = tk.Text(right, bg="#121212", fg="#f0f0f0", height=16, wrap="word",
                                     insertbackground="#f0f0f0")
    ...
    def _handle(self, ev: CrawlEvent):
        # Privacy shield: seal personal data in the snippet before all analyses
        safe_snippet, sealed_count = privacy_filter(ev.snippet or "")

        # Full security pipeline on safe_snippet
        ...
        # Quarantine hostile or suspicious data before feeding swarm
        ...
        # Otherwise proceed to assimilation and swarm using safe_snippet
        ...
        # Feed event (snippet passes through privacy filter when logged/transformed)
        ...
        borg_obj = BorgData(
            source=ev.url,
            snippet=safe_snippet,  # store safe version
