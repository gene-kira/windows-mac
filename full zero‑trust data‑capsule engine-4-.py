...
# ============================================================
# Hardware root of trust (stub)
# ============================================================

class RootOfTrustOrgan:
    """
    Stub for hardware root of trust.
    In real deployment, this would talk to TPM / HSM / secure element.
    """
    ...
    def get_device_secret(self) -> bytes:
        ...
        # In real hardware-backed mode, fetch from TPM/HSM.
        # Here we simulate with a local file-based secret.
...
# ============================================================
# Swarm coordinator (in-process HTTPS-capable HTTP server)
# ============================================================
    ...
    def do_POST(self):
        ...
        if self.path == "/policy":
            ...
            if not swarm_id or not node_id:
                self._send_json(400, {"error": "missing_fields"})
                return
            # Single-leader consensus: only node_id == "leader" can update
            if node_id != "leader":
...
# ============================================================
# Swarm sync organ (networked, HTTPS-enforced)
# ============================================================
...
class SwarmSyncOrgan:
    ...
    def __init__(self, swarm_id: str, node_id: str, coord_url: str):
    ...
    def register_peer(self, info: Dict[str, Any]):
        ...
        try:
            r = requests.post(f"{self.coord_url}/register", json=payload, timeout=3, verify=True)
            r.raise_for_status()
            data = r.json()
    ...
    def push_policy(self, policy_blob: Dict[str, Any]):
        try:
            ...
            r = requests.post(f"{self.coord_url}/policy", json=payload, timeout=3, verify=True)
            r.raise_for_status()
            data = r.json()
            version = data.get("version", 0)
            blob = data.get("policy", {})
...
# ============================================================
# Consensus organ (single-leader consensus wrapper)
# ============================================================

class ConsensusOrgan:
    """
    Simple single-leader consensus:
      - only leader node can push policy
      - followers only pull
    """
    ...
    def update_policy(self, policy_blob: Dict[str, Any]):
        if not self.is_leader:
            print("[Consensus] Not leader; refusing to update policy")
            return
...
class PolicyOrgan:
    def __init__(self, audit_organ: AuditOrgan, consensus: ConsensusOrgan):
...
class CapsuleRuntime:
    def __init__(self, swarm_id: str = "default-swarm", node_id: str = "node-1", is_leader: bool = False):
        ...
        self.consensus = ConsensusOrgan(self.swarm_sync, is_leader=is_leader)
        self.policy = PolicyOrgan(self.audit, self.consensus)
...
def start_gui(runtime: CapsuleRuntime):
    ...
    # No authentication: show console directly
    main_window.show()
