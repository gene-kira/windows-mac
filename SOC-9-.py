#!/usr/bin/env python3
"""
Hive Guardian - ASI Queen v11
(Self-contained Online Learning + Admin Tab + Risk Evolution Graph + Slope-aware Prediction)
...
- Queen brain + workers:
    ...
    - Swarm sync (multi-node state sharing, simple HTTP-based)
- Habit learning (per-process, time-of-day aware)
- Predictive-ish risk scoring:
    - Behavior vs habits (CPU/MEM ratios)
    - Time-of-day anomaly
    - Network anomaly
    - Trust tier (system/installed/user/temp) + dynamic trust evolution
    - Global load + file spike context
    - Cluster distance (behavioral clustering)
    - Signature change flags (hashes)
- Threat timeline:
    - Per-identity chain of events (risk over time, actions, signals)
    - Risk evolution graph per identity in GUI
...
- Persistence (JSON brain file):
    ...
    - Paranoia level, strategy, profiles, signatures, trust, cluster labels,
      network backup path, swarm config, online-ML weights
- Optional network backup of brain file
- Memory+disk-aware behavior
- Swarm sync hooks (simple JSON push/pull endpoint)
- Self-contained online learning:
    - Small weight vector over risk features
    - Learns from own actions to refine risk scores over time
    - Tuned learning rates, target risk zones, normalization, weight decay
...
- Main GUI:
    ...
    - Events
    - Process Map
    - Threat Timeline (with risk evolution graph)
    - Admin tab (admin-only controls, locked until password entered)
...
@dataclass
class TrustProfile:
    identity: str  # name|exe
    base_tier: str  # system/installed/user/temp/unknown
    dynamic_trust: float = 0.5  # 0..1
    good_events: int = 0
    bad_events: int = 0
    ...
    def trust_score(self) -> float:
        ...
        base = base_map.get(self.base_tier, 0.3)
        return max(0.0, min(1.0, 0.5 * base + 0.5 * self.dynamic_trust))

    def register_good(self):
        self.good_events += 1
        self.dynamic_trust = min(1.0, self.dynamic_trust + 0.01)

    def register_bad(self):
        self.bad_events += 1
        self.dynamic_trust = max(0.0, self.dynamic_trust - 0.05)
...
class QueenBrain:
    def __init__(self, brain_path: Optional[str] = None):
        ...
        # Swarm config
        self.swarm_endpoint: Optional[str] = None
        self.swarm_node_id: str = platform.node()
        ...
        self.global_cpu_history = deque(maxlen=20)
        ...
        self.signature_hashes: Dict[str, str] = {}
        self.cluster_labels: Dict[str, int] = {}
        self.cluster_distances: Dict[str, float] = {}
        ...
        self.threat_timeline: Dict[str, ThreatRecord] = {}

        # Online "ML" (simple linear adjustment)
        self.ml_enabled: bool = False
        # Features: [cpu_ratio, mem_ratio, time_anomaly, net_anomaly,
        #            recent_file_spikes, global_stress, signature_anomaly,
        #            cluster_anomaly, trust_score]
        self.ml_weights: List[float] = [0.0] * 9
        ...
        self.cluster_thread = threading.Thread(target=self._clustering_loop, daemon=True)
    ...
    def _load_brain(self):
        ...
        self.swarm_endpoint = data.get("swarm_endpoint")
        self.swarm_enabled = bool(self.swarm_endpoint)
        ...
        trust_data = data.get("trust_profiles", {})
        for identity, t in trust_data.items():
            self.trust_profiles[identity] = TrustProfile(
                identity=identity,
                base_tier=t.get("base_tier", "unknown"),
                dynamic_trust=t.get("dynamic_trust", 0.5),
                good_events=t.get("good_events", 0),
                bad_events=t.get("bad_events", 0),
            )

        self.signature_hashes = data.get("signature_hashes", {})
        self.cluster_labels = {k: int(v) for k, v in data.get("cluster_labels", {}).items()}
        self.cluster_distances = {k: float(v) for k, v in data.get("cluster_distances", {}).items()}

        self.ml_enabled = data.get("ml_enabled", False)
        self.ml_weights = data.get("ml_weights", [0.0] * 9)
        if len(self.ml_weights) != 9:
            self.ml_weights = [0.0] * 9
    ...
    def set_swarm_endpoint(self, endpoint: Optional[str]):
        self.swarm_endpoint = endpoint or None
        self.swarm_enabled = bool(self.swarm_endpoint)
        self._save_brain()
        self.log(f"[ADMIN] Swarm endpoint updated to: {self.swarm_endpoint or 'None'}")

    def set_ml_enabled(self, enabled: bool):
        self.ml_enabled = enabled
        self._save_brain()
        self.log(f"[ADMIN] Online ML-enhanced risk set to: {self.ml_enabled}")
    ...
    def _brain_to_dict(self) -> dict:
        data = {
            ...
            "swarm_endpoint": self.swarm_endpoint,
        ...
        for identity, t in self.trust_profiles.items():
            data["trust_profiles"][identity] = {
                "base_tier": t.base_tier,
                "dynamic_trust": t.dynamic_trust,
    ...
    # -------- Habits / Trust / Signatures -------- #

    def _get_identity_for_node(self, node: NodeInfo) -> str:
        exe = node.extra.get("exe") or ""
        name = node.extra.get("name", node.label)
        if exe:
            return f"{name}|{exe}"
        return name

    def _get_or_create_trust_profile(self, identity: str, base_tier: str) -> TrustProfile:
        tp = self.trust_profiles.get(identity)
        if tp is None:
            tp = TrustProfile(identity=identity, base_tier=base_tier)
            self.trust_profiles[identity] = tp
        return tp
    ...
    def _maybe_hash_executable(self, exe_path: str) -> Tuple[Optional[str], bool]:
        if not exe_path or not os.path.isfile(exe_path):
            return None, False

        known_hash = self.signature_hashes.get(exe_path)
        do_hash = False
        if known_hash is None:
            do_hash = True
        else:
            do_hash = (uniform(0, 1) < 0.05)
        ...
        self.signature_hashes[exe_path] = new_hash
    ...
    def _apply_ml_adjustment(self, base_risk: float, features: List[float]) -> float:
        if not self.ml_enabled:
            return base_risk
        if len(features) != len(self.ml_weights):
            return base_risk
        adjustment = sum(w * x for w, x in zip(self.ml_weights, features))
        return max(0.0, min(20.0, base_risk + adjustment))

    def _update_ml_weights(self, risk: float, features: List[float], action: str):
        if not self.ml_enabled:
            return
        if len(features) != len(self.ml_weights):
            return
        ...
        decay = 0.001
        for i in range(len(self.ml_weights)):
            self.ml_weights[i] *= (1.0 - decay)

        norm_features = [max(-3.0, min(3.0, x)) for x in features]

        for i in range(len(self.ml_weights)):
            self.ml_weights[i] += lr * error * norm_features[i]

    # -------- Clustering -------- #

    def _extract_features_for_identity(self, identity: str) -> Optional[List[float]]:
        name = identity.split("|", 1)[0]
        profile = self.process_profiles.get(name)
        if not profile or profile.count < 5:
            return None

        trust_profile = self.trust_profiles.get(identity)
        trust_score = trust_profile.trust_score() if trust_profile else 0.5
        cluster_distance = self.cluster_distances.get(identity, 0.0)
    ...
    def _clustering_loop(self):
        while self.running:
            time.sleep(180)
            try:
                self._run_clustering_once()
            except Exception as e:
                self.log(f"[CLUSTER] Error: {e}")

    def _run_clustering_once(self):
        identities = list(self.trust_profiles.keys())
        features = []
        ids = []
        for identity in identities:
            f = self._extract_features_for_identity(identity)
            if f is None:
                continue
            features.append(f)
            ids.append(identity)
        ...
        k = min(5, max(2, len(features) // 20))
        centers = [features[i][:] for i in range(k)]
        ...
        for _ in range(5):
            ...
            for idx, f in enumerate(features):
                best_j = 0
                best_d = dist2(f, centers[0])
                for j in range(1, k):
                    d = dist2(f, centers[j])
                    if d < best_d:
                        best_d = d
                        best_j = j
                assignments[best_j].append(idx)
            ...
            for j in range(k):
                ...
                dim = len(features[0])
                avg = [0.0] * dim
                for idx in assignments[j]:
                    for d in range(dim):
                        avg[d] += features[idx][d]
                count = len(assignments[j])
            ...
            for idx, identity in enumerate(ids):
                ...
                for j in range(1, k):
                    ...
                    if d < best_d:
                        best_d = d
                        best_j = j
                self.cluster_labels[identity] = best_j
                self.cluster_distances[identity] = best_d ** 0.5

        self.log(f"[CLUSTER] Updated clusters for {len(ids)} identities into {k} groups.")
    ...
    def _swarm_loop(self):
        while self.running:
            time.sleep(300)
            try:
                if self.swarm_enabled and self.swarm_endpoint:
                    self._swarm_sync_once()
            except Exception as e:
                self.log(f"[SWARM] Error: {e}")

    def _build_swarm_payload(self) -> dict:
        with self.lock:
            trust_summary = {
                identity: {
                    "base_tier": tp.base_tier,
                    "dynamic_trust": tp.dynamic_trust,
                    "good_events": tp.good_events,
                    "bad_events": tp.bad_events,
                }
                for identity, tp in self.trust_profiles.items()
            }
            sigs = self.signature_hashes.copy()
            actions = list(self.recent_actions[-50:])
        return {
            "node_id": self.swarm_node_id,
            "trust_profiles": trust_summary,
            "signature_hashes": sigs,
            "recent_actions": actions,
        }

    def _merge_swarm_state(self, data: dict):
        remote_trust = data.get("trust_profiles", {})
        remote_sigs = data.get("signature_hashes", {})

        with self.lock:
            for identity, t in remote_trust.items():
                local = self.trust_profiles.get(identity)
                if local is None:
                    self.trust_profiles[identity] = TrustProfile(
                        identity=identity,
                        base_tier=t.get("base_tier", "unknown"),
                    ...
                    local.good_events = max(local.good_events, t.get("good_events", 0))
                    local.bad_events += t.get("bad_events", 0)
                    local.dynamic_trust = (local.dynamic_trust + t.get("dynamic_trust", 0.5)) / 2.0

            for exe_path, hval in remote_sigs.items():
                if exe_path not in self.signature_hashes:
                    self.signature_hashes[exe_path] = hval

        self.log("[SWARM] Merged remote trust/signature data.")

    def _swarm_sync_once(self):
        payload = self._build_swarm_payload()
        try:
            resp = requests.post(self.swarm_endpoint, json=payload, timeout=5)
            if resp.status_code == 200:
                data = resp.json()
                self._merge_swarm_state(data)
            else:
                self.log(f"[SWARM] Non-200 response: {resp.status_code}")
        except Exception as e:
            self.log(f"[SWARM] Sync failed: {e}")

    # -------- Risk scoring & threat timeline -------- #

    def _compute_risk_features(self, node: NodeInfo) -> Tuple[List[float], float, float]:
        name = node.extra.get("name", node.label)
        profile = self.process_profiles.get(name)
        cpu = node.cpu
        mem = node.memory

        identity = self._get_identity_for_node(node)
        trust_profile = self.trust_profiles.get(identity)
        trust_score = trust_profile.trust_score() if trust_profile else 0.5
        ...
        exe_path = node.extra.get("exe") or ""
        _, sig_changed = self._maybe_hash_executable(exe_path)
        signature_anomaly = 1.5 if sig_changed else 0.0

        cluster_distance = self.cluster_distances.get(identity, 0.0)
        cluster_anomaly = min(1.5, cluster_distance / 50.0)

        behavior_component = max(cpu_ratio, mem_ratio) - 1.0
        behavior_component = max(0.0, behavior_component)

        base = 0.0
        base += behavior_component * 1.2
        base += time_anomaly * 0.8
        base += net_anomaly * 1.0
        base += recent_file_spikes * 0.5
        base += global_stress * 0.7
        base += signature_anomaly * 1.2
        base += cluster_anomaly * 1.0

        trust_factor = 1.5 - trust_score
        base *= trust_factor

        base *= (0.5 + self.paranoia_level)
        base *= (0.5 + self.current_strategy.enforcement_aggressiveness)

        base = max(0.0, min(base, 20.0))
        ...
        return features, base, trust_score

    def _compute_risk_score(self, node: NodeInfo) -> Tuple[float, List[float]]:
        features, base_risk, _ = self._compute_risk_features(node)
        risk = self._apply_ml_adjustment(base_risk, features)
        return risk, features

    def _risk_to_action(self, risk: float, name: str, identity: str) -> str:
        ...
        # Predictive adjustment: consider risk trend (slope) for this identity
        tr = self.threat_timeline.get(identity)
        if tr and len(tr.events) >= 3:
            r0 = tr.events[0].risk
            r1 = tr.events[-1].risk
            slope = (r1 - r0) / max(1, len(tr.events) - 1)
            if slope > 2.0 and r1 >= 4.0:
                if action == "WATCH":
                    action = "QUARANTINE"
                elif action == "QUARANTINE" and self.paranoia_level > 0.6:
                    action = "KILL"
    ...
    def _update_trust_from_action(self, identity: str, risk: float, action: str):
        tp = self.trust_profiles.get(identity)
        if tp is None:
            return
        if action in ("QUARANTINE", "KILL") and risk >= 5.0:
            tp.register_bad()
        elif action == "WATCH" and risk < 3.0:
            tp.register_good()

    def _record_action(self, node: NodeInfo, risk: float, action: str, signals: Dict, features: List[float]):
        identity = self._get_identity_for_node(node)
        name = node.extra.get("name", node.label)
        ...
        with self.lock:
            ...
            tr = self.threat_timeline.get(identity)
            if tr is None:
                tr = ThreatRecord(identity=identity, first_seen=now)
                self.threat_timeline[identity] = tr
            tr.events.append(ThreatEvent(
                time=now,
                risk=risk,
                action=action,
                cpu=node.cpu,
        ...
        self._update_trust_from_action(identity, risk, action)
        self._update_ml_weights(risk, features, action)
    ...
    def _ingest_snapshot(self, snapshot: SystemSnapshot):
        ...
        for node in hot_nodes:
            ...
            identity = self._get_identity_for_node(node)
            exe_path = node.extra.get("exe") or ""
            base_tier = node.extra.get("trust", "unknown")
            self._get_or_create_trust_profile(identity, base_tier)

            risk, features = self._compute_risk_score(node)
            action = self._risk_to_action(risk, name, identity)
            ...
            self.log(msg)
            self._record_action(node, risk, action, signals, features)
    ...
    def _attempt_enforcement(self, node: NodeInfo, action: str):
        ...
        if not self.enforce_actions_for_real:
            self.log(
                ...
                f"(Enable real actions via Admin tab.)"
            )
            return
...
class SystemMetricsWorker(BaseWorker):
    ...
    def step(self):
        ...
        if mem > self.queen.mem_threshold:
            self.queen.log(f"[GLOBAL] High memory usage: {mem:.1f}%")
...
# -------------- MAIN GUI WITH ADMIN TAB & RISK GRAPH -------------- #
...
class HiveGUI:
    ...
    def _build_threats_tab(self):
        ...
        self.notebook.add(self.threats_frame, text="Threat Timeline")

        top_frame = ttk.Frame(self.threats_frame)
        top_frame.pack(fill=tk.BOTH, expand=True)

        columns = ("identity", "first_seen", "events", "last_risk", "last_action")
        self.tree_threats = ttk.Treeview(
            top_frame, columns=columns, show="headings", height=15
        )
        for col in columns:
            self.tree_threats.heading(col, text=col)
            width = 260 if col == "identity" else 120
            self.tree_threats.column(col, width=width, anchor="w")
        self.tree_threats.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=8, pady=8)

        scroll = ttk.Scrollbar(top_frame, command=self.tree_threats.yview)
        scroll.pack(side=tk.LEFT, fill=tk.Y)
        self.tree_threats.configure(yscrollcommand=scroll.set)

        right_frame = ttk.Frame(self.threats_frame)
        right_frame.pack(fill=tk.BOTH, expand=False, padx=8, pady=(0,8))

        self.fig_threat = Figure(figsize=(5, 2.5), dpi=100)
        self.ax_threat = self.fig_threat.add_subplot(111)
        self.ax_threat.set_title("Risk Evolution")
        self.ax_threat.set_xlabel("Event index")
        self.ax_threat.set_ylabel("Risk")

        self.canvas_threat = FigureCanvasTkAgg(self.fig_threat, master=right_frame)
    ...
    def _build_admin_tab(self):
        ...
        self.admin_status_label = tk.Label(
            self.admin_frame,
            text="Admin panel locked. Click 'Unlock Admin Panel' and enter password.",
            anchor="w",
            justify="left",
            font=("Consolas", 10),
            fg="red",
        )
        self.admin_status_label.pack(fill=tk.X, padx=8, pady=(8,4))

        self.btn_unlock_admin = ttk.Button(
            self.admin_frame,
            text="Unlock Admin Panel",
            command=self._unlock_admin_panel,
        )
        self.btn_unlock_admin.pack(anchor="w", padx=8, pady=(0,8))

        self.admin_controls_frame = ttk.Frame(self.admin_frame)
        self.admin_controls_frame.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)

        self.enforce_var = tk.BooleanVar(value=self.queen.enforce_actions_for_real)
        self.chk_enforce = tk.Checkbutton(
        ...
        self.chk_enforce.pack(anchor="w", pady=4)

        tk.Label(self.admin_controls_frame, text="Paranoia level (0 = cautious, 1 = aggressive):").pack(anchor="w")
        self.paranoia_scale = tk.Scale(
            self.admin_controls_frame, from_=0.0, to=1.0, resolution=0.05,
        ...
        self.ml_var = tk.BooleanVar(value=self.queen.ml_enabled)
        self.chk_ml = tk.Checkbutton(
            self.admin_controls_frame,
            text="Enable online ML-enhanced risk",
            variable=self.ml_var,
            command=self._on_toggle_ml,
            state=tk.DISABLED,
        )
        self.chk_ml.pack(anchor="w", pady=4)

        settings_frame = ttk.Frame(self.admin_controls_frame)
        settings_frame.pack(fill=tk.X, pady=6)

        self.btn_net_backup = ttk.Button(settings_frame, text="Network Backup Path", command=self._on_set_network_backup, state=tk.DISABLED)
        self.btn_net_backup.pack(side=tk.LEFT, padx=4)
        self.btn_swarm = ttk.Button(settings_frame, text="Swarm Endpoint", command=self._on_set_swarm, state=tk.DISABLED)
        self.btn_swarm.pack(side=tk.LEFT, padx=4)
        self.btn_change_pw = ttk.Button(settings_frame, text="Change Admin Password", command=self._on_change_password, state=tk.DISABLED)
        self.btn_change_pw.pack(side=tk.LEFT, padx=4)

        bottom_frame = ttk.Frame(self.admin_controls_frame)
        bottom_frame.pack(fill=tk.X, pady=8)

        self.btn_save = ttk.Button(bottom_frame, text="Save Brain Now", command=self._on_save, state=tk.DISABLED)
        self.btn_save.pack(side=tk.LEFT, padx=4)
        self.btn_shutdown = ttk.Button(bottom_frame, text="Shutdown Queen (exit program)", command=self._on_shutdown, state=tk.DISABLED)
        self.btn_shutdown.pack(side=tk.RIGHT, padx=4)

        self.admin_controls_widgets = [
            self.chk_enforce,
            self.paranoia_scale,
            self.chk_ml,
            self.btn_net_backup,
    ...
    # -------- Threat selection (risk evolution graph) -------- #

    def _on_threat_selected(self, event):
        selection = self.tree_threats.selection()
        if not selection:
            return
        item_id = selection[0]
        identity = self.tree_threats.item(item_id, "values")[0]

        with self.queen.lock:
            tr = self.queen.threat_timeline.get(identity)

        self.ax_threat.clear()
        self.ax_threat.set_title(f"Risk Evolution: {identity}")
        self.ax_threat.set_xlabel("Event index")
        self.ax_threat.set_ylabel("Risk")

        if tr and tr.events:
            risks = [ev.risk for ev in tr.events]
            xs = list(range(len(risks)))
            self.ax_threat.plot(xs, risks, marker="o", linestyle="-", color="red")
            self.ax_threat.grid(True, alpha=0.3)
        else:
            self.ax_threat.text(0.5, 0.5, "No events", ha="center", va="center")

        self.canvas_threat.draw()

    # -------- Admin tab helpers -------- #

    def _unlock_admin_panel(self):
        pwd = simpledialog.askstring("Admin Authentication", "Enter admin password:", show="*", parent=self.root)
        if pwd is None:
            return
        if self.queen.verify_admin_password(pwd):
            self.admin_unlocked = True
            self.admin_status_label.config(text="Admin panel unlocked.", fg="green")
            self.btn_unlock_admin.config(state=tk.DISABLED)
            for w in self.admin_controls_widgets:
                w.config(state=tk.NORMAL)
            self._update_admin_status_text()
        else:
            messagebox.showerror("Access Denied", "Incorrect password.")
            self.queen.log("[ADMIN] Admin tab unlock denied (wrong password).")

    def _update_admin_status_text(self):
        ...
        lines = [
            ...
            f"Swarm endpoint: {self.queen.swarm_endpoint or 'None'}",
            f"Online ML-enhanced risk: {'ENABLED' if self.queen.ml_enabled else 'DISABLED'}",
        ]
    ...
    def _update_overview(self):
        ...
        lines.append(f"Swarm: {'ENABLED' if self.queen.swarm_enabled else 'DISABLED'}")
        lines.append(f"Online ML risk: {'ENABLED' if self.queen.ml_enabled else 'DISABLED'}")
        lines.append("")
    ...
    def _update_threats(self):
        ...
        with self.queen.lock:
            threats = list(self.queen.threat_timeline.values())

        for tr in threats:
            if not tr.events:
                continue
            last_event = tr.events[-1]
            first_seen_str = time.strftime("%H:%M:%S", time.localtime(tr.first_seen))
            self.tree_threats.insert(
                "",
                tk.END,
                values=(
                    tr.identity,
    ...
    def _on_toggle_ml(self):
        val = bool(self.ml_var.get())
        self.queen.set_ml_enabled(val)
        self._update_admin_status_text()

    def _on_save(self):
        self.queen.log("[ADMIN] Manual brain save from Admin tab.")
        self.queen._save_brain()
        messagebox.showinfo("Save Brain", "Brain state saved.")
        self._update_admin_status_text()
    ...
    def _on_set_swarm(self):
        endpoint = simpledialog.askstring(
            "Swarm Endpoint",
            "Enter swarm endpoint URL (leave empty to disable):",
            initialvalue=self.queen.swarm_endpoint or "",
            parent=self.root,
        )
        if endpoint is None:
            return
        endpoint = endpoint.strip()
        self.queen.set_swarm_endpoint(endpoint or None)
        self._update_admin_status_text()

    def _on_change_password(self):
        current = simpledialog.askstring("Change Password", "Enter current admin password:", show="*", parent=self.root)
        if current is None:
            return

        new1 = simpledialog.askstring("Change Password", "Enter new admin password:", show="*", parent=self.root)
        if new1 is None or not new1.strip():
            messagebox.showerror("Error", "New password cannot be empty.")
            return

        new2 = simpledialog.askstring("Change Password", "Confirm new admin password:", show="*", parent=self.root)
        if new2 is None:
            return
    ...
    def _on_shutdown(self):
        if messagebox.askyesno("Confirm Shutdown", "Shut down Queen and exit program?"):
            self.queen.log("[ADMIN] Shutting down Queen from Admin tab.")
            self.queen.stop()
            os._exit(0)
