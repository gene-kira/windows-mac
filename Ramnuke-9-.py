...
class RaidMode(Enum):
    RAID0 = "stripe"   # split blocks across devices
    RAID1 = "mirror"   # duplicate blocks on all
...
# -------------------------------------------------------------------
# RAID cache manager (3-way Ramnuke RAID organ)
# -------------------------------------------------------------------

class RaidCacheManager:
    """
    Ramnuke RAID organ:
      - wraps three VRAMCacheManager instances
      - exposes read_block / telemetry_snapshot
      - supports RAID0 (stripe) and RAID1 (mirror)
    """
    ...
    def __init__(self, logger, mode: RaidMode,
        ...
        self.b = dev_b
        self.c = dev_c
        self.devs = [self.a, self.b, self.c]
        self.running = False

    def start(self):
        self.log(f"[Ramnuke][RAID] Starting RAID {self.mode.value} over A+B+C")
        for d in self.devs:
            d.start()
        self.running = True
    ...
    def _map_block_raid0(self, block_id: int):
        """
        Stripe across 3 devices:
          block_id % 3 → which device
          block_id // 3 → local block id
        """
        idx = block_id % 3
        local_id = block_id // 3
    ...
    def _map_block_raid1(self, block_id: int):
        """
        Mirror: same logical block_id on all three
        """
...
class CachedFileInterface:
    def __init__(self, cache_manager):
        ...
        self.block_size = cache_manager.BLOCK_SIZE if hasattr(cache_manager, "BLOCK_SIZE") else 256 * 1024
...
class SelfIntegrityOrgan:
    """
    Watches Ramnuke’s own organs and signals:
      - stale sensors
...
class AutoLoaderGUI(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ramnuke – 3‑Way RAID VRAM/RAM Control System")
        ...
        self.info_label = QtWidgets.QLabel(
            "Ramnuke – 3‑way RAID VRAM/RAM cybernetic control system with predictive policy, learning engine, and self-integrity."
        )
    ...
    def start_caching_engine(self):
        try:
            ...
            self.log(f"[Ramnuke][ENGINE] RAID {raid_mode.value} (3-way) cache manager + app-level interfaces are active.")
        except Exception as e:
    ...
    def background_tick(self):
        ...
        # Use first device as sensor spine
        ...
        if isinstance(self.cache, RaidCacheManager):
            appetite, ra, workers, vram_bias, prewarm = self.cache.devs[0].master_policy(state)
            for dev in self.cache.devs:
                dev.apply_master_policy(appetite, ra, workers, vram_bias)
                dev.adjust_workers()
        else:
