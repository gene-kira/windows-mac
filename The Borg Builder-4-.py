...
def extract_text_from_html(html: str) -> str:
    ...
    Not perfect, but good enough for distillation input.
    """
    import re
...
def call_local_llm(prompt: str, system: str | None = None, max_tokens: int = 512) -> str:
    """
    Call a real local model via Ollama.
    Make sure Ollama is running on localhost:11434 (ollama serve)
    and a model is pulled (e.g., 'ollama pull llama3').
    """
    if system:
    ...
    try:
        # Use non-streaming generate for simplicity
...
# ========= EXTERNAL LLM CONNECTORS (CONFIG-DRIVEN, OPTIONAL) =========
...
def fanout_to_llms(prompt: str, system: str | None = None, task: str = "chat") -> dict:
    """
    Fan out to local + any configured remote LLMs.
    'task' can be used later for routing; for now we just call all.
    """
...
class WebProbe:
    ...
    Web probe organ.
    - Checks connectivity.
    - Fetches simple text resources for future distillation.
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _init_ui(self):
        ...
        # Left: organs + vitals + prediction + web probe + health/safe mode/net
