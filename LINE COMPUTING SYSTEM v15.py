...
# Autonomous, predictive, adaptive, defensive, self-tuning guardian
#
# Major layers:
...
# - Tuning Manager (self-tuning: thresholds, weights, policies)
# ============================================================
...
class ReplayController:
    ...
    def _run(self, history, config_override):
        # fresh predictive engine for replay
...
class ProcessPolicyEngine:
    ...
    def apply_action(self, pid: str, name: str, action: str, event_type: str, source: str,
        ...
        # Tag regret cases for tuning: high risk but allowed/logged, or low risk but strict action
        regret_tags = []
...
# -----------------------------
# TUNING MANAGER (SELF-TUNING)
# -----------------------------
class TuningManager:
    """
    Self-tuning engine:
    - Watches risk history tags for false_positive_candidate / false_negative_candidate.
    - Adjusts predictive weights and thresholds in small steps.
    - Adjusts folding threshold.
    - Adjusts per-process policies based on repeated regret tags.
    - Logs all changes and persists to settings.
    """
    ...
    def _tune_once(self):
        ...
        for entry in hist[-2000:]:  # recent window
        ...
        # Adjust predictive weights: if FPs dominate, reduce aggression; if FNs dominate, increase
    ...
    def _extract_process_from_source(self, source):
        # source formats: "sys-proc:name", "sys-proc-wl:name", "sys-proc-bl:name"
        if ":" in source:
    ...
    def _adjust_thresholds(self, direction, step):
        ...
        if direction > 0:  # more strict: lower thresholds
            rt_warn = max(10, rt_warn - step * 5)
            rt_strict = max(rt_warn + 5, rt_strict - step * 5)
        else:  # more tolerant: raise thresholds
    ...
    def _adjust_folding_threshold(self, direction, step):
        ...
        if direction > 0:  # more strict: increase N
            n = min(15, n + int(max(1, step * 3)))
        else:  # more tolerant: decrease N
    ...
    def _adjust_policies(self, process_fp, process_fn):
        ...
        # If a process often tagged FP -> too harsh -> downgrade policy
        ...
        # If a process often tagged FN -> too lenient -> upgrade policy
...
class LineGUI:
    def __init__(self, root, engine: LineComputer, swarm: SwarmManager,
        ...
        self.root.title("Line Computing Cockpit v12 (Self-Tuning)")
...
# -----------------------------
def main():
    global fold_manager, tuning_manager
    ...
    globals()["tuning_manager"] = tuning_manager_local
