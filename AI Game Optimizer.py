...
NETWORK_ROOT = os.getenv("GAME_OPTIMIZER_SMB_ROOT", "")
...
class MovidiusInferenceEngine(InferenceEngine):
    ...
    def init(self):
        try:
            ...
            # For now, keep unavailable but wired.
            self.available = False
        except Exception:
            self.available = False
...
def decide_posture(readings: dict, profile: GameProfile | None, risk: float, brain_mode: str) -> str:
    ...
    # Adjust thresholds based on brain mode
...
def main():
    network_root = NETWORK_ROOT if NETWORK_ROOT else None
    if network_root:
