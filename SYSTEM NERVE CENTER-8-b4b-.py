#!/usr/bin/env python3
# ============================================================
# SYSTEM NERVE CENTER – HybridBrain + Tri-Stance + B4B Trainer v2
# - Multi-engine Best-Guess with regime-aware fusion + B4B signals
# - Pattern memory, reinforcement, meta-states (with momentum), integrity
# - Tri-Stance decision engine (Conservative / Balanced / Beast)
# - Predictive dampening (short/mid/long horizons) + micro-recovery loops
# - SelfIntegrityOrgan with drift-aware self-correction
# - Movidius/ONNX training hook stub + GUI
# - Back4BloodAnalyzer + Back4BloodPhysicsOrgan from JSON metrics
# - InteractionCoachOrgan: B4B as human↔AI training ground (episodes)
# - GameDetector: auto-detect Back 4 Blood process
# - Full B4B memory persistence (metrics, physics, episodes, game state)
# - B4B integrated into Best-Guess voting + reasoning heatmap
# - Reboot Memory (SMB/local)
# - Tkinter GUI: Brain Cortex + Reboot Memory + Reasoning Heatmap + ONNX/B4B
# - RestorationProtocol: "Serve Again Mode" recovery cycle
# ============================================================
...
class Back4BloodAnalyzer:
    ...
    Reads metrics from a JSON file:
    {
      ...
      "cpu_load": 0.7,
      "mem_load": 0.65,
      "outcome": "overload" | "stable" | "beast_win" | "victory" | "defeat"
    }
    """
...
class Back4BloodPhysicsOrgan:
    """
    Interprets B4B metrics JSON as a physics/behavior organ.
    Uses analyzer.last_metrics as source.
    Exposes:
      - physics_stress
      - movement_intensity
      - server_stress
    ...
    def __init__(self, analyzer: Back4BloodAnalyzer):
...
class InteractionEpisode:
    ...
    outcome: str  # "victory" / "defeat" / "overload" / "stable" / "beast_win"

class InteractionCoachOrgan:
    """
    Uses Back4Blood as a training ground for human ↔ AI interaction.
    Records episodes and updates reinforcement.
    """
    ...
    def start_if_needed(self):
        if self.current is None and self.b4b_analyzer.last_metrics:
            now = utc_ts()
...
class GameDetector:
    """
    Detects Back 4 Blood by scanning running processes.
    GAME_NAMES list can be extended.
    """
...
class PredictionEngine:
    """
    Enhanced engine:
    - Regime-aware prediction fusion
    - Multi-engine voting with B4B + integrity + meta-state
    - Short/mid/long horizon awareness
    """
    ...
    def __init__(self, brain: HybridBrain):
        ...
        if HAS_ONNX:
            self.onnx_session = None  # load real model here
    ...
    def _pattern_influence(self) -> float:
        # Rough global pattern influence using counts
    ...
    def _engine_votes(self) -> Tuple[float, Dict[str, float], str, Dict[str, float]]:
        ...
        ewma_short = self.cpu.ewma(0.5)   # ~1s
        ewma_mid = self.cpu.ewma(0.2)     # ~5s
        ewma_long = self.cpu.ewma(0.05)   # ~30–120s
        ...
        if self.b4b_provider:
            ...
            # predictive outcome: stress + server + movement
...
class NerveCenterApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("System Nerve Center – HybridBrain Tri-Stance")
        ...
        self.back4blood_physics = Back4BloodPhysicsOrgan(self.back4blood_analyzer)
    ...
    def _build_gui(self):
        ...
        notebook.add(self.tab_b4b, text="Back4Blood")
    ...
    def _build_b4b_tab(self, tab):
        frame = ttk.LabelFrame(tab, text="Back4Blood Analyzer, Physics & Interaction Coach")
        frame.pack(fill="both", expand=True, padx=10, pady=10)

        ttk.Label(frame, text="Metrics JSON Path:").grid(row=0, column=0, sticky="w", padx=5, pady=3)
        ...
        self.lbl_b4b_last = ttk.Label(frame, text="Last metrics: (none)")
    ...
    def cmd_save_reboot_memory(self):
        ...
        meta = {"ts": time.time(), "version": "2.0-b4b-predictive"}
    ...
    def _try_load_reboot_memory(self):
        ...
        try:
            ...
            pm = brain_data.get("pattern_memory", {}).get("patterns", {})
            self.brain.pattern_memory = PatternMemory(
                patterns={k: PatternMemoryEntry(**v) for k, v in pm.items()}
            )
    ...
    def _bg_loop(self):
        while True:
            try:
                ...
                if running and not self.game_detector.last_state:
                    print("[GameDetector] Back 4 Blood detected — auto-training engaged")
                    self.lbl_top_status.config(
                        text="Back 4 Blood detected — Training Active",
                        fg="#55ff55",
                    )
                elif not running and self.game_detector.last_state:
                    print("[GameDetector] Back 4 Blood closed — training paused")
                    self.lbl_top_status.config(
    ...
    def _update_heatmap(self):
        ...
        for k, v in sorted(hm.items(), key=lambda kv: kv[1], reverse=True):
            bar = "#" * int(v * 20)
            self.txt_heatmap.insert("end", f"{k:20s} [{bar:<20s}] {v:.2f}\n")

    def _update_b4b_panel(self):
        m = self.back4blood_analyzer.last_metrics
        if not m:
            self.lbl_b4b_last.config(text="Last metrics: (none)")
            ...
            self.lbl_b4b_last.config(
                text=f"Last metrics: outcome={out}, fps={fps}, ping={ping}"
            )
        ...
        if self.game_detector.game_running:
            ...
            self.lbl_b4b_game.config(
                text=f"Game: RUNNING (pid={info.get('pid','?')}, cpu={info.get('cpu','?')}%)",
                foreground="#00cc66",
            )
        else:
