...
class AIGovernor:
    def __init__(self, assets, tier_disk, tier_ram, tier_vram,
        ...
        self.recent_accesses = deque(maxlen=32)
        ...
        self.sys_cpu = 0.0
        self.sys_ram_used = 0.0
        self.sys_ram_total = 0.0
        self.sys_disk_r = 0.0
        self.sys_disk_w = 0.0
        self.sys_gpu = 0.0
        self.sys_vram_used = 0.0
        self.sys_vram_total = 0.0
        self.sys_bottleneck = "stable"

    def update_system_view(self, snapshot):
        self.sys_cpu = snapshot.get("cpu", 0.0)
        self.sys_ram_used = snapshot.get("ram_used", 0.0)
        self.sys_ram_total = snapshot.get("ram_total", 0.0)
        self.sys_disk_r = snapshot.get("disk_r", 0.0)
        self.sys_disk_w = snapshot.get("disk_w", 0.0)
        self.sys_gpu = snapshot.get("gpu", 0.0)
        self.sys_vram_used = snapshot.get("vram_used", 0.0)
        self.sys_vram_total = snapshot.get("vram_total", 0.0)
        self.sys_bottleneck = snapshot.get("bottleneck", "stable")
    ...
    def apply_decay(self):
        factor = 1.0
        if self.sys_cpu > 80.0:
            factor *= 0.98
        if self.sys_bottleneck in ("cpu", "ram"):
            factor *= 0.985
        for asset in self.assets:
            asset.access_count *= self.decay * factor
    ...
    def decide_moves(self):
        ...
        cpu = self.sys_cpu
        bottleneck = self.sys_bottleneck
        hot_adj = 1.0
        warm_adj = 1.0

        if cpu > 80 or bottleneck in ("cpu", "ram"):
            hot_adj *= 1.15
            warm_adj *= 1.1
        elif bottleneck in ("gpu", "vram"):
            hot_adj *= 1.05
        elif bottleneck == "disk":
            warm_adj *= 1.1
        elif bottleneck == "stable" and cpu < 40 and self.sys_ram_total > 0:
            hot_adj *= 0.95
            warm_adj *= 0.95
    ...
    def _log_decision(self, asset, decision_type, conf, vram_util, action):
        ...
        event = {
            ...
            "sys_bottleneck": self.sys_bottleneck,
        }
        self.trace.log_decision(event)
        key = self.pattern_key(asset, decision_type)
        reward = (health_now - 50.0) / 50.0
...
def scan_game_folder(root_path, log_fn):
    ...
    for dirpath, dirnames, filenames in os.walk(root_path):
        for fname in filenames:
            ...
            asset = Asset(asset_id=full, size_mb=size_mb, kind=kind, meta={"path": full})
            assets.append(asset)
...
class SystemTelemetry:
    ...
    def start(self):
        if not psutil:
            self.log_fn("psutil not installed; System Cortex telemetry disabled.")
            return
        if self._running:
            return
        self._running = True
        threading.Thread(target=self._loop, daemon=True).start()
        self.log_fn("System Cortex telemetry started.")
    ...
    def _loop(self):
        ...
        while self._running:
            try:
                ...
                ram_used = (vm.total - vm.available) / (1024 * 1024 * 1024)
                ram_total = vm.total / (1024 * 1024 * 1024)

                disk = psutil.disk_io_counters()
                if last_disk:
                    r_bytes = disk.read_bytes - last_disk.read_bytes
                    w_bytes = disk.write_bytes - last_disk.write_bytes
                else:
                    r_bytes = 0
                    w_bytes = 0
                last_disk = disk
                disk_r = (r_bytes / dt) / (1024 * 1024)
                disk_w = (w_bytes / dt) / (1024 * 1024)
                ...
                try:
                    ...
                    procs = []
                procs_sorted = sorted(procs, key=lambda p: p.info.get("cpu_percent", 0.0), reverse=True)[:5]
                top_list = []
                for p in procs_sorted:
                    try:
                        name = p.info.get("name", "proc")
                        cpu_p = p.info.get("cpu_percent", 0.0)
                        mem = p.info.get("memory_info")
                        mem_mb = mem.rss / (1024 * 1024) if mem else 0.0
                        top_list.append((name, cpu_p, mem_mb))
                    except Exception:
                        continue

                foreground = top_list[0][0] if top_list else ""

                bottleneck = self._compute_bottleneck(cpu, ram_used, ram_total, disk_r, disk_w, gpu, vram_used, vram_total)
    ...
    def _compute_bottleneck(self, cpu, ram_used, ram_total, disk_r, disk_w, gpu, vram_used, vram_total):
        scores = {}
        if cpu > 75:
            scores["cpu"] = cpu / 100.0
        if ram_total > 0:
            ram_ratio = ram_used / ram_total
            if ram_ratio > 0.8:
                scores["ram"] = ram_ratio
        io_load = (disk_r + disk_w) / 500.0
        if io_load > 0.5:
            scores["disk"] = min(1.0, io_load)
        if scores:
...
class MemoryGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Hybrid Memory Overmind")
        ...
        self.telemetry = SystemTelemetry(log_fn=self._log)
        self.sys_gradient_phase = 0.0
        ...
        self.root.after(80, self._animate_system_cortex)
    ...
    def _init_styles(self):
        ...
        style.configure("TLabelframe", background=frame_bg, foreground=text_fg)
        style.configure("TLabelframe.Label", background=frame_bg, foreground=text_fg)
        style.configure("TLabel", background=frame_bg, foreground=text_fg)
        style.configure("TButton", background="#202040", foreground=text_fg)
        style.map("TButton", background=[("active", "#303060")])

        style.configure("SystemCortex.TFrame", background=dark_bg)
        style.configure("GameLibrary.TFrame", background=frame_bg)
    ...
    def _init_ui(self):
        ...
        self.tab_system = ttk.Frame(self.notebook, style="SystemCortex.TFrame")
        self.tab_library = ttk.Frame(self.notebook, style="GameLibrary.TFrame")

        self.notebook.add(self.tab_hybrid, text="Hybrid Cortex")
        self.notebook.add(self.tab_system, text="System Cortex")
        self.notebook.add(self.tab_library, text="Game Library")

        self._init_hybrid_tab(self.tab_hybrid)
        self._init_system_cortex_tab(self.tab_system)
    ...
    def _init_hybrid_tab(self, parent):
        top = ttk.Frame(parent)
        top.pack(fill=tk.X)

        hybrid_frame = ttk.Labelframe(top, text="Hybrid Cortex", padding=5)
        ...
        vram_frame = ttk.Labelframe(parent, text="VRAM Reservoir", padding=5)
        vram_frame.pack(fill=tk.X, padx=5, pady=5)

        self.vram_canvas = tk.Canvas(vram_frame, height=24, bg="#101010", highlightthickness=0)
        self.vram_canvas.pack(fill=tk.X, expand=True)

        middle = ttk.Labelframe(parent, text="Assets (Predictive Cache View)", padding=5)
        ...
        bottom = ttk.Labelframe(parent, text="Cortex Log", padding=5)
        bottom.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.log_text = tk.Text(bottom, height=8, wrap="word", bg="#05070a", fg="#d0d0ff")
        self.log_text.pack(fill=tk.BOTH, expand=True)
        self.log_text.config(state="disabled")

    # ---------- System Cortex Tab ----------

    def _init_system_cortex_tab(self, parent):
        parent.configure(style="SystemCortex.TFrame")
        self.sys_canvas = tk.Canvas(parent, bg="#05050a", highlightthickness=0)
        self.sys_canvas.pack(fill=tk.BOTH, expand=True)

        self.sys_center_x = 700
        self.sys_center_y = 200
        self.sys_center_radius = 70
    ...
    def _init_library_tab(self, parent):
        parent_column = ttk.Frame(parent, style="GameLibrary.TFrame")
        parent_column.pack(fill=tk.BOTH, expand=True)

        library_frame = ttk.Labelframe(parent_column, text="Game Library", padding=5)
        library_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.game_listbox = tk.Listbox(library_frame, height=20, exportselection=False,
                                       bg="#080810", fg="#f0f0ff", selectbackground="#303060")
        self.game_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.game_listbox.bind("<<ListboxSelect>>", self._on_game_selected)

        scroll = ttk.Scrollbar(library_frame, orient=tk.VERTICAL, command=self.game_listbox.yview)
        scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.game_listbox.config(yscrollcommand=scroll.set)

        btn_frame = ttk.Frame(parent_column)
    ...
    def _refresh_ui(self):
        if self.assets and self.governor:
            ...
            self.governor.update_system_view(snapshot)
            ...
            self.status_strip.config(
                text=f"[{self.profile_name}] VRAM {self.tier_vram.used_mb:.0f}/{self.tier_vram.capacity_mb} MB | "
                     ...
                     f"| CPU {snapshot.get('cpu', 0.0):.1f}% Bottleneck {snapshot.get('bottleneck', 'stable')}"
            )
    ...
    def _draw_vram_bar(self, health_score):
        ...
        bg = "#101010"
        self.vram_canvas.create_rectangle(2, 2, w - 2, h + 2, outline="#444444", fill=bg)
    ...
    # ---------- System Cortex rendering & gradient ----------

    def _animate_system_cortex(self):
        snap = self.telemetry.get_snapshot()
        self._draw_system_cortex(snap)
        self.root.after(80, self._animate_system_cortex)
    ...
    def _draw_system_cortex(self, snap):
        c = self.sys_canvas
        ...
        bottleneck = snap.get("bottleneck", "stable")
        speed = 0.03 + (cpu / 100.0) * 0.07
        if bottleneck != "stable":
            speed *= 1.3
        self.sys_gradient_phase = (self.sys_gradient_phase + speed) % 1.0

        bg_color = "#05050a"
        c.create_rectangle(0, 0, w, h, fill=bg_color, outline=bg_color)

        center_color = self._gradient_color(self.sys_gradient_phase)
        outer_phase = (self.sys_gradient_phase + 0.25) % 1.0
        outer_color = self._gradient_color(outer_phase)

        cx = self.sys_center_x
        cy = self.sys_center_y
        r = self.sys_center_radius

        c.create_oval(cx - r - 10, cy - r - 10, cx + r + 10, cy + r + 10,
                      outline=outer_color, width=4)
        c.create_oval(cx - r, cy - r, cx + r, cy + r,
                      outline=center_color, width=2, fill="#080814")

        label = bottleneck.upper()
        c.create_text(cx, cy, text=label, fill="#f0e0ff", font=("Consolas", 12, "bold"))

        cpu = snap.get("cpu", 0.0)
        ram_used = snap.get("ram_used", 0.0)
        ram_total = snap.get("ram_total", 0.0)
        disk_r = snap.get("disk_r", 0.0)
        disk_w = snap.get("disk_w", 0.0)
        gpu = snap.get("gpu", 0.0)
        vram_used = snap.get("vram_used", 0.0)
        vram_total = snap.get("vram_total", 0.0)

        bar_specs = [
            ("CPU", cpu / 100.0,       "cpu"),
            ("RAM", ram_used / (ram_total + 1e-6), "ram"),
            ("GPU", gpu / 100.0,       "gpu"),
            ("VRAM", vram_used / (vram_total + 1e-6) if vram_total > 0 else 0.0, "vram"),
            ("DISK", min(1.0, (disk_r + disk_w) / 500.0), "disk"),
        ]

        for label_text, frac, key in bar_specs:
            frac = max(0.0, min(1.0, frac))
            x, y = self.sys_bars[key]
            bw = 140
            bh = 18
            c.create_rectangle(x - bw//2, y - bh//2, x + bw//2, y + bh//2,
                               outline="#202030", fill="#05050a")
            phase = (self.sys_gradient_phase + frac * 0.3) % 1.0
            color = self._gradient_color(phase)
            fill_w = int(bw * frac)
            c.create_rectangle(x - bw//2, y - bh//2, x - bw//2 + fill_w, y + bh//2,
                               outline=color, fill=color)
            c.create_text(x, y - bh, text=label_text, fill="#c0b0ff", font=("Consolas", 9))
            c.create_text(x, y + bh, text=f"{frac*100:.1f}%", fill="#f0f0ff", font=("Consolas", 8))

        fg = snap.get("foreground", "")
        top_procs = snap.get("top_processes", [])

        c.create_text(w//2, 420, text="Foreground Process", fill="#b0a0ff", font=("Consolas", 11, "bold"))
        fg_text = fg if fg else "(none)"
        aura_phase = (self.sys_gradient_phase + (cpu / 100.0) * 0.2) % 1.0
        aura_color = self._gradient_color(aura_phase)
        c.create_text(w//2, 445, text=fg_text, fill=aura_color, font=("Consolas", 12, "bold"))

        c.create_text(w//2, 480, text="Top Processes (CPU%, RAM MB)", fill="#9080d0", font=("Consolas", 10))
        y = 505
        for name, cpu_p, mem_mb in top_procs:
            txt = f"{name[:30]:<30}  {cpu_p:5.1f}%  {mem_mb:6.1f} MB"
            c.create_text(w//2, y, text=txt, fill="#e0d0ff", font=("Consolas", 9))
            y += 18
