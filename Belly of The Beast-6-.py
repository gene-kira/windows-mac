...
# ============================================================
# Dev mode manager (global shadow system state)
# ============================================================

class DevModeManager:
    def __init__(self):
        self.enabled = False
        self.view_mode = "REAL"  # REAL, SHADOW, AUTO
        self.load()
...
class ProcessesPanel(QWidget):
    ...
    def kill_selected(self):
        if DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "In Development Mode, process killing is disabled (shadow only).")
            return
...
# ============================================================
# Registry panel (real, but we’ll respect Dev Mode)
# ============================================================
...
class RegistryPanel(QWidget):
    ...
    def _dev_mode_block(self):
        if DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "Registry writes are disabled in Development Mode (shadow only).")
            return True
        return False
...
class PowerShellPanel(QWidget):
    ...
    def run_command(self):
        if DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "PowerShell execution is disabled in Development Mode (shadow only).")
            return
...
class AppsPanel(QWidget):
    ...
    def _dev_mode_block(self):
        if DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "AppX modifications are disabled in Development Mode (shadow only).")
            return True
        return False
...
# ============================================================
# UNKNOWN SYSTEMS panel (REAL / SHADOW / AUTO-MERGE)
# ============================================================
...
class UnknownSystemsPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self.current_real_rows = []  # list of dicts
        ...
        info = QLabel(
            "⧫⧖ᛉ — UNKNOWN SYSTEMS\n"
            "Shadow of the real system. In Development Mode, SHADOW and AUTO-MERGE use JSON overrides only."
        )
        ...
        self.table.setColumnCount(5)
        self.table.setHorizontalHeaderLabels(
            ["Property", "Value (editable)", "Type", "Source", "Identifier"]
        )
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        layout.addWidget(self.table)

        bottom_layout = QHBoxLayout()
        self.btn_apply = QPushButton("⧖ᛞ⟟ — Apply Changes (Save JSON)", self)
    ...
    def update_banner(self):
        if DEV_MODE.enabled:
            self.banner.setText(
                ...
                "REAL: real system (read-only)\n"
                "SHADOW: JSON shadow (editable)\n"
                "AUTO-MERGE: real system overlaid with shadow values (editable, JSON only)"
            )
        else:
            self.banner.setText("Development Mode OFF — REAL VIEW only, read-only shadow.")
    ...
    def _load_rows_into_table(self, rows):
        ...
        for r in rows:
            ...
            self.table.setItem(row, 3, QTableWidgetItem(r["source"]))
            self.table.setItem(row, 4, QTableWidgetItem(r["identifier"]))

    def _build_view_rows(self):
        mode = DEV_MODE.view_mode
        if not self.current_real_rows:
            return []

        if not DEV_MODE.enabled or mode == "REAL":
            # pure real
            return self.current_real_rows

        if mode == "SHADOW":
            # only rows that exist in overrides for this source/identifier
            shadow_rows = []
            for key, data in self.overrides.items():
                src = data.get("source", "")
                ident = data.get("identifier", "")
                if src == self.current_source and ident == self.current_identifier:
                    shadow_rows.append({
        ...
        if mode == "AUTO":
            # real rows, but overlay values from overrides if present
            merged = []
            for r in self.current_real_rows:
                key = get_override_key(r["source"], r["identifier"], r["property"])
                if key in self.overrides:
                    ov = self.overrides[key]
                    merged.append({
                        "property": r["property"],
                        "value": ov.get("value", r["value"]),
                        "type": ov.get("type", r["type"]),
                        "source": r["source"],
    ...
    def apply_changes(self):
        if not DEV_MODE.enabled:
            QMessageBox.information(self, "Dev Mode", "Development Mode is OFF. No shadow writes allowed.")
            return
        mode = DEV_MODE.view_mode
        if mode not in ("SHADOW", "AUTO"):
            QMessageBox.information(self, "Dev Mode", "Edits only apply in SHADOW or AUTO-MERGE view.")
            return
        ...
        for row in range(rows):
            ...
            if not (prop_item and val_item and src_item and id_item):
                continue
        ...
        QMessageBox.information(self, "Overrides Saved", f"Shadow changes saved to {OVERRIDES_FILE}.")
...
class DevModePanel(QWidget):
    def __init__(self, parent=None, unknown_panel: UnknownSystemsPanel = None):
        ...
        desc = QLabel(
            ...
            "SHADOW: JSON shadow system, fully editable.\n"
            "AUTO: Real system overlaid with shadow values, edits go to JSON only."
        )
...
# ============================================================
# Main window with alien layout + pulsing nav
# ============================================================

class AlienAdminMainWindow(QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("⟒⧫ᛟ  ALIEN ADMIN COCKPIT  ⧖ᛉ⧗")
