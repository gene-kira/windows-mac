#!/usr/bin/env python3
# guardian_hybrid_edr_ai_encoding_thermal.py
...
# -----------------------------
# Thermal control thread (Peltier organ)
# -----------------------------
...
def score_serial_port(port) -> int:
    """Score how likely this port is a microcontroller-based thermal device."""
    score = 0
    ...
    # Known VID/PID patterns (Arduino, CH340, CP210x, Pico, etc.)
    known_patterns = [
        "2341:0043",  # Arduino Uno
        "2341:0001",
        "1a86:7523",  # CH340
        "10c4:ea60",  # CP210x
        "2e8a:000a",  # RP2040 Pico
        "0403:6001",  # FTDI
    ]
    ...
    # Description / manufacturer hints
...
def auto_detect_thermal_port(log_func=None) -> Optional[str]:
    ...
    # If all scores are zero, still allow using the "closest" one
...
class GuardianApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Guardian Hybrid EDR - SYSTEM + AI + Encoding + Thermal")
    ...
    # ---------- Thermal organ (auto-detect + control) ----------
