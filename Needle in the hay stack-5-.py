...
with Tkinter GUI, memory vault, zero-hesitation responders, and safe host isolation.

This version integrates:
- Robust GUI and orchestration (fix: added switch_bot_mode wrapper)
        ...
        with self.lock:
            try:
                with open(self.path, "w", encoding="utf-8") as f:
                    json.dump({"episodes": list(self.episodes)}, f, ensure_ascii=False, indent=2)
...
# -----------------------------------------------------------------------------
class Responder:
    def __init__(self, bridge: Bridge, memory: MemoryVault, ace: ACE):
        self.bridge = bridge
        self.memory = memory
        self.ace = ace
        # Defaults enabled
    ...
    def kill_process_tree(self, pid):
        ...
        try:
            ...
            self.memory.add("responder","process_kill",tags=["responder","process_kill"],payload={"pid":pid,"gone":len(gone),"alive":len(alive)})
            return True
        except Exception as e:
    ...
    def quarantine_file(self, path):
        try:
            ...
            self.bridge.threat.emit(f"{EMBLEMS['lock']} {msg}", tags=["responder","file_quarantine"], payload={"src":path,"dest":dest})
            self.memory.add("responder","file_quarantine",tags=["responder","file_quarantine"],payload={"src":path,"dest":dest})
            return True
    ...
    def throttle_dest(self, dest_str):
        # dest_str must be normalized 'ip:port'
        msg = f"Egress throttle applied to {dest_str}"
        self.bridge.threat.emit(f"{EMBLEMS['globe']} {msg}", tags=["responder","egress_throttle"], payload={"dest":dest_str})
        self.memory.add("responder","egress_throttle",tags=["responder","egress_throttle"],payload={"dest":dest_str})
        return True
...
# -----------------------------------------------------------------------------
class Collectors:
    def __init__(self, bridge: Bridge, trust: TrustModel, ace: ACE, responder: Responder, quarantine_dir: str):
        self.bridge = bridge
        self.trust = trust
        self.ace = ace
            ...
            try:
                for p in psutil.process_iter(attrs=["pid","ppid","name","cmdline","username"]):
                    ...
                    if any(x in cmd for x in ["rundll32","powershell","mshta","certutil","wmic","bash","sh","osascript"]) and cmd:
                        ...
                        if self.responder.process_termination:
                            self.responder.kill_process_tree(info["pid"])
                time.sleep(5)
            except Exception as e:
            ...
            try:
                for conn in psutil.net_connections(kind='inet'):
                    if conn.status == 'ESTABLISHED' and conn.raddr:
                        ...
                        if self.responder.egress_throttle:
                            self.responder.throttle_dest(dest_str)
                time.sleep(10)
            except Exception as e:
        ...
        class Handler(watchdog_events.FileSystemEventHandler):
            def on_modified(_, event):
                try:
                    ...
                    if any(sp in event.src_path for sp in suspicious) and self.responder.file_quarantine:
                        self.responder.quarantine_file(event.src_path)
...
class DefenderUI:
    ...
    def append_line(self, msg):
        ...
        if "LOLBin" in msg or "Egress throttle" in msg or "Quarantined file" in msg or "Terminated process tree" in msg:
    ...
    def _update_status_loop(self):
        while True:
            try:
                ...
                status = f"ACE: {ace} | Host Isolation: {iso}"
...
class Orchestrator:
    def __init__(self, use_ui=True):
        ...
        self.responder = Responder(self.bridge, self.memory, self.ace)
        self.collectors = Collectors(self.bridge, self.trust, self.ace, self.responder, quarantine_dir=self.responder.quarantine_dir)
    ...
    def start(self):
        ...
        if self.use_ui:
            ...
            log.info("Running CLI. Commands: switch | escalate | isolate on/off | ace on/off | save | quit")
            try:
                while True:
                    ...
                    if cmd in ("switch","s"):
                        ...
                        log.info("Commands: switch | escalate | isolate on/off | ace on/off | save | quit")
