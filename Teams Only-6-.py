...
class SentinelGUI:
    ...
    def _build_ui(self):
        # Top bar: lockdown toggle + filter
        ...
        # Notebook for Processes / Network
    ...
    def _selected_path(self):
        ...
        return v[5] if v and len(v) >= 6 and v[5] else None
    ...
    def add_selected_block(self):
        ...
        if p:
            BLOCKLIST.add(p)
            save_lists()
            log_event(f"Added to BLOCKLIST: {p}")
            self.refresh_lists_ui()
            self.refresh_process_view()
    ...
    def refresh_network_view(self):
        rows = getattr(self, "current_net_rows", [])
