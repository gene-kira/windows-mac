...
if IS_GUI:
    ...
    class MainWindow(QtWidgets.QMainWindow):
        ...
        def on_metrics(self, metrics: dict):
            ...
            # Beast Mode LED logic: average load of Beast cores (all except first 2)
            beast_active = False
            if cpu_perc:
                if len(cpu_perc) > 2:
                    beast_slice = cpu_perc[2:]
                else:
                    beast_slice = cpu_perc
                if beast_slice:
                    avg_beast = sum(beast_slice) / len(beast_slice)
                    beast_active = avg_beast >= 50.0
