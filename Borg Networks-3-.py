...
class ThreatDetector(threading.Thread):
    def __init__(self, memory_engine, alert_queue, hive_shared=None, node_name="Node"):
        ...
        self.hive_shared = hive_shared
        self.node_name = node_name

    def _log_alert(self, level, message):
        text = f"[{self.node_name}][{level}] {message}"
        ...
        if self.hive_shared is not None:
            with self.hive_shared["lock"]:
                node_stats = self.hive_shared["anomalies"]["per_node"].setdefault(
                    self.node_name,
                    {"new": 0, "rare": 0, "unusual": 0}
                )
                if level == "INFO":
                    node_stats["new"] += 1
                elif level == "WATCH":
...
class TimeAwareBrain(threading.Thread):
    def __init__(self, config, gaming_manager, hive_shared=None, node_name="Node"):
        super().__init__(daemon=True)
        self.config = config
    ...
    def run(self):
        while self.running:
            ...
            desired_mode = "gaming" if 18 <= hour <= 23 else "balanced"

            if self.hive_shared is not None:
                with self.hive_shared["lock"]:
                    self.hive_shared["modes"]["per_node"][self.node_name] = desired_mode

            with self.hive_shared["lock"]:
                hive_mode = self.hive_shared["modes"]["global_mode"]
...
# ============================================================
# PREDICTIVE AI ENGINE (HIVE-AWARE)
# ============================================================

class PredictiveAIEngine(threading.Thread):
    def __init__(self, node, shared_registry, hive_shared=None, node_name="Node"):
        super().__init__(daemon=True)
        self.node = node
        self.shared_registry = shared_registry
        self.registry_lock = shared_registry["_lock"]
    ...
    def _compute_collective_risk(self, anomaly_risk, drive_risk):
        ...
        for s in states:
            ...
            conn_gap = abs(conn - q_conn)
            ep_gap = abs(endpoints - q_endpoints)

            if q_conn > 0 and conn_gap > q_conn * 0.5:
                divergence_score += 15
                notes.append(f"{s.get('name')} connection count diverges from Queen.")
            if q_endpoints > 0 and ep_gap > q_endpoints * 0.5:
                divergence_score += 15
    ...
    def run(self):
        while self.running and self.node.is_running():
            try:
                ...
                if self.hive_shared is not None:
                    with self.hive_shared["lock"]:
                        self.hive_shared["predictive"]["per_node"][self.node_name] = dict(self._summary)
                ...
                with self._lock:
                    self._summary = {
                        ...
                        "notes": [f"Predictive engine error: {e}"]
                    }
            time.sleep(15)
...
# ============================================================
# HIVE MIND MANAGER (QUEEN COLLECTIVE LAYER)
# ============================================================

class HiveMindManager(threading.Thread):
    """
    Full hive mind:
    - Aggregates queens' predictive + anomaly + mode info
    - Produces a global hive-level prediction for GUI
    - Maintains a global mode (gaming/balanced) by consensus
    """
    def __init__(self, hive_shared, shared_registry):
        super().__init__(daemon=True)
        self.hive_shared = hive_shared
        self.shared_registry = shared_registry
        self.running = True
    ...
    def _compute_hive_mode(self):
        with self.hive_shared["lock"]:
            per_node_modes = dict(self.hive_shared["modes"]["per_node"])

        if not per_node_modes:
            return None

        gaming_votes = sum(1 for m in per_node_modes.values() if m == "gaming")
        balanced_votes = sum(1 for m in per_node_modes.values() if m == "balanced")

        if gaming_votes > balanced_votes:
            return "gaming"
        elif balanced_votes > gaming_votes:
            return "balanced"
        else:
            return None
    ...
    def run(self):
        while self.running:
            try:
                with self.hive_shared["lock"]:
                    predictive_per_node = dict(self.hive_shared["predictive"]["per_node"])
                ...
                for node_name, pred in predictive_per_node.items():
                    anomaly_levels.append(pred.get("anomaly_risk", "LOW"))
                    drive_levels.append(pred.get("drive_risk", "LOW"))
                    scores.append(pred.get("collective_risk_score", 100))
                    notes.extend(pred.get("notes", []))

                if not predictive_per_node:
                    anomaly = "LOW"
                    drive = "LOW"
                    score = 100
                    notes.append("Hive idle; no predictive data.")
                else:
                    anomaly = self._merge_levels(anomaly_levels)
                    drive = self._merge_levels(drive_levels)
                    if scores:
                        score = int(sum(scores) / len(scores))
                    else:
                        score = 100

                hive_mode = self._compute_hive_mode()
                with self.hive_shared["lock"]:
                    self.hive_shared["modes"]["global_mode"] = hive_mode

                with self._lock:
                    self._hive_summary = {
                        "anomaly_risk": anomaly,
                        "drive_risk": drive,
                        "collective_risk_score": score,
                        "notes": notes[:8]
                    }
    ...
    def get_hive_summary(self):
        with self._lock:
            return dict(self._hive_summary)
...
class BorgNode:
    def __init__(
        ...
        self.time_brain = TimeAwareBrain(
            self.config,
            self.gaming_manager,
            hive_shared=self.hive_shared,
            node_name=self.name
        )
...
class BorgGUI:
    def __init__(self, root, queen, node_list, shared_dirs_ref, config_ref, shared_registry, hive_manager):
        self.root = root
        self.queen = queen
        ...
        self.root.title("BorgNet Guardian Hive")
    ...
    def _build_layout(self):
        ...
        summary_frame = ttk.LabelFrame(top_frame, text="Hive Node Status")
        ...
        self.aggregate_label = ttk.Label(summary_frame, text="")
        self.aggregate_label.pack(anchor="w")
        ...
        predictive_frame = ttk.LabelFrame(top_frame, text="Hive Predictive Intelligence")
        ...
        self.report_button = ttk.Button(
            controls_frame,
            text="Show Queen Anomaly Report",
        ...
        self.endpoint_button = ttk.Button(
            controls_frame,
            text="Show Queen Endpoint Details",
        ...
        self.spawn_button = ttk.Button(
            controls_frame,
            text="Spawn Worker Drone",
            command=self._spawn_worker
        )
    ...
    def _toggle_gaming_mode(self):
        from_mode = None
        hive_summary = self.hive_manager.get_hive_summary()
        if hive_summary:
            from_mode = None

        with self.queen.hive_shared["lock"]:
            current = self.queen.hive_shared["modes"]["global_mode"]
            if current == "gaming":
                self.queen.hive_shared["modes"]["global_mode"] = "balanced"
                mode_str = "balanced"
            else:
                self.queen.hive_shared["modes"]["global_mode"] = "gaming"
                mode_str = "gaming"

        messagebox.showinfo("Hive Gaming Mode", f"Hive global mode set to {mode_str.upper()}")

    def _show_report(self):
        summary = self.queen.threat_detector.get_summary()
        lines = [
            "=== Queen Anomaly / Rogue-like Activity Summary ===",
        ...
        report_text = "\n".join(lines)
        messagebox.showinfo("Queen Anomaly Report", report_text)
    ...
    def _show_endpoints(self):
        ...
        win.title("Queen Endpoint Details")
    ...
    def _spawn_worker(self):
        name = simpledialog.askstring("Spawn Worker Drone", "Enter worker name:", parent=self.root)
        if not name:
            return
        ...
        hive_shared = self.queen.hive_shared

        def get_primary_dir():
            return self.shared_dirs_ref.get("primary", "")

        def get_secondary_dir():
            return self.shared_dirs_ref.get("secondary", "")

        worker = BorgNode(
            is_queen=False,
            name=name,
            persist_file=f"borgnet_{name.lower()}_memory.json",
            get_primary_dir_func=get_primary_dir,
            get_secondary_dir_func=get_secondary_dir,
            shared_registry=shared_registry,
            hive_shared=hive_shared
        )
        self.queen.register_drone(worker)
        self.node_list.append(worker)
        worker.start()
        messagebox.showinfo("Spawn Worker", f"Worker drone '{name}' spawned and joined the hive.")
    ...
    def _update_nodes_table(self):
        ...
        self.aggregate_label.config(
            text=f"Hive connections: {total_conn}  |  Hive endpoints: {total_endpoints}"
        )

    def _update_predictive_panel(self):
        hive_summary = self.hive_manager.get_hive_summary()
        self.pred_anomaly_var.set(hive_summary.get("anomaly_risk", "UNKNOWN"))
        self.pred_drive_var.set(hive_summary.get("drive_risk", "UNKNOWN"))
        self.pred_collective_var.set(str(hive_summary.get("collective_risk_score", 0)))
        notes = hive_summary.get("notes", [])
    ...
    def _update_gui(self):
        ...
        with self.queen.hive_shared["lock"]:
            global_mode = self.queen.hive_shared["modes"]["global_mode"]

        mode_display = global_mode if global_mode else cfg["mode"]
        self.node_label.config(text=f"Main Queen: {self.queen.name}")
        self.mode_label.config(text=f"Hive Mode: {mode_display}")
...
def main():
    ...
    hive_manager = HiveMindManager(hive_shared, shared_registry)
    hive_manager.start()
    ...
    root = tk.Tk()
    gui = BorgGUI(root, queen, node_list, shared_dirs_ref, cfg, shared_registry, hive_manager)

    try:
        root.mainloop()
    finally:
        print("\n[Main] Shutting down BorgNet Guardian Hive...")
        for node in node_list:
            node.stop()
        hive_manager.stop()

        summary = queen.threat_detector.get_summary()
        print("\n=== Final Queen Anomaly / Rogue-like Activity Summary ===")
        ...
        print("===================================================")
        time.sleep(1)
        print("[Main] Hive shutdown complete.")
