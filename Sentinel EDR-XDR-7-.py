...
# Split radioactive into telemetry vs exfiltration
RADIO_TELEMETRY = set()
RADIO_EXFIL = set()

# Legacy union (for backward compatibility / migration)
RADIOACTIVE = set()
...
def load_lists():
    global ALLOWLIST, BLOCKLIST, RADIO_TELEMETRY, RADIO_EXFIL, RADIOACTIVE, WHITELIST_NAMES
    with LIST_LOCK:
        ...
        # New split radioactive sets
        RADIO_TELEMETRY = set(data.get("radio_telemetry", []))
        RADIO_EXFIL = set(data.get("radio_exfil", []))

        # Legacy "radio" key – migrate into telemetry bucket if present
        legacy_radio = set(data.get("radio", []))
        if legacy_radio and not (RADIO_TELEMETRY or RADIO_EXFIL):
            RADIO_TELEMETRY |= legacy_radio

        RADIOACTIVE = RADIO_TELEMETRY | RADIO_EXFIL
...
def save_lists():
    with LIST_LOCK:
        data = {
            ...
            "radio_telemetry": sorted(RADIO_TELEMETRY),
            "radio_exfil": sorted(RADIO_EXFIL),
...
class ScoringPipeline:
    ...
    def compute_fingerprint(features, behavior_record, extra_features):
        ...
        if behavior_record.get("grey_zone"):
            fp.append("grey_zone")
        if behavior_record.get("chameleon_name"):
            fp.append("chameleon_name")
        if behavior_record.get("mirror_system_name"):
            fp.append("mirror_system_name")
        if behavior_record.get("exfil_tool"):
            fp.append("exfil_tool")
        if behavior_record.get("pii_exfil_suspected"):
            fp.append("pii_exfil_suspected")
        return fp
    ...
    def compute_score(info, behavior_record, bad_fingerprints, cluster_stats, campaign_stats, now, gpu_detector=None):
        ...
        elif status == "RADIO_EXFIL":
            score += 90
            anomalies.append("Radioactive EXFIL classification")
        elif status == "RADIO_TELEMETRY":
            score += 70
            anomalies.append("Radioactive TELEMETRY classification")
        elif status == "RADIOACTIVE":
            score += 70
            anomalies.append("Legacy radioactive classification")
        ...
        if behavior_record.get("grey_zone"):
            score += 10
            anomalies.append("Windows grey-zone binary (mismatched location)")

        if behavior_record.get("chameleon_name"):
            score += 8
            anomalies.append("Chameleon-style executable name")

        if behavior_record.get("mirror_system_name"):
            score += 12
            anomalies.append("System-like name from non-system location")

        if behavior_record.get("exfil_tool"):
            score += 12
            anomalies.append("Potential exfiltration tool")

        if behavior_record.get("pii_exfil_suspected"):
            score += 18
            anomalies.append("Outbound PII / data-leak suspected")
...
class SentinelBrain:
    def classify(self, proc):
        ...
        # Auto-classification with split radioactive tiers
        if exe in BLOCKLIST or name_l in BLOCKLIST:
            status = "BLOCKED"
        elif exe in ALLOWLIST:
            status = "ALLOW"
        elif exe in RADIO_EXFIL or name_l in RADIO_EXFIL:
            status = "RADIO_EXFIL"
        elif exe in RADIO_TELEMETRY or name_l in RADIO_TELEMETRY:
            status = "RADIO_TELEMETRY"
    ...
    def threat_level(status):
        if status in ("BLOCKED", "RADIO_EXFIL", "RADIO_TELEMETRY", "RADIOACTIVE"):
            return "HIGH"
    ...
    def apply_patch(self, patch):
        ...
        for p in patch.get("allowlist_paths", []):
            ...
            if np and np not in ALLOWLIST:
                ...
                RADIO_TELEMETRY.discard(np)
                RADIO_EXFIL.discard(np)
                changed = True
...
# ============================================================
# AI-DRIVEN DATA-LEAK / PII ADVISOR
# ============================================================

class DataLeakAdvisor:
    """
    Uses LLM to suggest:
      - which telemetry-radioactive apps should be promoted to EXFIL
      - which exfil-radioactive apps might be downgraded to TELEMETRY
      - which apps might be safe to allowlist
    """

    def __init__(self, llm_manager):
        self.llm_manager = llm_manager

    def propose_reclassifications(self, behavior_db_snapshot):
        prompt = (
            "You are an AI data-leak and PII exfiltration advisor for an EDR/XDR system.\n"
            "You are given a snapshot of behavior records. Each record may contain:\n"
            "- has_network, exfil_tool, pii_exfil_suspected, grey_zone, last_score, user_label\n"
            "- cluster_dir, parent_type, fingerprints, etc.\n\n"
            "Your job:\n"
            "- Identify processes that look like true EXFIL radioactive (pii_exfil_suspected, exfil_tool, high scores).\n"
            "- Identify processes that look like harmless telemetry (network but low scores, stable behavior).\n"
            "- Identify processes that should be allowlisted (stable, low score, no suspicious flags).\n\n"
            "Return STRICT JSON with keys:\n"
            "  promote_to_exfil: [list of paths or keys]\n"
            "  downgrade_to_telemetry: [list of paths or keys]\n"
            "  allowlist_candidates: [list of paths or keys]\n\n"
            f"Behavior_snapshot: {json.dumps(behavior_db_snapshot, indent=2)[:8000]}\n"
        )
        resp = self.llm_manager.ask_role("anomaly", prompt)
        if not resp:
            resp = {"default": self.llm_manager.ask(prompt)}
        text = next(iter(resp.values())) if isinstance(resp, dict) else resp
        try:
    ...
    def apply_reclassifications(self, recs):
        changed = False
        for p in recs.get("promote_to_exfil", []):
            np = norm_path(p)
            if np:
                RADIO_EXFIL.add(np)
                RADIO_TELEMETRY.discard(np)
                ALLOWLIST.discard(np)
                BLOCKLIST.discard(np)
                changed = True
                log_event(f"DATA-LEAK AI: promoted to RADIO_EXFIL {np}")
        for p in recs.get("downgrade_to_telemetry", []):
            np = norm_path(p)
            if np:
                RADIO_TELEMETRY.add(np)
                RADIO_EXFIL.discard(np)
                changed = True
                log_event(f"DATA-LEAK AI: downgraded to RADIO_TELEMETRY {np}")
        for p in recs.get("allowlist_candidates", []):
            np = norm_path(p)
            if np:
                ALLOWLIST.add(np)
                RADIO_TELEMETRY.discard(np)
                ...
                BLOCKLIST.discard(np)
                changed = True
                log_event(f"DATA-LEAK AI: allowlist candidate accepted {np}")
        if changed:
            save_lists()
...
class EngineCore:
    def __init__(self, db_path="engine.db", llm_manager=None):
        ...
        self.data_leak_advisor = DataLeakAdvisor(self.llm_manager)
    ...
    def _monitor_loop(self):
        while self.running:
            ...
            self._mark_pii_exfil_suspicions(net_rows, pid_to_key)
            save_behavior()
    ...
    def _apply_greyzone_and_chameleon(self, rec, path, name_l):
        rec.setdefault("grey_zone", False)
        rec.setdefault("chameleon_name", False)
        rec.setdefault("mirror_system_name", False)
        rec.setdefault("exfil_tool", False)

        base = os.path.basename(path) or name_l
        base_l = base.lower()

        system_root = os.path.normcase(os.environ.get("SystemRoot", r"C:\Windows"))
        system32 = os.path.join(system_root, "System32")
        windows_dir = system_root

        if base_l in ("svchost.exe", "lsass.exe", "csrss.exe", "winlogon.exe", "services.exe"):
            if not path.lower().startswith(system32.lower()):
                rec["grey_zone"] = True
                rec["mirror_system_name"] = True

        if base_l in ("explorer.exe", "taskmgr.exe"):
            if not path.lower().startswith(windows_dir.lower()):
                rec["grey_zone"] = True
                rec["mirror_system_name"] = True

        alpha_count = sum(1 for c in base if c.isalpha())
        digit_count = sum(1 for c in base if c.isdigit())
        total_len = len(base)
        if total_len >= 8 and alpha_count > 0:
            ratio = (alpha_count + digit_count) / total_len
            if ratio > 0.8 and not any(x in base_l for x in ["chrome", "edge", "firefox", "word", "excel", "powerpnt", "outlook", "teams", "slack"]):
                rec["chameleon_name"] = True

        exfil_names = [
            "7z.exe", "scp.exe", "pscp.exe", "winscp.exe", "filezilla.exe",
            "rclone.exe", "curl.exe", "wget.exe", "powershell.exe", "robocopy.exe",
        ]
        if base_l in exfil_names:
            rec["exfil_tool"] = True
    ...
    def update_behavior(self, proc, info, bad_fingerprints, cluster_stats, campaign_stats, now):
        ...
        with BEHAVIOR_LOCK:
            rec = BEHAVIOR_DB.get(key, {
                ...
                "grey_zone": False,
                "chameleon_name": False,
                "mirror_system_name": False,
                "exfil_tool": False,
                "pii_exfil_suspected": False,
            })
            ...
            self._apply_greyzone_and_chameleon(rec, path, name_l)
    ...
    def _mark_pii_exfil_suspicions(self, net_rows, pid_to_key):
        ...
        with BEHAVIOR_LOCK:
            for n in net_rows:
                ...
                if any(host.startswith(p) for p in private_prefixes):
                    continue
                port_str = raddr.split(":")[1] if ":" in raddr else ""
                try:
                    port = int(port_str)
                except ValueError:
                    port = 0
                if port in (80, 443, 8080, 8443):
                    rec["pii_exfil_suspected"] = True
                    BEHAVIOR_DB[key] = rec
    ...
    def collect_bad_fingerprints(self):
        ...
        with BEHAVIOR_LOCK:
            for key, rec in BEHAVIOR_DB.items():
                ...
                if label in ("block", "radio", "radio_exfil", "radio_telemetry") or last_score >= 80:
    ...
    def compute_cluster_stats(self, now):
        ...
        with BEHAVIOR_LOCK:
            for key, rec in BEHAVIOR_DB.items():
                ...
                c["score_count"] += 1
                label = rec.get("user_label")
                if label in ("block", "radio", "radio_exfil", "radio_telemetry") or score >= 80:
                    c["bad_count"] += 1
    ...
    def auto_update_lists(self, info):
        ...
        if info["status"] == "TRUSTED" and np not in ALLOWLIST:
        ...
        elif info["status"] == "SUSPICIOUS" and np not in RADIO_TELEMETRY and np not in RADIO_EXFIL and np not in ALLOWLIST:
            try:
            ...
            with BEHAVIOR_LOCK:
                key = self._key_for_behavior(info["path"], info["name_l"])
                rec = BEHAVIOR_DB.get(key, {})
                exfil_flag = rec.get("pii_exfil_suspected") or rec.get("exfil_tool")
                if exfil_flag:
                    RADIO_EXFIL.add(np)
                    log_event(f"AUTO: Suspicious path added to RADIO_EXFIL: {np}")
                else:
                    RADIO_TELEMETRY.add(np)
                    log_event(f"AUTO: Suspicious path added to RADIO_TELEMETRY: {np}")
            changed = True
    ...
    def enforce_lockdown(self, info):
        ...
        if score < self.dynamic_lockdown_threshold and status not in ("BLOCKED", "RADIO_EXFIL", "RADIO_TELEMETRY", "RADIOACTIVE"):
            return
    ...
    def _watchdog_loop(self):
        while self.running:
            ...
            if now - self._last_dla_ts > 600:
                self._last_dla_ts = now
                with BEHAVIOR_LOCK:
                    snapshot = dict(list(BEHAVIOR_DB.items())[:80])
                recs = self.data_leak_advisor.propose_reclassifications(snapshot)
                if recs:
                    self.data_leak_advisor.apply_reclassifications(recs)
...
class EDRConsole(QMainWindow):
    ...
    def build_sentinel_panel(self):
        ...
        right_layout.addWidget(QLabel("RADIOACTIVE – TELEMETRY"))
        self.radio_telemetry_list = QListWidget()
        right_layout.addWidget(self.radio_telemetry_list)

        right_layout.addWidget(QLabel("RADIOACTIVE – EXFIL"))
        self.radio_exfil_list = QListWidget()
        right_layout.addWidget(self.radio_exfil_list)
        ...
        self.btn_radio = QPushButton("☢ Add Selected to Radioactive (auto tier)")
    ...
    def refresh_lists_ui(self):
        ...
        self.radio_telemetry_list.clear()
        for item in sorted(RADIO_TELEMETRY):
            self.radio_telemetry_list.addItem(QListWidgetItem(item))
        self.radio_exfil_list.clear()
        for item in sorted(RADIO_EXFIL):
            self.radio_exfil_list.addItem(QListWidgetItem(item))
    ...
    def on_remove_from_lists(self):
        ...
        for widget, s in (
            ...
            (self.radio_telemetry_list, RADIO_TELEMETRY),
            (self.radio_exfil_list, RADIO_EXFIL),
        ):
    ...
    def on_add_allow(self):
        ...
        RADIO_TELEMETRY.discard(norm)
        RADIO_EXFIL.discard(norm)
        save_lists()
    ...
    def on_add_radio(self):
        ...
        with BEHAVIOR_LOCK:
            ...
            exfil_flag = rec.get("pii_exfil_suspected") or rec.get("exfil_tool")
        if exfil_flag:
            RADIO_EXFIL.add(norm)
            RADIO_TELEMETRY.discard(norm)
            label = "radio_exfil"
            log_event(f"RADIO OVERRIDE (EXFIL): {key}")
        else:
            RADIO_TELEMETRY.add(norm)
            RADIO_EXFIL.discard(norm)
            label = "radio_telemetry"
            log_event(f"RADIO OVERRIDE (TELEMETRY): {key}")
    ...
    def _color_for_row(self, row_info):
        status = row_info.get("status", "")
        threat = row_info.get("threat", "")
        if status == "BLOCKED":
            return QColor(255, 0, 0)
        if status == "RADIO_EXFIL":
            return QColor(255, 80, 80)
        if status == "RADIO_TELEMETRY":
