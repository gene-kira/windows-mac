...
"""
Foresight (cross-platform, autonomous, secure, resource-aware, intrusion-aware, next-gen prediction):
- System-wide habit learner: apps, utilities, games, and website-like titles (via browser window title).
- Next-gen predictor:
  * Sequence modeling: first-order Markov chain with recency decay.
  * Context features: time-of-day, day-of-week, and system load.
...
- Intrusion detection: flags unfamiliar, spiking processes at odd hours or spawned by scripting parents.
...
def get_foreground_window_title() -> Optional[str]:
    ...
    # Linux (X11, best-effort)
        ...
        try:
            ...
            if lines:
                # Heuristic: pick last window title token
                parts = lines[-1].split()
...
# ----------------------------
# Next-gen predictor (sequence + context + decay + clustering)
# ----------------------------
...
# ----------------------------
# Foresight engine (encrypted cache + security + alerts + next-gen prediction)
# ----------------------------
class Foresight:
    ...
    def _load_cache(self) -> None:
        ...
        with self._lock:
            try:
                ...
                self.event_log = [(float(ts), str(h)) for ts, h in data.get("event_log", [])][-40000:]
                ...
                # Predictor state (optional future extension): transition_counts/context_bias could be saved too
            except Exception:
                self.habits = {}
    ...
    def _save_cache(self) -> None:
        with self._lock:
            data = {
                ...
                "event_log": self.event_log[-40000:],
    ...
    def record_habit(self, habit: str, ts: Optional[float] = None) -> None:
        ...
        with self._lock:
            ...
            if len(self.event_log) > 40000:
                self.event_log = self.event_log[-40000:]
            # Activity score
            ...
            try:
                ...
                if advisory:
                    with self.fs._lock:
                        ...
                        if len(self.fs.event_log) > 40000:
                            self.fs.event_log = self.fs.event_log[-40000:]
                time.sleep(interval)
            except Exception:
                time.sleep(3.0)
