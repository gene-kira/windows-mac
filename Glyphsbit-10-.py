...
def ttl_auto(w,a):
    ...
    QTimer.singleShot(3000,lambda: log(w,f"💥 Vaporized. Entropy={entropy()}"))
...
class CodexGUI(QWidget):
    ...
    def activate(self):
        ...
        for f in [self.watchdog,lambda: mutate(self.log,self.active),lambda: memory_diag(self.log,self.active),lambda: animate(self.log,self.glyph,self.active),lambda: ttl_auto(self.log,self.active)]:
