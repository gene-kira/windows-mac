#!/usr/bin/env python3
"""
Persistent Fused Organism with Prometheus Borg Hive Mode:
- Guardian Hybrid Brain with Foresight and Baselines
- Capsule Cortex Organ with Symbolic Capsules, Daemon, Swarm, Node
- Borg Hive Mode upgraded to Prometheus-style network scanner:
  - Maps network endpoints and detects rogue addresses vs baseline
- External Flask API (port 6666)
- Tkinter GUI (Guardian console + Capsule Cortex + Borg Hive tiles + details)
- Encrypted persistent memory (local + SMB mirroring)
- FULL persistence of:
  - Capsule engine state (active, self-destruct, visibility, key)
  - Consciousness state (CALM/FOCUSED/ALERT/PARANOID)
  - Baselines, histories, events, profiles
  - Capsule metrics
  - Borg Hive baseline of known addresses
  - Node identity + entropy
  - SMB drive paths / storage location
"""
...
class BorgHiveMode(BaseMode):
    ...
    def _capture_snapshot(self) -> Dict[str, Any]:
        # Collect network endpoints from current connections
        endpoints = []
        try:
            conns = psutil.net_connections(kind="inet")
            for c in conns:
        ...
        # Represent each endpoint as a string key for baseline comparison
        ...
        if baseline is None:
            ...
            # Baseline endpoints missing in current scan
            missing_set = baseline_set - current_set
            # We treat all differences as potential changes
        ...
        snapshot = {
            "mode_name": "BorgHive",
            "process_count": len(endpoints),          # how many connections/endpoints we see
            "connection_count": len(endpoints),
            "rogue_count": rogue_count,
            "anomaly_count": anomaly_count,
            "rogue_endpoints": rogue_endpoints[:50],   # cap to keep snapshot small
    ...
    def _maybe_record_event(self, brain: BrainState, details: Dict[str, Any]):
        # In addition to standard event recording, include rogue info
        if brain.level not in ("yellow", "red"):
            return
        event = {
            "timestamp": time.time(),
...
# ---------------------------------------------------------------------------
# Guardian Engine (with Capsule Cortex + Borg Hive + persistence)
# ---------------------------------------------------------------------------
...
class GuardianApp(tk.Tk):
    def __init__(self, engine: GuardianEngine):
        ...
        self.title("Fused Guardian – Hybrid Brain with Capsule Cortex & Prometheus Borg Hive (Persistent)")
