...
"""
Foresight (cross-platform, autonomous, secure, resource-aware, intrusion-aware):
- System-wide habit learner: apps, utilities, games, and website-like titles (via browser window title).
- Adaptive predictor (bandit-lite): blends time-of-day, recent activity, and global frequency, reinforced by outcomes.
...
- Intrusion detection: flags unfamiliar, spiking processes at odd hours or spawned by scripting/terminal parents.
- Compact GUI (~450x290): Suggestion banner, advisory/alert line, Top habits, Frequency chart, Learning toggle.
- Cross-platform: Windows, Linux, macOS (foreground window via platform-aware wrappers; psutil for processes).
...
def get_foreground_window_title() -> Optional[str]:
    ...
    # Linux (X11, best-effort)
        ...
        try:
            ...
            # Focused window marked with '*' sometimes; fallback to last line
            # Heuristic: pick line with current desktop focus
            # Format: 0x04200007 -1 N/A <class> <host> <title>
            lines = [l for l in out.splitlines() if l.strip()]
            if lines:
                # Try to find the active window via 'wmctrl -a' not available; fallback: last entry's title
                title = lines[-1].split()[-1]
                return title.strip()
        except Exception:
            pass
        # Try xprop as fallback: requires DISPLAY and active X session
        try:
            import subprocess as sp
