...
class PredictiveAIEngine(threading.Thread):
    ...
    def _compute_drive_prediction(self):
        ...
        drive_signal = min(1.0, total_score / 60.0)
        decision_high = self.drive_guesser.guess(drive_signal)
        if decision_high and level != "HIGH":
            level = "HIGH"
...
# ============================================================
# GAME AWARENESS ENGINE (BACK 4 BLOOD)
# ============================================================

class GameAwarenessEngine(threading.Thread):
    """
    Tracks awareness of playing Back 4 Blood:
    - Detects when the game is running
    - Accumulates 'experience' time
    - Uses simple guessing + JudgmentEngine confidence to decide when learning is 'good enough'
    """
    def __init__(self, node, game_process_names=None):
        ...
        self._learning_threshold_seconds = 60.0 * 30.0  # ~30 minutes baseline
...
# ============================================================
# JUDGMENT ENGINE (GAME SITUATIONAL AWARENESS)
# ============================================================

class JudgmentEngine(threading.Thread):
    """
    Learns judgment from:
    - Player input (keystrokes + mouse)
    - Bot behavior (future)
    Builds episodes and a global judgment confidence.
    """
    ...
    def run(self):
        while self.running and self.node.is_running():
            try:
                ...
                print(f"[JudgmentEngine] Error: {e}")
                time.sleep(5.0)
...
# ============================================================
# SITUATIONAL AWARENESS ENGINE (FULL CORTEX)
# ============================================================

class SituationalAwarenessEngine(threading.Thread):
    """
    Full situational awareness cortex:
    - Reads signals from ThreatDetector, PredictiveAIEngine, Hybrid, Game, Judgment, Drives
    - Maintains mission weights: stability, learning, protection, efficiency
    - Detects unexpected events
    - Scores opportunity and risk
    - Chooses plan adjustment
    - Uses guessing when mission preferences are close
    - Tracks outcome confidence over time
    """
    def __init__(self, node):
        super().__init__(daemon=True)
        self.node = node
        ...
        # High-level state
        self.current_mission = "STABILITY"  # STABILITY / PROTECT / LEARN / OPTIMIZE
        self.unexpected_event = "None"
        self.opportunity_score = 0.0  # 0–100
        self.risk_score = 0.0         # 0–100
        self.plan_adjustment = "None"
        self.guessing_factor = 0.0    # 0–1
        self.outcome_confidence = 0.0 # 0–1

        # Internal last state for change detection
        self._last_threat_level = 0.0
        self._last_volatility = 0.0
        self._last_drive_risk_num = 0
        self._last_anomaly_risk_num = 0
    ...
    def _compute_situation_vector(self):
        ...
        # threat_level: based on unusual_time + sequence + anomaly_risk
        ...
        # stability_risk: volatility + drive_risk + failing drives
        ...
        stability_risk = min(1.0, volatility * 0.5 + drive_risk_num * 0.3 + drive_fail * 0.5)

        # learning_opportunity: game learning + judgment confidence + low threat
        ...
        # efficiency_pressure: how much we can care about efficiency (low risk, high score)
        collective = pred.get("collective_risk_score", 100)
        eff_base = max(0.0, (collective - 60) / 40.0)  # 60->0, 100->1
    ...
    def _choose_mission(self, vec):
        t = vec["threat_level"]
        s = vec["stability_risk"]
        l = vec["learning_opportunity"]
        e = vec["efficiency_pressure"]

        # Mission scores: PROTECT > STABILITY > LEARN > OPTIMIZE
        m_protect = t * 1.0 + s * 0.5
        m_stability = s * 1.0 + t * 0.3
        m_learn = l * 1.0 * (1.0 - t) * (1.0 - s)
        m_optimize = e * 1.0 * (1.0 - t) * (1.0 - s)
        ...
        # Guessing factor: how close top two are
        sorted_scores = sorted(scores.items(), key=lambda x: x[1], reverse=True)
        best_mission, best_score = sorted_scores[0]
        second_score = sorted_scores[1][1]
        diff = max(0.0, best_score - second_score)
        guessing_factor = max(0.0, min(1.0, 1.0 - diff * 3.0))

        return best_mission, scores, guessing_factor

    def _detect_unexpected_event(self, vec):
        t = vec["threat_level"]
        v = vec["volatility"]
        a = vec["anomaly_risk_num"]
        d = vec["drive_risk_num"]
        ga = vec["ga"]
        js = vec["js"]
        ...
        # Threat spike
        if t - self._last_threat_level > 0.3:
            events.append("Threat spike")
        # Volatility surge
        if v - self._last_volatility > 0.3:
            events.append("Volatility surge")
        # Drive risk jump
        if d > self._last_drive_risk_num:
            events.append("Drive risk increase")
        # Anomaly risk jump
        if a > self._last_anomaly_risk_num:
            events.append("Anomaly risk increase")
        # Game learning start
        if ga.get("mode") == "LEARNING":
            events.append("Game learning active")
        # Judgment confidence change
        if js.get("sample_count", 0) > 0 and js.get("judgment_confidence", 0.0) < 0.3:
            events.append("Low judgment confidence")

        if not events:
            return "None"

        # Choose the most serious descriptor prioritizing threat/drive
        priority_order = [
            "Threat spike",
            "Drive risk increase",
            "Anomaly risk increase",
            "Volatility surge",
            "Game learning active",
            "Low judgment confidence",
        ]
    ...
    def _compute_opportunity_and_risk(self, mission, vec, unexpected_event):
        t = vec["threat_level"]
        s = vec["stability_risk"]
        l = vec["learning_opportunity"]
        e = vec["efficiency_pressure"]

        # Risk: mostly from threat + stability risk
        risk = min(1.0, t * 0.6 + s * 0.6)
        # Opportunity: depends on mission
        if mission == "PROTECT":
            opp = min(1.0, t * 0.7 + s * 0.3)
        elif mission == "STABILITY":
            opp = min(1.0, s * 0.7 + (1.0 - t) * 0.3)
        elif mission == "LEARN":
            opp = min(1.0, l * 0.8 + (1.0 - t) * 0.2)
        else:  # OPTIMIZE
            opp = min(1.0, e * 0.8 + (1.0 - t) * 0.2)

        # Slight bump in opportunity if there's a named unexpected event
        if unexpected_event != "None":
            opp = min(1.0, opp + 0.1)

        return opp * 100.0, risk * 100.0

    def _select_plan_adjustment(self, mission, unexpected_event, vec):
        pred = vec["pred"]
        ga = vec["ga"]
        js = vec["js"]

        if mission == "PROTECT":
            if unexpected_event == "Drive risk increase":
                return "Favor secondary drive, reduce load"
            if unexpected_event == "Threat spike" or unexpected_event == "Anomaly risk increase":
                return "Switch Hybrid to REFLEX, increase anomaly sensitivity"
            return "Maintain defensive posture, monitor anomalies"

        if mission == "STABILITY":
            if unexpected_event == "Volatility surge":
                return "Dampen sensitivity, stabilize Hybrid mode"
            if unexpected_event == "Drive risk increase":
                return "Throttle non-critical IO, favor stable drives"
            return "Hold STABILITY mode, smooth fluctuations"

        if mission == "LEARN":
            if ga.get("mode") == "LEARNING":
                return "Prioritize game learning episodes and judgment updates"
            if js.get("judgment_confidence", 0.0) < 0.3:
                return "Seek diverse episodes to improve judgment"
            return "Maintain learning focus while environment is safe"

        if mission == "OPTIMIZE":
            return "Reduce overhead, streamline scans, favor efficiency"

        return "None"

    def _update_outcome_confidence(self, mission, vec, prev_risk, current_risk):
        # If risk decreased while holding same mission, increase confidence.
        # If risk increased, decrease confidence.
        if prev_risk is None:
            return

        if current_risk < prev_risk - 5.0:
            self.outcome_confidence = min(1.0, self.outcome_confidence + 0.05)
        elif current_risk > prev_risk + 5.0:
            self.outcome_confidence = max(0.0, self.outcome_confidence - 0.07)
        else:
            # small drift upward in calm conditions
            self.outcome_confidence = min(1.0, self.outcome_confidence + 0.01)
    ...
    def snapshot(self):
        with self._lock:
            return {
                ...
                "guessing_factor": self.guessing_factor,
    ...
    def run(self):
        ...
        while self.running and self.node.is_running():
            try:
                ...
                mission, scores, guessing = self._choose_mission(vec)
                unexpected = self._detect_unexpected_event(vec)
                opp, risk = self._compute_opportunity_and_risk(mission, vec, unexpected)
                plan = self._select_plan_adjustment(mission, unexpected, vec)

                self._update_outcome_confidence(mission, vec, prev_risk_score, risk)
                prev_risk_score = risk

                with self._lock:
                    self.current_mission = mission
                    self.unexpected_event = unexpected
                    self.opportunity_score = opp
                    self.risk_score = risk
                    self.plan_adjustment = plan
                    self.guessing_factor = guessing
                ...
                # Update last state for next tick comparison
                self._last_threat_level = vec["threat_level"]
                self._last_volatility = vec["volatility"]
                self._last_drive_risk_num = vec["drive_risk_num"]
                self._last_anomaly_risk_num = vec["anomaly_risk_num"]

            except Exception as e:
                print(f"[SituationalAwarenessEngine] Error: {e}")
...
class BorgNode:
    def __init__(
        ...
        self.game_awareness = GameAwarenessEngine(self)
        self.judgment_engine = JudgmentEngine(self)
        self.situational_engine = SituationalAwarenessEngine(self)
...
class BorgGUI:
    def __init__(self, root, queen, node_list, shared_dirs_ref, config_ref, shared_registry, cfg):
        ...
        # Hybrid brain vars
        self.hybrid_mode_var = tk.StringVar(value="STABILITY")
        self.hybrid_volatility_var = tk.StringVar(value="0.00")
        self.hybrid_trust_var = tk.StringVar(value="0.50")

        # Game cortex vars
        self.game_status_var = tk.StringVar(value="Offline")
        self.game_judgment_conf_var = tk.StringVar(value="0.00")
        self.game_episodes_var = tk.StringVar(value="0")
        self.game_good_bad_var = tk.StringVar(value="0 / 0")
        self.game_sources_var = tk.StringVar(value="Player, Bots")

        # Situational awareness cortex vars
        self.sit_mission_var = tk.StringVar(value="STABILITY")
        self.sit_event_var = tk.StringVar(value="None")
        self.sit_opp_var = tk.StringVar(value="0")
        self.sit_risk_var = tk.StringVar(value="0")
        self.sit_plan_var = tk.StringVar(value="None")
        self.sit_guess_var = tk.StringVar(value="0.00")
        self.sit_conf_var = tk.StringVar(value="0.00")
    ...
    def _build_layout(self):
        ...
        # ====== TOP ROW: Node Status / Health / Predictive / Hybrid / Game / Situational ======
        ...
        predictive_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))
        ...
        hybrid_frame = ttk.LabelFrame(top_frame, text="Hybrid Brain")
        hybrid_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))

        ttk.Label(hybrid_frame, text="Mode:").pack(anchor="w")
        self.hybrid_mode_label = ttk.Label(hybrid_frame, textvariable=self.hybrid_mode_var)
        self.hybrid_mode_label.pack(anchor="w")

        ttk.Label(hybrid_frame, text="Volatility:").pack(anchor="w")
        self.hybrid_volatility_label = ttk.Label(hybrid_frame, textvariable=self.hybrid_volatility_var)
        self.hybrid_volatility_label.pack(anchor="w")

        ttk.Label(hybrid_frame, text="Trust:").pack(anchor="w")
        self.hybrid_trust_label = ttk.Label(hybrid_frame, textvariable=self.hybrid_trust_var)
        self.hybrid_trust_label.pack(anchor="w")

        # ====== GAME CORTEX COLUMN ======
        game_frame = ttk.LabelFrame(top_frame, text="Game Cortex (Back 4 Blood)")
        game_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))

        ttk.Label(game_frame, text="Status:").pack(anchor="w")
        self.game_status_label = ttk.Label(game_frame, textvariable=self.game_status_var)
        self.game_status_label.pack(anchor="w")

        ttk.Label(game_frame, text="Judgment Confidence:").pack(anchor="w")
        self.game_judgment_conf_label = ttk.Label(
            game_frame,
            textvariable=self.game_judgment_conf_var
        )
        self.game_judgment_conf_label.pack(anchor="w")

        ttk.Label(game_frame, text="Episodes Learned:").pack(anchor="w")
        self.game_episodes_label = ttk.Label(game_frame, textvariable=self.game_episodes_var)
        self.game_episodes_label.pack(anchor="w")

        ttk.Label(game_frame, text="Good / Bad Outcomes:").pack(anchor="w")
        self.game_good_bad_label = ttk.Label(game_frame, textvariable=self.game_good_bad_var)
        self.game_good_bad_label.pack(anchor="w")

        ttk.Label(game_frame, text="Learning From:").pack(anchor="w")
        self.game_sources_label = ttk.Label(game_frame, textvariable=self.game_sources_var)
        self.game_sources_label.pack(anchor="w")

        # ====== SITUATIONAL AWARENESS CORTEX COLUMN ======
        sit_frame = ttk.LabelFrame(top_frame, text="Situational Awareness Cortex")
        sit_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 0))

        ttk.Label(sit_frame, text="Current Mission:").pack(anchor="w")
        self.sit_mission_label = ttk.Label(sit_frame, textvariable=self.sit_mission_var)
        self.sit_mission_label.pack(anchor="w")

        ttk.Label(sit_frame, text="Unexpected Event:").pack(anchor="w")
        self.sit_event_label = ttk.Label(sit_frame, textvariable=self.sit_event_var, wraplength=220)
        self.sit_event_label.pack(anchor="w")

        ttk.Label(sit_frame, text="Opportunity / Risk:").pack(anchor="w")
        self.sit_opp_risk_label = ttk.Label(sit_frame, textvariable=self.sit_opp_var)
        self.sit_opp_risk_label.pack(anchor="w")

        ttk.Label(sit_frame, text="Plan Adjustment:").pack(anchor="w")
        self.sit_plan_label = ttk.Label(sit_frame, textvariable=self.sit_plan_var, wraplength=220)
        self.sit_plan_label.pack(anchor="w")

        ttk.Label(sit_frame, text="Guess / Outcome Conf:").pack(anchor="w")
        self.sit_guess_conf_label = ttk.Label(sit_frame, textvariable=self.sit_guess_var)
        self.sit_guess_conf_label.pack(anchor="w")
    ...
    def _update_hybrid_brain_panel(self):
        summary = self.queen.predictive_engine.get_summary()
        mode = summary.get("mode", "stability")
        vol = summary.get("volatility", 0.0)
        trust = summary.get("trust", 0.5)

        self.hybrid_mode_var.set(mode.upper())
        self.hybrid_volatility_var.set(f"{vol:.2f}")
        self.hybrid_trust_var.set(f"{trust:.2f}")
        ...
        self.hybrid_mode_label.config(foreground=fg)
        self.hybrid_volatility_label.config(foreground="black")
        self.hybrid_trust_label.config(foreground="black")

    def _update_game_cortex_panel(self):
        try:
            ga = self.queen.game_awareness.snapshot()
        except Exception:
            ga = {}
        mode = ga.get("mode", "OFFLINE")

        if mode == "OFFLINE":
            status_text = "Offline"
        elif mode == "LEARNING":
            status_text = "Learning how to play Back 4 Blood"
        elif mode == "MISSION_ACCOMPLISHED":
            status_text = "Mission accomplished — playing Back 4 Blood"
        else:
            status_text = mode

        try:
            js = self.queen.judgment_engine.snapshot()
        except Exception:
            js = {}
        j_conf = js.get("judgment_confidence", 0.0)
        samples = js.get("sample_count", 0)
        good = js.get("good_outcome_count", 0)
        bad = js.get("bad_outcome_count", 0)
        lp = js.get("learning_from_player", True)
        lb = js.get("learning_from_bots", True)

        self.game_status_var.set(status_text)
        self.game_judgment_conf_var.set(f"{j_conf:.2f}")
        self.game_episodes_var.set(str(samples))
        self.game_good_bad_var.set(f"{good} / {bad}")
        ...
        if not sources:
            sources.append("None")
        self.game_sources_var.set(", ".join(sources))

        if mode == "MISSION_ACCOMPLISHED" and j_conf >= 0.7:
            fg = "green"
        elif j_conf >= 0.8:
            fg = "green"
        elif j_conf >= 0.4:
        ...
        self.game_status_label.config(foreground=fg)
        self.game_judgment_conf_label.config(foreground=fg)
        self.game_episodes_label.config(foreground="black")
        self.game_good_bad_label.config(foreground="black")
        self.game_sources_label.config(foreground="black")

    def _update_situational_panel(self):
        snap = self.queen.situational_engine.snapshot()
        mission = snap.get("current_mission", "STABILITY")
        event = snap.get("unexpected_event", "None")
        opp = snap.get("opportunity_score", 0.0)
        risk = snap.get("risk_score", 0.0)
        plan = snap.get("plan_adjustment", "None")
        guessing = snap.get("guessing_factor", 0.0)
        conf = snap.get("outcome_confidence", 0.0)

        self.sit_mission_var.set(mission)
        self.sit_event_var.set(event)
        self.sit_opp_var.set(f"{opp:.0f} / {risk:.0f}")
        self.sit_plan_var.set(plan)
        self.sit_guess_var.set(f"G={guessing:.2f}  OC={conf:.2f}")

        # Color mission label: PROTECT red, STABILITY yellow, LEARN blue, OPTIMIZE green
        mission_upper = mission.upper()
        if mission_upper == "PROTECT":
            fg = "red"
        elif mission_upper == "STABILITY":
            fg = "orange"
        elif mission_upper == "LEARN":
            fg = "blue"
        elif mission_upper == "OPTIMIZE":
            fg = "green"
        else:
            fg = "black"

        self.sit_mission_label.config(foreground=fg)
    ...
    def _update_hud_panel(self):
        ...
        p_status, s_status = self.queen.shared_sync.get_statuses()
        sit = self.queen.situational_engine.snapshot()

        pressure = summary["unusual_time_endpoints"] + summary["sequence_anomalies"]
        if pressure > 5:
            pressure_flag = "HIGH"
        elif pressure > 0:
            pressure_flag = "ELEVATED"
        else:
            pressure_flag = "CALM"
        ...
        static_lines.append(
            f"HYBRID: mode={pred.get('mode','?')} "
            f"vol={pred.get('volatility',0.0):.2f} "
            f"trust={pred.get('trust',0.5):.2f}"
        )
        static_lines.append(
            f"GAME: {self.game_status_var.get()} "
            f"J={self.game_judgment_conf_var.get()} "
            f"E={self.game_episodes_var.get()}"
        )
        static_lines.append(
            f"SIT: M={sit.get('current_mission','?')} "
            f"EV={sit.get('unexpected_event','None')} "
            f"O={sit.get('opportunity_score',0.0):.0f} "
            f"R={sit.get('risk_score',0.0):.0f}"
        )
        ...
        feed_line = (
            ...
            f"B2={pressure_flag} "
            f"HYB={pred.get('mode','?')} "
            f"GAMEJ={self.game_judgment_conf_var.get()} "
            f"SITM={sit.get('current_mission','?')}"
        )
    ...
    def _update_gui(self):
        ...
        self._update_hybrid_brain_panel()
        self._update_game_cortex_panel()
        self._update_situational_panel()
