...
# - GUI visuals for health, safe mode, net status, context memory, critique cycles, mind stream, evolution log
...
class ContextMemoryOrgan:
    """
    Tracks:
    - Active topics (weighted)
    - Entities (names, tools, key concepts)
    - Goals (free-form)
    - Recent actions
    - Live summary (via local LLM)

    This is a mid-term "situation awareness" organ, not full history.
    """
    ...
    def _detect_entities(self, text: str):
        # Very simple heuristic: capitalized tokens or "organ"/"engine"/"guardian" etc.
        entities = set()
    ...
    def _detect_goals(self, text: str):
        # Look for phrases that sound like goals.
        hints = []
        lower = text.lower()
        ...
        for p in prefixes:
            if p in lower:
                hints.append(p)
        # For now just treat these hints as "goal markers"
        return hints
    ...
    def update(self, user_text: str, draft: str, critique: str, refined: str):
        # Combine to see what the conversation is about.
        combined = f"USER:\n{user_text}\n\nREFINED:\n{refined}"
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _init_ui(self):
        ...
        # Left: organs + vitals + prediction + web probe + health/safe mode/net + clipboard toggle
        ...
        self.organs_list.setFixedHeight(280)
        ...
        # Center: thought stream + context memory
...
# ========= BRAIN LOOP (QUEUE-DRIVEN, SELF-CRITIQUE + CONTEXT MEMORY) =========
...
def process_one_input(user: str, thought_model: ThoughtStreamModel, cfg: dict):
    ...
    if STATE["safe_mode"]:
        ...
        # Update context memory minimally even in safe mode.
...
def brain_loop(thought_model: ThoughtStreamModel, cfg: dict):
    ...
    # Initialize context memory panel at startup
    thought_model.update_context(CONTEXT_MEMORY.render())
