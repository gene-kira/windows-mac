...
class RaidCacheManager:
    """
    Ramnuke RAID organ:
      - wraps six VRAMCacheManager instances
      - supports RAID0 (stripe) and RAID1 (mirror)
      - can mute/throttle misbehaving spines
    """
    ...
    def read_block(self, block_id: int):
        if self.mode == RaidMode.RAID0:
            ...
            # if chosen dev is muted, try others
            if dev.muted:
...
class AutoLoaderGUI(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Ramnuke – 6‑Spine RAID VRAM/RAM Control System")
        ...
        self.info_label = QtWidgets.QLabel(
            "Ramnuke – 6‑spine RAID VRAM/RAM organism with predictive policy, learning engine, self‑integrity, and per‑spine health."
        )
