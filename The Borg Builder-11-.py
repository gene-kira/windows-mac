#!/usr/bin/env python3
# neural_core_v5_tk.py
# Phase 9 (Tkinter, predictive+adaptive+persistent+policy+friction):
# - Event-driven organs
...
# - JSON persistence (personas, critique depth, outcome stats, mode transitions, policy)
...
# ============================================================
#  STATE CORE (with personas, policy, templates, feedback, session, persistence)
# ============================================================
...
class PredictionOrgan(Organ):
    ...
    def on_intent(self, payload):
        ...
        # Occasionally persist transitions
...
# ============================================================
#  FRICTION ORGAN (detects pain clusters)
# ============================================================
...
class FrictionOrgan(Organ):
    ...
    def _update_clusters(self):
        ...
        # Simple heuristic: fast oscillation between CODE/DEBUG/DOCS + anomalies → friction
        oscillation = 0
        for i in range(1, len(intent_pattern)):
            if intent_pattern[i] != intent_pattern[i - 1]:
                oscillation += 1

        if oscillation >= 3 and anomalies_recent:
            ts = time.strftime("%H:%M:%S")
            cluster_label = f"{ts} - Friction cluster: pattern={intent_pattern}"
            self.state.friction_clusters.append(cluster_label)
            self.recent_anomaly_flags.clear()

            # Surface as a brain output hint
...
class ClarificationOrgan(Organ):
    ...
    def on_input(self, payload):
        ...
        if intent in ("CODE", "DEBUG") and conf >= 0.75 and not weak_prev:
            return
...
# ============================================================
#  BRAIN ORGAN (persona-based, policy-aware, adaptive depth)
# ============================================================
...
class BrainOrgan(Organ):
    ...
    def on_input(self, payload):
        ...
        policy_text = f"""
Policy:
- Banned libraries: {', '.join(self.state.policy.get('banned_libs', []))}
- Preferred libraries: {', '.join(self.state.policy.get('preferred_libs', []))}
- Architecture preference: {self.state.policy.get('architecture')}
- Risk tolerance: {self.state.policy.get('risk_tolerance')}
- Style:
  - avoid_heavy_frameworks: {self.state.policy.get('style', {}).get('avoid_heavy_frameworks', True)}
  - prefer_transparency: {self.state.policy.get('style', {}).get('prefer_transparency', True)}
  - prefer_self_healing: {self.state.policy.get('style', {}).get('prefer_self_healing', True)}
...
- Obey policy strictly (no banned libs, no fragile stacks).
...
class GUIOrgan(Organ):
    def __init__(self, root, bus, state):
        ...
        self.root = root
        self.root.title("NEURAL CORE V5 — Tkinter organism (predictive, adaptive, persistent, policy-aware)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(center, text="Brain State / Policy / Friction").grid(row=10, column=0, sticky="w")
