"""
Turbo Booster Twin Cache Organ (All Live + Brain + SMB Scanner + Scrollable Sources)
...
- Real ingestion sources:
    ...
    * cpu, ram, disk, gpu_vram : system metrics -> real synthetic payloads
    * smb:<name>  : SMB/network folder, scanned every 3 seconds

GUI:
- Shows VRAM/RAM usage, mode, backend, Movidius status, Brain state, and per-source stats.
- Sources panel:
...
class CacheMonitorWidget(QtWidgets.QWidget):
    ...
    def _setup_ui(self):
        self.setWindowTitle("Turbo Booster Twin Cache Organ (All Live + Brain + SMB)")
    ...
    def _update_ui_from_stats(self, stats):
        ...
        if total_map:
            ...
            # Auto-truncate to last 100 lines
            max_lines = 100
