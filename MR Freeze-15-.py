# ===== MR FREEZE v4.8 – SINGLE FILE =====
# Adaptive Thread-Pool Scanner + Persistent Encrypted Logs + Admin Mode God View + Privacy Firewall
...
MEMORY_FILENAME = "mr_freeze_memory_v4_8.json.enc"
LOG_FILENAME = "mr_freeze_log_v4_8.log.enc"
...
class PersistentLog:
    ...
    def _background_load_older(self, older_lines):
        # older_lines is full list of older lines (from start to just before initial)
        # we load them in batches at the *top* of buffer
        idx = len(older_lines)
        while idx > 0:
...
class MrFreezeCockpit:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Mr Freeze v4.8 – Predictive Swarm Cockpit")
        ...
        self.log_event("[MR FREEZE v4.8] Cockpit initialized.")
    ...
    def _estimate_thread_count(self, num_ips):
        ...
        # Start with 10 * cores, clamped to [min, max]
        base = max(min_threads, min(max_threads, cores * 10))

        # Scale down if very few IPs
        if num_ips < base:
            base = max(min_threads, num_ips)

        # Could add more adaptive logic (latency, etc.) later
        return base
